#!/usr/bin/env python3
"""Write Origen Joshua homily OET artifacts and optionally emit stamp helpers."""
...
BOOK = ROOT / "books/origen-joshua-homilies"
...
def write_homily(num: int, claim: str, prev_tip: str, sections: list[dict]) -> None:
    ...
    wl = [
        f"Origen Joshua (Iesu Nave) Homilia {num} — working Latin "
    ...
    for s in sections:
        ...
        notes = [
            ...
            "True OET: no ANF; do not copy Bruce FOTC 105 (Joshua).",
        ...
        eng.append(
            {
                ...
                "baehrens": f"GCS 30 Iesu Nave Hom. {num}.{sec}",
                "claim": claim,
            }
        )
        src.append(
            {
                ...
                "work": "homiliae",
                "part": f"homilia-{num}",
                "locus": f"Baehrens GCS 30 Iesu Nave Hom. {num} §{sec}",
                "head": f"Iesu Nave Hom. {num}.{sec}",
                "latin": [latin],
                "edition": "baehrens-gcs30-1921",
            }
        )
        j = {
            "excerpt_id": f"josh_hom{num}_{sec:02d}",
            "edition": {
            ...
            "notes": [
                "Pass A ≠ Pass B.",
                "True OET; no Bruce FOTC 105.",
                "Copy-text Baehrens GCS 30 Rufinus Latin.",
            ],
            "bible_links": [
                {"ref": a["reference"], "why": a["reason"]} for a in allusions
            ],
            "anf_compare": "no ANF for Rufinus Joshua homilies; do not copy Bruce FOTC 105",
            "checks": {
        ...
        (BOOK / "reviews/justifications" / f"josh_hom{num}_{sec:02d}.json").write_text(
            json.dumps(j, ensure_ascii=False, indent=2) + "\n"
        )

    (BOOK / "sources" / f"josh_hom{num}_working_latin.txt").write_text("".join(wl))
    (BOOK / "translations" / f"josh_hom{num}_english.json").write_text(
        json.dumps(eng, ensure_ascii=False, indent=2) + "\n"
    )
    (BOOK / "translations" / f"josh_hom{num}_source.json").write_text(
        json.dumps(src, ensure_ascii=False, indent=2) + "\n"
    )
    (ROOT / "docs/claim-locks" / claim).write_text(
        f"agent: Air\nclaimed: 2026-09-13\n"
        f"slice: Origen Joshua Homilia {num} CLOSEOUT Pass A≠B OET\n"
        f"status: done\n"
    )
