"""Build Epiphanius Anacephalaeosis Logos Personal Book DOCX."""
...
OUT_DOCX = BOOK_DIR / "epiphanius-anacephalaeosis.docx"
RECEIPT = BOOK_DIR / "build_receipt.json"

ENGLISH_FILE = "anacephalaeosis_english.json"
SOURCE_FILE = "anacephalaeosis_source.json"
META_FILE = "anacephalaeosis_meta.json"
...
FRONT_MATTER = [
    ...
    "Scope: Epiphanius Anacephalaeosis. Tip covers the recapitulation opening from locked PG 41 Greek; remainder remains.",
    "Editorial note: Greek copy-text is PG 41 page-image OCR (tesseract grc+eng).",
...
def main() -> None:
    doc = setup_document(
        title="Epiphanius: Anacephalaeosis (New English)",
        author="Epiphanius of Salamis",
        subject="New English rendering for private Logos study",
        keywords="Epiphanius, Anacephalaeosis, Panarion, heresies",
    )
    linker = BibleLinker()
    doc.add_paragraph("Epiphanius: Anacephalaeosis", style="Title")
    ...
    add_heading_with_headword(doc, bookmarks, "Anacephalaeosis", 1, "anacephalaeosis-work")
    doc.add_paragraph(edition_line, style="Caption")

    for entry in entries:
        section = entry["section"]
        label = section_label(section, entry.get("title"))
        key = f"anaceph-{section}"
