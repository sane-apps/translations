"""Build Paulus Silentarius: Descriptio Ambonis Logos Personal Book DOCX."""
...
OUT_DOCX = BOOK_DIR / "paulus-silentarius-ambonis English.docx"
RECEIPT = BOOK_DIR / "build_receipt.json"

CODEX_FILES = [
    "ambon_prologue_english.json",
    "ambon_body_english.json",
]
...
_STOP = {
    ...
    "with", "would", "paul", "silentiary", "hagia", "sophia", "ambo", "greek",
...
def main() -> None:
    doc = setup_document(
        title="Paulus Silentarius: Descriptio Ambonis (New English)",
        author="Paulus Silentarius",
        subject="New English rendering for private Logos study",
        keywords="Paulus Silentarius, Hagia Sophia, Ambo, Ekphrasis, 563 AD",
    )
    doc.add_paragraph("Paulus Silentarius: Descriptio Ambonis", style="Title")
    ...
    add_heading_with_headword(doc, bookmarks, "Descriptio Ambonis", 1, "ambon-work")
    doc.add_paragraph("Paul the Silentiary, Ekphrasis of the Ambo of Hagia Sophia (563 AD). Greek locked from PG 86b / Friedländer 1912.", style="Caption")
    ...
    for fname in CODEX_FILES:
        ...
        if not entries:
            continue
        if not any(e.get("part") != "prologue" for e in entries):
            # Prologue file
            add_heading_with_headword(doc, bookmarks, "Prologue", 1, "ambon-prologue")
            doc.add_paragraph("Paul the Silentiary, Ekphrasis of the Ambo of Hagia Sophia (563 AD). Greek locked from PG 86b / Friedländer 1912.", style="Caption")
        ...
        for entry in entries:
            ...
            if part == "prologue":
                label = section_label("Prologue", section, entry.get("title"))
                key = f"ambon-prologue-{section}"
            else:
                part_num = entry.get("part")
                section_counter += 1
                label = section_label(f"Part {part_num}", section, entry.get("title"))
                key = f"ambon-body-{section_counter}"
