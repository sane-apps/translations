# -*- coding: utf-8 -*-
"""Pars IV IX-XIV densify: lock Latin, append secs 127-132, justifications, packet, review, handoff."""
...
PACKET_STEM = "authoritate_scripturae_pars_iv_ix_densify"
HANDOFF = BOOK / "SESSION_HANDOFF.md"
DATA = Path("/tmp/leblanc_pars4_ix")

SECS = ["127", "128", "129", "130", "131", "132"]

RANGE_SHORT = "Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XIV"
RANGE_TITLE = (
    "Theological Theses (De Theologia I-XLIII; De Fide I-XXII; "
    "De Authoritate Scripturae Pars I I-XLVII; Pars II I-XXXV; Pars III I-XLI; Pars IV I-XIV)"
)
EDITION = (
...
BLURB = (
    ...
    "through XIV (immediate belief / de fide proofs; Spirit unction; recognition retorsion). "
    "Not the collected folio."
)
METHOD = (
    "English follows the locked 1675 Pitt Latin of De Theologia I-XLIII, De Fide I-XXII, "
    "De Authoritate Scripturae Pars I I-XLVII, Pars II I-XXXV, Pars III I-XLI, and Pars IV I-XIV, "
    ...
    "through XLI; Pars IV through XIV (I-VIII retained; IX-XIV immediate belief / de fide / "
    "recognition). XV+ (Scripture doubtful without Church) remains."
)

LOCK_HEADER = (
    "\n---\n\n"
    "Expand lock: De Authoritate Scripturae Pars IV theses IX-XIV "
    "(contiguous after I-VIII - immediate belief / de fide proofs; Spirit unction; recognition retorsion).\n"
    "Same 1675 Pitt copy-text. Book pp. 44-46 / PDF 56-58. pdftotext + prior tess OCR (+ DjVu checks). "
    "Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: Part IV continue through XIV only (natural close after recognition retorsion). "
    "XV+ (Scripture doubtful without Church) remains.\n"
    "Note: IX page-turn after tantum; X fenestram/disci/notas Ecclesiae; XI Hebrews/Apocalypse/James blind test; "
    "XII 1 John unction / Phil 3; XIII portiuncula; XIV germanum/supposititio restored from damaged OCR.\n\n"
)
...
def append_lock(LATIN):
    ...
    if "Expand lock: De Authoritate Scripturae Pars IV theses IX-XIV" in text:
        print("lock already expanded")
        return
    blob = LOCK_HEADER + "\n".join(LATIN[s] + "\n" for s in SECS) + "\n"
    LOCK_PATH.write_text(text.rstrip() + "\n" + blob, encoding="utf-8")
    print("lock appended Pars IV IX-XIV")
...
def append_translations(LATIN, TITLES, PASS_B, NOTES, ALLUSIONS):
    ...
    if len(eng) != 126 or str(eng[-1]["section"]) != "126":
        raise SystemExit(f"refuse append: tip not 126 (eng={len(eng)} last={eng[-1].get('section')})")
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        n = int(sid)
        if 127 <= n <= 132:
            notes = f"Section {sid}: new densify Pars IV IX-XIV; Pass A/B checked."
        else:
            notes = (
                f"Section {sid}: prior verified densify retained; Pass A/B and lock identity "
                "rechecked in Pars IV IX-XIV packet scope covering all current sections."
            )
        reviews.append(
            {
                "section": sid,
    ...
    receipt = {
        ...
        "scope_review": {
            ...
            "uncertainties": [],
            "notes": (
                "Scope review: densify Pars IV IX-XIV only (sections 127-132). Meta discloses "
                f"{RANGE_SHORT}. Honest partial; Pars IV XV+ remains. "
...
def prepend_handoff(before: int, after: int):
    entry = (
        ...
        f"- Before: **{before}** (Pars I I–XLVII + Pars II I–XXXV + Pars III I–XLI + Pars IV I–VIII live). "
        f"After: **{after}** (contiguous Pars IV IX–XIV → §§127–132).\n"
        ...
        "`sources/_le_blanc_authoritate_latin_lock.txt` (PDF 56–58 / book pp. 44–46).\n"
        f"- Meta range bumped to **{RANGE_SHORT}** (title, edition, blurb, text_history.method).\n"
        "- Honest **partial**: Pars IV through XIV (immediate belief / de fide proofs; "
        "two senses of immediate; recognition objections; 1 John unction / Phil 3; retorsion on councils). "
        "Pars IV XV+ remains. De Theologia closed. De Fide I–XXII untouched. "
        "Not folio. Not shipped this slice.\n"
        "- Pass A ≠ B for §§127–132. Inline 1 John unction / Philippians 3:15–16. OCR restorations noted "
        "in translator_notes.\n"
    ...
    if "Pars IV IX–XIV densify" in prev[:1500]:
...
def main():
    ...
    if before != 126:
        raise SystemExit(f"refuse: expected tip 126 before append, got {before}")
    append_lock(LATIN)
