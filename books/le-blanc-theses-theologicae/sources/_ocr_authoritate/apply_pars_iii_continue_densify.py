# -*- coding: utf-8 -*-
"""Pars III XIX-XXVI densify: lock Latin, append secs 96-103, justifications, packet, review, handoff."""
...
PACKET_STEM = "authoritate_scripturae_pars_iii_continue_densify"
HANDOFF = BOOK / "SESSION_HANDOFF.md"
DATA = Path("/tmp/leblanc_cont")

SECS = ["96", "97", "98", "99", "100", "101", "102", "103"]

RANGE_SHORT = "Pars I I-XLVII + Pars II I-XXXV + Pars III I-XXVI"
RANGE_TITLE = (
    "Theological Theses (De Theologia I-XLIII; De Fide I-XXII; "
    "De Authoritate Scripturae Pars I I-XLVII; Pars II I-XXXV; Pars III I-XXVI)"
)
EDITION = (
...
BLURB = (
    ...
    "Scripturae Pars I through XLVII, Pars II through XXXV, and Pars III through XXVI "
    "(real state of the controversy: first external rule is Scripture, not present pastors). "
    "Not the collected folio."
)
METHOD = (
    "English follows the locked 1675 Pitt Latin of De Theologia I-XLIII, De Fide I-XXII, "
    "De Authoritate Scripturae Pars I I-XLVII, Pars II I-XXXV, and Pars III I-XXVI, "
    ...
    "through XXVI (I-XVIII retained; XIX-XXVI real status controversiae / first external "
    "principle). XXVII+ (Scripture better known than Church; further proofs) and Part IV remain."
)

LOCK_HEADER = (
    "\n---\n\n"
    "Expand lock: De Authoritate Scripturae Pars III theses XIX-XXVI "
    "(contiguous after XIV-XVIII - real state of the controversy; first external rule of faith).\n"
    "Same 1675 Pitt copy-text. Book pp. 36-38 / PDF 48-50. pdftotext + tesseract (400 dpi). "
    "Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: Part III continue through XXVI only (natural close once controversia is stated). "
    "XXVII+ and Part IV remain.\n"
    "Note: XIX Balaam/Jude-Enoch / infinite regress restored; XX page-turn after Satanae "
    "praestigiis restored; XXIV mutuam sumptam across page-turn restored; XXV analogy and "
    "XXVI Prius nos / Posterius Pontificii restored from damaged OCR.\n\n"
)
...
def load_data():
    ...
    return latin, engd["titles"], engd["pass_a"], engd["pass_b"], engd["notes"], engd["bible"], engd["lemmas"], engd["choices"]


def append_lock(LATIN):
    text = LOCK_PATH.read_text(encoding="utf-8")
    if "Expand lock: De Authoritate Scripturae Pars III theses XIX-XXVI" in text:
        print("lock already expanded")
        return
    blob = LOCK_HEADER + "\n".join(LATIN[s] + "\n" for s in SECS) + "\n"
    LOCK_PATH.write_text(text.rstrip() + "\n" + blob, encoding="utf-8")
    print("lock appended XIX-XXVI")
...
def append_translations(LATIN, TITLES, PASS_B, NOTES):
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        n = int(sid)
        if 96 <= n <= 103:
            notes = f"Section {sid}: new densify Pars III XIX-XXVI; Pass A/B checked."
        else:
            notes = (
                f"Section {sid}: prior verified densify retained; Pass A/B and lock identity "
                "rechecked in Pars III XIX-XXVI packet scope covering all current sections."
            )
        reviews.append(
            {
                "section": sid,
    ...
    receipt = {
        ...
        "scope_review": {
            ...
            "uncertainties": [],
            "notes": (
                "Scope review: densify Pars III XIX-XXVI only (sections 96-103). Meta discloses "
                f"{RANGE_SHORT}. Honest partial; Pars III XXVII+ and Part IV remain. "
...
def prepend_handoff(before: int, after: int):
    entry = (
        ...
        f"- Before: **{before}** (Pars I I–XLVII + Pars II I–XXXV + Pars III I–XVIII live). "
        f"After: **{after}** (contiguous Pars III XIX–XXVI → §§96–103).\n"
        ...
        "`sources/_le_blanc_authoritate_latin_lock.txt` (PDF 48–50 / book pp. 36–38).\n"
        f"- Meta range bumped to **{RANGE_SHORT}** (title, edition, blurb, text_history.method).\n"
        "- Honest **partial**: Pars III through XXVI (real status controversiae; first external "
        "rule is Scripture). XXVII+ (Scripture better known than Church; further proofs) and "
        "Part IV remain. De Theologia closed. De Fide I–XXII untouched. Not folio. Not shipped this slice.\n"
        "- Pass A ≠ B for §§96–103. Jude/Enoch and baptism formula named without chapter:verse. "
        "XXIV mutuam sumptam / XX page-turn restored (translator_notes).\n"
    ...
    if "Pars III XIX–XXVI densify" in prev[:1200]:
...
def main():
    LATIN, TITLES, PASS_A, PASS_B, NOTES, BIBLE, LEMMAS, CHOICES = load_data()
    ...
    append_translations(LATIN, TITLES, PASS_B, NOTES)
    update_meta()
    tip_ready()
