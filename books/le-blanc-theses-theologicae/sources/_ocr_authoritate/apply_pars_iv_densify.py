# -*- coding: utf-8 -*-
"""Pars IV I-VIII densify: lock Latin, append secs 119-126, justifications, packet, review, handoff."""
...
PACKET_STEM = "authoritate_scripturae_pars_iv_densify"
HANDOFF = BOOK / "SESSION_HANDOFF.md"
DATA = Path("/tmp/leblanc_pars4")

SECS = ["119", "120", "121", "122", "123", "124", "125", "126"]

RANGE_SHORT = "Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-VIII"
RANGE_TITLE = (
    "Theological Theses (De Theologia I-XLIII; De Fide I-XXII; "
    "De Authoritate Scripturae Pars I I-XLVII; Pars II I-XXXV; Pars III I-XLI; Pars IV I-VIII)"
)
EDITION = (
    ...
    "De Authoritate Scripturae Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + "
    "Pars IV I-VIII (not whole folio)."
)
BLURB = (
    ...
    "Scripturae Pars I through XLVII, Pars II through XXXV, Pars III through XLI, and "
    "Pars IV through VIII (adversaries' first argument: Scripture cannot prove Scripture; "
    "retorsion; first-principle reply; Spirit efficacy; divinity marks). Not the collected folio."
)
METHOD = (
    "English follows the locked 1675 Pitt Latin of De Theologia I-XLIII, De Fide I-XXII, "
    "De Authoritate Scripturae Pars I I-XLVII, Pars II I-XXXV, Pars III I-XLI, and Pars IV I-VIII, "
    ...
    "through XLI; Pars IV through VIII (adversaries' first argument and first clarifications). "
    "Pars IV IX+ remains."
)

LOCK_HEADER = (
    "\n---\n\n"
    "Expand lock: De Authoritate Scripturae Pars IV theses I-VIII "
    "(contiguous after Pars III XLI - adversaries' arguments open; first argument "
    "Scripture cannot prove Scripture; retorsion; first-principle form; Spirit efficacy; "
    "divinity marks caveat).\n"
    "Same 1675 Pitt copy-text. Book pp. 43-44 / PDF 55-56. pdftotext + tesseract checks. "
    "Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: Part IV open through VIII only (natural close after first caveat on self-proof). "
    "Pars IV IX+ remains.\n"
    "Note: I open/excutiamus; II Apocrypha self-claim; III retorqueri; IV major false / "
    "sistendum; V inevidentia / a posteriori ad hominem; VI mutuatur; VII summa responsionis; "
    "VIII dextre / Aristotle Euclid restored from damaged OCR.\n\n"
)
...
def append_lock(LATIN):
    ...
    if "Expand lock: De Authoritate Scripturae Pars IV theses I-VIII" in text:
        print("lock already expanded")
        return
    blob = LOCK_HEADER + "\n".join(LATIN[s] + "\n" for s in SECS) + "\n"
    LOCK_PATH.write_text(text.rstrip() + "\n" + blob, encoding="utf-8")
    print("lock appended Pars IV I-VIII")
...
def append_translations(LATIN, TITLES, PASS_B, NOTES, ALLUSIONS):
    ...
    if len(eng) != 118 or str(eng[-1]["section"]) != "118":
        raise SystemExit(f"refuse append: tip not 118 (eng={len(eng)} last={eng[-1].get('section')})")
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        n = int(sid)
        if 119 <= n <= 126:
            notes = f"Section {sid}: new densify Pars IV I-VIII; Pass A/B checked."
        else:
            notes = (
                f"Section {sid}: prior verified densify retained; Pass A/B and lock identity "
                "rechecked in Pars IV I-VIII packet scope covering all current sections."
            )
        reviews.append(
            {
                "section": sid,
    ...
    receipt = {
        ...
        "scope_review": {
            ...
            "uncertainties": [],
            "notes": (
                "Scope review: densify Pars IV I-VIII only (sections 119-126). Meta discloses "
                f"{RANGE_SHORT}. Honest partial; Pars IV IX+ remains. "
...
def prepend_handoff(before: int, after: int):
    entry = (
        ...
        f"- Before: **{before}** (Pars I I–XLVII + Pars II I–XXXV + Pars III I–XLI live). "
        f"After: **{after}** (contiguous Pars IV I–VIII → §§119–126).\n"
        ...
        "`sources/_le_blanc_authoritate_latin_lock.txt` (PDF 55–56 / book pp. 43–44).\n"
        f"- Meta range bumped to **{RANGE_SHORT}** (title, edition, blurb, text_history.method).\n"
        "- Honest **partial**: Pars IV through VIII (adversaries' first argument: Scripture "
        "cannot prove Scripture; retorsion; first-principle form; Spirit efficacy; divinity "
        "marks caveat). Pars IV IX+ remains. De Theologia closed. De Fide I–XXII untouched. "
        "Not folio. Not shipped this slice.\n"
        "- Pass A ≠ B for §§119–126. Aristotle/Euclid analogy named. OCR restorations noted "
        "in translator_notes.\n"
    ...
    if "Pars IV I–VIII densify" in prev[:1500]:
...
def main():
    ...
    if before != 118:
        raise SystemExit(f"refuse: expected tip 118 before append, got {before}")
    append_lock(LATIN)
