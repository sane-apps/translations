# -*- coding: utf-8 -*-
"""Pars III XXXV-XLI densify: lock Latin, append secs 112-118, justifications, packet, review, handoff."""
...
PACKET_STEM = "authoritate_scripturae_pars_iii_xxxv_densify"
HANDOFF = BOOK / "SESSION_HANDOFF.md"
DATA = Path("/tmp/leblanc_xxxv")

SECS = ["112", "113", "114", "115", "116", "117", "118"]

RANGE_SHORT = "Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI"
RANGE_TITLE = (
...
BLURB = (
    ...
    "Scripturae Pars I through XLVII, Pars II through XXXV, and Pars III through XLI "
    "(Symbol reply; pre-conciliar Canon; Stapleton tacit testimony; disputed books; "
    "prophets vs priests; pastors own certainty). Not the collected folio."
)
METHOD = (
    "English follows the locked 1675 Pitt Latin of De Theologia I-XLIII, De Fide I-XXII, "
    "De Authoritate Scripturae Pars I I-XLVII, Pars II I-XXXV, and Pars III I-XLI, "
    ...
    "through XLI (I-XXXIV retained; XXXV-XLI Symbol reply, pre-conciliar Canon, Stapleton "
    "tacit testimony, disputed books, prophets vs priests, pastors own certainty). "
    "Part IV remains."
)

LOCK_HEADER = (
    "\n---\n\n"
    "Expand lock: De Authoritate Scripturae Pars III theses XXXV-XLI "
    "(contiguous after XXVII-XXXIV - Stapleton Symbol reply; pre-conciliar Canon; "
    "tradition; tacit testimony; disputed books; prophets vs priests; pastors certainty).\n"
    "Same 1675 Pitt copy-text. Book pp. 41-43 / PDF 53-56. pdftotext + DjVu + tesseract "
    "checks. Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: Part III continue through XLI only (natural close before Pars IV opens). "
    "Part IV remains.\n"
    "Note: XXXV Symbol/Creed; XXXVI Laodicenum; XXXVII tradition; XXXVIII duplex "
    "testimonium; XXXIX Eusebius Greek + Tertullian + Hieronymus ad Dardanum; XL Jeremiah "
    "8/18/36; XLI pastors as Church circularity restored from damaged OCR.\n\n"
)
...
def append_lock(LATIN):
    ...
    if "Expand lock: De Authoritate Scripturae Pars III theses XXXV-XLI" in text:
        print("lock already expanded")
        return
    blob = LOCK_HEADER + "\n".join(LATIN[s] + "\n" for s in SECS) + "\n"
    LOCK_PATH.write_text(text.rstrip() + "\n" + blob, encoding="utf-8")
    print("lock appended XXXV-XLI")
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        n = int(sid)
        if 112 <= n <= 118:
            notes = f"Section {sid}: new densify Pars III XXXV-XLI; Pass A/B checked."
        else:
            notes = (
                f"Section {sid}: prior verified densify retained; Pass A/B and lock identity "
                "rechecked in Pars III XXXV-XLI packet scope covering all current sections."
            )
        reviews.append(
            {
                "section": sid,
    ...
    receipt = {
        ...
        "scope_review": {
            ...
            "uncertainties": [],
            "notes": (
                "Scope review: densify Pars III XXXV-XLI only (sections 112-118). Meta discloses "
                f"{RANGE_SHORT}. Honest partial; Part IV remains. "
...
def prepend_handoff(before: int, after: int):
    entry = (
        ...
        f"- Before: **{before}** (Pars I I–XLVII + Pars II I–XXXV + Pars III I–XXXIV live). "
        f"After: **{after}** (contiguous Pars III XXXV–XLI → §§112–118).\n"
        ...
        "`sources/_le_blanc_authoritate_latin_lock.txt` (PDF 53–56 / book pp. 41–43).\n"
        f"- Meta range bumped to **{RANGE_SHORT}** (title, edition, blurb, text_history.method).\n"
        "- Honest **partial**: Pars III through XLI (Symbol/Creed reply; pre-conciliar Canon; "
        "tradition; Stapleton tacit testimony; disputed NT books; prophets vs priests; pastors "
        "own certainty). Part IV remains. De Theologia closed. De Fide I–XXII untouched. Not "
        "folio. Not shipped this slice.\n"
        "- Pass A ≠ B for §§112–118. Inline Jeremiah 8 / 18 / 36; Eusebius Greek; Tertullian; "
        "Jerome to Dardanus named. OCR restorations noted in translator_notes.\n"
    ...
    if "Pars III XXXV–XLI densify" in prev[:1500]:
