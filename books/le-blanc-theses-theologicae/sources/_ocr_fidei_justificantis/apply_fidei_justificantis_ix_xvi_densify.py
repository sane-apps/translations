#!/usr/bin/env python3
"""Build + apply De Fidei justificantis natura IX-XVI densify (tip 684 → 692).

Rivetus→Placeus→Zanchi→Thorun/English→definition variance. Live floor 5604.
After tip-ready: HOLD live>5604 OR 12m. Punch X=NO. No ship.
"""
...
DATA = Path('/tmp/leblanc_fidei_ix_xvi_densify')
PAYLOAD = Path('/tmp/fidei_ix_xvi_payload.json')
PACKET_STEM = 'fidei_justificantis_ix_xvi_densify'
APPLY_COPY = BOOK / 'sources/_ocr_fidei_justificantis/apply_fidei_justificantis_ix_xvi_densify.py'
...
SECS = ['685', '686', '687', '688', '689', '690', '691', '692']
ROMANS = {
    '685': 'IX', '686': 'X', '687': 'XI', '688': 'XII',
    '689': 'XIII', '690': 'XIV', '691': 'XV', '692': 'XVI',
}
TIP_BEFORE = 684
PRIOR_START = 599
LIVE_FLOOR = 5604
...
RANGE_SHORT = (
    ...
    "De Fidei justificantis natura I-XVI"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine I-XLVIII + Gratia I-XXX + De Fidei justificantis natura I-XVI (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Fidei justificantis natura I-XVI. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Fidei justificantis natura I-XVI. "
    "1675 PDF 217-219 for IX-XVI. No modern English. "
    "This slice densifies Fidei justificantis IX-XVI (Rivetus/Moulin through confessional defs + variance)."
)
LOCK_HEADER = (
    "Le Blanc Latin lock — De Fidei justificantis natura\n"
    "Edition: Theses theologicae (London: Moses Pitt, 1675). ESTC R17887.\n"
    "Scope: De Fidei justificantis natura I-XVI tip. Next: Fidei justificantis XVII+.\n"
    "Source: 1675 PDF pages 216-219 (book ~191-194).\n"
...
def write_lock(new_latin: dict):
    """Preserve I-VIII latin already in lock; append/replace IX-XVI."""
    prior = {}
    if LOCK.exists():
        text = LOCK.read_text(encoding='utf-8')
        # strip header lines until first roman numeral thesis
        body = text
        for marker in ('I. ', 'I.\n'):
            idx = text.find(marker)
            if idx >= 0 and (idx == 0 or text[idx-1] == '\n' or 'Scope:' in text[:idx]):
                # find first thesis start more carefully
                break
        m = re.search(r'(?m)^(I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|XIII|XIV|XV|XVI)\.\s', text)
        if m:
            body = text[m.start():]
        chunks = re.split(r'(?m)(?=^(?:I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|XIII|XIV|XV|XVI)\.\s)', body)
        roman_to_n = {
        ...
        for ch in chunks:
            ...
            if not ch:
                continue
            rm = re.match(r'^(I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|XIII|XIV|XV|XVI)\.\s', ch)
            if not rm:
                continue
            sec = roman_to_n.get(rm.group(1))
            if sec:
                prior[sec] = ' '.join(ch.split())
    # Prefer existing I-VIII from prior lock; overlay new IX-XVI
    merged = dict(prior)
    merged.update({k: ' '.join(new_latin[k].split()) for k in SECS})
    order = ['677', '678', '679', '680', '681', '682', '683', '684'] + SECS
    parts = [LOCK_HEADER]
    for sec in order:
        if sec not in merged:
            raise SystemExit('lock missing section %s after merge' % sec)
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 685 <= n <= 692:
            notes = (
                'Section %s: new densify De Fidei justificantis natura IX-XVI; '
                'Pass A!=B; lock-grounded 1675 PDF 217-219; Rivetus through confessional defs.' % sid
            )
        else:
            notes = (
                'Section %s: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in fidei_justificantis_ix_xvi packet scope covering all current sections.'
                % sid
            )
        reviews.append({
            'section': sid,
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Fidei justificantis natura IX-XVI only '
                '(sections %s-%s). Meta discloses %s. '
                'Next De Fidei justificantis natura XVII+. Not shipped.'
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Fidei justificantis natura IX-XVI '
            '(Rivetus/Moulin; Maresius/Wendelin/Alting; Bucanus; Ames/Wittich/Mestrezat; '
            'Vermigli/Musculus/Placeus; Zanchi; Thorun/English confessions; definition variance)'
        ),
        'next_locus': (
            'De Fidei justificantis natura XVII+ '
            '(complete vs minimal faith; further analysis)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Fidei justificantis natura IX-XVI densify LOCAL)\n\n'
        '- Before: **%s**. After: **%s** (contiguous De Fidei justificantis natura IX-XVI → §§%s–%s).\n'
    ...
    repo_block = (
        '## %s ~ET (Scribe — Le Blanc De Fidei justificantis IX-XVI densify)\n\n'
        'CoS densify: De Fidei justificantis natura IX-XVI (**%s→%s**). Packet %s. '
        'Next: De Fidei justificantis natura XVII+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_692.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-692 post-ready')
