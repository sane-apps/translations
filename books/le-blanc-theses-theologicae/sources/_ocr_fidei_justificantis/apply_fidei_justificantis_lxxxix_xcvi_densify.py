#!/usr/bin/env python3
"""Build + apply De Fidei justificantis natura LXXXIX-XCVI densify (tip 764 → 772).

Placeus preferred; faith object; special mercy; notitia; assent; love; four fiducia senses.
Live floor 5926. After tip-ready: HOLD live>5926 OR 12m. Punch X=NO. No ship.
"""
...
DATA = Path('/tmp/leblanc_fidei_lxxxix_xcvi_densify')
PAYLOAD = Path('/tmp/fidei_lxxxix_xcvi_payload.json')
PACKET_STEM = 'fidei_justificantis_lxxxix_xcvi_densify'
APPLY_COPY = BOOK / 'sources/_ocr_fidei_justificantis/apply_fidei_justificantis_lxxxix_xcvi_densify.py'
...
SECS = ['765', '766', '767', '768', '769', '770', '771', '772']
ROMANS = {
    '765': 'LXXXIX', '766': 'XC', '767': 'XCI', '768': 'XCII',
    '769': 'XCIII', '770': 'XCIV', '771': 'XCV', '772': 'XCVI',
}
TIP_BEFORE = 764
LIVE_FLOOR = 5926
HOLD_MINUTES = 12
...
RANGE_SHORT = (
    ...
    "De Fidei justificantis natura I-XCVI"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine I-XLVIII + Gratia I-XXX + De Fidei justificantis natura I-XCVI (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Fidei justificantis natura I-XCVI. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Fidei justificantis natura I-XCVI. "
    "1675 PDF 234-236 for LXXXIX-XCVI. No modern English. "
    "This slice densifies Fidei justificantis LXXXIX-XCVI (Placeus; object; notitia; assent; love; fiducia senses)."
)
LOCK_HEADER = (
    "Le Blanc Latin lock — De Fidei justificantis natura\n"
    "Edition: Theses theologicae (London: Moses Pitt, 1675). ESTC R17887.\n"
    "Scope: De Fidei justificantis natura I-XCVI tip. Next: Fidei justificantis XCVII+.\n"
    "Source: 1675 PDF pages 216-236 (book ~191-211).\n"
...
ORDER = [str(n) for n in range(677, 773)]
...
def write_lock(new_latin: dict):
    """Merge prior lock/source latin with new SECS; write I–XCVI lock."""
    prior = {}
    ...
    if SRC.exists():
        for row in json.loads(SRC.read_text(encoding='utf-8')):
            ...
            if sec.isdigit() and 677 <= int(sec) <= 764 and lat:
                prior[sec] = ' '.join(lat.split())
    if LOCK.exists() and len(prior) < 32:
        text = LOCK.read_text(encoding='utf-8')
        long_rom = (
            'XCVI|XCV|XCIV|XCIII|XCII|XCI|XC|LXXXIX|LXXXVIII|LXXXVII|LXXXVI|LXXXV|LXXXIV|LXXXIII|LXXXII|LXXXI|LXXX|LXXIX|LXXVIII|LXXVII|LXXVI|LXXV|LXXIV|LXXIII|LXXII|LXXI|LXX|LXIX|LXVIII|LXVII|LXVI|LXV|LXIV|LXIII|LXII|LXI|LX|LIX|LVIII|LVII|LVI|LV|LIV|LIII|LII|LI|L|'
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 765 <= n <= 772:
            notes = (
                'Section %s: new densify De Fidei justificantis natura LXXXIX-XCVI; '
                'Pass A!=B; lock-grounded 1675 PDF 234-236; Placeus through four fiducia senses.'
                % sid
            )
        else:
            notes = (
                'Section %s: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in fidei_justificantis_lxxxix_xcvi packet scope covering all current sections.'
                % sid
            )
        reviews.append({
            'section': sid,
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Fidei justificantis natura LXXXIX-XCVI only '
                '(sections %s-%s). Meta discloses %s. '
                'Next De Fidei justificantis natura XCVII+. Not shipped.'
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Fidei justificantis natura LXXXIX-XCVI '
            '(Placeus preferred; object generic/special; special mercy wording; '
            'notitia; assent; love; four fiducia senses)'
        ),
        'next_locus': (
            'De Fidei justificantis natura XCVII+ '
            '(fiducia senses 3–4 as effects; refuge; grace-persuasion debate)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Fidei justificantis natura LXXXIX-XCVI densify LOCAL)\n\n'
        '- Before: **%s**. After: **%s** (contiguous De Fidei justificantis natura LXXXIX-XCVI → §§%s–%s).\n'
    ...
    repo_block = (
        '## %s ~ET (Scribe — Le Blanc De Fidei justificantis LXXXIX-XCVI densify)\n\n'
        'CoS densify: De Fidei justificantis natura LXXXIX-XCVI (**%s→%s**). Packet %s. '
        'Next: De Fidei justificantis natura XCVII+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_772.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-772 post-ready')
