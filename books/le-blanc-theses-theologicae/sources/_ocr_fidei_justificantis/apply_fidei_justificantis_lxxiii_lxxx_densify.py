#!/usr/bin/env python3
"""Build + apply De Fidei justificantis natura LXXIII-LXXX densify (tip 748 → 756).

Beza/Garissolius; application modes; Baron two acts; Bucanus; Perkins–Ames; whole-Word; English Confession; Placeus.
Live floor 5866. After tip-ready: HOLD live>5866 OR 12m. Punch X=NO. No ship.
"""
...
DATA = Path('/tmp/leblanc_fidei_lxxiii_lxxx_densify')
PAYLOAD = Path('/tmp/fidei_lxxiii_lxxx_payload.json')
PACKET_STEM = 'fidei_justificantis_lxxiii_lxxx_densify'
APPLY_COPY = BOOK / 'sources/_ocr_fidei_justificantis/apply_fidei_justificantis_lxxiii_lxxx_densify.py'
...
SECS = ['749', '750', '751', '752', '753', '754', '755', '756']
ROMANS = {
    '749': 'LXXIII', '750': 'LXXIV', '751': 'LXXV', '752': 'LXXVI',
    '753': 'LXXVII', '754': 'LXXVIII', '755': 'LXXIX', '756': 'LXXX',
}
TIP_BEFORE = 748
LIVE_FLOOR = 5866
HOLD_MINUTES = 12
...
RANGE_SHORT = (
    ...
    "De Fidei justificantis natura I-LXXX"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine I-XLVIII + Gratia I-XXX + De Fidei justificantis natura I-LXXX (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Fidei justificantis natura I-LXXX. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Fidei justificantis natura I-LXXX. "
    "1675 PDF 232-233 for LXXIII-LXXX. No modern English. "
    "This slice densifies Fidei justificantis LXXIII-LXXX (Beza/Garissolius; application; Baron; Bucanus; Perkins–Ames; Placeus)."
)
LOCK_HEADER = (
    "Le Blanc Latin lock — De Fidei justificantis natura\n"
    "Edition: Theses theologicae (London: Moses Pitt, 1675). ESTC R17887.\n"
    "Scope: De Fidei justificantis natura I-LXXX tip. Next: Fidei justificantis LXXXI+.\n"
    "Source: 1675 PDF pages 216-233 (book ~191-208).\n"
...
ORDER = [str(n) for n in range(677, 757)]
...
def write_lock(new_latin: dict):
    """Merge prior lock/source latin with new SECS; write I–LXXX lock."""
    prior = {}
    ...
    if SRC.exists():
        for row in json.loads(SRC.read_text(encoding='utf-8')):
            ...
            if sec.isdigit() and 677 <= int(sec) <= 748 and lat:
                prior[sec] = ' '.join(lat.split())
    if LOCK.exists() and len(prior) < 32:
        text = LOCK.read_text(encoding='utf-8')
        long_rom = (
            'LXXX|LXXIX|LXXVIII|LXXVII|LXXVI|LXXV|LXXIV|LXXIII|LXXII|LXXI|LXX|LXIX|LXVIII|LXVII|LXVI|LXV|LXIV|LXIII|LXII|LXI|LX|LIX|LVIII|LVII|LVI|LV|LIV|LIII|LII|LI|L|'
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 749 <= n <= 756:
            notes = (
                'Section %s: new densify De Fidei justificantis natura LXXIII-LXXX; '
                'Pass A!=B; lock-grounded 1675 PDF 232-233; Beza through Placeus.'
                % sid
            )
        else:
            notes = (
                'Section %s: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in fidei_justificantis_lxxiii_lxxx packet scope covering all current sections.'
                % sid
            )
        reviews.append({
            'section': sid,
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Fidei justificantis natura LXXIII-LXXX only '
                '(sections %s-%s). Meta discloses %s. '
                'Next De Fidei justificantis natura LXXXI+. Not shipped.'
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Fidei justificantis natura LXXIII-LXXX '
            '(Beza/Garissolius; application consensus; Baron two acts; Bucanus; '
            'Perkins–Ames refuge; whole-Word; English Confession; Placeus)'
        ),
        'next_locus': (
            'De Fidei justificantis natura LXXXI+ '
            '(historical vs saving faith; definitions; Placeus preferred)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Fidei justificantis natura LXXIII-LXXX densify LOCAL)\n\n'
        '- Before: **%s**. After: **%s** (contiguous De Fidei justificantis natura LXXIII-LXXX → §§%s–%s).\n'
        '- Packet %s. Punch X = NO. Not shipped.\n'
        '- Post tip-ready hold cleared (%s) live %s.\n'
        '- Next: De Fidei justificantis natura LXXXI+.\n\n'
        % (
            day,
            receipt['before'],
            receipt['after'],
    ...
    repo_block = (
        '## %s ~ET (Scribe — Le Blanc De Fidei justificantis LXXIII-LXXX densify)\n\n'
        'CoS densify: De Fidei justificantis natura LXXIII-LXXX (**%s→%s**). Packet %s. '
        'Next: De Fidei justificantis natura LXXXI+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_756.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-756 post-ready')
