#!/usr/bin/env python3
"""Build + apply De Fidei justificantis natura LXV-LXXII densify (tip 740 → 748).

Maresius threefold act; Paraeus special mercy; Rivetus/Baron/Ames; Alting; Chamier; Molinaeus.
Live floor 5838. After tip-ready: HOLD live>5838 OR 12m. Punch X=NO. No ship.
"""
...
DATA = Path('/tmp/leblanc_fidei_lxv_lxxii_densify')
PAYLOAD = Path('/tmp/fidei_lxv_lxxii_payload.json')
PACKET_STEM = 'fidei_justificantis_lxv_lxxii_densify'
APPLY_COPY = BOOK / 'sources/_ocr_fidei_justificantis/apply_fidei_justificantis_lxv_lxxii_densify.py'
...
SECS = ['741', '742', '743', '744', '745', '746', '747', '748']
ROMANS = {
    '741': 'LXV', '742': 'LXVI', '743': 'LXVII', '744': 'LXVIII',
    '745': 'LXIX', '746': 'LXX', '747': 'LXXI', '748': 'LXXII',
}
TIP_BEFORE = 740
LIVE_FLOOR = 5838
HOLD_MINUTES = 12
...
RANGE_SHORT = (
    ...
    "De Fidei justificantis natura I-LXXII"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine I-XLVIII + Gratia I-XXX + De Fidei justificantis natura I-LXXII (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Fidei justificantis natura I-LXXII. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Fidei justificantis natura I-LXXII. "
    "1675 PDF 230-232 for LXV-LXXII. No modern English. "
    "This slice densifies Fidei justificantis LXV-LXXII (Maresius; Paraeus; Rivetus/Baron/Ames; Alting; Chamier; Molinaeus)."
)
LOCK_HEADER = (
    "Le Blanc Latin lock — De Fidei justificantis natura\n"
    "Edition: Theses theologicae (London: Moses Pitt, 1675). ESTC R17887.\n"
    "Scope: De Fidei justificantis natura I-LXXII tip. Next: Fidei justificantis LXXIII+.\n"
    "Source: 1675 PDF pages 216-232 (book ~191-207).\n"
...
ORDER = [str(n) for n in range(677, 749)]
...
def write_lock(new_latin: dict):
    """Merge prior lock/source latin with new SECS; write I–LXXII lock."""
    prior = {}
    ...
    if SRC.exists():
        for row in json.loads(SRC.read_text(encoding='utf-8')):
            ...
            if sec.isdigit() and 677 <= int(sec) <= 740 and lat:
                prior[sec] = ' '.join(lat.split())
    if LOCK.exists() and len(prior) < 32:
        text = LOCK.read_text(encoding='utf-8')
        long_rom = (
            'LXXII|LXXI|LXX|LXIX|LXVIII|LXVII|LXVI|LXV|LXIV|LXIII|LXII|LXI|LX|LIX|LVIII|LVII|LVI|LV|LIV|LIII|LII|LI|L|'
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 741 <= n <= 748:
            notes = (
                'Section %s: new densify De Fidei justificantis natura LXV-LXXII; '
                'Pass A!=B; lock-grounded 1675 PDF 230-232; Maresius through Molinaeus.'
                % sid
            )
        else:
            notes = (
                'Section %s: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in fidei_justificantis_lxv_lxxii packet scope covering all current sections.'
                % sid
            )
        reviews.append({
            'section': sid,
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Fidei justificantis natura LXV-LXXII only '
                '(sections %s-%s). Meta discloses %s. '
                'Next De Fidei justificantis natura LXXIII+. Not shipped.'
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Fidei justificantis natura LXV-LXXII '
            '(Maresius threefold; Paraeus special mercy; Rivetus/Baron/Ames; '
            'Alting; Chamier; Molinaeus eternal justif. & fiducia-fruit)'
        ),
        'next_locus': (
            'De Fidei justificantis natura LXXIII+ '
            '(Beza/Garissolius; application modes; Baron two acts)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Fidei justificantis natura LXV-LXXII densify LOCAL)\n\n'
        '- Before: **%s**. After: **%s** (contiguous De Fidei justificantis natura LXV-LXXII → §§%s–%s).\n'
    ...
    repo_block = (
        '## %s ~ET (Scribe — Le Blanc De Fidei justificantis LXV-LXXII densify)\n\n'
        'CoS densify: De Fidei justificantis natura LXV-LXXII (**%s→%s**). Packet %s. '
        'Next: De Fidei justificantis natura LXXIII+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_748.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-748 post-ready')
