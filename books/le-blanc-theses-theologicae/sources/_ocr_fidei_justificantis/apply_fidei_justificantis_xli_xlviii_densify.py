#!/usr/bin/env python3
"""Build + apply De Fidei justificantis natura XLI-XLVIII densify (tip 716 → 724).

Explicit vs implicit knowledge; fiducia form vs effect; Zanchi/Wittich/Ames.
Live floor 5751. After tip-ready: HOLD live>5751 OR 12m. Punch X=NO. No ship.
"""
...
DATA = Path('/tmp/leblanc_fidei_xli_xlviii_densify')
PAYLOAD = Path('/tmp/fidei_xli_xlviii_payload.json')
PACKET_STEM = 'fidei_justificantis_xli_xlviii_densify'
APPLY_COPY = BOOK / 'sources/_ocr_fidei_justificantis/apply_fidei_justificantis_xli_xlviii_densify.py'
...
SECS = ['717', '718', '719', '720', '721', '722', '723', '724']
ROMANS = {
    '717': 'XLI', '718': 'XLII', '719': 'XLIII', '720': 'XLIV',
    '721': 'XLV', '722': 'XLVI', '723': 'XLVII', '724': 'XLVIII',
}
TIP_BEFORE = 716
LIVE_FLOOR = 5751
HOLD_MINUTES = 12
...
RANGE_SHORT = (
    ...
    "De Fidei justificantis natura I-XLVIII"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine I-XLVIII + Gratia I-XXX + De Fidei justificantis natura I-XLVIII (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Fidei justificantis natura I-XLVIII. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Fidei justificantis natura I-XLVIII. "
    "1675 PDF 223-225 for XLI-XLVIII. No modern English. "
    "This slice densifies Fidei justificantis XLI-XLVIII (explicit/implicit knowledge; fiducia debates)."
)
LOCK_HEADER = (
    "Le Blanc Latin lock — De Fidei justificantis natura\n"
    "Edition: Theses theologicae (London: Moses Pitt, 1675). ESTC R17887.\n"
    "Scope: De Fidei justificantis natura I-XLVIIIVIII tip. Next: Fidei justificantis XLIX+.\n"
    "Source: 1675 PDF pages 216-225 (book ~191-200).\n"
...
ORDER = [str(n) for n in range(677, 725)]
...
def write_lock(new_latin: dict):
    ...
    if SRC.exists():
        for row in json.loads(SRC.read_text(encoding='utf-8')):
            ...
            if sec.isdigit() and 677 <= int(sec) <= 716 and lat:
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 717 <= n <= 724:
            notes = (
                'Section %s: new densify De Fidei justificantis natura XLI-XLVIII; '
                'Pass A!=B; lock-grounded 1675 PDF 223-225; explicit/implicit knowledge through Ames special mercy.'
                % sid
            )
        else:
            notes = (
                'Section %s: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in fidei_justificantis_xli_xlviii packet scope covering all current sections.'
                % sid
            )
        reviews.append({
            'section': sid,
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'notes': (
                ...
                'Next De Fidei justificantis natura XLIX+. Not shipped.'
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Fidei justificantis natura XLI-XLVIII '
            '(explicit/implicit knowledge; Garissol/Beza fiducia as effect; '
            'Zanchi; Wittich/Ames fiducia senses; special mercy)'
        ),
        'next_locus': (
            'De Fidei justificantis natura XLIX+ '
            '(remaining fiducia/act analysis)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Fidei justificantis natura XLI-XLVIII densify LOCAL)\n\n'
        ...
        '- Next: De Fidei justificantis natura XLIX+.\n\n'
        % (
            day,
            receipt['before'],
            receipt['after'],
    ...
    repo_block = (
        '## %s ~ET (Scribe — Le Blanc De Fidei justificantis XLI-XLVIII densify)\n\n'
        'CoS densify: De Fidei justificantis natura XLI-XLVIII (**%s→%s**). Packet %s. '
        'Next: De Fidei justificantis natura XLIX+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_724.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-724 post-ready')
