#!/usr/bin/env python3
"""Build + apply De Fidei justificantis natura XCVII-CIV densify (tip 772 → 780).

Fiducia senses 3–4; refuge; grace-persuasion debate; Le Blanc; pastoral; Baron inconsistency; Chamier.
Live floor 5955. After tip-ready: HOLD live>5955 OR 12m. Punch X=NO. No ship.
"""
...
DATA = Path('/tmp/leblanc_fidei_xcvii_civ_densify')
PAYLOAD = Path('/tmp/fidei_xcvii_civ_payload.json')
PACKET_STEM = 'fidei_justificantis_xcvii_civ_densify'
APPLY_COPY = BOOK / 'sources/_ocr_fidei_justificantis/apply_fidei_justificantis_xcvii_civ_densify.py'
...
SECS = ['773', '774', '775', '776', '777', '778', '779', '780']
ROMANS = {
    '773': 'XCVII', '774': 'XCVIII', '775': 'XCIX', '776': 'C',
    '777': 'CI', '778': 'CII', '779': 'CIII', '780': 'CIV',
}
TIP_BEFORE = 772
LIVE_FLOOR = 5955
HOLD_MINUTES = 12
...
RANGE_SHORT = (
    ...
    "De Fidei justificantis natura I-CIV"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine I-XLVIII + Gratia I-XXX + De Fidei justificantis natura I-CIV (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Fidei justificantis natura I-CIV. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Fidei justificantis natura I-CIV. "
    "1675 PDF 236-238 for XCVII-CIV. No modern English. "
    "This slice densifies Fidei justificantis XCVII-CIV (fiducia senses; grace-persuasion; Chamier)."
)
LOCK_HEADER = (
    "Le Blanc Latin lock — De Fidei justificantis natura\n"
    "Edition: Theses theologicae (London: Moses Pitt, 1675). ESTC R17887.\n"
    "Scope: De Fidei justificantis natura I-CIV tip. Next: Fidei justificantis CV+.\n"
    "Source: 1675 PDF pages 216-238 (book ~191-213).\n"
...
ORDER = [str(n) for n in range(677, 781)]
...
def write_lock(new_latin: dict):
    """Merge prior lock/source latin with new SECS; write I–CIV lock."""
    prior = {}
    ...
    if SRC.exists():
        for row in json.loads(SRC.read_text(encoding='utf-8')):
            ...
            if sec.isdigit() and 677 <= int(sec) <= 772 and lat:
                prior[sec] = ' '.join(lat.split())
    if LOCK.exists() and len(prior) < 32:
        text = LOCK.read_text(encoding='utf-8')
        long_rom = (
            'CIV|CIII|CII|CI|C|XCIX|XCVIII|XCVII|XCVI|XCV|XCIV|XCIII|XCII|XCI|XC|LXXXIX|LXXXVIII|LXXXVII|LXXXVI|LXXXV|LXXXIV|LXXXIII|LXXXII|LXXXI|LXXX|LXXIX|LXXVIII|LXXVII|LXXVI|LXXV|LXXIV|LXXIII|LXXII|LXXI|LXX|LXIX|LXVIII|LXVII|LXVI|LXV|LXIV|LXIII|LXII|LXI|LX|LIX|LVIII|LVII|LVI|LV|LIV|LIII|LII|LI|L|'
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 773 <= n <= 780:
            notes = (
                'Section %s: new densify De Fidei justificantis natura XCVII-CIV; '
                'Pass A!=B; lock-grounded 1675 PDF 236-238; fiducia senses through Chamier.'
                % sid
            )
        else:
            notes = (
                'Section %s: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in fidei_justificantis_xcvii_civ packet scope covering all current sections.'
                % sid
            )
        reviews.append({
            'section': sid,
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Fidei justificantis natura XCVII-CIV only '
                '(sections %s-%s). Meta discloses %s. '
                'Next De Fidei justificantis natura CV+. Not shipped.'
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Fidei justificantis natura XCVII-CIV '
            '(fiducia 3–4 fruits; refuge; grace-persuasion debate; Le Blanc; '
            'pastoral; Baron inconsistency; Chamier reductio)'
        ),
        'next_locus': (
            'De Fidei justificantis natura CV+ '
            '(threefold fiducia acts; instant justification; formal act)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Fidei justificantis natura XCVII-CIV densify LOCAL)\n\n'
        '- Before: **%s**. After: **%s** (contiguous De Fidei justificantis natura XCVII-CIV → §§%s–%s).\n'
        '- Packet %s. Punch X = NO. Not shipped.\n'
        '- Post tip-ready hold cleared (%s) live %s.\n'
        '- Next: De Fidei justificantis natura CV+.\n\n'
        % (
            day,
            receipt['before'],
            receipt['after'],
    ...
    repo_block = (
        '## %s ~ET (Scribe — Le Blanc De Fidei justificantis XCVII-CIV densify)\n\n'
        'CoS densify: De Fidei justificantis natura XCVII-CIV (**%s→%s**). Packet %s. '
        'Next: De Fidei justificantis natura CV+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_780.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-780 post-ready')
