#!/usr/bin/env python3
"""Build + apply De Fidei justificantis natura XXXIII-XL densify (tip 708 → 716).

Perkins/Ames/Moulin on assurance vs essence; English Confession; love-as-act; notitia.
Live floor 5721. After tip-ready: HOLD live>5721 OR 12m. Punch X=NO. No ship.
"""
...
DATA = Path('/tmp/leblanc_fidei_xxxiii_xl_densify')
PAYLOAD = Path('/tmp/fidei_xxxiii_xl_payload.json')
PACKET_STEM = 'fidei_justificantis_xxxiii_xl_densify'
APPLY_COPY = BOOK / 'sources/_ocr_fidei_justificantis/apply_fidei_justificantis_xxxiii_xl_densify.py'
...
SECS = ['709', '710', '711', '712', '713', '714', '715', '716']
ROMANS = {
    '709': 'XXXIII', '710': 'XXXIV', '711': 'XXXV', '712': 'XXXVI',
    '713': 'XXXVII', '714': 'XXXVIII', '715': 'XXXIX', '716': 'XL',
}
TIP_BEFORE = 708
LIVE_FLOOR = 5721
HOLD_MINUTES = 12
...
RANGE_SHORT = (
    ...
    "De Fidei justificantis natura I-XL"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine I-XLVIII + Gratia I-XXX + De Fidei justificantis natura I-XL (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Fidei justificantis natura I-XL. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Fidei justificantis natura I-XL. "
    "1675 PDF 222-223 for XXXIII-XL. No modern English. "
    "This slice densifies Fidei justificantis XXXIII-XL (assurance vs essence; love debate; notitia)."
)
LOCK_HEADER = (
    "Le Blanc Latin lock — De Fidei justificantis natura\n"
    "Edition: Theses theologicae (London: Moses Pitt, 1675). ESTC R17887.\n"
    "Scope: De Fidei justificantis natura I-XL tip. Next: Fidei justificantis XLI+.\n"
    "Source: 1675 PDF pages 216-223 (book ~191-198).\n"
...
ORDER = [str(n) for n in range(677, 717)]
...
def write_lock(new_latin: dict):
    ...
    if SRC.exists():
        for row in json.loads(SRC.read_text(encoding='utf-8')):
            ...
            if sec.isdigit() and 677 <= int(sec) <= 708 and lat:
                prior[sec] = ' '.join(lat.split())
    if LOCK.exists() and len(prior) < 32:
        text = LOCK.read_text(encoding='utf-8')
        long_rom = (
            'XXXVIII|XXXVII|XXXVI|XXXV|XXXIV|XXXIII|XXXII|XXXI|XXX|XXIX|'
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 709 <= n <= 716:
            notes = (
                'Section %s: new densify De Fidei justificantis natura XXXIII-XL; '
                'Pass A!=B; lock-grounded 1675 PDF 222-223; Perkins through notitia consensus.'
                % sid
            )
        else:
            notes = (
                'Section %s: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in fidei_justificantis_xxxiii_xl packet scope covering all current sections.'
                % sid
            )
        reviews.append({
            'section': sid,
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'notes': (
                ...
                'Next De Fidei justificantis natura XLI+. Not shipped.'
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Fidei justificantis natura XXXIII-XL '
            '(Perkins/Ames/Moulin assurance vs essence; English Confession; '
            'Wendelin/Chamier/Cocceius love; notitia consensus)'
        ),
        'next_locus': (
            'De Fidei justificantis natura XLI+ '
            '(explicit vs implicit knowledge; fiducia as form or effect)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Fidei justificantis natura XXXIII-XL densify LOCAL)\n\n'
        ...
        '- Next: De Fidei justificantis natura XLI+.\n\n'
        % (
            day,
            receipt['before'],
            receipt['after'],
    ...
    repo_block = (
        '## %s ~ET (Scribe — Le Blanc De Fidei justificantis XXXIII-XL densify)\n\n'
        'CoS densify: De Fidei justificantis natura XXXIII-XL (**%s→%s**). Packet %s. '
        'Next: De Fidei justificantis natura XLI+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_716.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-716 post-ready')
