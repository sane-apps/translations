#!/usr/bin/env python3
"""Build + apply De Fidei justificantis natura XLIX-LVI densify (tip 724 → 732).

Ames object Christ; Davenant/Moulin refuge vs sense; Perkins/Walaeus; Confession; Wendelin/Rivetus.
Live floor 5780. After tip-ready: HOLD live>5780 OR 12m. Punch X=NO. No ship.
"""
...
DATA = Path('/tmp/leblanc_fidei_xlix_lvi_densify')
PAYLOAD = Path('/tmp/fidei_xlix_lvi_payload.json')
PACKET_STEM = 'fidei_justificantis_xlix_lvi_densify'
APPLY_COPY = BOOK / 'sources/_ocr_fidei_justificantis/apply_fidei_justificantis_xlix_lvi_densify.py'
...
SECS = ['725', '726', '727', '728', '729', '730', '731', '732']
ROMANS = {
    '725': 'XLIX', '726': 'L', '727': 'LI', '728': 'LII',
    '729': 'LIII', '730': 'LIV', '731': 'LV', '732': 'LVI',
}
TIP_BEFORE = 724
LIVE_FLOOR = 5780
HOLD_MINUTES = 12
...
RANGE_SHORT = (
    ...
    "De Fidei justificantis natura I-LVI"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine I-XLVIII + Gratia I-XXX + De Fidei justificantis natura I-LVI (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Fidei justificantis natura I-LVI. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Fidei justificantis natura I-LVI. "
    "1675 PDF 226-228 for XLIX-LVI. No modern English. "
    "This slice densifies Fidei justificantis XLIX-LVI (refuge vs sense fiducia; Ames/Davenant/Moulin)."
)
LOCK_HEADER = (
    "Le Blanc Latin lock — De Fidei justificantis natura\n"
    "Edition: Theses theologicae (London: Moses Pitt, 1675). ESTC R17887.\n"
    "Scope: De Fidei justificantis natura I-LVI tip. Next: Fidei justificantis LVII+.\n"
    "Source: 1675 PDF pages 216-228 (book ~191-203).\n"
...
ORDER = [str(n) for n in range(677, 733)]
...
def write_lock(new_latin: dict):
    ...
    if SRC.exists():
        for row in json.loads(SRC.read_text(encoding='utf-8')):
            ...
            if sec.isdigit() and 677 <= int(sec) <= 724 and lat:
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 725 <= n <= 732:
            notes = (
                'Section %s: new densify De Fidei justificantis natura XLIX-LVI; '
                'Pass A!=B; lock-grounded 1675 PDF 226-228; Ames/Davenant/Moulin refuge vs sense through Rivetus/Sohn.'
                % sid
            )
        else:
            notes = (
                'Section %s: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in fidei_justificantis_xlix_lvi packet scope covering all current sections.'
                % sid
            )
        reviews.append({
            'section': sid,
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'notes': (
                ...
                'Next De Fidei justificantis natura LVII+. Not shipped.'
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Fidei justificantis natura XLIX-LVI '
            '(Ames object Christ; Davenant/Moulin refuge vs sense; '
            'Perkins/Walaeus; English Confession; Wendelin/Rivetus/Sohn)'
        ),
        'next_locus': (
            'De Fidei justificantis natura LVII+ '
            '(Paraeus; Baron on senses of fiducia)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Fidei justificantis natura XLIX-LVI densify LOCAL)\n\n'
        ...
        '- Next: De Fidei justificantis natura LVII+.\n\n'
        % (
            day,
            receipt['before'],
            receipt['after'],
    ...
    repo_block = (
        '## %s ~ET (Scribe — Le Blanc De Fidei justificantis XLIX-LVI densify)\n\n'
        'CoS densify: De Fidei justificantis natura XLIX-LVI (**%s→%s**). Packet %s. '
        'Next: De Fidei justificantis natura LVII+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_732.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-732 post-ready')
