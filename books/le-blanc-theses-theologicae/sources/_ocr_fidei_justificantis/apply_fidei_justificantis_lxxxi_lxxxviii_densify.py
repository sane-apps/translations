#!/usr/bin/env python3
"""Build + apply De Fidei justificantis natura LXXXI-LXXXVIII densify (tip 756 → 764).

Historical vs saving; genus quarrel; definition families; Calvin/Beza; Ames refuge; critique; English Confession 1647.
Live floor 5896. After tip-ready: HOLD live>5896 OR 12m. Punch X=NO. No ship.
"""
...
DATA = Path('/tmp/leblanc_fidei_lxxxi_lxxxviii_densify')
PAYLOAD = Path('/tmp/fidei_lxxxi_lxxxviii_payload.json')
PACKET_STEM = 'fidei_justificantis_lxxxi_lxxxviii_densify'
APPLY_COPY = BOOK / 'sources/_ocr_fidei_justificantis/apply_fidei_justificantis_lxxxi_lxxxviii_densify.py'
...
SECS = ['757', '758', '759', '760', '761', '762', '763', '764']
ROMANS = {
    '757': 'LXXXI', '758': 'LXXXII', '759': 'LXXXIII', '760': 'LXXXIV',
    '761': 'LXXXV', '762': 'LXXXVI', '763': 'LXXXVII', '764': 'LXXXVIII',
}
TIP_BEFORE = 756
LIVE_FLOOR = 5896
HOLD_MINUTES = 12
...
RANGE_SHORT = (
    ...
    "De Fidei justificantis natura I-LXXXVIII"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine I-XLVIII + Gratia I-XXX + De Fidei justificantis natura I-LXXXVIII (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Fidei justificantis natura I-LXXXVIII. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Fidei justificantis natura I-LXXXVIII. "
    "1675 PDF 233-234 for LXXXI-LXXXVIII. No modern English. "
    "This slice densifies Fidei justificantis LXXXI-LXXXVIII (historical vs saving; definitions; English Confession 1647)."
)
LOCK_HEADER = (
    "Le Blanc Latin lock — De Fidei justificantis natura\n"
    "Edition: Theses theologicae (London: Moses Pitt, 1675). ESTC R17887.\n"
    "Scope: De Fidei justificantis natura I-LXXXVIII tip. Next: Fidei justificantis LXXXIX+.\n"
    "Source: 1675 PDF pages 216-234 (book ~191-209).\n"
...
ORDER = [str(n) for n in range(677, 765)]
...
def write_lock(new_latin: dict):
    """Merge prior lock/source latin with new SECS; write I–LXXXVIII lock."""
    prior = {}
    ...
    if SRC.exists():
        for row in json.loads(SRC.read_text(encoding='utf-8')):
            ...
            if sec.isdigit() and 677 <= int(sec) <= 756 and lat:
                prior[sec] = ' '.join(lat.split())
    if LOCK.exists() and len(prior) < 32:
        text = LOCK.read_text(encoding='utf-8')
        long_rom = (
            'LXXXVIII|LXXXVII|LXXXVI|LXXXV|LXXXIV|LXXXIII|LXXXII|LXXXI|LXXX|LXXIX|LXXVIII|LXXVII|LXXVI|LXXV|LXXIV|LXXIII|LXXII|LXXI|LXX|LXIX|LXVIII|LXVII|LXVI|LXV|LXIV|LXIII|LXII|LXI|LX|LIX|LVIII|LVII|LVI|LV|LIV|LIII|LII|LI|L|'
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 757 <= n <= 764:
            notes = (
                'Section %s: new densify De Fidei justificantis natura LXXXI-LXXXVIII; '
                'Pass A!=B; lock-grounded 1675 PDF 233-234; historical vs saving through Eng. Conf. 1647.'
                % sid
            )
        else:
            notes = (
                'Section %s: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in fidei_justificantis_lxxxi_lxxxviii packet scope covering all current sections.'
                % sid
            )
        reviews.append({
            'section': sid,
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Fidei justificantis natura LXXXI-LXXXVIII only '
                '(sections %s-%s). Meta discloses %s. '
                'Next De Fidei justificantis natura LXXXIX+. Not shipped.'
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Fidei justificantis natura LXXXI-LXXXVIII '
            '(historical vs saving; genus; definition families; Calvin/Beza; '
            'Ames refuge; critique; English Confession 1647)'
        ),
        'next_locus': (
            'De Fidei justificantis natura LXXXIX+ '
            '(Placeus preferred def.; faith object; special mercy; parts/acts)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Fidei justificantis natura LXXXI-LXXXVIII densify LOCAL)\n\n'
        '- Before: **%s**. After: **%s** (contiguous De Fidei justificantis natura LXXXI-LXXXVIII → §§%s–%s).\n'
        '- Packet %s. Punch X = NO. Not shipped.\n'
        '- Post tip-ready hold cleared (%s) live %s.\n'
        '- Next: De Fidei justificantis natura LXXXIX+.\n\n'
        % (
            day,
            receipt['before'],
            receipt['after'],
    ...
    repo_block = (
        '## %s ~ET (Scribe — Le Blanc De Fidei justificantis LXXXI-LXXXVIII densify)\n\n'
        'CoS densify: De Fidei justificantis natura LXXXI-LXXXVIII (**%s→%s**). Packet %s. '
        'Next: De Fidei justificantis natura LXXXIX+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_764.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-764 post-ready')
