#!/usr/bin/env python3
"""Build + apply De Fidei justificantis natura XVII-XXIV densify (tip 692 → 700).

Complete vs minimal defs; two marks; object of faith; Rivetus/Maresius/Alting/Wendelin.
Live floor 5646. After tip-ready: HOLD live>5646 OR 12m. Punch X=NO. No ship.
"""
...
DATA = Path('/tmp/leblanc_fidei_xvii_xxiv_densify')
PAYLOAD = Path('/tmp/fidei_xvii_xxiv_payload.json')
PACKET_STEM = 'fidei_justificantis_xvii_xxiv_densify'
APPLY_COPY = BOOK / 'sources/_ocr_fidei_justificantis/apply_fidei_justificantis_xvii_xxiv_densify.py'
...
SECS = ['693', '694', '695', '696', '697', '698', '699', '700']
ROMANS = {
    '693': 'XVII', '694': 'XVIII', '695': 'XIX', '696': 'XX',
    '697': 'XXI', '698': 'XXII', '699': 'XXIII', '700': 'XXIV',
}
TIP_BEFORE = 692
LIVE_FLOOR = 5646
HOLD_MINUTES = 12
...
RANGE_SHORT = (
    ...
    "De Fidei justificantis natura I-XXIV"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine I-XLVIII + Gratia I-XXX + De Fidei justificantis natura I-XXIV (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Fidei justificantis natura I-XXIV. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Fidei justificantis natura I-XXIV. "
    "1675 PDF 219-220 for XVII-XXIV. No modern English. "
    "This slice densifies Fidei justificantis XVII-XXIV (complete vs minimal; two marks; object of faith)."
)
LOCK_HEADER = (
    "Le Blanc Latin lock — De Fidei justificantis natura\n"
    "Edition: Theses theologicae (London: Moses Pitt, 1675). ESTC R17887.\n"
    "Scope: De Fidei justificantis natura I-XXIV tip. Next: Fidei justificantis XXV+.\n"
    "Source: 1675 PDF pages 216-220 (book ~191-195).\n"
...
ORDER = [str(n) for n in range(677, 701)]
...
def write_lock(new_latin: dict):
    ...
    if LOCK.exists():
        ...
        chunks = re.split(
            r'(?m)(?=^(?:I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|XIII|XIV|XV|XVI|XVII|XVIII|XIX|XX|XXI|XXII|XXIII|XXIV)\.\s)',
            text,
        )
        for ch in chunks:
            ch = ch.strip()
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 693 <= n <= 700:
            notes = (
                'Section %s: new densify De Fidei justificantis natura XVII-XXIV; '
                'Pass A!=B; lock-grounded 1675 PDF 219-220; complete vs minimal through Wendelin object.'
                % sid
            )
        else:
            notes = (
                'Section %s: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in fidei_justificantis_xvii_xxiv packet scope covering all current sections.'
                % sid
            )
        reviews.append({
            'section': sid,
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'notes': (
                ...
                'Next De Fidei justificantis natura XXV+. Not shipped.'
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Fidei justificantis natura XVII-XXIV '
            '(complete vs minimal defs; two marks; object of faith; '
            'Rivetus/Maresius/Alting; particular fiducia; Wendelin special mercy)'
        ),
        'next_locus': (
            'De Fidei justificantis natura XXV+ '
            '(Paraeus/Ames quae/qua; Trelcatius; acts of faith)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Fidei justificantis natura XVII-XXIV densify LOCAL)\n\n'
    ...
    repo_block = (
        '## %s ~ET (Scribe — Le Blanc De Fidei justificantis XVII-XXIV densify)\n\n'
        'CoS densify: De Fidei justificantis natura XVII-XXIV (**%s→%s**). Packet %s. '
        'Next: De Fidei justificantis natura XXV+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_700.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-700 post-ready')
