#!/usr/bin/env python3
"""Build + apply De Fidei justificantis natura XXV-XXXII densify (tip 700 → 708).

Paraeus/Ames quae/qua; Trelcatius; acts; Musculus; Bucanus; Perkins; Ames; Wittich.
Live floor 5693. After tip-ready: HOLD live>5693 OR 12m. Punch X=NO. No ship.
"""
...
DATA = Path('/tmp/leblanc_fidei_xxv_xxxii_densify')
PAYLOAD = Path('/tmp/fidei_xxv_xxxii_payload.json')
PACKET_STEM = 'fidei_justificantis_xxv_xxxii_densify'
APPLY_COPY = BOOK / 'sources/_ocr_fidei_justificantis/apply_fidei_justificantis_xxv_xxxii_densify.py'
...
SECS = ['701', '702', '703', '704', '705', '706', '707', '708']
ROMANS = {
    '701': 'XXV', '702': 'XXVI', '703': 'XXVII', '704': 'XXVIII',
    '705': 'XXIX', '706': 'XXX', '707': 'XXXI', '708': 'XXXII',
}
TIP_BEFORE = 700
LIVE_FLOOR = 5693
HOLD_MINUTES = 12
...
RANGE_SHORT = (
    ...
    "De Fidei justificantis natura I-XXXII"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine I-XLVIII + Gratia I-XXX + De Fidei justificantis natura I-XXXII (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Fidei justificantis natura I-XXXII. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Fidei justificantis natura I-XXXII. "
    "1675 PDF 220-222 for XXV-XXXII. No modern English. "
    "This slice densifies Fidei justificantis XXV-XXXII (Paraeus/Ames; acts of faith; Wittich)."
)
LOCK_HEADER = (
    "Le Blanc Latin lock — De Fidei justificantis natura\n"
    "Edition: Theses theologicae (London: Moses Pitt, 1675). ESTC R17887.\n"
    "Scope: De Fidei justificantis natura I-XXXII tip. Next: Fidei justificantis XXXIII+.\n"
    "Source: 1675 PDF pages 216-222 (book ~191-197).\n"
...
ORDER = [str(n) for n in range(677, 709)]
...
def write_lock(new_latin: dict):
    ...
    if LOCK.exists():
        ...
        chunks = re.split(
            r'(?m)(?=^(?:I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|XIII|XIV|XV|XVI|XVII|XVIII|XIX|XX|XXI|XXII|XXIII|XXIV|XXV|XXVI|XXVII|XXVIII|XXIX|XXX|XXXI|XXXII)\.\s)',
            text,
        )
        for ch in chunks:
            ch = ch.strip()
            if not ch:
                continue
            rm = re.match(
                r'^(I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|XIII|XIV|XV|XVI|XVII|XVIII|XIX|XX|XXI|XXII|XXIII|XXIV)\.\s',
                ch,
            )
            if not rm:
                continue
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 701 <= n <= 708:
            notes = (
                'Section %s: new densify De Fidei justificantis natura XXV-XXXII; '
                'Pass A!=B; lock-grounded 1675 PDF 220-222; Paraeus/Ames through Wittich acts.'
                % sid
            )
        else:
            notes = (
                'Section %s: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in fidei_justificantis_xxv_xxxii packet scope covering all current sections.'
                % sid
            )
        reviews.append({
            'section': sid,
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'notes': (
                ...
                'Next De Fidei justificantis natura XXXIII+. Not shipped.'
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Fidei justificantis natura XXV-XXXII '
            '(Paraeus/Ames quae/qua; Trelcatius; three acts; Musculus; Bucanus; '
            'Perkins; Ames five concurrents; Wittich essential vs accidental)'
        ),
        'next_locus': (
            'De Fidei justificantis natura XXXIII+ '
            '(Perkins/Ames on implicit faith; Moulin; English Confession)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Fidei justificantis natura XXV-XXXII densify LOCAL)\n\n'
    ...
    repo_block = (
        '## %s ~ET (Scribe — Le Blanc De Fidei justificantis XXV-XXXII densify)\n\n'
        'CoS densify: De Fidei justificantis natura XXV-XXXII (**%s→%s**). Packet %s. '
        'Next: De Fidei justificantis natura XXXIII+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_708.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-708 post-ready')
