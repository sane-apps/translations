#!/usr/bin/env python3
"""Build + apply De Aeterna Hominum Electione XXIX-XXXVI densify (tip 589 → 597).

Comparative Roman/Reformed word-use; Scholastic end/means vs Reformed medium voice;
parallel effect/object disputes; logomachy; lexical rule.
Live floor 5243. After tip-ready: HOLD live>5243 OR 12m. Punch X=NO. No ship.
"""
...
SEED = Path('/tmp/leblanc_xxix_data')
PACKET_STEM = 'aeterna_electio_xxix_xxxvi_densify'
APPLY_COPY = BOOK / 'sources/_ocr_aeterna_electio/apply_aeterna_electio_xxix_xxxvi_densify.py'
...
SECS = ['590', '591', '592', '593', '594', '595', '596', '597']
ROMANS = {
    '590': 'XXIX', '591': 'XXX', '592': 'XXXI', '593': 'XXXII',
    '594': 'XXXIII', '595': 'XXXIV', '596': 'XXXV', '597': 'XXXVI',
}
TIP_BEFORE = 589
PRIOR_START = 562
LIVE_FLOOR = 5243
...
RANGE_SHORT = (
    ...
    "De Causa Praedestinationis I-XXXIV + De Aeterna Hominum Electione et Praedestinatione I-XXXVI"
)
RANGE_TITLE = (
    ...
    "De Aeterna Hominum Electione et Praedestinatione I-XXXVI)"
)
EDITION = (
    "Theses theologicae (London: Moses Pitt, 1675). ESTC R17887; Wing L802. IA bub_gb_eOkHAW4G0-wC. "
    "Densify through De Aeterna Hominum Electione et Praedestinatione I-XXXVI (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Aeterna Hominum Electione et Praedestinatione I-XXXVI. Not the collected folio."
)
METHOD = (
    "English follows the locked 1675 Pitt Latin through De Aeterna Hominum Electione et Praedestinatione I-XXXVI, "
    "reconstructed from IA PDF page images with pdftotext (+ DjVu checks). "
    "1683 not copy-text. No modern English. This slice densifies Aeterna Electio XXIX-XXXVI "
    "(comparative Roman/Reformed word-use; Scholastic end/means vs Reformed medium voice; "
    "parallel effect/object disputes; logomachy; lexical rule)."
)
...
LOCK_HEADER = (
    ...
    "Same 1675 Pitt copy-text. Book pp. 127-132 / PDF 139-144 (I-XXXVI; this packet XXIX-XXXVI). "
    "pdftotext layout (+ DjVu checks). Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: De Aeterna Hominum Electione et Praedestinatione I-XXXVI tip. Next: Aeterna Electio XXXVII+.\n"
    "Note: Comparative Roman/Reformed word-use; parallel object/effect disputes; logomachy; lexical rule.\n\n"
)
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 590 <= n <= 597:
            notes = (
                'Section %s: new densify De Aeterna Hominum Electione XXIX-XXXVI; '
                'Pass A!=B; lock-grounded PDF 143-144 / book pp. 131-132.' % sid
            )
        else:
            notes = (
                'Section %s: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in aeterna_electio XXIX-XXXVI packet scope covering all current sections.'
                % sid
            )
        reviews.append({
            'section': sid,
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Aeterna Hominum Electione XXIX-XXXVI only '
                '(sections %s-%s). Meta discloses %s. '
                'Next Aeterna Electio XXXVII+. Not shipped.'
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Aeterna Hominum Electione et Praedestinatione XXIX-XXXVI '
            '(comparative Roman/Reformed word-use; logomachy; lexical rule)'
        ),
        'next_locus': (
            'De Aeterna Hominum Electione et Praedestinatione XXXVII+ '
            '(why some extend/restrict praedestinatio)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Aeterna Hominum Electione XXIX–XXXVI densify LOCAL)\n\n'
        '- Before: **%s**. After: **%s** (contiguous Aeterna Electio XXIX–XXXVI → §§%s–%s).\n'
        '- Packet %s. Punch X = NO. Not shipped.\n'
        '- Post tip-ready hold cleared (%s) live %s.\n'
        '- Next: Aeterna Electio XXXVII+.\n\n'
        % (
            day,
            receipt['before'],
            receipt['after'],
    ...
    repo_block = (
        '## %s ~ET (Scribe — Le Blanc De Aeterna Electione XXIX–XXXVI densify)\n\n'
        'CoS densify: Aeterna Electio XXIX–XXXVI (**%s→%s**). Packet %s. '
        'Next: Aeterna Electio XXXVII+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_597.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-597 post-ready')
