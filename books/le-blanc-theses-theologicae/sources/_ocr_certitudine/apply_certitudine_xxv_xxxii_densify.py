#!/usr/bin/env python3
"""Build + apply De Certitudine qua Fidei competit XXV-XXXII densify (tip 622 → 630).

Shared difficulty; internal Spirit testimony; moral motives; apparent collapse;
logomachy; Spirit-boost escape; rational scruple; Bonaventure adhesion.
Live floor 5366. After tip-ready: HOLD live>5366 OR 12m. Punch X=NO. No ship.
"""
...
SEED = Path('/tmp/leblanc_certitudine_xxv_data')
PACKET_STEM = 'certitudine_xxv_xxxii_densify'
APPLY_COPY = BOOK / 'sources/_ocr_certitudine/apply_certitudine_xxv_xxxii_densify.py'
...
SECS = ['623', '624', '625', '626', '627', '628', '629', '630']
ROMANS = {
    '623': 'XXV', '624': 'XXVI', '625': 'XXVII', '626': 'XXVIII',
    '627': 'XXIX', '628': 'XXX', '629': 'XXXI', '630': 'XXXII',
}
TIP_BEFORE = 622
PRIOR_START = 599
LIVE_FLOOR = 5366
...
RANGE_SHORT = (
    ...
    "De Certitudine qua Fidei competit I-XXXII"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine qua Fidei competit I-XXXII (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Certitudine qua Fidei competit I-XXXII. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Certitudine qua Fidei competit I-XXXII. "
    "1675 PDF 148-149 (+ 1683 check) for XXV-XXXII. No modern English. "
    "This slice densifies Certitudine XXV-XXXII (shared difficulty; Spirit testimony; Bonaventure adhesion)."
)
...
LOCK_HEADER = (
    ...
    "VIII+ from 1675 PDF 146-149. pdftotext + DjVu. No modern English.\n"
    "Scope: De Certitudine qua Fidei competit I-XXXII tip. Next: Certitudine XXXIII+.\n"
    "Note: Shared difficulty; internal Spirit testimony; moral motives; Bonaventure adhesion.\n\n"
)
...
def write_lock(latin: dict):
    ...
    # refresh header; keep prior theses XXV-XXXII latin bodies
    body_lines = []
    if LOCK.exists():
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 623 <= n <= 630:
            notes = (
                'Section %s: new densify De Certitudine qua Fidei competit XXV-XXXII; '
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Certitudine qua Fidei competit XXV-XXXII only '
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Certitudine qua Fidei competit XXV-XXXII densify LOCAL)\n\n'
        '- Before: **%s**. After: **%s** (contiguous Certitudine XXV-XXXII → §§%s–%s).\n'
    ...
    repo_block = (
        ...
        'CoS densify: Certitudine XXV-XXXII (**%s→%s**). Packet %s. '
...
def main():
    ...
    (DATA / 'hold_post_tipready_630.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-630 post-ready')
