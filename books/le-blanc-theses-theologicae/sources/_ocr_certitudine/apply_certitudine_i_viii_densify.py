#!/usr/bin/env python3
"""Build + apply De Certitudine qua Fidei competit I-VIII densify (tip 598 → 606).

Certainty genus; subjective/objective; experience / intelligence / science / moral signs.
1675 IA scan lacks leaf 164 (I–mid-VII): filled from 1683 Pitt Latin of same tract;
VIII+ cross-checked on 1675 PDF 146 / book p.165. Live floor 5288.
After tip-ready: HOLD live>5288 OR 12m. Punch X=NO. No ship.
"""
...
SEED = Path('/tmp/leblanc_certitudine_data')
PACKET_STEM = 'certitudine_i_viii_densify'
APPLY_COPY = BOOK / 'sources/_ocr_certitudine/apply_certitudine_i_viii_densify.py'
...
SECS = ['599', '600', '601', '602', '603', '604', '605', '606']
ROMANS = {
    '599': 'I', '600': 'II', '601': 'III', '602': 'IV',
    '603': 'V', '604': 'VI', '605': 'VII', '606': 'VIII',
}
TIP_BEFORE = 598
PRIOR_START = 599  # unused for lock; Certitudine lock is self-contained
LIVE_FLOOR = 5288
HOLD_MINUTES = 12
...
RANGE_SHORT = (
    ...
    "De Certitudine qua Fidei competit I-VIII"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine qua Fidei competit I-VIII (not whole folio). "
    "I-VII start filled from 1683 Pitt where 1675 IA scan lacks leaf 164."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Certitudine qua Fidei competit I-VIII. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Certitudine qua Fidei competit I-VIII. "
    "1675 IA PDF lacks leaf 164 (Certitudine I–mid-VII): those theses reconstructed from 1683 Pitt "
    "Latin of the same tract; VIII+ cross-checked on 1675 PDF 146 / book p.165. "
    "No modern English. This slice densifies Certitudine I-VIII "
    "(genus; subjective/objective; experience / intelligence / science / moral signs)."
)

LOCK_HEADER = (
    "Locked Latin: Louis Le Blanc, Theses theologicae (London: Moses Pitt).\n"
    "Tract: De Certitudine qua Fidei competit.\n"
    "Copy-text intent: 1675 Pitt. IA bub_gb_eOkHAW4G0-wC lacks leaf 164 (book pp. 164 / I–mid-VII); "
    "I–VII start filled from 1683 Pitt Latin of the same tract; VIII+ from 1675 PDF 146 / book p.165. "
    "pdftotext + DjVu. Long-s and ligatures normalized. No modern English.\n"
    "Scope: De Certitudine qua Fidei competit I-VIII tip. Next: Certitudine IX+.\n"
    "Note: Genus; subjective/objective; experience / intelligence / science / moral-sign assent.\n\n"
)

LATIN = json.loads((SEED / 'latin.json').read_text(encoding='utf-8'))
SECTIONS = json.loads((SEED / 'sections.json').read_text(encoding='utf-8'))
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 599 <= n <= 606:
            notes = (
                'Section %s: new densify De Certitudine qua Fidei competit I-VIII; '
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Certitudine qua Fidei competit I-VIII only '
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': (
            'De Aeterna Hominum Electione et Praedestinatione XXXVII '
            '(certainty genus; subjective/objective; experience/intelligence/science/moral signs)'
        ),
        'next_locus': (
            'De Certitudine qua Fidei competit I+ '
            '(certainty belonging to faith)'
        ),
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Certitudine qua Fidei competit I-VIII densify LOCAL)\n\n'
        '- Before: **%s**. After: **%s** (contiguous Certitudine I-VIII → §§%s–%s).\n'
    ...
    repo_block = (
        ...
        'CoS densify: Certitudine I-VIII (**%s→%s**). Packet %s. '
        'Next: De Certitudine qua Fidei competit IX+. Punch X: **NO**.\n\n---\n\n'
...
def main():
    ...
    (DATA / 'hold_post_tipready_606.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-606 post-ready')
