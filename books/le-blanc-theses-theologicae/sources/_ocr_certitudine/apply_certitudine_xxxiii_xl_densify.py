#!/usr/bin/env python3
"""Build + apply De Certitudine qua Fidei competit XXXIII-XL densify (tip 630 → 638).

Speculation rank; shaken faith still true; evidence measure; moral still binds;
word-is-God judgment; adhesion cling; faith > scientia by cling; firmer though less clear.
Live floor 5392. After tip-ready: HOLD live>5392 OR 12m. Punch X=NO. No ship.
"""
...
SEED = Path('/tmp/leblanc_certitudine_xxxiii_data')
PACKET_STEM = 'certitudine_xxxiii_xl_densify'
APPLY_COPY = BOOK / 'sources/_ocr_certitudine/apply_certitudine_xxxiii_xl_densify.py'
...
SECS = ['631', '632', '633', '634', '635', '636', '637', '638']
ROMANS = {
    '631': 'XXXIII', '632': 'XXXIV', '633': 'XXXV', '634': 'XXXVI',
    '635': 'XXXVII', '636': 'XXXVIII', '637': 'XXXIX', '638': 'XL',
}
TIP_BEFORE = 630
PRIOR_START = 599
LIVE_FLOOR = 5392
...
RANGE_SHORT = (
    ...
    "De Certitudine qua Fidei competit I-XL"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine qua Fidei competit I-XL (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Certitudine qua Fidei competit I-XL. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Certitudine qua Fidei competit I-XL. "
    "1675 PDF 149-150 (+ 1683 check) for XXXIII-XL. No modern English. "
    "This slice densifies Certitudine XXXIII-XL (speculation rank; moral bind; adhesion cling)."
)
...
LOCK_HEADER = (
    ...
    "Scope: De Certitudine qua Fidei competit I-XL tip. Next: Certitudine XLI+.\n"
...
def write_lock(latin: dict):
    ...
    # refresh header; keep prior theses XXXIII-XL latin bodies
    body_lines = []
    if LOCK.exists():
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 631 <= n <= 638:
            notes = (
                'Section %s: new densify De Certitudine qua Fidei competit XXXIII-XL; '
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Certitudine qua Fidei competit XXXIII-XL only '
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Certitudine qua Fidei competit XXXIII-XL densify LOCAL)\n\n'
        '- Before: **%s**. After: **%s** (contiguous Certitudine XXXIII-XL → §§%s–%s).\n'
    ...
    repo_block = (
        ...
        'CoS densify: Certitudine XXXIII-XL (**%s→%s**). Packet %s. '
...
def main():
    ...
    (DATA / 'hold_post_tipready_638.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-638 post-ready')
