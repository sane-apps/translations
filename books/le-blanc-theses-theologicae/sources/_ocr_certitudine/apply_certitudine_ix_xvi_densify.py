#!/usr/bin/env python3
"""Build + apply De Certitudine qua Fidei competit IX-XVI densify (tip 606 → 614).

Moral certainty (fama); grade vs scientia; true-name; life-sufficiency;
cognition vs adhesion; divine revelation as faith-object.
Live floor 5314. After tip-ready: HOLD live>5314 OR 12m. Punch X=NO. No ship.
"""
...
SEED = Path('/tmp/leblanc_certitudine_ix_data')
PACKET_STEM = 'certitudine_ix_xvi_densify'
APPLY_COPY = BOOK / 'sources/_ocr_certitudine/apply_certitudine_ix_xvi_densify.py'
...
SECS = ['607', '608', '609', '610', '611', '612', '613', '614']
ROMANS = {
    '607': 'IX', '608': 'X', '609': 'XI', '610': 'XII',
    '611': 'XIII', '612': 'XIV', '613': 'XV', '614': 'XVI',
}
TIP_BEFORE = 606
PRIOR_START = 599
LIVE_FLOOR = 5314
...
RANGE_SHORT = (
    ...
    "De Certitudine qua Fidei competit I-XVI"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine qua Fidei competit I-XVI (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Certitudine qua Fidei competit I-XVI. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Certitudine qua Fidei competit I-XVI. "
    "1675 PDF 146-147 for IX-XVI; earlier I-VII gap-fill noted in lock. No modern English. "
    "This slice densifies Certitudine IX-XVI (moral certainty; cognition/adhesion; divine revelation)."
)
...
LOCK_HEADER = (
    ...
    "VIII+ from 1675 PDF 146-147 / book pp. 165-166. pdftotext + DjVu. No modern English.\n"
    "Scope: De Certitudine qua Fidei competit I-XVI tip. Next: Certitudine XVII+.\n"
    "Note: Moral certainty; cognition vs adhesion; divine testimony as faith-object.\n\n"
)
...
def write_lock(latin: dict):
    ...
    # refresh header; keep prior theses IX-XVI latin bodies
    body_lines = []
    if LOCK.exists():
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 607 <= n <= 614:
            notes = (
                'Section %s: new densify De Certitudine qua Fidei competit IX-XVI; '
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Certitudine qua Fidei competit IX-XVI only '
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Certitudine qua Fidei competit IX-XVI densify LOCAL)\n\n'
        '- Before: **%s**. After: **%s** (contiguous Certitudine IX-XVI → §§%s–%s).\n'
    ...
    repo_block = (
        ...
        'CoS densify: Certitudine IX-XVI (**%s→%s**). Packet %s. '
...
def main():
    ...
    (DATA / 'hold_post_tipready_614.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-614 post-ready')
