#!/usr/bin/env python3
"""Build + apply De Certitudine qua Fidei competit XVII-XXIV densify (tip 614 → 622).

Faith essence/existence; objective summit; formal ratio; two testimony-conditions;
God veracious; hard question; no infinite regress.
Live floor 5340. After tip-ready: HOLD live>5340 OR 12m. Punch X=NO. No ship.
"""
...
SEED = Path('/tmp/leblanc_certitudine_xvii_data')
PACKET_STEM = 'certitudine_xvii_xxiv_densify'
APPLY_COPY = BOOK / 'sources/_ocr_certitudine/apply_certitudine_xvii_xxiv_densify.py'
...
SECS = ['615', '616', '617', '618', '619', '620', '621', '622']
ROMANS = {
    '615': 'XVII', '616': 'XVIII', '617': 'XIX', '618': 'XX',
    '619': 'XXI', '620': 'XXII', '621': 'XXIII', '622': 'XXIV',
}
TIP_BEFORE = 614
PRIOR_START = 599
LIVE_FLOOR = 5340
...
RANGE_SHORT = (
    ...
    "De Certitudine qua Fidei competit I-XXIV"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    "Densify through De Certitudine qua Fidei competit I-XXIV (not whole folio)."
)
BLURB = (
    "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. "
    "Covers De Theologia through De Certitudine qua Fidei competit I-XXIV. Not the collected folio."
)
METHOD = (
    "English follows locked Pitt Latin through De Certitudine qua Fidei competit I-XXIV. "
    "1675 PDF 147-148 for XVII-XXIV. No modern English. "
    "This slice densifies Certitudine XVII-XXIV (essence/existence; objective summit; testimony conditions; regress)."
)
...
LOCK_HEADER = (
    ...
    "VIII+ from 1675 PDF 146-148. pdftotext + DjVu. No modern English.\n"
    "Scope: De Certitudine qua Fidei competit I-XXIV tip. Next: Certitudine XXV+.\n"
    "Note: Faith essence/existence; objective summit; formal ratio; no infinite regress of testimonies.\n\n"
)
...
def write_lock(latin: dict):
    ...
    # refresh header; keep prior theses XVII-XXIV latin bodies
    body_lines = []
    if LOCK.exists():
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 615 <= n <= 622:
            notes = (
                'Section %s: new densify De Certitudine qua Fidei competit XVII-XXIV; '
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Certitudine qua Fidei competit XVII-XXIV only '
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        '## %s (Scribe — De Certitudine qua Fidei competit XVII-XXIV densify LOCAL)\n\n'
        '- Before: **%s**. After: **%s** (contiguous Certitudine XVII-XXIV → §§%s–%s).\n'
    ...
    repo_block = (
        ...
        'CoS densify: Certitudine XVII-XXIV (**%s→%s**). Packet %s. '
...
def main():
    ...
    (DATA / 'hold_post_tipready_622.json').write_text(
        json.dumps({
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-622 post-ready')
