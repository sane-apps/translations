#!/usr/bin/env python3
"""De Scripturae plenitudine Pars I XXXIII-XXXVIII densify on Mini.

Lock + justifs (check_pass_ab each) first. Append eng/src only after live >3638
OR 12 minutes from tip-194 hold start (~19:42 EDT), then re-read tip.
...
DATA = Path("/tmp/leblanc_plen_xxxiii_densify")
PACKET_STEM = "scripturae_plenitudine_pars_i_xxxiii_densify"
APPLY_COPY = BOOK / "sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_i_xxxiii_densify.py"
...
EDITION = (
    ...
    " Scripturae plenitudine Pars I I-XXXVIII (not whole folio)."
)
BLURB = (
    ...
    "III (sum of inspired teaching in Scripture; unwritten silence; sole rule; negativ"
    "e argument). Not the collected folio."
)
METHOD = (
    ...
    "rs IV I-XLIV, and De Scripturae plenitudine Pars I I-XXXVIII, reconstructed from "
    "the Internet Archive PDF page images with pdftotext + tesseract (+ DjVu checks). "
    "The 1683 third edition was not used as copy-text. No modern English was copied. "
    "This slice continues Plenitudine from XXXIII (sum in Scripture) through XXXVIII "
    "(historical/polity traditions vs faith dogmas). Pars II remains."
)
...
LOCK_HEADER = (
    ...
    "am non scripti.\nPARS PRIMA — theses I-XXXVIII (XXXIII-XXXVIII densify contiguous "
    "after I-XXXII).\nSame 1675 Pitt copy-text. Book pp. 54-60 / PDF 67-72 (XXXIII-XXXVI"
    "II on PDF 72 / p. 60). pdftotext + tess OCR (+ DjVu checks). Long-s and ligatures "
    "normalized. 1683 not copy-text. No modern English.\nScope: Plenitudine Pars I thr"
    "ough XXXVIII (natural close before PARS SECUNDA). Pars II remains.\nNote: I-XXXII "
    "retained from prior lock; XXXIII sum of inspired speech in Scripture; XXXIV rite/"
    "polity left to Church (1 Cor 14:40); XXXV no more ἀδιάπτωτοι; XXXVI sole rule; XXX"
    "VII negative argument; XXXVIII historical/polity traditions admitted, not faith/"
    "morals.\n\n"
)
def load_data():
...
def write_lock(latin: dict):
    # Expand lock: keep I-XXXII body from existing lock, append XXXIII-XXXVIII.
...
def wait_hold():
    """Owner hold: live >3638 OR 12 minutes from tip-194 hold, then re-read tip."""
    ...
    while True:
        ...
        if live_ok or timed_out:
            reason = f"live>{LIVE_FLOOR}" if live_ok else f"{HOLD_MINUTES}m since tip-194 hold start"
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 195 <= n <= 200:
            notes = (
                f"Section {sid}: new densify De Scripturae plenitudine Pars I XXXIII-XXXVIII; "
                "Pass A!=B; lock-grounded PDF 72 / book p. 60."
            )
        else:
            notes = (
                f"Section {sid}: prior verified densify retained; Pass A/B and lock identity "
                "rechecked in plenitudine Pars I XXXIII packet scope covering all current sections."
            )
        reviews.append(
            {
                "section": sid,
    ...
    receipt = {
        ...
        "scope_review": {
            ...
            "uncertainties": [],
            "notes": (
                "Scope review: densify De Scripturae plenitudine Pars I XXXIII-XXXVIII only "
                f"(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. "
                "Honest partial; plenitudine Pars II remains. Prior tracts untouched. Not shipped."
            ),
        },
        "reviews": reviews,
    }
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        "locus": "De Scripturae plenitudine Pars I theses XXXIII-XXXVIII (sum / silence)",
        "next_locus": "De Scripturae plenitudine Pars II (Pontificiorum sententia)",
        "gates": {
...
def prepend_handoff(receipt: dict):
    day = datetime.now().strftime("%Y-%m-%d")
    parts = [
        "## " + day + " (Scribe — De Scripturae plenitudine Pars I XXXIII–XXXVIII densify LOCAL)\n\n",
        "- Before: **" + str(receipt["before"]) + "** (Plenitudine Pars I I–XXXII). After: **"
        + str(receipt["after"]) + "** ",
        "(contiguous Plenitudine Pars I XXXIII–XXXVIII → §§" + SECS[0] + "–" + SECS[-1] + ").\n",
        "- Packet `" + PACKET_STEM + "` (`reviews/audit/" + PACKET_STEM + ".packet.json` + `.review.json`). ",
        "Reviewer: scribe-leblanc, " + day + ". Verdict pass grounded in ",
        "`sources/_le_blanc_plenitudine_latin_lock.txt` (PDF 72 / book p. 60).\n",
        "- Meta range bumped to **" + RANGE_SHORT + "** (title, edition, blurb, text_history.method).\n",
        "- Honest **partial**: Plenitudine Pars I through XXXVIII (sum of inspired teaching in "
        "Scripture; unwritten silence; sole rule; negative argument; historical/polity traditions "
        "admitted). Pars II remains. De Theologia / De Fide / Authoritate closed as before. "
        "Not folio. Not shipped this slice.\n",
        "- Pass A ≠ B for §§" + SECS[0] + "–" + SECS[-1] + ". Greek ἀδιάπτωτοι restored; 1 Cor 14:40 sense. "
    ...
    repo_block = (
        "## " + day + " ~ET (Scribe — Le Blanc Plenitudine Pars I XXXIII–XXXVIII densify)\n\n"
        "CoS densify: De Scripturae plenitudine Pars I XXXIII–XXXVIII (sum / unwritten silence). "
        "Punch X: **NO**. No ship/commit/Logos/ai_promote.\n\n"
        "### Bound this job (not shipped)\n"
        "- **Le Blanc** `le-blanc-theses-theologicae`: Plenitudine Pars I tip (**"
        + str(receipt["before"]) + "→" + str(receipt["after"]) + "** sections). Packet `"
        + PACKET_STEM + "`. Pass A≠B; tip-ready ok. Honest partial. Next: Pars II. "
        "Punch X: **NO**.\n\n---\n\n"
    )
...
def main():
    ...
    if mode in ("all", "justifs"):
        ...
        if mode == "justifs":
            ...
            return
    # HARD HOLD — owner: live >3638 OR 12m, then re-read tip before append past 194
