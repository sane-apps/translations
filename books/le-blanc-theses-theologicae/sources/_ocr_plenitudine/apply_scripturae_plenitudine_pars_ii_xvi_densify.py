#!/usr/bin/env python3
"""De Scripturae plenitudine Pars II XVI-XXIII densify on Mini.

Lock + justifs (check_pass_ab each) first. Append eng/src only after live >3744
OR 12 minutes from tip-215 catch-up-ship hold start (~20:17 EDT), then re-read tip.
...
DATA = Path('/tmp/leblanc_plen_pars_ii_xvi_densify')
PACKET_STEM = 'scripturae_plenitudine_pars_ii_xvi_densify'
APPLY_COPY = BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_ii_xvi_densify.py'
...
SECS = ['216', '217', '218', '219', '220', '221', '222', '223']
ROMANS = {
    '216': 'XVI', '217': 'XVII', '218': 'XVIII', '219': 'XIX',
    '220': 'XX', '221': 'XXI', '222': 'XXII', '223': 'XXIII',
}

TIP_BEFORE = 215
LIVE_FLOOR = 3744
HOLD_START = datetime(2026, 9, 22, 20, 17, 0)
HOLD_MINUTES = 12

RANGE_SHORT = (
    'Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XLIV + De Scriptura'
    'e plenitudine Pars I I-XXXVIII + Pars II I-XXIII'
)
RANGE_TITLE = (
...
EDITION = (
    ...
    ' Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XXIII (not whole folio).'
)
BLURB = (
    ...
    'III plus Pars II through XXIII (tradition discernment rules; controversy status). '
...
METHOD = (
    ...
    'rs IV I-XLIV, and De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XXIII, '
    ...
    'glish was copied. This slice continues Plenitudine Pars II through XXIII (discern'
    'ment rules XVI-XIX; controversy status XX-XXIII). Pars II XXIV+ remains.'
)
...
LOCK_HEADER = (
    ...
    'am non scripti.\nPARS PRIMA — theses I-XXXVIII retained.\nPARS SECUNDA — theses I-XXIII'
    ' (Pontificiorum sententia + discernment rules + controversy status).\nSame 1675 Pitt '
    'copy-text. Book pp. 54-64 / PDF 67-76 (Pars II XVI-XXIII on PDF 75-76 / pp. 63-64). '
    ...
    'through XXIII. Pars II XXIV+ remains.\nNote: I-XV retained; XVI-XIX three rules; '
    'XX-XXI Reformed concessions; XXII Roman concessions; XXIII real difference.\n\n'
)
...
def write_lock(latin: dict):
    ...
    for sec in [str(n) for n in range(201, 216)]:
...
def wait_hold():
    ...
    while True:
        ...
        if live_ok or timed_out:
            reason = f'live>{LIVE_FLOOR}' if live_ok else f'{HOLD_MINUTES}m since tip-215 hold start'
...
def build_packet_and_review():
    ...
    packet = make_audit_packet(
        ...
        raw_sources=[
            LOCK,
            pdf,
            BOOK / 'sources/_ocr_plenitudine/pdf_75_77_raw.txt',
            BOOK / 'sources/_ocr_plenitudine/tesseng-p-075.txt',
            BOOK / 'sources/_ocr_plenitudine/tesseng-p-076.txt',
            BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_ii_xvi_densify.py',
        ],
    ...
    for sid in section_ids:
        ...
        if 216 <= n <= 223:
            notes = (
                f'Section {sid}: new densify De Scripturae plenitudine Pars II XVI-XXIII; '
                'Pass A!=B; lock-grounded PDF 75-76 / book pp. 63-64.'
            )
        else:
            notes = (
                f'Section {sid}: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in plenitudine Pars II XVI packet scope covering all current sections.'
            )
        reviews.append({
            'section': sid,
            'verdict': 'pass',
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Scripturae plenitudine Pars II XVI-XXIII only '
                f'(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. '
                'Honest partial; plenitudine Pars II XXIV+ remains. Prior tracts untouched. Not shipped.'
            ),
        },
        'reviews': reviews,
    }
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': 'De Scripturae plenitudine Pars II theses XVI-XXIII (discernment rules / controversy status)',
        'next_locus': 'De Scripturae plenitudine Pars II XXIV+ (two-point question / Reformed affirmations)',
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    parts = [
        "## " + day + " (Scribe — De Scripturae plenitudine Pars II XVI–XXIII densify LOCAL)\n\n",
        "- Before: **" + str(receipt["before"]) + "** (Plenitudine Pars II I–XV). After: **"
        + str(receipt["after"]) + "** ",
        "(contiguous Plenitudine Pars II XVI–XXIII → §§" + SECS[0] + "–" + SECS[-1] + ").\n",
        "- Packet " + packet_ref + " (" + packet_files + "). ",
        "Reviewer: scribe-leblanc, " + day + ". Verdict pass grounded in ",
        lock_ref + " (PDF 75–76 / book pp. 63–64).\n",
        ...
        "Pars I / Pars II I–XV closed as before. Not folio. Not shipped this slice.\n",
    ...
    repo_block = (
        "## " + day + " ~ET (Scribe — Le Blanc Plenitudine Pars II XVI–XXIII densify)\n\n"
        "CoS densify: De Scripturae plenitudine Pars II XVI–XXIII (discernment rules / status). "
        "Punch X: **NO**. No ship/commit/Logos/ai_promote.\n\n"
        "### Bound this job (not shipped)\n"
        "- **Le Blanc** " + slug_ref + ": Plenitudine Pars II XVI–XXIII (**"
        + str(receipt["before"]) + "→" + str(receipt["after"]) + "** sections). Packet "
        + packet_ref + ". Pass A≠B; tip-ready ok. Honest partial. Next: Pars II XXIV+. "
        "Punch X: **NO**.\n\n---\n\n"
    )
