#!/usr/bin/env python3
"""De Scripturae plenitudine Pars II XXXII-XXXVIII densify on Mini.

Lock + justifs (check_pass_ab each) first. Append eng/src only after live >3809
OR 12 minutes from tip-231 catch-up-ship hold clear (~20:38 EDT), then re-read tip.
...
DATA = Path('/tmp/leblanc_plen_pars_ii_xxxii_densify')
PACKET_STEM = 'scripturae_plenitudine_pars_ii_xxxii_densify'
APPLY_COPY = BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_ii_xxxii_densify.py'
...
SECS = ['232', '233', '234', '235', '236', '237', '238']
ROMANS = {
    '232': 'XXXII', '233': 'XXXIII', '234': 'XXXIV', '235': 'XXXV',
    '236': 'XXXVI', '237': 'XXXVII', '238': 'XXXVIII',
}

TIP_BEFORE = 231
LIVE_FLOOR = 3809
HOLD_START = datetime(2026, 9, 22, 20, 38, 0)
HOLD_MINUTES = 12

RANGE_SHORT = 'Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XLIV + De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XXXVIII'
RANGE_TITLE = 'Theological Theses (De Theologia I-XLIII; De Fide I-XXII; De Authoritate Scripturae Pars I I-XLVII; Pars II I-XXXV; Pars III I-XLI; Pars IV I-XLIV; De Scripturae plenitudine Pars I I-XXXVIII; Pars II I-XXXVIII)'
EDITION = 'Theses theologicae (London: Moses Pitt, 1675). ESTC R17887; Wing L802. IA bub_gb_eOkHAW4G0-wC. Densify: De Theologia I-XLIII + De Fide I-XXII + De Authoritate Scripturae Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XLIV + De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XXXVIII (not whole folio).'
BLURB = 'Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. Covers De Theologia I-XLIII, De Fide I-XXII, De Authoritate Scripturae through Pars IV XLIV, and De Scripturae plenitudine Pars I through XXXVIII plus Pars II through XXXVIII (memory/corruption of viva-voce deposit). Not the collected folio.'
METHOD = 'English follows the locked 1675 Pitt Latin of De Theologia I-XLIII, De Fide I-XXII, De Authoritate Scripturae Pars I I-XLVII, Pars II I-XXXV, Pars III I-XLI, Pars IV I-XLIV, and De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XXXVIII, reconstructed from the Internet Archive PDF page images with pdftotext + tesseract (+ DjVu checks). The 1683 third edition was not used as copy-text. No modern English was copied. This slice continues Plenitudine Pars II through XXXVIII (memory/corruption XXXII; pre-Mosaic reply XXXIII; why-written XXXIV; continuous-use XXXV; Fathers fallible XXXVI; early contests XXXVII; infant communion XXXVIII). Pars II XXXIX+ remains.'

LOCK_HEADER = 'Locked Latin: Louis Le Blanc, Theses theologicae (London: Moses Pitt, 1675).\nTract: De Scripturae Plenitudine et Sufficientia adversus necessitatem verbi cujusdam non scripti.\nPARS PRIMA — theses I-XXXVIII retained.\nPARS SECUNDA — theses I-XXXVIII (through memory/corruption of viva-voce deposit XXXII-XXXVIII).\nSame 1675 Pitt copy-text. Book pp. 54-69 / PDF 67-81 (Pars II XXXII-XXXVIII on PDF 78-81 / pp. 66-69). pdftotext + tess OCR (+ DjVu checks). Long-s and ligatures normalized. 1683 not copy-text. No modern English.\nScope: Plenitudine Pars I through XXXVIII + Pars II through XXXVIII. Pars II XXXIX+ remains.\nNote: I-XXXI retained; XXXII viva-voce corruption; XXXIII pre-Mosaic reply; XXXIV why-written; XXXV continuous use; XXXVI Fathers fallible; XXXVII early contests; XXXVIII infant communion.\n\n'
...
def write_lock(latin: dict):
    ...
    for sec in [str(n) for n in range(201, 232)]:
...
def wait_hold():
    ...
    while True:
        ...
        if live_ok or timed_out:
            reason = f'live>{LIVE_FLOOR}' if live_ok else f'{HOLD_MINUTES}m since tip-231 hold start'
...
def build_packet_and_review():
    ...
    packet = make_audit_packet(
        ...
        raw_sources=[
            LOCK,
            pdf,
            BOOK / 'sources/_ocr_plenitudine/pdf_76_80_raw.txt',
            BOOK / 'sources/_ocr_plenitudine/tesseng-p-078.txt',
            BOOK / 'sources/_ocr_plenitudine/tesseng-p-079.txt',
            BOOK / 'sources/_ocr_plenitudine/pdf_76_80_raw.txt',
            Path('/tmp/lb_plen_78_82.txt'),
            BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_ii_xxxii_densify.py',
        ],
    ...
    for sid in section_ids:
        ...
        if 232 <= n <= 238:
            notes = (
                f'Section {sid}: new densify De Scripturae plenitudine Pars II XXXII-XXXVIII; '
                'Pass A!=B; lock-grounded PDF 78-81 / book pp. 66-69.'
            )
        else:
            notes = (
                f'Section {sid}: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in plenitudine Pars II XXXII packet scope covering all current sections.'
            )
        reviews.append({
            'section': sid,
            'verdict': 'pass',
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Scripturae plenitudine Pars II XXXII-XXXVIII only '
                f'(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. '
                'Honest partial; plenitudine Pars II XXXIX+ remains. Prior tracts untouched. Not shipped.'
            ),
        },
        'reviews': reviews,
    }
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': 'De Scripturae plenitudine Pars II theses XXXII-XXXVIII (memory/corruption of viva-voce deposit)',
        'next_locus': 'De Scripturae plenitudine Pars II XXXIX+ (spurious vs true apostolic writings objection)',
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    parts = [
        "## " + day + " (Scribe — De Scripturae plenitudine Pars II XXXII–XXXVIII densify LOCAL)\n\n",
        "- Before: **" + str(receipt["before"]) + "** (Plenitudine Pars II I–XXXI). After: **"
        + str(receipt["after"]) + "** ",
        "(contiguous Plenitudine Pars II XXXII–XXXVIII → §§" + SECS[0] + "–" + SECS[-1] + ").\n",
        "- Packet " + packet_ref + " (" + packet_files + "). ",
        "Reviewer: scribe-leblanc, " + day + ". Verdict pass grounded in ",
        lock_ref + " (PDF 78–81 / book pp. 66–69).\n",
        "- Meta range bumped to **" + RANGE_SHORT + "** (title, edition, blurb, text_history.method).\n",
                "- Honest **partial**: Plenitudine Pars II through XXXVIII (viva-voce memory/corruption; "
        "pre-Mosaic reply; why God had the law written; continuous-use vernacular analogy; "
        "Fathers fallible vs inspired Scripture; early contested traditions; infant communion "
        "abrogated). XXXIX+ remains. De Theologia / De Fide / Authoritate / Plenitudine "
        "Pars I / Pars II I–XXXI closed as before. Not folio. Not shipped this slice.\n",
        "- Pass A ≠ B for §§" + SECS[0] + "–" + SECS[-1] + ". viva voce; ἀθεωρήτως; "
        "Sixtus Senensis; Polycrates/Victor; Tridentino damnata. OCR restorations noted in translator_notes.\n",
    ...
    repo_block = (
        "## " + day + " ~ET (Scribe — Le Blanc Plenitudine Pars II XXXII–XXXVIII densify)\n\n"
        "CoS densify: De Scripturae plenitudine Pars II XXXII–XXXVIII (memory/corruption viva-voce). "
        "Punch X: **NO**. No ship/commit/Logos/ai_promote.\n\n"
        "### Bound this job (not shipped)\n"
        "- **Le Blanc** " + slug_ref + ": Plenitudine Pars II XXXII–XXXVIII (**"
        + str(receipt["before"]) + "→" + str(receipt["after"]) + "** sections). Packet "
        + packet_ref + ". Pass A≠B; tip-ready ok. Honest partial. Next: Pars II XXXIX+. "
        "Punch X: **NO**.\n\n---\n\n"
    )
