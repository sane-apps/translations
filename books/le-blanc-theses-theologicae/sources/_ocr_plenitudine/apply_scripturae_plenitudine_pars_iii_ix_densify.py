#!/usr/bin/env python3
"""De Scripturae plenitudine Pars III IX-XV densify on Mini.

Lock + justifs (check_pass_ab each) first. Append eng/src after live >3884
OR 12m from tip-252 proceed (~21:03 EDT), then re-read tip.
...
DATA = Path('/tmp/leblanc_plen_pars_iii_ix_densify')
PACKET_STEM = 'scripturae_plenitudine_pars_iii_ix_densify'
APPLY_COPY = BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iii_ix_densify.py'
...
SECS = ['253', '254', '255', '256', '257', '258', '259']
ROMANS = {
    '253': 'IX', '254': 'X', '255': 'XI', '256': 'XII',
    '257': 'XIII', '258': 'XIV', '259': 'XV',
}

TIP_BEFORE = 252
LIVE_FLOOR = 3884
HOLD_START = datetime(2026, 9, 22, 21, 3, 30)
HOLD_MINUTES = 12

RANGE_SHORT = "Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XLIV + De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-XV"
RANGE_TITLE = "Theological Theses (De Theologia I-XLIII; De Fide I-XXII; De Authoritate Scripturae Pars I I-XLVII; Pars II I-XXXV; Pars III I-XLI; Pars IV I-XLIV; De Scripturae plenitudine Pars I I-XXXVIII; Pars II I-XLIV; Pars III I-XV)"
EDITION = "Theses theologicae (London: Moses Pitt, 1675). ESTC R17887; Wing L802. IA bub_gb_eOkHAW4G0-wC. Densify: De Theologia I-XLIII + De Fide I-XXII + De Authoritate Scripturae Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XLIV + De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-XV (not whole folio)."
BLURB = "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. Covers De Theologia I-XLIII, De Fide I-XXII, De Authoritate Scripturae through Pars IV XLIV, and De Scripturae plenitudine Pars I through XXXVIII, Pars II through XLIV, and Pars III through XV (Laodiceans / command-to-write). Not the collected folio."
METHOD = "English follows the locked 1675 Pitt Latin of De Theologia I-XLIII, De Fide I-XXII, De Authoritate Scripturae Pars I I-XLVII, Pars II I-XXXV, Pars III I-XLI, Pars IV I-XLIV, and De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-XV, reconstructed from the Internet Archive PDF page images with pdftotext + tesseract (+ DjVu checks). The 1683 third edition was not used as copy-text. No modern English was copied. This slice densifies Plenitudine Pars III IX-XV (Laodiceans ambiguity; command-to-write; inspiration as mandate; John/Apocalypse; occasions; catechism; few writers suffice). XVI+ remains."
...
LOCK_HEADER = (
    ...
    "PARS TERTIA — theses I-XV (Laodiceans / command-to-write replies).\n"
    "Same 1675 Pitt copy-text. Book pp. 54-74 / PDF 67-87 (Pars III IX-XV on PDF 86-87 / pp. 73-74). "
    "pdftotext + tess OCR (+ DjVu checks). Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: Plenitudine Pars I through XXXVIII + Pars II through XLIV + Pars III through XV. Pars III XVI+ remains.\n"
    "Note: Pars III I-VIII retained; IX Laodiceans; X-XV command-to-write / Bellarmine replies.\n\n"
)
...
def write_lock(latin: dict):
    ...
    for sec in [str(n) for n in range(245, 253)]:
        parts.append(by_prior[sec].rstrip() + chr(10))
    for sec in SECS:
...
def wait_hold():
    ...
    while True:
        ...
        if live_ok or timed_out:
            reason = f'live>{LIVE_FLOOR}' if live_ok else f'{HOLD_MINUTES}m since tip-252 hold start'
...
def build_packet_and_review():
    ...
    packet = make_audit_packet(
        ...
        raw_sources=[
            ...
            Path('/tmp/lb_plen_85_89.txt'),
            Path('/tmp/leblanc_plen_pars_iii_densify/pdf_83_88_raw.txt'),
            BOOK / 'sources/_ocr_plenitudine/tess_085.txt',
            BOOK / 'sources/_ocr_plenitudine/tess_086.txt',
            BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iii_densify.py',
        ],
    ...
    for sid in section_ids:
        ...
        if 253 <= n <= 259:
            notes = (
                f'Section {sid}: new densify De Scripturae plenitudine Pars III IX-XV; '
                'Pass A!=B; lock-grounded PDF 86-87 / book pp. 73-74.'
            )
        else:
            notes = (
                f'Section {sid}: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in plenitudine Pars III IX packet scope covering all current sections.'
            )
        reviews.append({
            'section': sid,
            'verdict': 'pass',
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Scripturae plenitudine Pars III IX-XV only '
                f'(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. '
                'Honest partial; plenitudine Pars III through XV; XVI+ remains. Prior tracts untouched. Not shipped.'
            ),
        },
        'reviews': reviews,
    }
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': 'De Scripturae plenitudine Pars III theses IX-XV (Laodiceans / command-to-write / inspiration mandate / John / occasions / catechism / few writers)',
        'next_locus': 'De Scripturae plenitudine Pars III XVI+ (things not in Scripture / OT remedies / further Bellarmine instances)',
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        f"## {day} (Scribe — De Scripturae plenitudine Pars III IX–XV densify LOCAL)\n\n"
        f"- Before: **{receipt['before']}** (Plenitudine Pars III I–VIII). After: **{receipt['after']}** "
        f"(contiguous Plenitudine Pars III IX–XV → §§{SECS[0]}–{SECS[-1]}).\n"
        f"- Packet {packet_ref} ({packet_files}). "
        f"Reviewer: scribe-leblanc, {day}. Verdict pass grounded in "
        f"{lock_ref} (PDF 86–87 / book pp. 73–74).\n"
        f"- Meta range bumped to **{RANGE_SHORT}** (title, edition, blurb, text_history.method).\n"
        "- Honest **partial**: Plenitudine Pars III through XV (Laodiceans Col. 4 ambiguity; "
        "command-to-write attack; inspiration as stronger mandate; John/Apocalypse + Docete omnes "
        "Gentes; occasions; catechism reply; few writers suffice). XVI+ remains. De Theologia / "
        "De Fide / Authoritate / Plenitudine Pars I / Pars II / Pars III I–VIII closed as before. "
        "Not folio. Not shipped.\n"
        f"- Pass A ≠ B for §§{SECS[0]}–{SECS[-1]}. ten ek Laodikeias; mandatum internum; "
        "Docete omnes Gentes; calamo. OCR restorations noted in translator_notes.\n"
    ...
    block = block.replace("ten ek Laodikeias", "τὴν ἐκ Λαοδικείας")
    ...
    repo_block = (
        f"## {day} ~ET (Scribe — Le Blanc Plenitudine Pars III IX–XV densify)\n\n"
        "CoS densify: De Scripturae plenitudine Pars III IX–XV (Laodiceans / command-to-write). "
        "Punch X: **NO**. No ship/commit/Logos/ai_promote.\n\n"
        "### Bound this job (not shipped)\n"
        f"- **Le Blanc** {slug_ref}: Plenitudine Pars III IX–XV (**"
        f"{receipt['before']}→{receipt['after']}** sections). Packet "
        f"{packet_ref}. Pass A≠B; tip-ready ok. Honest partial. Next: Pars III XVI+. "
        "Punch X: **NO**.\n\n---\n\n"
    )
