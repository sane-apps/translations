#!/usr/bin/env python3
"""De Scripturae plenitudine Pars III XXXII-XXXIX densify on Mini.

Lock + justifs (check_pass_ab each) first. Append eng/src after live >3948
OR 12m from tip-275 proceed (~21:42 EDT), then re-read tip.
...
DATA = Path('/tmp/leblanc_plen_pars_iii_xxxii_densify')
PACKET_STEM = 'scripturae_plenitudine_pars_iii_xxxii_densify'
APPLY_COPY = BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iii_xxxii_densify.py'
...
SECS = ['276', '277', '278', '279', '280', '281', '282', '283']
ROMANS = {
    '276': 'XXXII', '277': 'XXXIII', '278': 'XXXIV', '279': 'XXXV',
    '280': 'XXXVI', '281': 'XXXVII', '282': 'XXXVIII', '283': 'XXXIX',
}

TIP_BEFORE = 275
LIVE_FLOOR = 3948
HOLD_START = datetime(2026, 9, 22, 21, 42, 33)
HOLD_MINUTES = 12

RANGE_SHORT = "Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XLIV + De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-XXXIX"
RANGE_TITLE = "Theological Theses (De Theologia I-XLIII; De Fide I-XXII; De Authoritate Scripturae Pars I I-XLVII; Pars II I-XXXV; Pars III I-XLI; Pars IV I-XLIV; De Scripturae plenitudine Pars I I-XXXVIII; Pars II I-XLIV; Pars III I-XXXIX)"
EDITION = "Theses theologicae (London: Moses Pitt, 1675). ESTC R17887; Wing L802. IA bub_gb_eOkHAW4G0-wC. Densify: De Theologia I-XLIII + De Fide I-XXII + De Authoritate Scripturae Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XLIV + De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-XXXIX (not whole folio)."
BLURB = "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. Covers De Theologia I-XLIII, De Fide I-XXII, De Authoritate Scripturae through Pars IV XLIV, and De Scripturae plenitudine Pars I through XXXVIII, Pars II through XLIV, and Pars III through XXXIX (faith accepts word vs things / innate notes / integrity). Not the collected folio."
METHOD = "English follows the locked 1675 Pitt Latin of De Theologia I-XLIII, De Fide I-XXII, De Authoritate Scripturae Pars I I-XLVII, Pars II I-XXXV, Pars III I-XLI, Pars IV I-XLIV, and De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-XXXIX, reconstructed from the Internet Archive PDF page images with pdftotext + tesseract (+ DjVu checks). The 1683 third edition was not used as copy-text. No modern English was copied. This slice densifies Plenitudine Pars III XXXII-XXXIX (faith accepts word vs things; innate notes of divinity; integrity retorsion; Spirit illumination). XL+ (sense of Scripture) remains."
...
LOCK_HEADER = (
    ...
    "PARS TERTIA — theses I-XXXIX (faith accepts word vs things / innate notes / integrity).\n"
    "Same 1675 Pitt copy-text. Book pp. 54-80 / PDF 67-92 (Pars III XXXII-XXXIX on PDF 90-92 / pp. 78-80). "
    "pdftotext + tess OCR (+ DjVu checks). Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: Plenitudine Pars I through XXXVIII + Pars II through XLIV + Pars III through XXXIX. Pars III XL+ remains.\n"
    "Note: Pars III I-XXXI retained; XXXII word vs things; XXXIII-XXXIV innate notes; XXXV-XXXIX integrity / Spirit illumination.\n\n"
)
...
def write_lock(latin: dict):
    ...
    for sec in [str(n) for n in range(245, 276)]:
        parts.append(by_prior[sec].rstrip() + chr(10))
    for sec in SECS:
...
def wait_hold():
    ...
    while True:
        ...
        if live_ok or timed_out:
            reason = f'live>{LIVE_FLOOR}' if live_ok else f'{HOLD_MINUTES}m since tip-275 hold start'
...
def build_packet_and_review():
    ...
    packet = make_audit_packet(
        ...
        raw_sources=[
            LOCK,
            pdf,
            Path('/tmp/leblanc_plen_pars_iii_xxxii_densify/pdf_90_96_layout.txt'),
            Path('/tmp/leblanc_plen_pars_iii_xxxii_densify/tess_90.txt'),
            Path('/tmp/leblanc_plen_pars_iii_xxxii_densify/tess_91.txt'),
            Path('/tmp/leblanc_plen_pars_iii_xxxii_densify/tess_92.txt'),
            Path('/tmp/leblanc_plen_pars_iii_xxxii_densify/latin.json'),
            BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iii_xxiv_densify.py',
        ],
    ...
    for sid in section_ids:
        ...
        if 276 <= n <= 283:
            notes = (
                f'Section {sid}: new densify De Scripturae plenitudine Pars III XXXII-XXXIX; '
                'Pass A!=B; lock-grounded PDF 90-92 / book pp. 78-80.'
            )
        else:
            notes = (
                f'Section {sid}: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in plenitudine Pars III XXXII packet scope covering all current sections.'
            )
        reviews.append({
            'section': sid,
            'verdict': 'pass',
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Scripturae plenitudine Pars III XXXII-XXXIX only '
                f'(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. '
                'Honest partial; plenitudine Pars III through XXXIX; XL+ remains. Prior tracts untouched. Not shipped.'
            ),
        },
        'reviews': reviews,
    }
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': 'De Scripturae plenitudine Pars III theses XXXII-XXXIX (faith accepts word vs things / innate notes / integrity)',
        'next_locus': 'De Scripturae plenitudine Pars III XL+ (sense of Scripture / pastors as interpreters / obscurity)',
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        f"## {day} (Scribe — De Scripturae plenitudine Pars III XXXII–XXXIX densify LOCAL)\n\n"
        f"- Before: **{receipt['before']}** (Plenitudine Pars III I–XXIII). After: **{receipt['after']}** "
        f"(contiguous Plenitudine Pars III XXXII–XXXIX → §§{SECS[0]}–{SECS[-1]}).\n"
    ...
    repo_block = (
        f"## {day} ~ET (Scribe — Le Blanc Plenitudine Pars III XXXII–XXXIX densify)\n\n"
        ...
        f"- **Le Blanc** {slug_ref}: Plenitudine Pars III XXXII–XXXIX (**"
