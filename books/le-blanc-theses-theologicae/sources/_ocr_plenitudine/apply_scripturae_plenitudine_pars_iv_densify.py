#!/usr/bin/env python3
"""De Scripturae plenitudine Pars IV I-VIII densify on Mini.

Pre-append hold CLEARED at live 57/4050. Append at tip-294.
After tip-ready: HOLD until live >4050 OR 12m.
No ship/commit/push. Punch X=NO.
"""
...
DATA = Path('/tmp/leblanc_plen_pars_iv_densify')
PACKET_STEM = 'scripturae_plenitudine_pars_iv_densify'
APPLY_COPY = BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iv_densify.py'
...
SECS = ['295', '296', '297', '298', '299', '300', '301', '302']
ROMANS = {'295': 'I', '296': 'II', '297': 'III', '298': 'IV', '299': 'V', '300': 'VI', '301': 'VII', '302': 'VIII'}
...
RANGE_SHORT = (
    ...
    "De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-L + Pars IV I-VIII"
)
RANGE_TITLE = (
...
METHOD = (
    ...
    "De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-L + Pars IV I-VIII, reconstructed from "
...
LOCK_HEADER = (
    ...
    "PARS QUARTA — theses I-VIII (unwritten tradition equal-authority objections).\n"
    "Same 1675 Pitt copy-text. Book pp. 54-84 / PDF 67-96 (Pars IV I-VIII on PDF 95-96 / pp. 83-84). "
    "pdftotext + tess OCR (+ DjVu checks). Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: Plenitudine Pars I–III closed + Pars IV through VIII. Pars IV IX+ remains.\n"
...
def wait_hold_post_tipready(hold_start: datetime):
    ...
    while True:
        ...
        if live_ok or timed_out:
            reason = f'live>{LIVE_FLOOR}' if live_ok else f'{HOLD_MINUTES}m since tip-294 post-ready hold start (live>{LIVE_FLOOR})'
...
def build_packet_and_review():
    ...
    packet = make_audit_packet(
        ...
        raw_sources=[
            LOCK,
            pdf,
            DATA / 'pdf_95_100_layout.txt',
            DATA / 'tess_095.txt',
            DATA / 'tess_096.txt',
            DATA / 'latin.json',
            BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iii_xlviii_densify.py',
        ],
    ...
    for sid in section_ids:
        ...
        if 295 <= n <= 302:
            notes = (
                f'Section {sid}: new densify De Scripturae plenitudine Pars IV I-VIII; '
                'Pass A!=B; lock-grounded PDF 95-96 / book pp. 83-84.'
            )
        else:
            notes = (
                f'Section {sid}: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in plenitudine Pars IV packet scope covering all current sections.'
            )
        reviews.append({
            'section': sid,
            'verdict': 'pass',
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Scripturae plenitudine Pars IV I-VIII only '
                f'(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. '
                'Honest partial; plenitudine Pars IV through VIII; IX+ remains. '
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': 'De Scripturae plenitudine Pars IV theses I-VIII (unwritten tradition equal-authority)',
        'next_locus': 'De Scripturae plenitudine Pars IV IX+ (1 Cor 11 / 2 Thess 2 traditions continue)',
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        f'## {day} (Scribe — De Scripturae plenitudine Pars IV I–VIII densify LOCAL)\n\n'
        f'- Before: **{receipt["before"]}**. After: **{receipt["after"]}** '
        f'(contiguous Plenitudine Pars IV I–VIII → §§{SECS[0]}–{SECS[-1]}; opens Pars IV).\n'
        f'- Packet {packet_ref} ({packet_files}). '
        f'Reviewer: scribe-leblanc, {day}. Verdict pass grounded in '
        f'{lock_ref} (PDF 95-96 / pp. 83-84).\n'
        f'- Meta range bumped to **{RANGE_SHORT}** (title, edition, blurb, text_history.method).\n'
        '- Honest **partial**: Plenitudine Pars IV through VIII (clarity vs sophistry; Scripture proves '
        ...
        f'- Pre-append hold cleared live>4025; post tip-ready hold cleared '
    ...
    repo_block = (
        f'## {day} ~ET (Scribe — Le Blanc Plenitudine Pars IV I–VIII densify)\n\n'
        'CoS densify: De Scripturae plenitudine Pars IV I–VIII (unwritten tradition equal-authority). '
        'Punch X: **NO**. No ship/commit/Logos/ai_promote.\n\n'
        '### Bound this job (not shipped)\n'
        f'- **Le Blanc** {slug_ref}: Plenitudine Pars IV I–VIII (**'
        f'{receipt["before"]}→{receipt["after"]}** sections). Packet '
        f'{packet_ref}. Pass A≠B; tip-ready ok. Honest partial; Pars IV through VIII. Next: Pars IV IX+. '
        'Punch X: **NO**.\n\n---\n\n'
    )
    ...
    if mode == 'finish':
        ...
        if hold_path.exists():
            ...
            hold_path.write_text(json.dumps({
                ...
                'note': 'post tip-ready hold live>4050 OR 12m',
            }, indent=2) + '\n', encoding='utf-8')
        ...
        return
    # all: pre-append hold already cleared at 57/4050
    write_lock(latin)
    write_and_check_justifications(latin, by_sec)
    eng, src_tip = reread_tip()
