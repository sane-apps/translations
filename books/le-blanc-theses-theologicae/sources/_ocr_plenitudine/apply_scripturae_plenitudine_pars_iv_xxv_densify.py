#!/usr/bin/env python3
"""De Scripturae plenitudine Pars IV XXV-XXXII densify on Mini.

Pre-append hold CLEARED at live 57/4149. Append at tip-318.
After tip-ready: HOLD until live >4149 OR 12m.
No ship. Punch X=NO.
"""
...
DATA = Path('/tmp/leblanc_plen_pars_iv_xxv_densify')
PACKET_STEM = 'scripturae_plenitudine_pars_iv_xxv_densify'
APPLY_COPY = BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iv_xxv_densify.py'
...
SECS = ['319', '320', '321', '322', '323', '324', '325', '326']
ROMANS = {'319': 'XXV', '320': 'XXVI', '321': 'XXVII', '322': 'XXVIII', '323': 'XXIX', '324': 'XXX', '325': 'XXXI', '326': 'XXXII'}
...
RANGE_SHORT = (
    ...
    "De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-L + Pars IV I-XXXII"
)
RANGE_TITLE = (
...
METHOD = (
    ...
    "De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-L + Pars IV I-XXXII, reconstructed from "
...
LOCK_HEADER = (
    ...
    "Same 1675 Pitt copy-text. Book pp. 54-84 / PDF 67-96 (Pars IV XXV-XXXII on PDF 99-101 / pp. 87-89). "
    "pdftotext + tess OCR (+ DjVu checks). Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: Plenitudine Pars I–III closed + Pars IV through XXXII. Pars IV XXXIII+ remains.\n"
...
def write_lock(latin: dict):
    ...
    for sec in [str(n) for n in range(295, 319)]:
        parts.append(by_prior[sec].rstrip() + chr(10))
    for sec in SECS:
...
def build_packet_and_review():
    ...
    packet = make_audit_packet(
        ...
        raw_sources=[
            ...
            DATA / 'tess_101.txt',
            DATA / 'latin.json',
            BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iv_xvii_densify.py',
        ],
    ...
    for sid in section_ids:
        ...
        if 319 <= n <= 326:
            notes = (
                f'Section {sid}: new densify De Scripturae plenitudine Pars IV XXV-XXXII; '
                'Pass A!=B; lock-grounded PDF 99-101 / book pp. 87-89.'
            )
        else:
            notes = (
                f'Section {sid}: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in plenitudine Pars IV XXV packet scope covering all current sections.'
            )
        reviews.append({
            'section': sid,
            'verdict': 'pass',
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Scripturae plenitudine Pars IV XXV-XXXII only '
                f'(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. '
                'Honest partial; plenitudine Pars IV through XXXII; XXXIII+ remains. '
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': 'De Scripturae plenitudine Pars IV theses XXV-XXXII (Epiphanius / nations / church dignity)',
        'next_locus': 'De Scripturae plenitudine Pars IV XXXIII+ (superstition silence / Mass mysteries reply)',
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        f'## {day} (Scribe — De Scripturae plenitudine Pars IV XXV–XXXII densify LOCAL)\n\n'
        f'- Before: **{receipt["before"]}**. After: **{receipt["after"]}** '
        f'(contiguous Plenitudine Pars IV XXV–XXXII → §§{SECS[0]}–{SECS[-1]}; continues Pars IV).\n'
        ...
        '- Honest **partial**: Plenitudine Pars IV through XXXII (clarity vs sophistry; Scripture proves '
        ...
        f'- Pre-append hold cleared live>4149 OR 12m; post tip-ready hold cleared '
    ...
    repo_block = (
        f'## {day} ~ET (Scribe — Le Blanc Plenitudine Pars IV XXV–XXXII densify)\n\n'
        'CoS densify: De Scripturae plenitudine Pars IV XXV–XXXII (unwritten tradition equal-authority). '
        'Punch X: **NO**. No ship/commit/Logos/ai_promote.\n\n'
        '### Bound this job (not shipped)\n'
        f'- **Le Blanc** {slug_ref}: Plenitudine Pars IV XXV–XXXII (**'
        f'{receipt["before"]}→{receipt["after"]}** sections). Packet '
        f'{packet_ref}. Pass A≠B; tip-ready ok. Honest partial; Pars IV through XXXII. Next: Pars IV XXXIII+. '
        'Punch X: **NO**.\n\n---\n\n'
    )
...
def main():
    ...
    if mode == 'justifs':
        ...
        print('JUSTIFS DONE')
        return
    # Pre-append hold already cleared at live 57/4149
    write_lock(latin)
    write_and_check_justifications(latin, by_sec)
    eng, _src = reread_tip()
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-318 post-ready')
