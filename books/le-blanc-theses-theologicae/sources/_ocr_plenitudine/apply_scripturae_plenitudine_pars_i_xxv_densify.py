#!/usr/bin/env python3
"""De Scripturae plenitudine Pars I XXV-XXXII densify on Mini.

Lock + justifs (check_pass_ab each) first. Append eng/src only after live >3638
OR 12 minutes from tip-186 hold start (~19:30 EDT), then re-read tip.
...
DATA = Path("/tmp/leblanc_plen_xxv_densify")
PACKET_STEM = "scripturae_plenitudine_pars_i_xxv_densify"
APPLY_COPY = BOOK / "sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_i_xxv_densify.py"
...
SECS = ["187", "188", "189", "190", "191", "192", "193", "194"]
ROMANS = {
    "187": "XXV",
    "188": "XXVI",
    "189": "XXVII",
...
EDITION = (
    ...
    " Scripturae plenitudine Pars I I-XXXII (not whole folio)."
)
BLURB = (
    ...
    "ripturae through Pars IV XLIV, and De Scripturae plenitudine Pars I through XXXI"
    "I (historical plenitude across ages; progressive revelation; Canon growth; post-"
    "apostolic closed deposit). Not the collected folio."
)
METHOD = (
    ...
    "rs IV I-XLIV, and De Scripturae plenitudine Pars I I-XXXII, reconstructed from t"
    "he Internet Archive PDF page images with pdftotext + tesseract (+ DjVu checks). "
    "The 1683 third edition was not used as copy-text. No modern English was copied. "
    "This slice continues Plenitudine from XXV (plenitude across ages) through XXXII "
    "(oral teaching / unwritten polity admitted). XXXIII+ remains."
)
...
LOCK_HEADER = (
    ...
    "am non scripti.\nPARS PRIMA — theses I-XXXII (XXV-XXXII densify contiguous after "
    "I-XXIV).\nSame 1675 Pitt copy-text. Book pp. 54-60 / PDF 67-72. pdftotext + tess O"
    "CR (+ DjVu checks). Long-s and ligatures normalized. 1683 not copy-text. No mode"
    "rn English.\nScope: Plenitudine Pars I through XXXII (natural close before XXXIII"
    " sum-in-Scripture). XXXIII+ remains.\nNote: I-XXIV retained from prior lock; XXV "
    "plenitude across ages; XXVI progressive revelation / Heb 1:1 Greek; XXVII Canon "
    "growth; XXVIII mensura fidei; XXIX Mosaic sufficiency; XXX θεοπνεύστοις living-"
    "voice caution; XXXI post-translation closed deposit; XXXII viva voce / polity / "
    "historia admitted.\n\n"
)
...
def write_lock(latin: dict):
    # Expand lock: keep I-XXIV body from existing lock, append XXV-XXXII.
...
def wait_hold():
    """Owner hold: live >3638 OR 12 minutes from tip-186 hold, then re-read tip."""
    ...
    while True:
        ...
        if live_ok or timed_out:
            reason = f"live>{LIVE_FLOOR}" if live_ok else f"{HOLD_MINUTES}m since tip-186 hold start"
...
def build_packet_and_review():
    ...
    for sid in section_ids:
        ...
        if 187 <= n <= 194:
            notes = (
                f"Section {sid}: new densify De Scripturae plenitudine Pars I XXV-XXXII; "
                "Pass A!=B; lock-grounded PDF 71-72 / book pp. 59-60."
            )
        else:
            notes = (
    ...
    receipt = {
        ...
        "scope_review": {
            ...
            "uncertainties": [],
            "notes": (
                "Scope review: densify De Scripturae plenitudine Pars I XXV-XXXII only "
                f"(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. "
                "Honest partial; plenitudine XXXIII+ remain. Prior tracts untouched. Not shipped."
            ),
        },
        "reviews": reviews,
    }
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        "locus": "De Scripturae plenitudine Pars I theses XXV-XXXII (after I-XVI)",
        "next_locus": "De Scripturae plenitudine Pars I XXXIII+ (sum in Scripture / unwritten silence)",
        "gates": {
...
def prepend_handoff(receipt: dict):
    day = datetime.now().strftime("%Y-%m-%d")
    parts = [
        "## " + day + " (Scribe — De Scripturae plenitudine Pars I XXV–XXXII densify LOCAL)\n\n",
        "- Before: **" + str(receipt["before"]) + "** (Plenitudine Pars I I–XXIV). After: **"
        + str(receipt["after"]) + "** ",
        "(contiguous Plenitudine Pars I XXV–XXXII → §§" + SECS[0] + "–" + SECS[-1] + ").\n",
    ...
    repo_block = (
        "## " + day + " ~ET (Scribe — Le Blanc Plenitudine Pars I XXV–XXXII densify)\n\n"
        "CoS densify: De Scripturae plenitudine Pars I XXV–XXXII (historical plenitude). "
        ...
        + PACKET_STEM + "`. Pass A≠B; tip-ready ok. Honest partial. Next: Pars I XXXIII+. "
        "Punch X: **NO**.\n\n---\n\n"
    )
...
def main():
    ...
    if mode in ("all", "justifs"):
        ...
        if mode == "justifs":
            ...
            return
    # HARD HOLD — owner: live >3638 OR 12m, then re-read tip before append past 186
