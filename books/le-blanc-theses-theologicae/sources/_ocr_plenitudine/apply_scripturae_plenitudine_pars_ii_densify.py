#!/usr/bin/env python3
"""De Scripturae plenitudine Pars II I-VIII densify on Mini.

Lock + justifs (check_pass_ab each) first. Append eng/src only after live >3696
OR 12 minutes from tip-200 ship hold start (~19:55 EDT), then re-read tip.
...
DATA = Path("/tmp/leblanc_plen_pars_ii_densify")
PACKET_STEM = "scripturae_plenitudine_pars_ii_densify"
APPLY_COPY = BOOK / "sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_ii_densify.py"
...
SECS = ["201", "202", "203", "204", "205", "206", "207", "208"]
ROMANS = {
    "201": "I",
    "202": "II",
    "203": "III",
...
TIP_BEFORE = 200
LIVE_FLOOR = 3696
HOLD_START = datetime(2026, 9, 22, 19, 55, 0)
HOLD_MINUTES = 12

RANGE_SHORT = (
    "Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XLIV + De Scriptura"
    "e plenitudine Pars I I-XXXVIII + Pars II I-VIII"
)
RANGE_TITLE = (
...
EDITION = (
    ...
    " Scripturae plenitudine Pars I I-XXXVIII + Pars II I-VIII (not whole folio)."
)
BLURB = (
    ...
    "III plus Pars II through VIII (Pontificiorum sententia open: Reformed summary; R"
    "oman sole-rule denial; Bellarmine/Stapleton; tradition still required). Not the "
    "collected folio."
)
METHOD = (
    ...
    "rs IV I-XLIV, and De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-VIII, r"
    ...
    "glish was copied. This slice opens Plenitudine Pars II (Pontificiorum sententia)"
    " through VIII (tradition and unwritten word still required). Pars II IX+ remains."
)
...
LOCK_HEADER = (
    ...
    "am non scripti.\nPARS PRIMA — theses I-XXXVIII retained.\nPARS SECUNDA — theses I-VIII"
    " (Pontificiorum sententia; densify contiguous after Pars I close).\nSame 1675 Pitt "
    "copy-text. Book pp. 54-62 / PDF 67-74 (Pars II I-VIII on PDF 73-74 / pp. 61-62). "
    ...
    "through VIII. Pars II IX+ remains.\nNote: Pars I retained from prior lock; Pars II "
    "I summary of Reformed plenitude; II not word-for-word; III principles=Scripture; "
    "IV sole rule; V Roman doctors / Bellarmine on Augustine; VI not expressly / Stap"
    "leton; VII Roman consensus in seed; VIII tradition still required.\n\n"
)
...
def write_lock(latin: dict):
    # Keep Pars I body (I-XXXVIII); append Pars II I-VIII after a PARS SECUNDA marker.
    ...
    if prior:
        ...
        # Drop any prior Pars II append if re-run
        if "\n\nPARS SECUNDA" in rest:
            rest = rest.split("\n\nPARS SECUNDA")[0].rstrip() + "\n"
        elif "\nPARS SECUNDA" in rest:
            rest = rest.split("\nPARS SECUNDA")[0].rstrip() + "\n"
        body = rest.rstrip() + "\n"
...
def wait_hold():
    """Owner hold: live >3696 OR 12 minutes from tip-200 ship hold, then re-read tip."""
    ...
    while True:
        ...
        if live_ok or timed_out:
            reason = f"live>{LIVE_FLOOR}" if live_ok else f"{HOLD_MINUTES}m since tip-200 hold start"
...
def build_packet_and_review():
    ...
    packet = make_audit_packet(
        ...
        raw_sources=[
            ...
            BOOK / "sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_ii_densify.py",
        ],
    ...
    for sid in section_ids:
        ...
        if 201 <= n <= 208:
            notes = (
                f"Section {sid}: new densify De Scripturae plenitudine Pars II I-VIII; "
                "Pass A!=B; lock-grounded PDF 73-74 / book pp. 61-62."
            )
        else:
            notes = (
                f"Section {sid}: prior verified densify retained; Pass A/B and lock identity "
                "rechecked in plenitudine Pars II packet scope covering all current sections."
            )
        reviews.append(
            {
                "section": sid,
    ...
    receipt = {
        ...
        "scope_review": {
            ...
            "uncertainties": [],
            "notes": (
                "Scope review: densify De Scripturae plenitudine Pars II I-VIII only "
                f"(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. "
                "Honest partial; plenitudine Pars II IX+ remains. Prior tracts untouched. Not shipped."
            ),
        },
        "reviews": reviews,
    }
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        "locus": "De Scripturae plenitudine Pars II theses I-VIII (Pontificiorum sententia)",
        "next_locus": "De Scripturae plenitudine Pars II IX+ (unwritten dogma / tradition kinds)",
        "gates": {
...
def prepend_handoff(receipt: dict):
    day = datetime.now().strftime("%Y-%m-%d")
    parts = [
        "## " + day + " (Scribe — De Scripturae plenitudine Pars II I–VIII densify LOCAL)\n\n",
        "- Before: **" + str(receipt["before"]) + "** (Plenitudine Pars I I–XXXVIII). After: **"
        + str(receipt["after"]) + "** ",
        "(contiguous Plenitudine Pars II I–VIII → §§" + SECS[0] + "–" + SECS[-1] + ").\n",
        "- Packet `" + PACKET_STEM + "` (`reviews/audit/" + PACKET_STEM + ".packet.json` + `.review.json`). ",
        "Reviewer: scribe-leblanc, " + day + ". Verdict pass grounded in ",
        "`sources/_le_blanc_plenitudine_latin_lock.txt` (PDF 73–74 / book pp. 61–62).\n",
        "- Meta range bumped to **" + RANGE_SHORT + "** (title, edition, blurb, text_history.method).\n",
        "- Honest **partial**: Plenitudine Pars II through VIII (Reformed summary; equipollent/"
        "consequence; principles=Scripture; sole rule; Roman sole-rule denial; Bellarmine/Stapleton; "
        "consensus in seed; tradition still required). IX+ remains. De Theologia / De Fide / "
        "Authoritate / Plenitudine Pars I closed as before. Not folio. Not shipped this slice.\n",
        "- Pass A ≠ B for §§" + SECS[0] + "–" + SECS[-1] + ". Bellarmine / Stapleton / Augustine sense; "
        "Patres docent; corde depositum. OCR restorations noted in translator_notes.\n",
    ...
    repo_block = (
        "## " + day + " ~ET (Scribe — Le Blanc Plenitudine Pars II I–VIII densify)\n\n"
        "CoS densify: De Scripturae plenitudine Pars II I–VIII (Pontificiorum sententia). "
        "Punch X: **NO**. No ship/commit/Logos/ai_promote.\n\n"
        "### Bound this job (not shipped)\n"
        "- **Le Blanc** `le-blanc-theses-theologicae`: Plenitudine Pars II tip (**"
        + str(receipt["before"]) + "→" + str(receipt["after"]) + "** sections). Packet `"
        + PACKET_STEM + "`. Pass A≠B; tip-ready ok. Honest partial. Next: Pars II IX+. "
        "Punch X: **NO**.\n\n---\n\n"
    )
...
def main():
    ...
    if mode in ("all", "justifs"):
        ...
        if mode == "justifs":
            ...
            return
    # HARD HOLD — owner: live >3696 OR 12m, then re-read tip before append past 200
