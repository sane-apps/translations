#!/usr/bin/env python3
"""De Scripturae plenitudine Pars IV IX-XVI densify on Mini.

HOLD until live >4076 OR 12m (prefer ship). Then append at tip-302.
...
DATA = Path('/tmp/leblanc_plen_pars_iv_ix_densify')
PACKET_STEM = 'scripturae_plenitudine_pars_iv_ix_densify'
APPLY_COPY = BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iv_ix_densify.py'
...
SECS = ['303', '304', '305', '306', '307', '308', '309', '310']
ROMANS = {'303': 'IX', '304': 'X', '305': 'XI', '306': 'XII', '307': 'XIII', '308': 'XIV', '309': 'XV', '310': 'XVI'}
...
RANGE_SHORT = (
    ...
    "De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-L + Pars IV I-XVI"
)
RANGE_TITLE = (
...
METHOD = (
    ...
    "De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-L + Pars IV I-XVI, reconstructed from "
...
LOCK_HEADER = (
    ...
    "PARS QUARTA — theses I-XVI (unwritten tradition; 1 Cor 11; 2 Thess 2).\n"
    "Same 1675 Pitt copy-text. Book pp. 54-84 / PDF 67-96 (Pars IV IX-XVI on PDF 97-98 / pp. 85-86). "
    "pdftotext + tess OCR (+ DjVu checks). Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: Plenitudine Pars I–III closed + Pars IV through XVI. Pars IV XVII+ remains.\n"
...
def write_lock(latin: dict):
    ...
    for sec in [str(n) for n in range(295, 303)]:
        parts.append(by_prior[sec].rstrip() + chr(10))
    for sec in SECS:
...
def build_packet_and_review():
    ...
    packet = make_audit_packet(
        ...
        raw_sources=[
            LOCK,
            pdf,
            DATA / 'pdf_96_99_layout.txt',
            DATA / 'tess_097.txt',
            DATA / 'tess_098.txt',
            DATA / 'latin.json',
            BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iv_densify.py',
        ],
    ...
    for sid in section_ids:
        ...
        if 303 <= n <= 310:
            notes = (
                f'Section {sid}: new densify De Scripturae plenitudine Pars IV IX-XVI; '
                'Pass A!=B; lock-grounded PDF 97-98 / book pp. 85-86.'
            )
        else:
            notes = (
                f'Section {sid}: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in plenitudine Pars IV IX packet scope covering all current sections.'
            )
        reviews.append({
            'section': sid,
            'verdict': 'pass',
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Scripturae plenitudine Pars IV IX-XVI only '
                f'(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. '
                'Honest partial; plenitudine Pars IV through XVI; XVII+ remains. '
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': 'De Scripturae plenitudine Pars IV theses IX-XVI (1 Cor 11 cetera / 2 Thess 2)',
        'next_locus': 'De Scripturae plenitudine Pars IV XVII+ (Antichrist timing / Augustine Civ. Dei)',
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        f'## {day} (Scribe — De Scripturae plenitudine Pars IV IX–XVI densify LOCAL)\n\n'
        f'- Before: **{receipt["before"]}**. After: **{receipt["after"]}** '
        f'(contiguous Plenitudine Pars IV IX–XVI → §§{SECS[0]}–{SECS[-1]}; continues Pars IV).\n'
        f'- Packet {packet_ref} ({packet_files}). '
        f'Reviewer: scribe-leblanc, {day}. Verdict pass grounded in '
        f'{lock_ref} (PDF 97-98 / pp. 85-86).\n'
        f'- Meta range bumped to **{RANGE_SHORT}** (title, edition, blurb, text_history.method).\n'
        '- Honest **partial**: Plenitudine Pars IV through XVI (clarity vs sophistry; Scripture proves '
        ...
        f'- Pre-append hold cleared live>4076 OR 12m; post tip-ready hold cleared '
    ...
    repo_block = (
        f'## {day} ~ET (Scribe — Le Blanc Plenitudine Pars IV IX–XVI densify)\n\n'
        'CoS densify: De Scripturae plenitudine Pars IV IX–XVI (unwritten tradition equal-authority). '
        'Punch X: **NO**. No ship/commit/Logos/ai_promote.\n\n'
        '### Bound this job (not shipped)\n'
        f'- **Le Blanc** {slug_ref}: Plenitudine Pars IV IX–XVI (**'
        f'{receipt["before"]}→{receipt["after"]}** sections). Packet '
        f'{packet_ref}. Pass A≠B; tip-ready ok. Honest partial; Pars IV through XVI. Next: Pars IV XVII+. '
        'Punch X: **NO**.\n\n---\n\n'
    )
...
def main():
    ...
    live_tuple = wait_hold(pre_start, LIVE_FLOOR, label='tip-302 pre-append')
    write_lock(latin)
    write_and_check_justifications(latin, by_sec)
    eng, _src = reread_tip()
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-302 post-ready')
