#!/usr/bin/env python3
"""De Scripturae plenitudine Pars III XLVIII-L densify on Mini.

Lock + justifs (check_pass_ab each) first. Pre-append hold CLEARED at live 57/4025.
Append eng/src at tip-291, tip-ready, packet. Then HOLD until live >4025 OR 12m.
...
DATA = Path('/tmp/leblanc_plen_pars_iii_xlviii_densify')
PACKET_STEM = 'scripturae_plenitudine_pars_iii_xlviii_densify'
APPLY_COPY = BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iii_xlviii_densify.py'
...
SECS = ['292', '293', '294']
ROMANS = {'292': 'XLVIII', '293': 'XLIX', '294': 'L'}
...
RANGE_SHORT = (
    ...
    "De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-L"
)
RANGE_TITLE = (
...
METHOD = (
    ...
    "De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-L, reconstructed from "
...
LOCK_HEADER = (
    ...
    "PARS TERTIA — theses I-L (sense of Scripture / pastors / obscurity / tradition lock / "
    "clarity vs sophistry / final sufficiency).\n"
    "Same 1675 Pitt copy-text. Book pp. 54-82 / PDF 67-94 (Pars III XLVIII-L on PDF 94 / p. 82). "
    "pdftotext + tess OCR (+ DjVu checks). Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: Plenitudine Pars I through XXXVIII + Pars II through XLIV + Pars III through L. Pars IV remains.\n"
    "Note: Pars III I-XLVII retained; XLVIII sophistry-clarity; XLIX Scripture-proves-sufficiency; "
    "L no rival word / duty stops at revelation. Closes Pars III.\n\n"
)
...
def write_lock(latin: dict):
    ...
    for sec in [str(n) for n in range(245, 292)]:
        parts.append(by_prior[sec].rstrip() + chr(10))
    for sec in SECS:
...
def wait_hold_post_tipready(hold_start: datetime):
    ...
    while True:
        ...
        if live_ok or timed_out:
            reason = f'live>{LIVE_FLOOR}' if live_ok else f'{HOLD_MINUTES}m since tip-291 post-ready hold start (live>{LIVE_FLOOR})'
...
def build_packet_and_review():
    ...
    packet = make_audit_packet(
        ...
        raw_sources=[
            LOCK,
            pdf,
            DATA / 'pdf_94_95_layout.txt',
            DATA / 'tess_94.txt',
            DATA / 'tess_95.txt',
            DATA / 'latin.json',
            BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iii_xl_densify.py',
        ],
    ...
    for sid in section_ids:
        ...
        if 292 <= n <= 294:
            notes = (
                f'Section {sid}: new densify De Scripturae plenitudine Pars III XLVIII-L; '
                'Pass A!=B; lock-grounded PDF 94 / book p. 82.'
            )
        else:
            notes = (
                f'Section {sid}: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in plenitudine Pars III XLVIII packet scope covering all current sections.'
            )
        reviews.append({
            'section': sid,
            'verdict': 'pass',
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Scripturae plenitudine Pars III XLVIII-L only '
                f'(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. '
                'Honest partial; plenitudine Pars III closed through L; Pars IV remains. '
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': 'De Scripturae plenitudine Pars III theses XLVIII-L (clarity vs sophistry / final sufficiency)',
        'next_locus': 'De Scripturae plenitudine Pars IV I+ (unwritten tradition equal-authority objections)',
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        f'## {day} (Scribe — De Scripturae plenitudine Pars III XLVIII–L densify LOCAL)\n\n'
        f'- Before: **{receipt["before"]}**. After: **{receipt["after"]}** '
        f'(contiguous Plenitudine Pars III XLVIII–L → §§{SECS[0]}–{SECS[-1]}; closes Pars III).\n'
        f'- Packet {packet_ref} ({packet_files}). '
        f'Reviewer: scribe-leblanc, {day}. Verdict pass grounded in '
        f'{lock_ref} (PDF 94 / book p. 82).\n'
        f'- Meta range bumped to **{RANGE_SHORT}** (title, edition, blurb, text_history.method).\n'
        '- Honest **partial**: Plenitudine Pars III closed (clarity vs sophistry; Scripture proves '
        ...
        f'- Pre-append hold cleared live>4012; post tip-ready hold cleared '
    ...
    repo_block = (
        f'## {day} ~ET (Scribe — Le Blanc Plenitudine Pars III XLVIII–L densify)\n\n'
        'CoS densify: De Scripturae plenitudine Pars III XLVIII–L (clarity vs sophistry / final sufficiency). '
        'Punch X: **NO**. No ship/commit/Logos/ai_promote.\n\n'
        '### Bound this job (not shipped)\n'
        f'- **Le Blanc** {slug_ref}: Plenitudine Pars III XLVIII–L (**'
        f'{receipt["before"]}→{receipt["after"]}** sections). Packet '
        f'{packet_ref}. Pass A≠B; tip-ready ok. Honest partial; Pars III closed. Next: Pars IV I+. '
        'Punch X: **NO**.\n\n---\n\n'
    )
...
def main():
    ...
    if mode == 'finish':
        eng, src = reread_tip()
        if len(eng) != TIP_BEFORE + len(SECS):
            raise SystemExit(f'finish expects tip {TIP_BEFORE + len(SECS)}, got {len(eng)}')
        tip_ready()
        hold_path = DATA / 'hold_post_tipready.json'
        if hold_path.exists():
            hold_start = datetime.fromisoformat(json.loads(hold_path.read_text())['hold_start'])
        else:
            hold_start = datetime.fromisoformat('2026-09-22T22:53:12')
            hold_path.write_text(
                json.dumps({
                    'hold_start': hold_start.isoformat(timespec='seconds'),
                    'live_floor': LIVE_FLOOR,
                    'tip_after': TIP_BEFORE + len(SECS),
                    'note': 'post tip-ready hold live>4025 OR 12m',
                }, indent=2) + '\n',
                encoding='utf-8',
            )
        print('post-tip hold start', hold_start.isoformat(timespec='seconds'))
        ...
        live_tuple = wait_hold_post_tipready(hold_start)
    ...
    if mode in ('all', 'justifs'):
        ...
        if mode == 'justifs':
            print('JUSTIFS DONE — append deferred')
            return
    eng, src = reread_tip()
