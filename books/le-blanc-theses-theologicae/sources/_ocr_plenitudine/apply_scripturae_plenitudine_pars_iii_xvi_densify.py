#!/usr/bin/env python3
"""De Scripturae plenitudine Pars III XVI-XXIII densify on Mini.

Lock + justifs (check_pass_ab each) first. Append eng/src after live >3896
OR 12m from tip-259 proceed (~21:15 EDT), then re-read tip.
...
DATA = Path('/tmp/leblanc_plen_pars_iii_xvi_densify')
PACKET_STEM = 'scripturae_plenitudine_pars_iii_xvi_densify'
APPLY_COPY = BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iii_xvi_densify.py'
...
SECS = ['260', '261', '262', '263', '264', '265', '266', '267']
ROMANS = {
    '260': 'XVI', '261': 'XVII', '262': 'XVIII', '263': 'XIX',
    '264': 'XX', '265': 'XXI', '266': 'XXII', '267': 'XXIII',
}

TIP_BEFORE = 259
LIVE_FLOOR = 3896
HOLD_START = datetime(2026, 9, 22, 21, 15, 24)
HOLD_MINUTES = 12

RANGE_SHORT = "Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XLIV + De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-XXIII"
RANGE_TITLE = "Theological Theses (De Theologia I-XLIII; De Fide I-XXII; De Authoritate Scripturae Pars I I-XLVII; Pars II I-XXXV; Pars III I-XLI; Pars IV I-XLIV; De Scripturae plenitudine Pars I I-XXXVIII; Pars II I-XLIV; Pars III I-XXIII)"
EDITION = "Theses theologicae (London: Moses Pitt, 1675). ESTC R17887; Wing L802. IA bub_gb_eOkHAW4G0-wC. Densify: De Theologia I-XLIII + De Fide I-XXII + De Authoritate Scripturae Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XLIV + De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-XXIII (not whole folio)."
BLURB = "Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. Covers De Theologia I-XLIII, De Fide I-XXII, De Authoritate Scripturae through Pars IV XLIV, and De Scripturae plenitudine Pars I through XXXVIII, Pars II through XLIV, and Pars III through XXIII (OT remedies / Bellarmine instances). Not the collected folio."
METHOD = "English follows the locked 1675 Pitt Latin of De Theologia I-XLIII, De Fide I-XXII, De Authoritate Scripturae Pars I I-XLVII, Pars II I-XXXV, Pars III I-XLI, Pars IV I-XLIV, and De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-XXIII, reconstructed from the Internet Archive PDF page images with pdftotext + tesseract (+ DjVu checks). The 1683 third edition was not used as copy-text. No modern English was copied. This slice densifies Plenitudine Pars III XVI-XXIII (things not in Scripture; OT remedies; false sign-hypothesis; circumcision as seal; Mary perpetual virginity; Pascha; infant baptism). XXIV+ remains."
...
LOCK_HEADER = (
    ...
    "PARS TERTIA — theses I-XXIII (OT remedies / Bellarmine instances).\n"
    "Same 1675 Pitt copy-text. Book pp. 54-76 / PDF 67-88 (Pars III XVI-XXIII on PDF 86-88 / pp. 74-76). "
    "pdftotext + tess OCR (+ DjVu checks). Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: Plenitudine Pars I through XXXVIII + Pars II through XLIV + Pars III through XXIII. Pars III XXIV+ remains.\n"
    "Note: Pars III I-XV retained; XVI-XIX OT remedies; XX-XXIII Mary / Pascha / infant baptism.\n\n"
)
...
def write_lock(latin: dict):
    ...
    for sec in [str(n) for n in range(245, 260)]:
        parts.append(by_prior[sec].rstrip() + chr(10))
    for sec in SECS:
...
def wait_hold():
    ...
    while True:
        ...
        if live_ok or timed_out:
            reason = f'live>{LIVE_FLOOR}' if live_ok else f'{HOLD_MINUTES}m since tip-259 hold start'
...
def build_packet_and_review():
    ...
    packet = make_audit_packet(
        ...
        raw_sources=[
            LOCK,
            pdf,
            Path('/tmp/leblanc_plen_pars_iii_xvi_densify/pdf_86_89_raw.txt'),
            Path('/tmp/leblanc_plen_pars_iii_xvi_densify/tess_086.txt'),
            Path('/tmp/leblanc_plen_pars_iii_xvi_densify/tess_087.txt'),
            Path('/tmp/leblanc_plen_pars_iii_xvi_densify/tess_088.txt'),
            BOOK / 'sources/_ocr_plenitudine/pdf_84_90_raw.txt',
            BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iii_ix_densify.py',
        ],
    ...
    for sid in section_ids:
        ...
        if 260 <= n <= 267:
            notes = (
                f'Section {sid}: new densify De Scripturae plenitudine Pars III XVI-XXIII; '
                'Pass A!=B; lock-grounded PDF 86-88 / book pp. 74-76.'
            )
        else:
            notes = (
                f'Section {sid}: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in plenitudine Pars III XVI packet scope covering all current sections.'
            )
        reviews.append({
            'section': sid,
            'verdict': 'pass',
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Scripturae plenitudine Pars III XVI-XXIII only '
                f'(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. '
                'Honest partial; plenitudine Pars III through XXIII; XXIV+ remains. Prior tracts untouched. Not shipped.'
            ),
        },
        'reviews': reviews,
    }
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': 'De Scripturae plenitudine Pars III theses XVI-XXIII (OT remedies / false sign-hypothesis / circumcision seal / Mary / Pascha / infant baptism)',
        'next_locus': 'De Scripturae plenitudine Pars III XXIV+ (rebaptism / Filioque / Mosaic immortality / Scripture self-witness)',
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        f"## {day} (Scribe — De Scripturae plenitudine Pars III XVI–XXIII densify LOCAL)\n\n"
        f"- Before: **{receipt['before']}** (Plenitudine Pars III I–XV). After: **{receipt['after']}** "
        f"(contiguous Plenitudine Pars III XVI–XXIII → §§{SECS[0]}–{SECS[-1]}).\n"
        f"- Packet {packet_ref} ({packet_files}). "
        f"Reviewer: scribe-leblanc, {day}. Verdict pass grounded in "
        f"{lock_ref} (PDF 86–88 / book pp. 74–76).\n"
        f"- Meta range bumped to **{RANGE_SHORT}** (title, edition, blurb, text_history.method).\n"
        "- Honest **partial**: Plenitudine Pars III through XXIII (things not in Scripture; OT "
        "remedies for women/day-eight males; tradition silence retorsion; false sign-hypothesis; "
        "circumcision as Rom. 4 seal; Mary perpetual virginity; Pascha indifferent; infant baptism "
        "Scripture-proved). XXIV+ remains. De Theologia / De Fide / Authoritate / Plenitudine "
        "Pars I / Pars II / Pars III I–XV closed as before. Not folio. Not shipped.\n"
        f"- Pass A ≠ B for §§{SECS[0]}–{SECS[-1]}. Sigillum justitiae fidei; Basil τὸ ἐφεξῆς; "
        "Quartodecimani; paedobaptismum. OCR restorations noted in translator_notes.\n"
    ...
    repo_block = (
        f"## {day} ~ET (Scribe — Le Blanc Plenitudine Pars III XVI–XXIII densify)\n\n"
        "CoS densify: De Scripturae plenitudine Pars III XVI–XXIII (OT remedies / Bellarmine instances). "
        "Punch X: **NO**. No ship/commit/Logos/ai_promote.\n\n"
        "### Bound this job (not shipped)\n"
        f"- **Le Blanc** {slug_ref}: Plenitudine Pars III XVI–XXIII (**"
        f"{receipt['before']}→{receipt['after']}** sections). Packet "
        f"{packet_ref}. Pass A≠B; tip-ready ok. Honest partial. Next: Pars III XXIV+. "
        "Punch X: **NO**.\n\n---\n\n"
    )
