#!/usr/bin/env python3
"""De Scripturae plenitudine Pars IV XVII-XXIV densify on Mini.

HOLD until live >4113 OR 12m (prefer ship). Append at tip-310.
After tip-ready: HOLD until live > current OR 12m.
No ship. Punch X=NO.
"""
...
DATA = Path('/tmp/leblanc_plen_pars_iv_xvii_densify')
PACKET_STEM = 'scripturae_plenitudine_pars_iv_xvii_densify'
APPLY_COPY = BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iv_xvii_densify.py'
...
SECS = ['311', '312', '313', '314', '315', '316', '317', '318']
ROMANS = {'311': 'XVII', '312': 'XVIII', '313': 'XIX', '314': 'XX', '315': 'XXI', '316': 'XXII', '317': 'XXIII', '318': 'XXIV'}
...
RANGE_SHORT = (
    ...
    "De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-L + Pars IV I-XXIV"
)
RANGE_TITLE = (
...
METHOD = (
    ...
    "De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XLIV + Pars III I-L + Pars IV I-XXIV, reconstructed from "
...
LOCK_HEADER = (
    ...
    "PARS QUARTA — theses I-XXIV (unwritten tradition; Antichrist; heretic-consensus).\n"
    "Same 1675 Pitt copy-text. Book pp. 54-84 / PDF 67-96 (Pars IV XVII-XXIV on PDF 98-99 / pp. 86-87). "
    "pdftotext + tess OCR (+ DjVu checks). Long-s and ligatures normalized. 1683 not copy-text. No modern English.\n"
    "Scope: Plenitudine Pars I–III closed + Pars IV through XXIV. Pars IV XXV+ remains.\n"
...
def write_lock(latin: dict):
    ...
    for sec in [str(n) for n in range(295, 311)]:
        parts.append(by_prior[sec].rstrip() + chr(10))
    for sec in SECS:
...
def build_packet_and_review():
    ...
    packet = make_audit_packet(
        ...
        raw_sources=[
            LOCK,
            pdf,
            DATA / 'pdf_98_101_layout.txt',
            DATA / 'tess_098.txt',
            DATA / 'tess_099.txt',
            DATA / 'latin.json',
            BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_iv_ix_densify.py',
        ],
    ...
    for sid in section_ids:
        ...
        if 311 <= n <= 318:
            notes = (
                f'Section {sid}: new densify De Scripturae plenitudine Pars IV XVII-XXIV; '
                'Pass A!=B; lock-grounded PDF 98-99 / book pp. 86-87.'
            )
        else:
            notes = (
                f'Section {sid}: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in plenitudine Pars IV XVII packet scope covering all current sections.'
            )
        reviews.append({
            'section': sid,
            'verdict': 'pass',
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Scripturae plenitudine Pars IV XVII-XXIV only '
                f'(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. '
                'Honest partial; plenitudine Pars IV through XXIV; XXV+ remains. '
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': 'De Scripturae plenitudine Pars IV theses XVII-XXIV (Antichrist / Augustine / heretics)',
        'next_locus': 'De Scripturae plenitudine Pars IV XXV+ (Epiphanius / more heretic examples)',
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    block = (
        f'## {day} (Scribe — De Scripturae plenitudine Pars IV XVII–XXIV densify LOCAL)\n\n'
        f'- Before: **{receipt["before"]}**. After: **{receipt["after"]}** '
        f'(contiguous Plenitudine Pars IV XVII–XXIV → §§{SECS[0]}–{SECS[-1]}; continues Pars IV).\n'
        f'- Packet {packet_ref} ({packet_files}). '
        f'Reviewer: scribe-leblanc, {day}. Verdict pass grounded in '
        f'{lock_ref} (PDF 98-99 / pp. 86-87).\n'
        f'- Meta range bumped to **{RANGE_SHORT}** (title, edition, blurb, text_history.method).\n'
        '- Honest **partial**: Plenitudine Pars IV through XXIV (clarity vs sophistry; Scripture proves '
        ...
        f'- Pre-append hold cleared live>4113 OR 12m; post tip-ready hold cleared '
    ...
    repo_block = (
        f'## {day} ~ET (Scribe — Le Blanc Plenitudine Pars IV XVII–XXIV densify)\n\n'
        'CoS densify: De Scripturae plenitudine Pars IV XVII–XXIV (unwritten tradition equal-authority). '
        'Punch X: **NO**. No ship/commit/Logos/ai_promote.\n\n'
        '### Bound this job (not shipped)\n'
        f'- **Le Blanc** {slug_ref}: Plenitudine Pars IV XVII–XXIV (**'
        f'{receipt["before"]}→{receipt["after"]}** sections). Packet '
        f'{packet_ref}. Pass A≠B; tip-ready ok. Honest partial; Pars IV through XXIV. Next: Pars IV XXV+. '
        'Punch X: **NO**.\n\n---\n\n'
    )
...
def main():
    ...
    if mode == 'justifs':
        ...
        print('JUSTIFS DONE')
        return
    hold_meta = json.loads((DATA / 'hold.json').read_text())
    pre_start = datetime.fromisoformat(hold_meta['hold_start'])
    print('pre-append hold from', pre_start.isoformat(timespec='seconds'), 'floor', LIVE_FLOOR)
    live_tuple = wait_hold(pre_start, LIVE_FLOOR, label='tip-310 pre-append')
    write_lock(latin)
    write_and_check_justifications(latin, by_sec)
    eng, _src = reread_tip()
    ...
    live_tuple = wait_hold(hold_start, post_floor, label='tip-310 post-ready')
