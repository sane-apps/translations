#!/usr/bin/env python3
"""De Scripturae plenitudine Pars II XXIV-XXXI densify on Mini.

Lock + justifs (check_pass_ab each) first. Append eng/src only after live >3757
OR 12 minutes from tip-223 catch-up-ship hold start (~20:21 EDT), then re-read tip.
...
DATA = Path('/tmp/leblanc_plen_pars_ii_xxiv_densify')
PACKET_STEM = 'scripturae_plenitudine_pars_ii_xxiv_densify'
APPLY_COPY = BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_ii_xxiv_densify.py'
...
SECS = ['224', '225', '226', '227', '228', '229', '230', '231']
ROMANS = {
    '224': 'XXIV', '225': 'XXV', '226': 'XXVI', '227': 'XXVII',
    '228': 'XXVIII', '229': 'XXIX', '230': 'XXX', '231': 'XXXI',
}

TIP_BEFORE = 223
LIVE_FLOOR = 3757
HOLD_START = datetime(2026, 9, 22, 20, 21, 0)
HOLD_MINUTES = 12

RANGE_SHORT = 'Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XLIV + De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XXXI'
RANGE_TITLE = 'Theological Theses (De Theologia I-XLIII; De Fide I-XXII; De Authoritate Scripturae Pars I I-XLVII; Pars II I-XXXV; Pars III I-XLI; Pars IV I-XLIV; De Scripturae plenitudine Pars I I-XXXVIII; Pars II I-XXXI)'
EDITION = 'Theses theologicae (London: Moses Pitt, 1675). ESTC R17887; Wing L802. IA bub_gb_eOkHAW4G0-wC. Densify: De Theologia I-XLIII + De Fide I-XXII + De Authoritate Scripturae Pars I I-XLVII + Pars II I-XXXV + Pars III I-XLI + Pars IV I-XLIV + De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XXXI (not whole folio).'
BLURB = 'Louis Le Blanc de Beaulieu - Sedan Theological Theses, newly rendered from the 1675 London Latin. Covers De Theologia I-XLIII, De Fide I-XXII, De Authoritate Scripturae through Pars IV XLIV, and De Scripturae plenitudine Pars I through XXXVIII plus Pars II through XXXI (two-point question; Reformed affirmations; Jewish-Church argument). Not the collected folio.'
METHOD = 'English follows the locked 1675 Pitt Latin of De Theologia I-XLIII, De Fide I-XXII, De Authoritate Scripturae Pars I I-XLVII, Pars II I-XXXV, Pars III I-XLI, Pars IV I-XLIV, and De Scripturae plenitudine Pars I I-XXXVIII + Pars II I-XXXI, reconstructed from the Internet Archive PDF page images with pdftotext + tesseract (+ DjVu checks). The 1683 third edition was not used as copy-text. No modern English was copied. This slice continues Plenitudine Pars II through XXXI (two-point question XXIV; Reformed affirmations XXV; Jewish-Church argument and replies XXVI-XXXI). Pars II XXXII+ remains.'

LOCK_HEADER = 'Locked Latin: Louis Le Blanc, Theses theologicae (London: Moses Pitt, 1675).\nTract: De Scripturae Plenitudine et Sufficientia adversus necessitatem verbi cujusdam non scripti.\nPARS PRIMA — theses I-XXXVIII retained.\nPARS SECUNDA — theses I-XXXI (Pontificiorum sententia + discernment + status + Reformed affirmations XXIV-XXXI).\nSame 1675 Pitt copy-text. Book pp. 54-66 / PDF 67-78 (Pars II XXIV-XXXI on PDF 76-78 / pp. 64-66). pdftotext + tess OCR (+ DjVu checks). Long-s and ligatures normalized. 1683 not copy-text. No modern English.\nScope: Plenitudine Pars I through XXXVIII + Pars II through XXXI. Pars II XXXII+ remains.\nNote: I-XXIII retained; XXIV two-point question; XXV two affirmations; XXVI-XXXI Jewish-Church argument and Roman replies.\n\n'
...
def write_lock(latin: dict):
    ...
    for sec in [str(n) for n in range(201, 224)]:
...
def wait_hold():
    ...
    while True:
        ...
        if live_ok or timed_out:
            reason = f'live>{LIVE_FLOOR}' if live_ok else f'{HOLD_MINUTES}m since tip-223 hold start'
...
def build_packet_and_review():
    ...
    packet = make_audit_packet(
        ...
        raw_sources=[
            ...
            BOOK / 'sources/_ocr_plenitudine/tesseng-p-076.txt',
            BOOK / 'sources/_ocr_plenitudine/tesseng-p-077.txt',
            BOOK / 'sources/_ocr_plenitudine/tesseng-p-078.txt',
            BOOK / 'sources/_ocr_plenitudine/apply_scripturae_plenitudine_pars_ii_xxiv_densify.py',
        ],
    ...
    for sid in section_ids:
        ...
        if 224 <= n <= 231:
            notes = (
                f'Section {sid}: new densify De Scripturae plenitudine Pars II XXIV-XXXI; '
                'Pass A!=B; lock-grounded PDF 76-78 / book pp. 64-66.'
            )
        else:
            notes = (
                f'Section {sid}: prior verified densify retained; Pass A/B and lock identity '
                'rechecked in plenitudine Pars II XXIV packet scope covering all current sections.'
            )
        reviews.append({
            'section': sid,
            'verdict': 'pass',
    ...
    receipt = {
        ...
        'scope_review': {
            ...
            'uncertainties': [],
            'notes': (
                'Scope review: densify De Scripturae plenitudine Pars II XXIV-XXXI only '
                f'(sections {SECS[0]}-{SECS[-1]}). Meta discloses {RANGE_SHORT}. '
                'Honest partial; plenitudine Pars II XXXII+ remains. Prior tracts untouched. Not shipped.'
            ),
        },
        'reviews': reviews,
    }
...
def write_receipt(packet_path: Path, review_path: Path, live_tuple, packet: dict):
    ...
    receipt = {
        ...
        'locus': 'De Scripturae plenitudine Pars II theses XXIV-XXXI (two-point question / Reformed affirmations / Jewish-Church argument)',
        'next_locus': 'De Scripturae plenitudine Pars II XXXII+ (memory/corruption of viva-voce deposit)',
        'gates': {
...
def prepend_handoff(receipt: dict):
    ...
    parts = [
        "## " + day + " (Scribe — De Scripturae plenitudine Pars II XXIV–XXXI densify LOCAL)\n\n",
        "- Before: **" + str(receipt["before"]) + "** (Plenitudine Pars II I–XXIII). After: **"
        + str(receipt["after"]) + "** ",
        "(contiguous Plenitudine Pars II XXIV–XXXI → §§" + SECS[0] + "–" + SECS[-1] + ").\n",
        "- Packet " + packet_ref + " (" + packet_files + "). ",
        "Reviewer: scribe-leblanc, " + day + ". Verdict pass grounded in ",
        lock_ref + " (PDF 76–78 / book pp. 64–66).\n",
        "- Meta range bumped to **" + RANGE_SHORT + "** (title, edition, blurb, text_history.method).\n",
        "- Honest **partial**: Plenitudine Pars II through XXIII (three discernment rules; "
        "collapse to present-Church judgment; Reformed polity/consequence concessions; "
        "Roman necessary-to-salvation concessions; real difference = second equal word). "
        "XXIV+ remains. De Theologia / De Fide / Authoritate / Plenitudine "
        "Pars I / Pars II I–XXIII closed as before. Not folio. Not shipped this slice.\n",
        "- Pass A ≠ B for §§" + SECS[0] + "–" + SECS[-1] + ". de fide; Quadragesimale; "
        "politiam; fides explicita; paris authoritatis. OCR restorations noted in translator_notes.\n",
    ...
    repo_block = (
        "## " + day + " ~ET (Scribe — Le Blanc Plenitudine Pars II XXIV–XXXI densify)\n\n"
        "CoS densify: De Scripturae plenitudine Pars II XXIV–XXXI (discernment rules / status). "
        "Punch X: **NO**. No ship/commit/Logos/ai_promote.\n\n"
        "### Bound this job (not shipped)\n"
        "- **Le Blanc** " + slug_ref + ": Plenitudine Pars II XXIV–XXXI (**"
        + str(receipt["before"]) + "→" + str(receipt["after"]) + "** sections). Packet "
        + packet_ref + ". Pass A≠B; tip-ready ok. Honest partial. Next: Pars II XXXII+. "
        "Punch X: **NO**.\n\n---\n\n"
    )
