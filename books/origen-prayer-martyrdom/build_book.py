"""Build the Origen Prayer + Martyrdom Logos Personal Book DOCX."""
...
OUT_DOCX = BOOK_DIR / "origen-prayer-martyrdom.docx"
RECEIPT = BOOK_DIR / "build_receipt.json"

WORKS = [
    ("gebet_english.json", "On Prayer", "prayer", "De oratione, GCS III (Koetschau 1899), Bd II."),
    ("martyrium_english.json", "Exhortation to Martyrdom", "mart", "Exhortatio ad martyrium, GCS II (Koetschau 1899), Bd I."),
]

FRONT_MATTER = [
    "New English rendering for private study, prepared with AI assistance from the Greek text of the GCS Koetschau edition. No modern copyrighted translation has been copied.",
    "Scope: the complete On Prayer (34 chapters) and the complete Exhortation to Martyrdom (proem and 51 chapters) — works of Origen lacking a public-domain English version.",
    ...
    "Passages marked rough await page-image verification against the GCS edition.",
]
...
def main() -> None:
    doc = setup_document(
        title="Origen: On Prayer and Exhortation to Martyrdom (New English)",
        author="Origen of Alexandria",
        subject="New English rendering for private Logos study",
        keywords="Origen, prayer, martyrdom, ante-nicene",
    )
    doc.add_paragraph("Origen: On Prayer and Exhortation to Martyrdom", style="Title")
    ...
    for fname, work_title, key_prefix, edition_line in WORKS:
        entries = json.loads((BOOK_DIR / "translations" / fname).read_text())
