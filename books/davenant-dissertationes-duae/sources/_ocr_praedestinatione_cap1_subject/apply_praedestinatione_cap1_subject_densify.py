#!/usr/bin/env python3
"""De praedestinatione Cap.1 subject densify — tip 207 → 214. Packet praedestinatione_cap1_subject_densify."""
...
def wait_hold() -> str:
    ...
    while True:
        ...
        print(f"live {works}/{secs}; elapsed {elapsed/60:.1f} min; floor>{HOLD_FLOOR}")
...
def write_latin_lock(secs: list[dict]) -> None:
    lock = BOOK / "sources/_davenant_praedestinatione_cap1_subject_latin_lock.txt"
    header = (
        "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. 1 subject densify lock\n"
        "Witnesses: IA 1650 Daniel PDF pdftotext pp.~110–114 (PDF 123–126) + tesseract lat+eng + DjVu + sense normalize.\n"
        "Scope: Cap. 1 Prolegomena III subject conditions (rationality / sin presupposed); after vocabulary; before distinguishing conditions.\n\n"
    )
...
def extend_morte_lock(secs: list[dict]) -> None:
    ...
    if "Cap. 1 subject densify append" in text or "Tertio in loco jam accedamus ad qualitates" in text:
        print("morte lock already has cap1 vocab; skip append")
        return
    block = "\n\n=== De praedestinatione Cap. 1 subject densify append ===\n" + "\n\n".join(
...
def update_meta(tip: int) -> None:
    ...
    note = "; De praedestinatione Cap. 1 subject conditions (rationality/sin) tip."
    if isinstance(blurb, str) and "Cap. 1 vocabulary" not in blurb:
        if "blurb" in meta:
...
def build_packet(tip: int) -> str:
    ...
    packet = {
        "identity": {
            ...
            "work": "Two Dissertations (De morte Christi Cap. 1-7 + De praedestinatione Prefatio/Cap.1 subject partial)",
            "edition": (
                ...
                "Cap. 1 Prolegomena I-III + vocabulary Electio/Praedestinatio/Reprobatio "
                "(before Cap. 1 subject-condition Prolegomena III)."
            ),
        ...
        "scope": {
            ...
            "locus": "De praedestinatione Cap. 1 Prolegomena III subject conditions (rationality / sin presupposed)",
            "next_locus": "Cap. 1 Prolegomena IV distinguishing conditions (works / faith foresight)",
            "punch_x": "NO",
            "ship": "NO",
        },
        "sections": [
    ...
    review = {
        ...
        "pass_a_ne_b": True,
        "notes": "De praedestinatione Cap.1 subject densify; distinguishing-condition Prolegomena IV remains.",
        "punch_x": "NO",
    }
...
def write_receipt(before: int, after: int, packet_id: str, gates: dict, hold_note: str) -> None:
    receipt = {
        ...
        "latin_lock": str(BOOK / "sources/_davenant_praedestinatione_cap1_subject_latin_lock.txt"),
        "hold_note": hold_note,
    }
...
def prepend_handoff(before: int, after: int, hold_note: str) -> None:
    ...
    block = f"""## {ts} (Cursor — Davenant De praedestinatione Cap. 1 subject densify)

CoS densify: Cap. 1 Prolegomena III subject conditions (rationality / sin presupposed). Punch X: **NO**. Cap. 1 mid-Thesis tip patch stays deferred. Did **not** edit `publication-review.json`; did not ship; did not commit; did not run Logos or ai_promote. {hold_note}

### Bound this job (not shipped)
- **Davenant** `davenant-dissertationes-duae`: Cap. 1 subject-condition tip (**{before}→{after}** sections). Locked densify Latin from IA 1650 Daniel PDF (pdftotext + tesseract + DjVu) with lock. Pass A≠B; OUR; English-first; no PBB/ops TNs; packet `{PACKET}`. Honest partial — **not** whole Dissertationes. Cap. 1 distinguishing-condition Prolegomena IV remains. Punch X: **NO**.
...
def main() -> None:
    ...
    gates = {
        ...
        "check_pass_ab": "ok fail=0 on morte_208..214",
        "tip_ready": tip_out,
        "punch_x": "NO",
        "ship": "NO",
        "claim": "davenant-dissertationes-densify stays claimed",
