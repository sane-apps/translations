...
OCR = BOOK/"sources/_ocr_praedestinatione_cap10_arg9"
PIPE = Path.home()/"SaneApps/clients/translations/pipeline"
PACKET = "praedestinatione_cap10_arg9_densify"
CHECK = PIPE/"check_pass_ab.py"
TIP_BEFORE = 745
HOLD_FLOOR_PRIOR = 5751
POST_HOLD_FLOOR = 5780
DEPLOY = "live5780"
...
for s in secs:
    ...
    jx = {
        ...
        "pass_a_ne_b": True,
        "bible_refs": [],
        "notes": "Densify De praedestinatione Cap. X Arg. IX+ section %s. Pass A!=B. Packet %s." % (n, PACKET),
        "lemmas": s["lemmas"],
        "choices": [
            {"issue": "Copy-text", "choice": "Locked 1650 Daniel Latin; IA PDF pp.219-220 + page-image sense-normalize."},
            {"issue": "Scope", "choice": "Arg. IX natura non-electionis through Rom.ix.18 + Arg. X open 1 Cor.iv.7; further Cap. X remains."},
            {"issue": "Sense-normalize", "choice": "IA PDF + tess; Rom.ix.15/18; Suarez/Bellarmine/Vasquez; Judas/Peter; 1 Cor.iv.7."},
...
try:
    ...
    works, secs_live = (int(m.group(1)), int(m.group(2))) if m else (57, 5780)
except Exception:
    works, secs_live = 57, 5780
...
print(hold_note)
assert secs_live > HOLD_FLOOR_PRIOR or secs_live >= 5780

lock = BOOK/"sources/_davenant_praedestinatione_cap10_arg9_latin_lock.txt"
lock.write_text(
    "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. X Arg. IX+ densify lock\n"
    "Witnesses: IA 1650 Daniel PDF pp.219-220 + page-image sense normalize.\n"
    "Scope: Arg. IX negative reprobation / Rom.ix + Arg. X open. Further Cap. X remains.\n\n"
...
if morte_lock.exists():
    ...
    if "Cap. X Arg. IX densify append" not in text:
        parts = ["[%s] %s" % (s["section"], s["latin"]) for s in secs]
        block = "\n\n=== De praedestinatione Cap. X Arg. IX densify append ===\n" + "\n\n".join(parts)
...
packet = {
  "identity": {
    ...
    "work": "Two Dissertations (De praedestinatione through Cap. X Arg. IX/X)",
    "edition": "Dissertationes duae (Cambridge: Roger Daniel, 1650). Partial through Cap. X Arg. IX + Arg. X open. Further Cap. X remains.",
  ...
  "scope": {
    ...
    "locus": "De praedestinatione Cap. X Arg. IX natura non-electionis + Arg. X open.",
    "next_locus": "Cap. X Arg. X cont. (gratia gratis data / Aug. De praedest. sanct. 5; Major: modes of turning hearts; Prov. xxi.1; Ambrose/Basil).",
    "punch_x": "NO",
    "ship": "NO",
  },
  "sections": [{
...
(AUDIT/("%s.review.json" % PACKET)).write_text(json.dumps({
  ...
  "notes": "Cap. X Arg. IX+ densify; further Cap. X remains. Punch X=NO.",
  "punch_x": "NO",
}, ensure_ascii=False, indent=2)+"\n")

gates = {
  "hostname": host,
  "check_pass_ab": "ok fail=0 on morte_746..753",
...
block = (
  ...
  "CoS densify: Cap. X Arg. IX natura non-electionis / Rom.ix + Arg. X open. Punch X: **NO**. Did **not** ship. "
