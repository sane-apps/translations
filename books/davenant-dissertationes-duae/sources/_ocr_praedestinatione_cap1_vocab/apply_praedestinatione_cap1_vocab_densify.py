#!/usr/bin/env python3
"""De praedestinatione Cap.1 vocabulary densify — tip 200 → 207. Packet praedestinatione_cap1_vocab_densify."""
...
def write_latin_lock(secs: list[dict]) -> None:
    lock = BOOK / "sources/_davenant_praedestinatione_cap1_vocab_latin_lock.txt"
    header = (
        "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. 1 vocabulary densify lock\n"
        "Witnesses: IA 1650 Daniel PDF pdftotext pp.~110–114 (PDF 123–126) + tesseract lat+eng + DjVu + sense normalize.\n"
        "Scope: Cap. 1 vocabulary Electio / Praedestinatio / Reprobatio; after Prolegomena I–III; before subject-condition Prolegomena III (Tertio in loco).\n\n"
    )
...
def extend_morte_lock(secs: list[dict]) -> None:
    ...
    if "Cap. 1 vocabulary densify append" in text or "Jam veniamus ad explicationem illorum trium vocabulorum" in text:
        print("morte lock already has cap1 vocab; skip append")
        return
    block = "\n\n=== De praedestinatione Cap. 1 vocabulary densify append ===\n" + "\n\n".join(
...
def update_meta(tip: int) -> None:
    ...
    note = "; De praedestinatione Cap. 1 vocabulary (Electio/Praedestinatio/Reprobatio) tip."
...
def build_packet(tip: int) -> str:
    ...
    packet = {
        "identity": {
            ...
            "work": "Two Dissertations (De morte Christi Cap. 1-7 + De praedestinatione Prefatio/Cap.1 vocab partial)",
            "edition": (
        ...
        "scope": {
            ...
            "locus": "De praedestinatione Cap. 1 vocabulary — Electio / Praedestinatio / Reprobatio",
            "next_locus": "Cap. 1 Prolegomena III subject conditions (rationality / sin presupposed)",
            "punch_x": "NO",
            "ship": "NO",
        },
        "sections": [
    ...
    review = {
        ...
        "pass_a_ne_b": True,
        "notes": "De praedestinatione Cap.1 vocabulary densify; subject-condition Prolegomena III remains.",
        "punch_x": "NO",
    }
...
def write_receipt(before: int, after: int, packet_id: str, gates: dict, hold_note: str) -> None:
    receipt = {
        ...
        "latin_lock": str(BOOK / "sources/_davenant_praedestinatione_cap1_vocab_latin_lock.txt"),
        "hold_note": hold_note,
    }
...
def prepend_handoff(before: int, after: int, hold_note: str) -> None:
    ...
    block = f"""## {ts} (Cursor — Davenant De praedestinatione Cap. 1 vocab densify)

CoS densify: Cap. 1 vocabulary (Electio / Praedestinatio / Reprobatio). Punch X: **NO**. Cap. 1 mid-Thesis tip patch stays deferred. Did **not** edit `publication-review.json`; did not ship; did not commit; did not run Logos or ai_promote. {hold_note}

### Bound this job (not shipped)
- **Davenant** `davenant-dissertationes-duae`: Cap. 1 vocabulary tip (**{before}→{after}** sections). Locked densify Latin from IA 1650 Daniel PDF (pdftotext + tesseract + DjVu) with lock. Pass A≠B; OUR; English-first; no PBB/ops TNs; packet `{PACKET}`. Honest partial — **not** whole Dissertationes. Cap. 1 subject-condition Prolegomena III remains. Punch X: **NO**.
...
def main() -> None:
    ...
    gates = {
        ...
        "check_pass_ab": "ok fail=0 on morte_201..207",
        "tip_ready": tip_out,
        "punch_x": "NO",
        "ship": "NO",
        "claim": "davenant-dissertationes-densify stays claimed",
