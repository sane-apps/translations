#!/usr/bin/env python3
"""Cap. 7 Scholastici densify — tip 171 → ~178. Packet morte_christi_cap7_scholastici_densify.
HOLD: do not append eng/src until live sections > 3556 (tip 171 ship in flight).
"""
...
OCR = BOOK / "sources/_ocr_cap7_scholastici"
PIPE = Path.home() / "SaneApps/clients/translations/pipeline"
PACKET = "morte_christi_cap7_scholastici_densify"
CHECK = PIPE / "check_pass_ab.py"
TIP_BEFORE = 171
HOLD_LIVE_FLOOR = 3556  # append ONLY when live sections > this (owner confirm)
...
def write_justification(sec: dict) -> Path:
    ...
    payload = {
        ...
        "bible_refs": [],
        "notes": (
            f"Densify Cap. 7 Scholastici & modern Pontificii section {sec['section']}. "
        ...
        "choices": [
            {
                ...
                "choice": (
                    ...
                    "with sense normalize; Cap. 7 Scholastici lock: "
                    "sources/_davenant_cap7_scholastici_latin_lock.txt."
                ),
            },
            {
                "issue": "Scope",
                "choice": (
                    "Cap. 7 Scholastics & modern Pontificii (Sed missis … Lombard through "
                    "Suarez/Vasquez) close; before Theologi nostri (Bucerus)."
                ),
            },
        ],
    }
...
def hold_cleared(secs: int) -> tuple[bool, str]:
    # Owner confirm 2026-09-22: append ONLY when live > 3556 (no 12-min escape).
    if secs > HOLD_LIVE_FLOOR:
        return True, f"live sections {secs} > {HOLD_LIVE_FLOOR}"
    return False, f"HOLD: live={secs} need >{HOLD_LIVE_FLOOR} (tip 171 ship in flight; no eng/src append)"


def write_latin_lock(secs: list[dict]) -> None:
    lock = BOOK / "sources/_davenant_cap7_scholastici_latin_lock.txt"
    header = (
        "CAPUT VII — Scholastici & modern Pontificii densify lock\n"
        "Witnesses: IA 1650 Daniel PDF pdftotext pp.105–120 + DjVu + sense normalize long-s/ligatures.\n"
        "Scope: Sed missis … Lombard through Suarez/Vasquez close before Theologi nostri (Bucerus).\n\n"
    )
...
def update_meta(tip: int) -> None:
    ...
    note = "; Cap. 7 Scholastici & modern Pontificii tip."
    if isinstance(blurb, str) and "Scholastici" not in blurb and "Scholastics" not in blurb:
        if "blurb" in meta:
...
def extend_morte_lock(secs: list[dict]) -> None:
    ...
    if "Sed missis antiquioribus" in text and "Vasquez" in text:
        print("morte lock already has Scholastici; skip append")
        return
    block = "\n\n=== Cap. 7 Scholastici densify append ===\n" + "\n\n".join(
...
def build_packet(tip: int) -> str:
    ...
    packet = {
        "identity": {
            ...
            "edition": (
                ...
                "Partial: De morte Christi Cap. 1-7 through Cap. 7 Scholastici (Suarez/Vasquez)."
            ),
        ...
        "publication_scope": {
            "status": "partial",
            "note": f"Tip densify Cap. 7 Scholastici (secs 172-{tip}). Not whole Dissertationes.",
            "packet": PACKET,
        },
        ...
        "raw_source_paths": [
            str(BOOK / "sources/_davenant_cap7_scholastici_latin_lock.txt"),
    ...
    review = {
        ...
        "scope_review": {
            ...
            "uncertainties": [],
            "notes": f"Scope rebind davenant-dissertationes-duae tip {tip} (Cap. 7 Scholastici).",
        },
        "reviews": reviews,
    }
...
def write_receipt(before: int, after: int, packet_id: str, gates: dict, hold_note: str) -> None:
    receipt = {
        ...
        "locus": "Cap. 7 Scholastici & modern Pontificii (Sed missis … Lombard through Suarez/Vasquez)",
        "next_locus": "Cap. 7 Theologi nostri (Bucerus …) through Cap. 7 remainder / Cap. 8+",
        "gates": gates,
        "punch_x": "NO",
        "ship": "NO",
        "claim": "davenant-dissertationes-densify stays claimed",
        "latin_lock": str(BOOK / "sources/_davenant_cap7_scholastici_latin_lock.txt"),
        "hold_note": hold_note,
    }
...
CoS densify: Cap. 7 Scholastici & modern Pontificii (`Sed missis` / Lombard through Suarez/Vasquez) close before Theologi nostri. Punch X: **NO**. Cap. 1 mid-Thesis tip patch stays deferred. Did **not** edit `publication-review.json`; did not ship; did not commit; did not run Logos or ai_promote. {hold_note}

### Bound this job (not shipped)
- **Davenant** `davenant-dissertationes-duae`: Cap. 7 Scholastici tip (**{before}→{after}** sections). Locked densify Latin from IA 1650 Daniel PDF (pdftotext + DjVu) with lock. Pass A≠B; OUR; English-first; no PBB/ops TNs; packet `morte_christi_cap7_scholastici_densify`. Honest partial — **not** whole Dissertationes. Cap. 7 Theologi nostri–remainder through Cap. 11 and De praedestinatione remain. Punch X: **NO**.
