#!/usr/bin/env python3
"""De praedestinatione Cap. III Rat. 3 densify — tip 282 → 287. Packet praedestinatione_cap3_rat3_densify."""
...
def wait_hold() -> str:
    ...
    while True:
        ...
        print(
            f"live {works}/{secs}; elapsed {elapsed/60:.1f} min; floor>{HOLD_FLOOR}",
            flush=True,
        )
        if secs > HOLD_FLOOR or elapsed >= 12 * 60:
            ...
            if secs > HOLD_FLOOR:
                clear = f"live sections {secs} > {HOLD_FLOOR}"
            else:
                clear = f"12-minute wait elapsed ({elapsed/60:.1f} min from hold_start={hold_start})"
            note = (
                f"Hold cleared ({clear}); "
...
def write_latin_lock(secs: list["dict"]) -> None:
    lock = BOOK / "sources/_davenant_praedestinatione_cap3_rat3_latin_lock.txt"
    header = (
        "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. III Rat. 3 densify lock\n"
        "Witnesses: IA 1650 Daniel PDF pdftotext pp.~143–144 + tesseract lat+eng + DjVu + sense normalize.\n"
        "Scope: Cap. III adversariorum rationes Rat. 3 (king/prize / victory foresight analogy) and Sol.; "
        "after Cap. III Rat. 2 Resp. close; before Cap. III Rat. 4.\n\n"
    )
...
def extend_morte_lock(secs: list["dict"]) -> None:
    ...
    if "Cap. III Rat. 3 densify append" in text or "Sicuti cum Rex aliquis proponit premium" in text:
        print("morte lock already has Cap III Rat. 3; skip append")
        return
    block = "\n\n=== De praedestinatione Cap. III Rat. 3 densify append ===\n" + "\n\n".join(
...
def update_meta(tip: int) -> None:
    ...
    note = "; De praedestinatione Cap. III Rat. 3 (king/prize / victory foresight) tip."
    if isinstance(blurb, str) and "Cap. III Rat. 3" not in blurb:
        if "blurb" in meta:
            meta["blurb"] = blurb.rstrip(".") + note
        elif "description" in meta:
            meta["description"] = blurb.rstrip(".") + note
    method = meta.get("method") or ""
    if isinstance(method, str) and "Cap. III Rat. 3" not in method:
        if "Cap. III Rat. 2" in method:
            meta["method"] = method.replace(
                "Cap. III Rat. 2",
                "Cap. III Rat. 2 + Cap. III Rat. 3",
            )
        elif "Cap. III Rat. 1" in method:
            meta["method"] = method.replace(
                "Cap. III Rat. 1",
                "Cap. III Rat. 1 + Cap. III Rat. 2 + Cap. III Rat. 3",
            )
...
def build_packet(tip: int) -> str:
    ...
    packet = {
        "identity": {
            ...
            "work": "Two Dissertations (De morte Christi Cap. 1-7 + De praedestinatione Prefatio/Cap.1–Cap.III Rat.3 partial)",
            "edition": (
                ...
                "Cap. 1 Prolegomena I-IV + Cap. II Variae + Cap. II Scholastici / Pontificii / Vasquez Arg. 1 + Sol. "
                "+ Cap. II Arg. 2–7 + Sol. + Cap. III Rat. 1 + Rej. + Cap. III Rat. 2 + Resp. "
                "+ Cap. III Rat. 3 + Sol. (before Cap. III Rat. 4)."
            ),
        ...
        "scope": {
            ...
            "locus": "De praedestinatione Cap. III Rat. 3 (king/prize / victory foresight analogy) and Sol.",
            "next_locus": "Cap. III Rat. 4 (optimum modus / crown of justice from merits foresight) and Sol.",
            "punch_x": "NO",
            "ship": "NO",
        },
        "sections": [
    ...
    review = {
        ...
        "pass_a_ne_b": True,
        "notes": "De praedestinatione Cap. III Rat. 3 densify; Cap. III Rat. 4+ remain.",
        "punch_x": "NO",
    }
...
def write_receipt(before: int, after: int, packet_id: str, gates: dict, hold_note: str) -> None:
    receipt = {
        ...
        "latin_lock": str(BOOK / "sources/_davenant_praedestinatione_cap3_rat3_latin_lock.txt"),
        "hold_note": hold_note,
    }
...
def prepend_handoff(before: int, after: int, hold_note: str) -> None:
    ...
    block = f"""## {ts} (Cursor — Davenant De praedestinatione Cap. III Rat. 3 densify)

CoS densify: Cap. III adversariorum rationes Rat. 3 (king/prize / victory foresight analogy) and Sol. Punch X: **NO**. Cap. 1 mid-Thesis tip patch stays deferred. Did **not** edit `publication-review.json`; did not ship; did not commit; did not run Logos or ai_promote. {hold_note}

### Bound this job (not shipped)
- **Davenant** `davenant-dissertationes-duae`: Cap. III Rat. 3 tip (**{before}→{after}** sections). Locked densify Latin from IA 1650 Daniel PDF (pdftotext + tesseract + DjVu) with lock. Pass A≠B; OUR; English-first; no PBB/ops TNs; packet `{PACKET}`. Honest partial — **not** whole Dissertationes. Cap. III Rat. 4+ remain. Punch X: **NO**.
...
def main() -> None:
    ...
    hold_note = wait_hold()
    eng2 = json.loads((TRANS / "morte_christi_english.json").read_text(encoding="utf-8"))
    tip2 = int(eng2[-1]["section"])
    print(f"re-read tip after hold: {tip2} (n={len(eng2)})")
    if tip2 != TIP_BEFORE:
        raise SystemExit(f"disk tip changed to {tip2} during hold; abort")
    ...
    gates = {
        ...
        "check_pass_ab": "ok fail=0 on morte_283..287",
        "tip_ready": tip_out,
        "punch_x": "NO",
        "ship": "NO",
        "claim": "davenant-dissertationes-densify stays claimed",
        "hold_floor": HOLD_FLOOR,
        "catchup_note": "catch-up ship retargeting tip≥282; cleared by 12m OR live>4001",
    }
