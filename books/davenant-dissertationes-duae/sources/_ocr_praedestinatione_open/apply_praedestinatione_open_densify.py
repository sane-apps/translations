#!/usr/bin/env python3
"""De praedestinatione Prefatio/Cap.1 Prolegomena densify — tip 192 → 200. Packet praedestinatione_open_densify."""
...
def write_latin_lock(secs: list[dict]) -> None:
    lock = BOOK / "sources/_davenant_praedestinatione_open_latin_lock.txt"
    header = (
        "DE PRAEDESTINATIONE ET REPROBATIONE — Prefatio / Cap. 1 Prolegomena densify lock\n"
        "Witnesses: IA 1650 Daniel PDF pdftotext pp.107–110 (PDF 117–123) + tesseract lat+eng + DjVu + sense normalize.\n"
        "Scope: title + Prefatio + Cap. 1 Prolegomena I–III; before Electio/Praedestinatio/Reprobatio vocabulary.\n\n"
    )
...
def extend_morte_lock(secs: list[dict]) -> None:
    ...
    if "Prefatio habita ad Academicos" in text and "Prolegomena" in text:
        print("morte lock already has praedestinatione open; skip append")
        return
    block = "\n\n=== De praedestinatione Prefatio/Prolegomena densify append ===\n" + "\n\n".join(
...
def update_meta(tip: int) -> None:
    ...
    note = "; De praedestinatione Prefatio + Cap. 1 Prolegomena tip."
    if isinstance(blurb, str) and "praedestinatione Prefatio" not in blurb:
        if "blurb" in meta:
...
def build_packet(tip: int) -> str:
    ...
    packet = {
        "identity": {
            ...
            "work": "Two Dissertations (De morte Christi Cap. 1-7 + De praedestinatione Prefatio/Prolegomena partial)",
            "edition": (
                ...
                "Cap. 1 Prolegomena I-III (before vocabulary Electio/Praedestinatio/Reprobatio)."
            ),
        ...
        "scope": {
            ...
            "locus": "De praedestinatione et reprobatione — title + Prefatio + Cap. 1 Prolegomena I–III",
            "next_locus": "Cap. 1 vocabulary (Electio / Praedestinatio / Reprobatio)",
            "punch_x": "NO",
            "ship": "NO",
        },
        "sections": [
    ...
    review = {
        ...
        "pass_a_ne_b": True,
        "notes": "De praedestinatione Prefatio/Prolegomena densify; Cap. 1 vocabulary remains.",
        "punch_x": "NO",
    }
...
def write_receipt(before: int, after: int, packet_id: str, gates: dict, hold_note: str) -> None:
    receipt = {
        ...
        "latin_lock": str(BOOK / "sources/_davenant_praedestinatione_open_latin_lock.txt"),
        "hold_note": hold_note,
    }
...
def prepend_handoff(before: int, after: int, hold_note: str) -> None:
    ...
    block = f"""## {ts} (Cursor — Davenant De praedestinatione Prefatio/Prolegomena densify)

CoS densify: De praedestinatione et reprobatione (title + Prefatio + Cap. 1 Prolegomena I–III). Punch X: **NO**. Cap. 1 mid-Thesis tip patch stays deferred. Did **not** edit `publication-review.json`; did not ship; did not commit; did not run Logos or ai_promote. {hold_note}

### Bound this job (not shipped)
- **Davenant** `davenant-dissertationes-duae`: De praedestinatione open tip (**{before}→{after}** sections). Locked densify Latin from IA 1650 Daniel PDF (pdftotext + tesseract + DjVu) with lock. Pass A≠B; OUR; English-first; no PBB/ops TNs; packet `{PACKET}`. Honest partial — **not** whole Dissertationes. Cap. 1 vocabulary (Electio / Praedestinatio / Reprobatio) remains. Punch X: **NO**.
...
def main() -> None:
    ...
    gates = {
        ...
        "check_pass_ab": "ok fail=0 on morte_193..200",
        "tip_ready": tip_out,
        "punch_x": "NO",
        "ship": "NO",
        "claim": "davenant-dissertationes-densify stays claimed",
