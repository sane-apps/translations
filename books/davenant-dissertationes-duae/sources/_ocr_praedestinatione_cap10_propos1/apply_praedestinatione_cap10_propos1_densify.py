...
OCR = BOOK/"sources/_ocr_praedestinatione_cap10_propos1"
PIPE = Path.home()/"SaneApps/clients/translations/pipeline"
PACKET = "praedestinatione_cap10_propos1_densify"
CHECK = PIPE/"check_pass_ab.py"
TIP_BEFORE = 777
HOLD_FLOOR_PRIOR = 5866
POST_HOLD_FLOOR = 5896
DEPLOY = "live5896"
...
for s in secs:
    ...
    jx = {
        ...
        "pass_a_ne_b": True,
        "bible_refs": [],
        "notes": "Densify De praedestinatione Cap. X Ultimum Propos. 1 section %s. Pass A!=B. Packet %s." % (n, PACKET),
        "lemmas": s["lemmas"],
        "choices": [
            {"issue": "Copy-text", "choice": "Locked 1650 Daniel Latin; IA PDF pp.224-225 + page-image sense-normalize."},
            {"issue": "Scope", "choice": "Propos. 1 Orthodox Grace vs Pelagius I–II + Arminius/Corvinus parallel; further Cap. X (Propos. 1.III Fathers' Grace of Christ) remains."},
            {"issue": "Sense-normalize", "choice": "IA PDF + tess; Aug. De grat. Christi; Prosper; Hilary/Faustus; Arminius Exam. Perkins; Corvinus."},
...
try:
    ...
    works, secs_live = (int(m.group(1)), int(m.group(2))) if m else (57, 5896)
except Exception:
    works, secs_live = 57, 5896
...
print(hold_note)
assert secs_live > HOLD_FLOOR_PRIOR or secs_live >= 5896

lock = BOOK/"sources/_davenant_praedestinatione_cap10_propos1_latin_lock.txt"
lock.write_text(
    "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. X Ultimum Propos. 1 densify lock\n"
    "Witnesses: IA 1650 Daniel PDF pp.224-225 + page-image sense normalize.\n"
    "Scope: Propos. 1 vs Pelagius + Arminius parallel. Further Cap. X (III Fathers' Grace) remains.\n\n"
...
if morte_lock.exists():
    ...
    if "Cap. X Propos.1 densify append" not in text:
        parts = ["[%s] %s" % (s["section"], s["latin"]) for s in secs]
        block = "\n\n=== De praedestinatione Cap. X Propos.1 densify append ===\n" + "\n\n".join(parts)
...
packet = {
  "identity": {
    ...
    "work": "Two Dissertations (De praedestinatione through Cap. X Ultimum Propos. 1)",
    "edition": "Dissertationes duae (Cambridge: Roger Daniel, 1650). Partial through Cap. X Propos. 1 Arminius parallel. Further Cap. X remains.",
  ...
  "scope": {
    ...
    "locus": "De praedestinatione Cap. X Ultimum Propos. 1 (Orthodox Grace vs Pelagius; Arminius parallel).",
    "next_locus": "Cap. X Ultimum Propos. 1.III (Fathers' Grace of Christ / Bishop of Salisbury; Aug. contra 2 Epist. Pelag.; then Propos. 2).",
    "punch_x": "NO",
    "ship": "NO",
  },
  "sections": [{
...
(AUDIT/("%s.review.json" % PACKET)).write_text(json.dumps({
  ...
  "notes": "Cap. X Ultimum Propos. 1 densify; further Cap. X remains. Punch X=NO.",
  "punch_x": "NO",
}, ensure_ascii=False, indent=2)+"\n")

gates = {
  "hostname": host,
  "check_pass_ab": "ok fail=0 on morte_778..785",
...
block = (
  ...
  "CoS densify: Cap. X Ultimum Propos. 1 Orthodox Grace vs Pelagius + Arminius parallel. Punch X: **NO**. Did **not** ship. "
