#!/usr/bin/env python3
"""De praedestinatione Cap. III Rat. 2 densify — tip 277 → 282. Packet praedestinatione_cap3_rat2_densify."""
...
def write_latin_lock(secs: list[dict]) -> None:
    lock = BOOK / "sources/_davenant_praedestinatione_cap3_rat2_latin_lock.txt"
    header = (
        "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. III Rat. 2 densify lock\n"
        "Witnesses: IA 1650 Daniel PDF pdftotext pp.~142–144 + tesseract lat+eng + DjVu + sense normalize.\n"
        "Scope: Cap. III adversariorum rationes Rat. 2 (Vasquez physical vs moral media) and Resp.; "
...
def extend_morte_lock(secs: list[dict]) -> None:
    ...
    if "Cap. III Rat. 2 densify append" in text or "discrimine mediorum Physicorum" in text:
        print("morte lock already has Cap III Rat. 2; skip append")
        return
    block = "\n\n=== De praedestinatione Cap. III Rat. 2 densify append ===\n" + "\n\n".join(
...
def update_meta(tip: int) -> None:
    ...
    note = "; De praedestinatione Cap. III Rat. 2 (Vasquez physical vs moral media) tip."
    if isinstance(blurb, str) and "Cap. III Rat. 2" not in blurb:
        if "blurb" in meta:
            meta["blurb"] = blurb.rstrip(".") + note
        elif "description" in meta:
            meta["description"] = blurb.rstrip(".") + note
    method = meta.get("method") or ""
    if isinstance(method, str) and "Cap. III Rat. 2" not in method and "Cap. III Rat. 1" in method:
        meta["method"] = method.replace(
            "Cap. III Rat. 1",
            "Cap. III Rat. 1 + Cap. III Rat. 2",
        )
...
def build_packet(tip: int) -> str:
    ...
    packet = {
        "identity": {
            ...
            "work": "Two Dissertations (De morte Christi Cap. 1-7 + De praedestinatione Prefatio/Cap.1–Cap.III Rat.2 partial)",
            "edition": (
                ...
                "+ Cap. II Arg. 2–7 + Sol. + Cap. III Rat. 1 + Rej. + Cap. III Rat. 2 + Resp. (before Cap. III Rat. 3)."
            ),
        ...
        "scope": {
            ...
            "locus": "De praedestinatione Cap. III Rat. 2 (Vasquez physical vs moral media) and Resp.",
            "next_locus": "Cap. III Rat. 3 (king/prize / victory foresight analogy) and Sol.",
            "punch_x": "NO",
            "ship": "NO",
        },
        "sections": [
    ...
    review = {
        ...
        "pass_a_ne_b": True,
        "notes": "De praedestinatione Cap. III Rat. 2 densify; Cap. III Rat. 3+ remain.",
        "punch_x": "NO",
    }
...
def write_receipt(before: int, after: int, packet_id: str, gates: dict, hold_note: str) -> None:
    receipt = {
        ...
        "latin_lock": str(BOOK / "sources/_davenant_praedestinatione_cap3_rat2_latin_lock.txt"),
        "hold_note": hold_note,
    }
...
def prepend_handoff(before: int, after: int, hold_note: str) -> None:
    ...
    block = f"""## {ts} (Cursor — Davenant De praedestinatione Cap. III Rat. 2 densify)

CoS densify: Cap. III adversariorum rationes Rat. 2 (Vasquez physical vs moral media) and Resp. Punch X: **NO**. Cap. 1 mid-Thesis tip patch stays deferred. Did **not** edit `publication-review.json`; did not ship; did not commit; did not run Logos or ai_promote. {hold_note}

### Bound this job (not shipped)
- **Davenant** `davenant-dissertationes-duae`: Cap. III Rat. 2 tip (**{before}→{after}** sections). Locked densify Latin from IA 1650 Daniel PDF (pdftotext + tesseract + DjVu) with lock. Pass A≠B; OUR; English-first; no PBB/ops TNs; packet `{PACKET}`. Honest partial — **not** whole Dissertationes. Cap. III Rat. 3+ remain. Punch X: **NO**.
...
def main() -> None:
    ...
    gates = {
        ...
        "check_pass_ab": "ok fail=0 on morte_278..282",
        "tip_ready": tip_out,
        "punch_x": "NO",
        "ship": "NO",
        "claim": "davenant-dissertationes-densify stays claimed",
        "hold_floor": HOLD_FLOOR,
        "placeus_262_catchup_note": "treated Placeus 262 as possible in-flight; cleared by 12m OR >4001",
    }
