...
OCR = BOOK/"sources/_ocr_praedestinatione_cap10_arg7"
PIPE = Path.home()/"SaneApps/clients/translations/pipeline"
PACKET = "praedestinatione_cap10_arg7_densify"
CHECK = PIPE/"check_pass_ab.py"
TIP_BEFORE = 737
HOLD_FLOOR_PRIOR = 5721
POST_HOLD_FLOOR = 5751
DEPLOY = "live5751"
...
for s in secs:
    ...
    jx = {
        ...
        "pass_a_ne_b": True,
        "bible_refs": [],
        "notes": "Densify De praedestinatione Cap. X Arg. VII cont. section %s. Pass A!=B. Packet %s." % (n, PACKET),
        "lemmas": s["lemmas"],
        "choices": [
            {"issue": "Copy-text", "choice": "Locked 1650 Daniel Latin; IA PDF pp.218-219 + page-image sense-normalize."},
            {"issue": "Scope", "choice": "Arg. VII Isa.liii.10 / Aug. De praedest. sanct. 10 close + Arg. VIII infants open/close; further Cap. X remains."},
            {"issue": "Sense-normalize", "choice": "IA PDF + tess; Isaiah liii.10; Augustine De praedest. sanct. 10/12; Prosper; Arminius."},
...
try:
    ...
    works, secs_live = (int(m.group(1)), int(m.group(2))) if m else (57, 5751)
except Exception:
    works, secs_live = 57, 5751
...
print(hold_note)
assert secs_live > HOLD_FLOOR_PRIOR or secs_live >= 5751

lock = BOOK/"sources/_davenant_praedestinatione_cap10_arg7_latin_lock.txt"
lock.write_text(
    "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. X Arg. VII+ densify lock\n"
    "Witnesses: IA 1650 Daniel PDF pp.218-219 + page-image sense normalize.\n"
    "Scope: Arg. VII Isa.liii.10 / Aug. De praedest. sanct. 10 + Arg. VIII infants. Further Cap. X remains.\n\n"
...
if morte_lock.exists():
    ...
    if "Cap. X Arg. VII densify append" not in text:
        parts = ["[%s] %s" % (s["section"], s["latin"]) for s in secs]
        block = "\n\n=== De praedestinatione Cap. X Arg. VII densify append ===\n" + "\n\n".join(parts)
...
packet = {
  "identity": {
    ...
    "work": "Two Dissertations (De praedestinatione through Cap. X Arg. VII/VIII)",
    "edition": "Dissertationes duae (Cambridge: Roger Daniel, 1650). Partial through Cap. X Arg. VII Isa/Aug + Arg. VIII infants. Further Cap. X remains.",
  ...
  "scope": {
    ...
    "locus": "De praedestinatione Cap. X Arg. VII Isa.liii.10 / Aug. De praedest. sanct. 10 + Arg. VIII infants.",
    "next_locus": "Cap. X Arg. IX+ (natura non-electionis / negative reprobation; Rom. ix.15; angels; Judas vs Peter).",
    "punch_x": "NO",
    "ship": "NO",
  },
  "sections": [{
...
(AUDIT/("%s.review.json" % PACKET)).write_text(json.dumps({
  ...
  "notes": "Cap. X Arg. VII+ densify; further Cap. X Args remain. Punch X=NO.",
  "punch_x": "NO",
}, ensure_ascii=False, indent=2)+"\n")

gates = {
  "hostname": host,
  "check_pass_ab": "ok fail=0 on morte_738..745",
...
block = (
  ...
  "CoS densify: Cap. X Arg. VII Isa.liii.10 / Aug. De praedest. sanct. 10 + Arg. VIII infants. Punch X: **NO**. Did **not** ship. "
...
print(json.dumps({
  "tip_before": TIP_BEFORE, "tip_after": tip_after, "tip_len": tip_after,
  "packet": PACKET, "slice_id": packet_id,
  "gates": gates, "hold": gates["post_densify_hold"],
