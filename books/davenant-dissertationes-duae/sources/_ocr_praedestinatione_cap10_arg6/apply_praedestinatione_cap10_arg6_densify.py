...
OCR = BOOK/"sources/_ocr_praedestinatione_cap10_arg6"
PIPE = Path.home()/"SaneApps/clients/translations/pipeline"
PACKET = "praedestinatione_cap10_arg6_densify"
CHECK = PIPE/"check_pass_ab.py"
TIP_BEFORE = 729
HOLD_FLOOR_PRIOR = 5693
POST_HOLD_FLOOR = 5721
DEPLOY = "live5721"
...
for s in secs:
    ...
    jx = {
        ...
        "pass_a_ne_b": True,
        "bible_refs": [],
        "notes": "Densify De praedestinatione Cap. X Arg. VI+ section %s. Pass A!=B. Packet %s." % (n, PACKET),
        "lemmas": s["lemmas"],
        "choices": [
            {"issue": "Copy-text", "choice": "Locked 1650 Daniel Latin; IA PDF pp.215-218 + page-image sense-normalize."},
            {"issue": "Scope", "choice": "Arg. V Minor close through Arg. VI Principatus + Arg. VII Arminius open; further Cap. X remains."},
            {"issue": "Sense-normalize", "choice": "IA PDF + tess page images; Bradwardine; Augustine De corr./praedest. sanct.; Arminius/Grevinchovius/Episcopius."},
...
try:
    ...
    works, secs_live = (int(m.group(1)), int(m.group(2))) if m else (57, 5721)
except Exception:
    works, secs_live = 57, 5721
...
print(hold_note)
assert secs_live > HOLD_FLOOR_PRIOR or secs_live >= 5721

lock = BOOK/"sources/_davenant_praedestinatione_cap10_arg6_latin_lock.txt"
lock.write_text(
    "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. X Arg. VI+ densify lock\n"
    "Witnesses: IA 1650 Daniel PDF pp.215-218 + page-image sense normalize.\n"
    "Scope: Arg. V Minor close + Arg. VI Principatus (Bradwardine) + Arg. VII Arminius open. Further Cap. X remains.\n\n"
...
if morte_lock.exists():
    ...
    if "Cap. X Arg. VI densify append" not in text:
        parts = ["[%s] %s" % (s["section"], s["latin"]) for s in secs]
        block = "\n\n=== De praedestinatione Cap. X Arg. VI densify append ===\n" + "\n\n".join(parts)
...
packet = {
  "identity": {
    ...
    "work": "Two Dissertations (De praedestinatione through Cap. X Arg. VI+)",
    "edition": "Dissertationes duae (Cambridge: Roger Daniel, 1650). Partial through Cap. X Arg. VI Principatus + Arg. VII Arminius open. Further Cap. X remains.",
  ...
  "scope": {
    ...
    "locus": "De praedestinatione Cap. X Arg. V Minor close + Arg. VI Principatus + Arg. VII Arminius open.",
    "next_locus": "Cap. X Arg. VII cont. (Isa. liii.10 / Augustine De praedest. sanct. 10 — promise of church; then infants Arg.).",
    "punch_x": "NO",
    "ship": "NO",
  },
  "sections": [{
...
(AUDIT/("%s.review.json" % PACKET)).write_text(json.dumps({
  ...
  "notes": "Cap. X Arg. VI+ densify; further Cap. X Args remain. Punch X=NO.",
  "punch_x": "NO",
}, ensure_ascii=False, indent=2)+"\n")

gates = {
  "hostname": host,
  "check_pass_ab": "ok fail=0 on morte_730..737",
...
block = (
  ...
  "CoS densify: Cap. X Arg. V Minor close + Arg. VI Principatus + Arg. VII Arminius open. Punch X: **NO**. Did **not** ship. "
