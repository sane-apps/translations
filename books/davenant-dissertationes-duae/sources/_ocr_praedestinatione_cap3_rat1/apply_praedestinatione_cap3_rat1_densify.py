#!/usr/bin/env python3
"""De praedestinatione Cap. III Rat. 1 densify — tip 270 → 277. Packet praedestinatione_cap3_rat1_densify."""
...
def write_latin_lock(secs: list[dict]) -> None:
    lock = BOOK / "sources/_davenant_praedestinatione_cap3_rat1_latin_lock.txt"
    header = (
        "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. III Rat. 1 densify lock\n"
        "Witnesses: IA 1650 Daniel PDF pdftotext pp.~141–142 + tesseract lat+eng + DjVu + sense normalize.\n"
        "Scope: Cap. III adversariorum rationes Rat. 1 (common inefficacious will) and Rej.; "
        "after Cap. II Arg. 7 Sol.; before Cap. III Rat. 2 Vasquez physical/moral media.\n\n"
    )
...
def extend_morte_lock(secs: list[dict]) -> None:
    ...
    if "Cap. III Rat. 1 densify append" in text or "Argumenta adversariorum expenduntur" in text:
        print("morte lock already has Cap III Rat. 1; skip append")
        return
    block = "\n\n=== De praedestinatione Cap. III Rat. 1 densify append ===\n" + "\n\n".join(
...
def update_meta(tip: int) -> None:
    ...
    note = "; De praedestinatione Cap. III Rat. 1 (common inefficacious will) tip."
    if isinstance(blurb, str) and "Cap. III Rat. 1" not in blurb:
        if "blurb" in meta:
            meta["blurb"] = blurb.rstrip(".") + note
        elif "description" in meta:
            meta["description"] = blurb.rstrip(".") + note
    # stretch identity work string lightly if present in method
    method = meta.get("method") or ""
    if isinstance(method, str) and "Cap. III Rat. 1" not in method and "Cap. II Arg. 7" in method:
        meta["method"] = method.replace(
            "Cap. II Arg. 7",
            "Cap. II Arg. 7 + Cap. III Rat. 1",
        )
...
def build_packet(tip: int) -> str:
    ...
    packet = {
        "identity": {
            ...
            "work": "Two Dissertations (De morte Christi Cap. 1-7 + De praedestinatione Prefatio/Cap.1–Cap.III Rat.1 partial)",
            "edition": (
                ...
                "+ Cap. II Arg. 2–7 + Sol. + Cap. III Rat. 1 + Rej. (before Cap. III Rat. 2)."
            ),
        ...
        "scope": {
            ...
            "locus": "De praedestinatione Cap. III Rat. 1 (common inefficacious will) and Rej.",
            "next_locus": "Cap. III Rat. 2 (Vasquez physical vs moral media) and Resp.",
            "punch_x": "NO",
            "ship": "NO",
        },
        "sections": [
    ...
    review = {
        ...
        "pass_a_ne_b": True,
        "notes": "De praedestinatione Cap. III Rat. 1 densify; Cap. III Rat. 2+ remain.",
        "punch_x": "NO",
    }
...
def write_receipt(before: int, after: int, packet_id: str, gates: dict, hold_note: str) -> None:
    receipt = {
        ...
        "latin_lock": str(BOOK / "sources/_davenant_praedestinatione_cap3_rat1_latin_lock.txt"),
        "hold_note": hold_note,
    }
...
def prepend_handoff(before: int, after: int, hold_note: str) -> None:
    ...
    block = f"""## {ts} (Cursor — Davenant De praedestinatione Cap. III Rat. 1 densify)

CoS densify: Cap. III adversariorum rationes Rat. 1 (common inefficacious will) and Rej. Punch X: **NO**. Cap. 1 mid-Thesis tip patch stays deferred. Did **not** edit `publication-review.json`; did not ship; did not commit; did not run Logos or ai_promote. {hold_note}

### Bound this job (not shipped)
- **Davenant** `davenant-dissertationes-duae`: Cap. III Rat. 1 tip (**{before}→{after}** sections). Locked densify Latin from IA 1650 Daniel PDF (pdftotext + tesseract + DjVu) with lock. Pass A≠B; OUR; English-first; no PBB/ops TNs; packet `{PACKET}`. Honest partial — **not** whole Dissertationes. Cap. III Rat. 2+ remain. Punch X: **NO**.
...
def main() -> None:
    ...
    gates = {
        ...
        "check_pass_ab": "ok fail=0 on morte_271..277",
        "tip_ready": tip_out,
        "punch_x": "NO",
        "ship": "NO",
        "claim": "davenant-dissertationes-densify stays claimed",
