...
OCR = BOOK/"sources/_ocr_praedestinatione_cap10_propos2"
PIPE = Path.home()/"SaneApps/clients/translations/pipeline"
PACKET = "praedestinatione_cap10_propos2_densify"
CHECK = PIPE/"check_pass_ab.py"
TIP_BEFORE = 793
HOLD_FLOOR_PRIOR = 5926
POST_HOLD_FLOOR = 5955
DEPLOY = "live5955"
...
for s in secs:
    ...
    jx = {
        ...
        "pass_a_ne_b": True,
        "bible_refs": [],
        "notes": "Densify De praedestinatione Cap. X Ultimum Propos. 2–3 section %s. Pass A!=B. Packet %s." % (n, PACKET),
        "lemmas": s["lemmas"],
        "choices": [
            {"issue": "Copy-text", "choice": "Locked 1650 Daniel Latin; IA PDF pp.228-230 + page-image sense-normalize."},
            {"issue": "Scope", "choice": "Propos. 2 Epist.107/Prosper/councils + Propos. 3 open through Fathers deny prior better disposition; further Cap. X remains."},
            {"issue": "Sense-normalize", "choice": "IA PDF + tess; Epist. 107; Prosper; Milevis/Orange/Sardinia; Pelagius/Faustus; merit=disposition."},
...
try:
    ...
    works, secs_live = (int(m.group(1)), int(m.group(2))) if m else (57, 5955)
except Exception:
    works, secs_live = 57, 5955
...
print(hold_note)
assert secs_live > HOLD_FLOOR_PRIOR or secs_live >= 5955

lock = BOOK/"sources/_davenant_praedestinatione_cap10_propos2_latin_lock.txt"
lock.write_text(
    "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. X Ultimum Propos. 2–3 densify lock\n"
    "Witnesses: IA 1650 Daniel PDF pp.228-230 + page-image sense normalize.\n"
    "Scope: Propos. 2 close + Propos. 3 open. Further Cap. X remains.\n\n"
...
if morte_lock.exists():
    ...
    if "Cap. X Propos.2 densify append" not in text:
        parts = ["[%s] %s" % (s["section"], s["latin"]) for s in secs]
        block = "\n\n=== De praedestinatione Cap. X Propos.2 densify append ===\n" + "\n\n".join(parts)
...
packet = {
  "identity": {
    ...
    "work": "Two Dissertations (De praedestinatione through Cap. X Ultimum Propos. 3)",
    "edition": "Dissertationes duae (Cambridge: Roger Daniel, 1650). Partial through Cap. X Propos. 3 open. Further Cap. X remains.",
  ...
  "scope": {
    ...
    "locus": "De praedestinatione Cap. X Ultimum Propos. 2 cont. + Propos. 3 open.",
    "next_locus": "Cap. X Ultimum Propos. 3 cont. (Aug. contra 2 Epist. Pelag. / De praedest. sanct. 14 / De bono pers. 15; Prosper; then close of Cap. X / Cap. XI if any).",
    "punch_x": "NO",
    "ship": "NO",
  },
  "sections": [{
...
(AUDIT/("%s.review.json" % PACKET)).write_text(json.dumps({
  ...
  "notes": "Cap. X Ultimum Propos. 2–3 densify; further Cap. X remains. Punch X=NO.",
  "punch_x": "NO",
}, ensure_ascii=False, indent=2)+"\n")

gates = {
  "hostname": host,
  "check_pass_ab": "ok fail=0 on morte_794..801",
...
block = (
  ...
  "CoS densify: Cap. X Ultimum Propos. 2 cont. + Propos. 3 open. Punch X: **NO**. Did **not** ship. "
