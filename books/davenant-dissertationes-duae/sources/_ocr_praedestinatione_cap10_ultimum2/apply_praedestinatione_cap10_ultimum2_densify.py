...
OCR = BOOK/"sources/_ocr_praedestinatione_cap10_ultimum2"
PIPE = Path.home()/"SaneApps/clients/translations/pipeline"
PACKET = "praedestinatione_cap10_ultimum2_densify"
CHECK = PIPE/"check_pass_ab.py"
TIP_BEFORE = 769
HOLD_FLOOR_PRIOR = 5838
POST_HOLD_FLOOR = 5866
DEPLOY = "live5866"
...
for s in secs:
    ...
    jx = {
        ...
        "pass_a_ne_b": True,
        "bible_refs": [],
        "notes": "Densify De praedestinatione Cap. X Ultimum Secunda cont. section %s. Pass A!=B. Packet %s." % (n, PACKET),
        "lemmas": s["lemmas"],
        "choices": [
            {"issue": "Copy-text", "choice": "Locked 1650 Daniel Latin; IA PDF pp.222-224 + page-image sense-normalize."},
            {"issue": "Scope", "choice": "Ultimum Secunda cont. through triple-cord Major; further Cap. X (Propos. 1 grace of Christ vs Pelagius) remains."},
            {"issue": "Sense-normalize", "choice": "IA PDF + tess; Aug. De bono pers. 23; Vasquez; Suarez; Chrysostom; triple cord."},
...
try:
    ...
    works, secs_live = (int(m.group(1)), int(m.group(2))) if m else (57, 5866)
except Exception:
    works, secs_live = 57, 5866
...
print(hold_note)
assert secs_live > HOLD_FLOOR_PRIOR or secs_live >= 5866

lock = BOOK/"sources/_davenant_praedestinatione_cap10_ultimum2_latin_lock.txt"
lock.write_text(
    "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. X Ultimum Secunda cont. densify lock\n"
    "Witnesses: IA 1650 Daniel PDF pp.222-224 + page-image sense normalize.\n"
    "Scope: De bono pers. 23 / Vasquez / Patres / triple-cord Major. Further Cap. X remains.\n\n"
...
if morte_lock.exists():
    ...
    if "Cap. X Ultimum2 densify append" not in text:
        parts = ["[%s] %s" % (s["section"], s["latin"]) for s in secs]
        block = "\n\n=== De praedestinatione Cap. X Ultimum2 densify append ===\n" + "\n\n".join(parts)
...
packet = {
  "identity": {
    ...
    "work": "Two Dissertations (De praedestinatione through Cap. X Ultimum Secunda)",
    "edition": "Dissertationes duae (Cambridge: Roger Daniel, 1650). Partial through Cap. X Ultimum triple-cord Major. Further Cap. X remains.",
  ...
  "scope": {
    ...
    "locus": "De praedestinatione Cap. X Ultimum Secunda cont. (De bono pers. 23; Vasquez; Patres; triple-cord).",
    "next_locus": "Cap. X Ultimum Propos. 1 (Orthodox Grace of Christ vs Pelagius: illuminating/exciting vs healing/regenerating; then Arminius parallel).",
    "punch_x": "NO",
    "ship": "NO",
  },
  "sections": [{
...
(AUDIT/("%s.review.json" % PACKET)).write_text(json.dumps({
  ...
  "notes": "Cap. X Ultimum Secunda cont. densify; further Cap. X remains. Punch X=NO.",
  "punch_x": "NO",
}, ensure_ascii=False, indent=2)+"\n")

gates = {
  "hostname": host,
  "check_pass_ab": "ok fail=0 on morte_770..777",
...
block = (
  ...
  "CoS densify: Cap. X Ultimum Secunda cont. De bono pers. 23 / Vasquez / Patres / triple-cord. Punch X: **NO**. Did **not** ship. "
