...
OCR = BOOK/"sources/_ocr_praedestinatione_cap10_propos1iii"
PIPE = Path.home()/"SaneApps/clients/translations/pipeline"
PACKET = "praedestinatione_cap10_propos1iii_densify"
CHECK = PIPE/"check_pass_ab.py"
TIP_BEFORE = 785
HOLD_FLOOR_PRIOR = 5896
POST_HOLD_FLOOR = 5926
DEPLOY = "live5926"
...
for s in secs:
    ...
    jx = {
        ...
        "pass_a_ne_b": True,
        "bible_refs": [],
        "notes": "Densify De praedestinatione Cap. X Ultimum Propos. 1.III/2 section %s. Pass A!=B. Packet %s." % (n, PACKET),
        "lemmas": s["lemmas"],
        "choices": [
            {"issue": "Copy-text", "choice": "Locked 1650 Daniel Latin; IA PDF pp.226-228 + page-image sense-normalize."},
            {"issue": "Scope", "choice": "Propos. 1.III Fathers' Grace / Salisbury + Propos. 2 open through Aug. De praedest. sanct. 8; further Cap. X (Propos. 2 Prosper/councils; Propos. 3) remains."},
            {"issue": "Sense-normalize", "choice": "IA PDF + tess; Salisbury; Aug.; Milevis/Orange; Aquinas; Propos. 2."},
...
try:
    ...
    works, secs_live = (int(m.group(1)), int(m.group(2))) if m else (57, 5926)
except Exception:
    works, secs_live = 57, 5926
...
print(hold_note)
assert secs_live > HOLD_FLOOR_PRIOR or secs_live >= 5926

lock = BOOK/"sources/_davenant_praedestinatione_cap10_propos1iii_latin_lock.txt"
lock.write_text(
    "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. X Ultimum Propos. 1.III/2 densify lock\n"
    "Witnesses: IA 1650 Daniel PDF pp.226-228 + page-image sense normalize.\n"
    "Scope: Propos. 1.III Salisbury/Fathers + Propos. 2 open. Further Cap. X remains.\n\n"
...
if morte_lock.exists():
    ...
    if "Cap. X Propos.1iii densify append" not in text:
        parts = ["[%s] %s" % (s["section"], s["latin"]) for s in secs]
        block = "\n\n=== De praedestinatione Cap. X Propos.1iii densify append ===\n" + "\n\n".join(parts)
...
packet = {
  "identity": {
    ...
    "work": "Two Dissertations (De praedestinatione through Cap. X Ultimum Propos. 2)",
    "edition": "Dissertationes duae (Cambridge: Roger Daniel, 1650). Partial through Cap. X Propos. 2 Aug. witnesses. Further Cap. X remains.",
  ...
  "scope": {
    ...
    "locus": "De praedestinatione Cap. X Ultimum Propos. 1.III + Propos. 2 open.",
    "next_locus": "Cap. X Ultimum Propos. 2 cont. (Epist. 107; Prosper; Milevis/Orange on non-universal saving grace; then Propos. 3 mere good pleasure).",
    "punch_x": "NO",
    "ship": "NO",
  },
  "sections": [{
...
(AUDIT/("%s.review.json" % PACKET)).write_text(json.dumps({
  ...
  "notes": "Cap. X Ultimum Propos. 1.III/2 densify; further Cap. X remains. Punch X=NO.",
  "punch_x": "NO",
}, ensure_ascii=False, indent=2)+"\n")

gates = {
  "hostname": host,
  "check_pass_ab": "ok fail=0 on morte_786..793",
...
block = (
  ...
  "CoS densify: Cap. X Ultimum Propos. 1.III Salisbury/Fathers + Propos. 2 open. Punch X: **NO**. Did **not** ship. "
