#!/usr/bin/env python3
"""De praedestinatione Cap.1 prol.IV densify — tip 214 → 221. Packet praedestinatione_cap1_prol4_densify."""
...
def write_latin_lock(secs: list[dict]) -> None:
    lock = BOOK / "sources/_davenant_praedestinatione_cap1_prol4_latin_lock.txt"
    header = (
        "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. 1 Prolegomena IV densify lock\n"
        "Witnesses: IA 1650 Daniel PDF pdftotext pp.~129–132 + tesseract lat+eng + DjVu + sense normalize.\n"
        "Scope: Cap. 1 Prolegomena IV distinguishing conditions (works / faith foresight); after subject conditions; before Cap. II.\n\n"
    )
...
def extend_morte_lock(secs: list[dict]) -> None:
    ...
    if "Cap. 1 prol. IV densify append" in text or "Jam de conditionibus distinguentibus pauca etiam dicenda sunt" in text:
        print("morte lock already has prol4; skip append")
        return
    block = "\n\n=== De praedestinatione Cap. 1 prol. IV densify append ===\n" + "\n\n".join(
...
def update_meta(tip: int) -> None:
    ...
    note = "; De praedestinatione Cap. 1 Prolegomena IV distinguishing conditions tip."
    if isinstance(blurb, str) and "Prolegomena IV" not in blurb:
        if "blurb" in meta:
...
def build_packet(tip: int) -> str:
    ...
    packet = {
        "identity": {
            ...
            "work": "Two Dissertations (De morte Christi Cap. 1-7 + De praedestinatione Prefatio/Cap.1 prol.IV partial)",
            "edition": (
                ...
                "Cap. 1 Prolegomena I-IV (vocabulary, subject conditions, distinguishing conditions) "
                "(before Cap. II theses on election from foresight merits)."
            ),
        ...
        "scope": {
            ...
            "locus": "De praedestinatione Cap. 1 Prolegomena IV distinguishing conditions (works / faith foresight)",
            "next_locus": "Cap. II — Variæ illorum sententiæ (election from foresight merits / Pelagian opening)",
            "punch_x": "NO",
            "ship": "NO",
        },
        "sections": [
    ...
    review = {
        ...
        "pass_a_ne_b": True,
        "notes": "De praedestinatione Cap.1 Prolegomena IV densify; Cap. II election theses remain.",
        "punch_x": "NO",
    }
...
def write_receipt(before: int, after: int, packet_id: str, gates: dict, hold_note: str) -> None:
    receipt = {
        ...
        "latin_lock": str(BOOK / "sources/_davenant_praedestinatione_cap1_prol4_latin_lock.txt"),
        "hold_note": hold_note,
    }
...
def prepend_handoff(before: int, after: int, hold_note: str) -> None:
    ...
    block = f"""## {ts} (Cursor — Davenant De praedestinatione Cap. 1 prol. IV densify)

CoS densify: Cap. 1 Prolegomena IV distinguishing conditions (works / faith foresight). Punch X: **NO**. Cap. 1 mid-Thesis tip patch stays deferred. Did **not** edit `publication-review.json`; did not ship; did not commit; did not run Logos or ai_promote. {hold_note}

### Bound this job (not shipped)
- **Davenant** `davenant-dissertationes-duae`: Cap. 1 prol. IV distinguishing-condition tip (**{before}→{after}** sections). Locked densify Latin from IA 1650 Daniel PDF (pdftotext + tesseract + DjVu) with lock. Pass A≠B; OUR; English-first; no PBB/ops TNs; packet `{PACKET}`. Honest partial — **not** whole Dissertationes. Cap. II foresight-merits theses remain. Punch X: **NO**.
...
def main() -> None:
    ...
    gates = {
        ...
        "check_pass_ab": "ok fail=0 on morte_215..221",
        "tip_ready": tip_out,
        "punch_x": "NO",
        "ship": "NO",
        "claim": "davenant-dissertationes-densify stays claimed",
