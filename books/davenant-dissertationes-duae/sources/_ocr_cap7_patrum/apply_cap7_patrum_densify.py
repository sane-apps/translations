#!/usr/bin/env python3
"""Cap. 7 Patrum densify — tip 164 → ~171. Packet morte_christi_cap7_patrum_densify."""
...
OCR = BOOK / "sources/_ocr_cap7_patrum"
PIPE = Path.home() / "SaneApps/clients/translations/pipeline"
PACKET = "morte_christi_cap7_patrum_densify"
CHECK = PIPE / "check_pass_ab.py"
TIP_BEFORE = 164
HOLD_LIVE_FLOOR = 3527  # append when live sections > this OR 15 min elapsed
...
def write_justification(sec: dict) -> Path:
    ...
    payload = {
        ...
        "bible_refs": [],
        "notes": (
            f"Densify Cap. 7 ultimum Patrum & modernorum section {sec['section']}. "
        ...
        "choices": [
            {
                ...
                "choice": (
                    ...
                    "with sense normalize; Cap. 7 Patrum lock: "
                    "sources/_davenant_cap7_patrum_latin_lock.txt."
                ),
            },
            {
                "issue": "Scope",
                "choice": (
                    "Cap. 7 ultimum Patrum & modernorum (Augustine In Joh. tract. 48 through Bernard) "
                    "close; before Sed missis Scholastics & modern Pontificii."
                ),
            },
        ],
    }
...
def update_meta(tip: int) -> None:
    ...
    note = "; Cap. 7 ultimum Patrum & modernorum tip."
    if isinstance(blurb, str) and "Patrum" not in blurb and "modernorum" not in blurb:
        if "blurb" in meta:
...
def write_latin_lock(secs: list[dict]) -> None:
    lock = BOOK / "sources/_davenant_cap7_patrum_latin_lock.txt"
    header = (
        "CAPUT VII — ultimum Patrum & modernorum densify lock\n"
        "Witnesses: IA 1650 Daniel PDF pdftotext pp.105–109 + tesseract p-106..p-108 "
        "(lat+eng) + sense normalize long-s/ligatures.\n"
        "Scope: Augustine In Joh. tract. 48 through Bernard close before Sed missis Scholastics.\n\n"
    )
...
def extend_morte_lock(secs: list[dict]) -> None:
    ...
    if "ultimum Patrum" in text and "Bernardus" in text and "Paschasius" in text:
        print("morte lock already has Patrum; skip append")
        return
    block = "\n\n=== Cap. 7 Patrum densify append ===\n" + "\n\n".join(
...
def build_packet(tip: int) -> str:
    ...
    packet = {
        ...
        "publication_scope": {
            "status": "partial",
            "note": f"Tip densify Cap. 7 ultimum Patrum (secs 165-{tip}). Not whole Dissertationes.",
            "packet": PACKET,
        },
        ...
        "raw_source_paths": [
            str(BOOK / "sources/_davenant_cap7_patrum_latin_lock.txt"),
    ...
    review = {
        ...
        "scope_review": {
            ...
            "uncertainties": [],
            "notes": f"Scope rebind davenant-dissertationes-duae tip {tip} (Cap. 7 ultimum Patrum).",
        },
        "reviews": reviews,
    }
...
def write_receipt(before: int, after: int, packet_id: str, gates: dict) -> None:
    receipt = {
        ...
        "locus": "Cap. 7 ultimum Patrum & modernorum (Augustine In Joh. tract. 48 through Bernard)",
        "next_locus": "Cap. 7 Scholastics & modern Pontificii (Sed missis … Lombard/Aquinas …)",
        "gates": gates,
        "punch_x": "NO",
        "ship": "NO",
        "claim": "davenant-dissertationes-densify stays claimed",
        "latin_lock": str(BOOK / "sources/_davenant_cap7_patrum_latin_lock.txt"),
    }
...
CoS densify: Cap. 7 ultimum Patrum & modernorum (Augustine In Joh. tract. 48 through Bernard) close before Sed missis Scholastics. Punch X: **NO**. Cap. 1 mid-Thesis tip patch stays deferred. Did **not** edit `publication-review.json`; did not ship; did not commit; did not run Logos or ai_promote. {hold_note}

### Bound this job (not shipped)
- **Davenant** `davenant-dissertationes-duae`: Cap. 7 Patrum tip (**{before}→{after}** sections). Locked densify Latin from IA 1650 Daniel PDF (pdftotext + DjVu) with lock. Pass A≠B; OUR; English-first; no PBB/ops TNs; packet `morte_christi_cap7_patrum_densify`. Honest partial — **not** whole Dissertationes. Cap. 7 Scholastics–remainder through Cap. 11 and De praedestinatione remain. Punch X: **NO**.
