#!/usr/bin/env python3
"""De praedestinatione Cap. II Scholastici densify — tip 228 → 235. Packet praedestinatione_cap2_scholastici_densify."""
...
def write_latin_lock(secs: list[dict]) -> None:
    lock = BOOK / "sources/_davenant_praedestinatione_cap2_scholastici_latin_lock.txt"
    header = (
        "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. II Scholastici densify lock\n"
        "Witnesses: IA 1650 Daniel PDF pdftotext pp.~132–136 + tesseract lat+eng + DjVu + sense normalize.\n"
        "Scope: Cap. II Scholastici (causes of election error; then Pontificii / Vasquez Arg. 1 + Sol.); after Cap. II Variae; before Cap. II Arg. 2 praescientia loci.\n\n"
    )
...
def extend_morte_lock(secs: list[dict]) -> None:
    ...
    if "Cap. II Scholastici densify append" in text or "Sed descendamus a Semipelagianis ad Scholasticos" in text:
        print("morte lock already has Cap II Scholastici; skip append")
        return
    block = "\n\n=== De praedestinatione Cap. II Scholastici densify append ===\n" + "\n\n".join(
...
def update_meta(tip: int) -> None:
    ...
    note = "; De praedestinatione Cap. II Scholastici / Vasquez Arg. 1 tip."
    if isinstance(blurb, str) and "Cap. II Scholastici" not in blurb:
        if "blurb" in meta:
...
def build_packet(tip: int) -> str:
    ...
    packet = {
        "identity": {
            ...
            "work": "Two Dissertations (De morte Christi Cap. 1-7 + De praedestinatione Prefatio/Cap.1–Cap.II Scholastici/Vasquez Arg.1 partial)",
            "edition": (
                ...
                "(before Cap. II Arg. 2 praescientia loci)."
            ),
        ...
        "scope": {
            ...
            "locus": "De praedestinatione Cap. II Scholastici (causes of election error; then Pontificii / Vasquez Arg. 1 + Sol.)",
            "next_locus": "Cap. II Arg. 2 (praescientia loci Rom. 8 / 11 / 1 Pet. 1) and Sol.",
            "punch_x": "NO",
            "ship": "NO",
        },
        "sections": [
    ...
    review = {
        ...
        "pass_a_ne_b": True,
        "notes": "De praedestinatione Cap. II Scholastici densify; Cap. II Arg. 2 praescientia loci remain.",
        "punch_x": "NO",
    }
...
def write_receipt(before: int, after: int, packet_id: str, gates: dict, hold_note: str) -> None:
    receipt = {
        ...
        "latin_lock": str(BOOK / "sources/_davenant_praedestinatione_cap2_scholastici_latin_lock.txt"),
        "hold_note": hold_note,
    }
...
def prepend_handoff(before: int, after: int, hold_note: str) -> None:
    ...
    block = f"""## {ts} (Cursor — Davenant De praedestinatione Cap. II Scholastici densify)

CoS densify: Cap. II Scholastici (causes of election error; then Pontificii / Vasquez Arg. 1 + Sol.). Punch X: **NO**. Cap. 1 mid-Thesis tip patch stays deferred. Did **not** edit `publication-review.json`; did not ship; did not commit; did not run Logos or ai_promote. {hold_note}

### Bound this job (not shipped)
- **Davenant** `davenant-dissertationes-duae`: Cap. II Scholastici tip (**{before}→{after}** sections). Locked densify Latin from IA 1650 Daniel PDF (pdftotext + tesseract + DjVu) with lock. Pass A≠B; OUR; English-first; no PBB/ops TNs; packet `{PACKET}`. Honest partial — **not** whole Dissertationes. Cap. II Arg. 2 praescientia loci remain. Punch X: **NO**.
...
def main() -> None:
    ...
    gates = {
        ...
        "check_pass_ab": "ok fail=0 on morte_229..235",
        "tip_ready": tip_out,
        "punch_x": "NO",
        "ship": "NO",
        "claim": "davenant-dissertationes-densify stays claimed",
