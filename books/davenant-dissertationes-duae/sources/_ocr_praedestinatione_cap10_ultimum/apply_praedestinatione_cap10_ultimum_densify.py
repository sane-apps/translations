...
OCR = BOOK/"sources/_ocr_praedestinatione_cap10_ultimum"
PIPE = Path.home()/"SaneApps/clients/translations/pipeline"
PACKET = "praedestinatione_cap10_ultimum_densify"
CHECK = PIPE/"check_pass_ab.py"
TIP_BEFORE = 761
HOLD_FLOOR_PRIOR = 5809
POST_HOLD_FLOOR = 5838
DEPLOY = "live5838"
...
for s in secs:
    ...
    jx = {
        ...
        "pass_a_ne_b": True,
        "bible_refs": [],
        "notes": "Densify De praedestinatione Cap. X Corvinus/Ultimum section %s. Pass A!=B. Packet %s." % (n, PACKET),
        "lemmas": s["lemmas"],
        "choices": [
            {"issue": "Copy-text", "choice": "Locked 1650 Daniel Latin; IA PDF pp.221-222 + page-image sense-normalize."},
            {"issue": "Scope", "choice": "Arg. X Corvinus close + Ultimum Prima/Secunda open; further Cap. X (grace-predestination knot; triple cord) remains."},
            {"issue": "Sense-normalize", "choice": "IA PDF + tess; Corvinus Contra Tilen. 148; Orange 6; Col. i.12; councils; Aug. De praedest. sanct. 10."},
...
try:
    ...
    works, secs_live = (int(m.group(1)), int(m.group(2))) if m else (57, 5838)
except Exception:
    works, secs_live = 57, 5838
...
print(hold_note)
assert secs_live > HOLD_FLOOR_PRIOR or secs_live >= 5838

lock = BOOK/"sources/_davenant_praedestinatione_cap10_ultimum_latin_lock.txt"
lock.write_text(
    "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. X Ultimum densify lock\n"
    "Witnesses: IA 1650 Daniel PDF pp.221-222 + page-image sense normalize.\n"
    "Scope: Corvinus reply + Ultimum Prima/Secunda open. Further Cap. X remains.\n\n"
...
if morte_lock.exists():
    ...
    if "Cap. X Ultimum densify append" not in text:
        parts = ["[%s] %s" % (s["section"], s["latin"]) for s in secs]
        block = "\n\n=== De praedestinatione Cap. X Ultimum densify append ===\n" + "\n\n".join(parts)
...
packet = {
  "identity": {
    ...
    "work": "Two Dissertations (De praedestinatione through Cap. X Ultimum)",
    "edition": "Dissertationes duae (Cambridge: Roger Daniel, 1650). Partial through Cap. X Ultimum Prima/Secunda open. Further Cap. X remains.",
  ...
  "scope": {
    ...
    "locus": "De praedestinatione Cap. X Corvinus + Ultimum Prima/Secunda open.",
    "next_locus": "Cap. X Ultimum Secunda cont. (Aug. De bono pers. 23; Vasquez grace=predestination knot; Patres ante Pelagium; triple-cord ratio).",
    "punch_x": "NO",
    "ship": "NO",
  },
  "sections": [{
...
(AUDIT/("%s.review.json" % PACKET)).write_text(json.dumps({
  ...
  "notes": "Cap. X Ultimum densify; further Cap. X remains. Punch X=NO.",
  "punch_x": "NO",
}, ensure_ascii=False, indent=2)+"\n")

gates = {
  "hostname": host,
  "check_pass_ab": "ok fail=0 on morte_762..769",
...
block = (
  ...
  "CoS densify: Cap. X Corvinus + Ultimum Prima/Secunda open. Punch X: **NO**. Did **not** ship. "
