#!/usr/bin/env python3
"""De praedestinatione Cap. II Arg. 5 densify — tip 250 → 256. Packet praedestinatione_cap2_arg5_densify."""
...
def write_latin_lock(secs: list[dict]) -> None:
    lock = BOOK / "sources/_davenant_praedestinatione_cap2_arg5_latin_lock.txt"
    header = (
        "DE PRAEDESTINATIONE ET REPROBATIONE — Cap. II Arg. 5 densify lock\n"
        "Witnesses: IA 1650 Daniel PDF pdftotext pp.~137–139 + tesseract lat+eng + DjVu + sense normalize.\n"
        "Scope: Cap. II Arg. 5 (1 Tim. 2:4 God wills all men to be saved) and Sol.; after Cap. II Arg. 4 Sol.; before Cap. II Arg. 6 / James 2:5.\n\n"
    )
...
def extend_morte_lock(secs: list[dict]) -> None:
    ...
    if "Cap. II Arg. 5 densify append" in text or "Quinto, ex 1 Timoth. ii. 4" in text:
        print("morte lock already has Cap II Arg. 5; skip append")
        return
    block = "\n\n=== De praedestinatione Cap. II Arg. 5 densify append ===\n" + "\n\n".join(
...
def update_meta(tip: int) -> None:
    ...
    note = "; De praedestinatione Cap. II Arg. 5 (1 Tim. 2:4) tip."
    if isinstance(blurb, str) and "Cap. II Arg. 5" not in blurb:
        if "blurb" in meta:
...
def build_packet(tip: int) -> str:
    ...
    packet = {
        "identity": {
            ...
            "work": "Two Dissertations (De morte Christi Cap. 1-7 + De praedestinatione Prefatio/Cap.1–Cap.II Arg.5 partial)",
            "edition": (
                ...
                "+ Cap. II Arg. 2–5 + Sol. (before Cap. II Arg. 6 / James 2:5)."
            ),
        ...
        "scope": {
            ...
            "locus": "De praedestinatione Cap. II Arg. 5 (1 Tim. 2:4 God wills all men to be saved) and Sol.",
            "next_locus": "Cap. II Arg. 6 (James 2:5 poor elect / rich in faith) and Sol.",
            "punch_x": "NO",
            "ship": "NO",
        },
        "sections": [
    ...
    review = {
        ...
        "pass_a_ne_b": True,
        "notes": "De praedestinatione Cap. II Arg. 5 densify; Cap. II Arg. 6 / James 2:5 remain.",
        "punch_x": "NO",
    }
...
def write_receipt(before: int, after: int, packet_id: str, gates: dict, hold_note: str) -> None:
    receipt = {
        ...
        "latin_lock": str(BOOK / "sources/_davenant_praedestinatione_cap2_arg5_latin_lock.txt"),
        "hold_note": hold_note,
    }
...
def prepend_handoff(before: int, after: int, hold_note: str) -> None:
    ...
    block = f"""## {ts} (Cursor — Davenant De praedestinatione Cap. II Arg. 5 densify)

CoS densify: Cap. II Arg. 5 (1 Tim. 2:4 God wills all men to be saved) and Sol. Punch X: **NO**. Cap. 1 mid-Thesis tip patch stays deferred. Did **not** edit `publication-review.json`; did not ship; did not commit; did not run Logos or ai_promote. {hold_note}

### Bound this job (not shipped)
- **Davenant** `davenant-dissertationes-duae`: Cap. II Arg. 5 tip (**{before}→{after}** sections). Locked densify Latin from IA 1650 Daniel PDF (pdftotext + tesseract + DjVu) with lock. Pass A≠B; OUR; English-first; no PBB/ops TNs; packet `{PACKET}`. Honest partial — **not** whole Dissertationes. Cap. II Arg. 6 / James 2:5 remain. Punch X: **NO**.
...
def main() -> None:
    ...
    gates = {
        ...
        "check_pass_ab": "ok fail=0 on morte_251..256",
        "tip_ready": tip_out,
        "punch_x": "NO",
        "ship": "NO",
        "claim": "davenant-dissertationes-densify stays claimed",
