# -*- coding: utf-8 -*-
"""Man. Post. Cap. X densify — continue after reatus tip."""
...
OCR = BOOK / "sources/_ocr_cap10_mp_rom5_cont"
...
HOLD_START_PATH = Path("/tmp/placeus_cap10_mp_rom5_cont_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5809
PACKET_STEM = "cap10_mp_rom5_cont_densify"
LOCK_NAME = "_placeus_cap10_mp_rom5_cont_latin_lock.txt"
JX_PREFIX = "cap10_mp_rom5_cont"
...
Scope: Manuscriptum posterius Cap. X — PLAC on reatus/corruption after GAR.3, printed pp. 362–364 tip. Stops before further Cap. X. Not whole Cap. X / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. X GAR on non imputatur+ / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.X cont densify tip-676 to 682; need live>5809 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. X reatus densify section {sid} "
                "from locked 1661 Latin (pp. 362–364 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput X (GAR.1–3 Rom. 5.13 tip) "
        "/ before PLAC on reatus-only"
    )
    remain = "Cap. X GAR on non imputatur+ / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. X three v.13 args tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap10_mp_rom5_reatus_densify.packet.json").read_text(encoding="utf-8"))
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. X reatus densify from locked 1661 Latin "
            "(pp. 362–364 tip); Pass A != B."
            if int(sid) >= 677
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. X cont densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput X three v.13 args tip "
                "from locked 1661 Saumur Latin "
                "(pp. 362–364 tip). Honest partial — Cap. X GAR on non imputatur+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. X cont densify

- Before: **{BEFORE}** sections (through Cap. X reatus tip)
- After: **{AFTER}** sections (through Cap. X three v.13 args tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 362–364 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: vitium innatum; usque ad legem; tria argumenta; Punch X=NO
- Gates: `check_pass_ab` ok {JX_PREFIX}_677–682; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5809 or 12m (`sources/_ocr_cap10_mp_rom5_cont/hold_receipt.json`; released {hold_note})
- Stopped before Cap. X GAR on non imputatur…
- Honest partial: Cap. X GAR on non imputatur+ / rest / ~494 pp Disputatio remain (**do not claim full Disputatio**)
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_677-682", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. X — GAR on non imputatur (after three v.13 tip)",
    }
