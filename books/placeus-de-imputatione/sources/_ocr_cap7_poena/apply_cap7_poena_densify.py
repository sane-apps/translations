# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — Cap. VII penalties / dying-you-die / meritorie grace tip; before sons-grandsons imputation."""
...
OCR = BOOK / "sources/_ocr_cap7_poena"
...
HOLD_START_PATH = Path("/tmp/placeus_cap7_poena_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4581
PACKET_STEM = "cap7_poena_densify"
LOCK_NAME = "_placeus_cap7_poena_latin_lock.txt"
JX_PREFIX = "cap7_poena"
...
Scope: Manuscriptum posterius Cap. VII first similitude — penalties / dying-you-die / meritorie grace comparison (printed pp. 265 tip–266 tip). Stops before sons/grandsons imputation. Not whole Cap. VII / Disputatio.
...
Remaining: Cap. VII sons-grandsons+ / second–third similitudes / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VII poena densify tip-377 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VII poena densify section {sid} "
                "from locked 1661 Latin (pp. 265 tip–266 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VII (Examen trium similitudinum; penalties/meritorie tip) "
        "/ before sons-grandsons imputation"
    )
    remain = "Cap. VII sons-grandsons imputation+ / second–third similitudes / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VII penalties/meritorie tip (before sons-grandsons imputation). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap7_sep_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap7_poena",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VII penalties/meritorie tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VII penalties/meritorie tip) before sons-grandsons, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VII sons-grandsons deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VII poena densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VII penalties/meritorie tip) before sons-grandsons from locked 1661 Saumur Latin "
                "(pp. 265 tip–266 tip). Honest partial — Cap. VII sons-grandsons deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VII poena densify (penalties / meritorie → before sons-grandsons)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VII separability/judicial tip / before penalties deepen)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VII penalties/meritorie tip / before sons-grandsons imputation)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 265 tip–266 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: poenae / morte morieris / meritorie / status gratiae; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_378–383; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap7_poena/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_378-383", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VII — sons/grandsons imputation / Deut 24",
    }
