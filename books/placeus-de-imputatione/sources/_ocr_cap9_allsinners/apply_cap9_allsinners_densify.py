# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — all-are-sinners proof; tip before eph ho."""
...
OCR = BOOK / "sources/_ocr_cap9_allsinners"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_allsinners_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5225
PACKET_STEM = "cap9_allsinners_densify"
LOCK_NAME = "_placeus_cap9_allsinners_latin_lock.txt"
JX_PREFIX = "cap9_allsinners"
...
Scope: Manuscriptum posterius Cap. IX — all-are-sinners proof (peccatum in mundo / in hominibus) printed pp. 330–332 tip. Stops before eph ho. Not whole Cap. IX / Disputatio.
...
Remaining: Cap. IX eph ho+ / rest of Cap. IX / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX allsinners densify tip-526; need live>5225 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX allsinners densify section {sid} "
                "from locked 1661 Latin (pp. 330–332 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (all-are-sinners proof) "
        "/ before eph ho"
    )
    remain = "Cap. IX eph ho+ / rest of Cap. IX / ~494 pp Disputatio remain."
    meta.update({
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_omit_densify.packet.json").read_text(encoding="utf-8"))
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Caput IX (all-are-sinners proof) before "
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX allsinners densify from locked 1661 Latin "
            "(pp. 330–332 tip); Pass A != B."
            if int(sid) >= 533
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX allsinners densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "before eph ho from locked 1661 Saumur Latin "
                "(pp. 330–332 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX allsinners densify (omit/death dilemma → before all-sinners)

- Before: **{BEFORE}** sections (through Cap. IX omit/death dilemma tip)
- After: **{AFTER}** sections (through Cap. IX all-are-sinners tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 330–332 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_533–538; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5225 or 12m (`sources/_ocr_cap9_allsinners/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_533-538", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — eph ho / eph ho pantes (in quo / because all sinned)",
    }
