# -*- coding: utf-8 -*-
"""Man. Post. Cap. VII densify — GAR Ezek 21 / just with unjust reply; tip before Quarto king."""
...
OCR = BOOK / "sources/_ocr_cap7_ezek"
...
HOLD_START_PATH = Path("/tmp/placeus_cap7_ezek_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4767
PACKET_STEM = "cap7_ezek_densify"
LOCK_NAME = "_placeus_cap7_ezek_latin_lock.txt"
JX_PREFIX = "cap7_ezek"
...
Scope: Manuscriptum posterius Cap. VII — GAR Ezek 21 / just with unjust reply (printed pp. 292 tip–295 tip). Stops before Quarto king analogy. Not whole Cap. VII / Disputatio.
...
Remaining: Cap. VII Quarto king+ / rest of Cap. VII / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VII ezek densify tip-419 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VII ezek densify section {sid} "
                "from locked 1661 Latin (pp. 292 tip–295 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VII (Examen trium similitudinum; GAR Ezek 21 / just-with-unjust) "
        "/ before Quarto king"
    )
    remain = "Cap. VII Quarto king+ / rest of Cap. VII / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VII GAR Ezek 21 / just-with-unjust (before Quarto). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap7_judge_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap7_ezek",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VII GAR Ezek 21 / just-with-unjust (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VII GAR Ezek 21 / just-with-unjust) before Quarto, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VII Quarto king deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. VII Ezek densify from locked 1661 Latin "
            "(pp. 292 tip–295 tip); Pass A != B."
            if int(sid) >= 420
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VII ezek densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VII GAR Ezek 21 / just-with-unjust) before Quarto from locked 1661 Saumur Latin "
                "(pp. 292 tip–295 tip). Honest partial — Cap. VII Quarto+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VII ezek densify (GAR Ezek 21 / just with unjust → before Quarto)

- Before: **{BEFORE}** sections (through Cap. VII God-as-judge / Abraham immediate tip)
- After: **{AFTER}** sections (through Cap. VII GAR Ezek 21 / just-with-unjust tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 292 tip–295 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_420–425; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap7_ezek/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_420-425", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VII — Quarto king / rebel justice analogy",
    }
