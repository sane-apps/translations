# -*- coding: utf-8 -*-
"""Man. Post. Cap. X densify — PLAC on reatus after GAR.3."""
...
OCR = BOOK / "sources/_ocr_cap10_mp_rom5_reatus"
...
HOLD_START_PATH = Path("/tmp/placeus_cap10_mp_rom5_reatus_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5780
PACKET_STEM = "cap10_mp_rom5_reatus_densify"
LOCK_NAME = "_placeus_cap10_mp_rom5_reatus_latin_lock.txt"
JX_PREFIX = "cap10_mp_rom5_reatus"
...
Scope: Manuscriptum posterius Cap. X — PLAC on reatus/corruption after GAR.3, printed pp. 360–362 tip. Stops before further Cap. X. Not whole Cap. X / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. X continue+ / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.X reatus densify tip-670 to 676; need live>5780 or 12m; Punch X=NO",
    }
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                ...
                "from locked 1661 Latin (pp. 360–362 tip)."
            ),
        }
...
def main() -> None:
    ...
    remain = "Cap. X continue+ / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. X reatus tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap10_mp_rom5_gar_densify.packet.json").read_text(encoding="utf-8"))
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. X reatus densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput X reatus tip "
                "from locked 1661 Saumur Latin "
                "(pp. 360–362 tip). Honest partial — Cap. X continue+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. X reatus densify

- Before: **{BEFORE}** sections (through Cap. X GAR.3 tip)
- After: **{AFTER}** sections (through Cap. X reatus tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 360–362 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: solus reatus; ἁμαρτία; ἄχρι νόμου; Punch X=NO
- Gates: `check_pass_ab` ok {JX_PREFIX}_671–676; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5780 or 12m (`sources/_ocr_cap10_mp_rom5_reatus/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_671-676", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. X — continue after reatus tip",
    }
