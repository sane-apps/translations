# -*- coding: utf-8 -*-
"""Cap. IV Noli/Quarta/Quinta densify — peccatum≠poena; Quarta ante judicium; Quinta Gen 5:5; GAR misnamed tertia; before GAR 2."""
...
OCR = BOOK / "sources/_ocr_cap4_noli_quarta"
...
HOLD_START_PATH = Path("/tmp/placeus_cap4_noli_quarta_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 3896  # tip-238 catch-up ship; need live>3896 or 12m
PACKET_STEM = "cap4_noli_quarta_densify"
LOCK_NAME = "_placeus_cap4_noli_quarta_latin_lock.txt"
JX_PREFIX = "cap4_noli_quarta"
...
Scope: Manuscriptum posterius Cap. IV after Tertia GAR three rationes: Noli mihi objicere; Quarta ratio; Quinta through Gen 5:5 וַיְחִי / interlocutory paradise judgment; Garissoles misnamed "tertia" + Resp. 1; Placeus syllogism / limitatio / gratia through Pergat igitur (printed pp. 202 tip–206 tip). Stops before GAR 2 on Quinta. Not Cap. IV remainder+.
Reconstruction: IA PDF page images via macOS Vision OCR + pdftotext; long-s/ligature OCR corrected. Locked Hebrew וַיְחִי. Do not revive ἔργον.
No modern English used as copy-text.
Remaining: Cap. IV GAR 2 on Quinta+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "tip-238 catch-up ship in flight; re-probe before append; need live>3896 or 12m",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IV Noli/Quarta densify section {sid} "
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Cap. IV Noli/Quarta/Quinta (Gen 5:5 וַיְחִי; GAR misnamed tertia; "
        "PLAC syllogism) / before GAR 2"
    )
    remain = (
        "Cap. IV GAR 2 on Quinta+ / rest of MS examination / ~494 pp Disputatio remain."
    )
    meta.update({
        ...
        "edition": (
            ...
            "through Cap. IV Noli/Quarta/Quinta (before GAR 2 on Quinta). "
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads(
                (BOOK / "reviews/audit/cap4_tertia_gar_densify.packet.json").read_text(
                    encoding="utf-8"
                )
            )
            wits = list(
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap4_noli_quarta",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": (
                "Locked Latin, Man. Post. Cap. IV Noli/Quarta/Quinta "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Cap. IV Noli mihi / Quarta / Quinta (Gen 5:5 וַיְחִי; interlocutory "
            "judgment; Garissoles misnamed tertia; Placeus syllogism) before GAR 2, "
            "reconstructed from IA PDF page images with Vision OCR and pdftotext as check. "
            "Locked Hebrew וַיְחִי. No modern English was copied. Cap. IV GAR 2+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IV Noli/Quarta/Quinta densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Cap. IV Noli/Quarta/Quinta "
                "(Gen 5:5 וַיְחִי; GAR misnamed tertia; PLAC syllogism/gratia) before GAR 2 "
                "from locked 1661 Saumur Latin (pp. 202 tip–206 tip). Honest partial — "
                "Cap. IV GAR 2+ remain. Locked Hebrew וַיְחִי. Do not revive ἔργον. "
                "Punch X: NO."
            ),
        },
        "reviews": reviews,
    ...
    block = f"""## 2026-09-22 Cap. IV Noli/Quarta/Quinta densify (Noli → Quarta → Quinta Gen 5:5 → GAR misnamed tertia → before GAR 2)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. IV Tertia GAR three rationes / before Noli/Quarta)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. IV Noli/Quarta/Quinta / PLAC syllogism–gratia / before GAR 2)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 202 tip–206 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: locked Hebrew **וַיְחִי** (Gen 5:5); do not revive ἔργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_239–244; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: tip-238 catch-up ship in flight; eng/src append after live>3896 or 12m (`sources/_ocr_cap4_noli_quarta/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_239-244",
        "tip_ready": "ok",
        "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IV GAR 2 on Quinta (non mirum / temporal deferral) / PLAC posterior",
        "prior_packet_done": "cap4_tertia_gar_densify (231→238)",
    }
