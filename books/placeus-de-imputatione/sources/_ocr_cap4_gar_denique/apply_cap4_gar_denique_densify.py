# -*- coding: utf-8 -*-
"""Cap. IV GAR Denique densify — Pontificii/Bellarmine/Chamier; Vir Doctus threefold; before Caput V."""
...
OCR = BOOK / "sources/_ocr_cap4_gar_denique"
...
HOLD_START_PATH = Path("/tmp/placeus_cap4_gar_denique_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 3983  # tip-256 catch-up ship; need live>3983 or 12m
PACKET_STEM = "cap4_gar_denique_densify"
LOCK_NAME = "_placeus_cap4_gar_denique_latin_lock.txt"
JX_PREFIX = "cap4_gar_denique"
...
Scope: Manuscriptum posterius Cap. IV after GAR 4 Decalogue tip: GAR Denique (Pontificii / Bellarmine / Coster / Chamier hoc est corpus); PLAC comparison reply + Vir Doctus intro; GAR Primura rationem triplicem; PL Distinguo mala (culpae/poenae); GAR 2 / PLAC integritas; GAR 3 / PLAC latitudo + Caeterum tip through before Caput V (printed pp. 211 tip-213 tip). Stops before Caput V. Not Caput V+ / rest of MS examination.
...
Remaining: Caput V+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "tip-256 catch-up ship in flight; re-probe before append; need live>3983 or 12m",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IV GAR Denique densify section {sid} "
                "from locked 1661 Latin (pp. 211 tip–213 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Cap. IV GAR Denique (Pontificii / Bellarmine / Chamier; PLAC; "
        "Vir Doctus threefold / Caeterum tip) / before Caput V"
    )
    remain = (
        "Caput V+ / rest of MS examination / ~494 pp Disputatio remain."
    )
    meta.update({
        ...
        "edition": (
            ...
            "through Cap. IV GAR Denique / PLAC / Vir Doctus threefold tip (before Caput V). "
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads(
                (BOOK / "reviews/audit/cap4_gar_ultimo_densify.packet.json").read_text(
                    encoding="utf-8"
                )
            )
            wits = list(
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap4_gar_denique",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": (
                "Locked Latin, Man. Post. Cap. IV GAR Denique / Vir Doctus threefold tip "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Cap. IV GAR Denique (Pontificii / Bellarmine / Chamier) through "
            "Placeus replies and Vir Doctus threefold (Primura / GAR 2-3 / Caeterum) before Caput V, "
            "reconstructed from IA PDF page images with Vision OCR and pdftotext as check. "
            "No modern English was copied. Caput V+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads(
        (BOOK / "reviews/audit/cap4_gar_ultimo_densify.packet.json").read_text(encoding="utf-8")
    )
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IV GAR Denique densify from locked 1661 Latin "
            "(pp. 211 tip–213 tip); Pass A != B."
            if int(sid) >= 257
            else (
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IV GAR Denique densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Cap. IV GAR Denique (Pontificii / Bellarmine / Chamier; Vir Doctus threefold / Caeterum) before Caput V from locked 1661 Saumur Latin (pp. 211 tip–213 tip). Honest partial — Caput V+ remain. Do not revive εργον. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. IV GAR Denique densify (Pontificii / Bellarmine / Chamier → Vir Doctus threefold → before Caput V)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. IV GAR Ultimo / Decalogue tip / before Denique)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. IV GAR Denique / Vir Doctus threefold / Caeterum tip / before Caput V)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 211 tip–213 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_257–262; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: tip-256 catch-up ship; eng/src append after live>3983 or 12m (`sources/_ocr_cap4_gar_denique/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_257-262",
        "tip_ready": "ok",
        "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Caput V (Examen Cap. III quid sit imputari / non imputari)",
        "prior_packet_done": "cap4_gar_ultimo_densify (250→256)",
    }
