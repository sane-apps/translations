# -*- coding: utf-8 -*-
"""Cap. III Observata quaedam densify — before Cap. IV Rursus observata."""
...
OCR = BOOK / "sources/_ocr_cap3_observata"
...
HOLD_START_PATH = Path("/tmp/placeus_cap3_observata_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 3809
PACKET_STEM = "cap3_observata_densify"
LOCK_NAME = "_placeus_cap3_observata_latin_lock.txt"
JX_PREFIX = "cap3_observata"
...
Scope: Manuscriptum posterius Cap. III Observata quaedam in primo & secundo Capite Tractatus through Laudo denique / Protestant consensus (printed pp. 186 tip–190 tip). Stops before Cap. IV Rursus quaedam observata. Not Cap. IV+.
...
Remaining: Cap. IV+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "catch-up ship tip 205 in flight; do not append eng/src until live>3809 or 12m",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. III Observata quaedam densify section {sid} "
                "from locked 1661 Latin (pp. 186 tip–190 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Cap. III Observata quaedam (Cap. 1–2 notes / personale / culpa-poena "
        "absurdities / fons-radix / Rom 5 transfusa / before Cap. IV)"
    )
    remain = (
        "Cap. IV+ / rest of MS examination / ~494 pp Disputatio remain."
    )
    meta.update({
        ...
        "edition": (
            ...
            "IA deimputationepri00lapl. Densify: Capita I–XII + Caput XIV + Man. Post. Cap. II–III "
            "through Cap. III Observata quaedam (before Cap. IV). "
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads(
                (BOOK / "reviews/audit/cap2_posterior_poena_densify.packet.json").read_text(
                    encoding="utf-8"
                )
            )
            wits = list(
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap3_observata",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": (
                "Locked Latin, Man. Post. Cap. III Observata quaedam before Cap. IV "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    for wid, path, name in [
        ...
        (
            "latin_lock_cap2_gar_habitus",
            "sources/_placeus_cap2_gar_habitus_latin_lock.txt",
            "Locked Latin, Man. Post. Cap. II GAR Deinde habitus before Posterior quaestio (Saumur 1661)",
        ),
    ]:
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Cap. III Observata quaedam in primo & secundo Capite "
            "(laudanda/refutanda; personale; culpa-poena; fons/radix; Rom 5) before Cap. IV, "
            "reconstructed from IA PDF page images with Vision OCR, tesseract, and pdftotext "
            "as check. No modern English was copied. Cap. IV+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads(
        (BOOK / "reviews/audit/cap2_posterior_poena_densify.packet.json").read_text(encoding="utf-8")
    )
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. III Observata quaedam densify from locked 1661 Latin "
            "(pp. 186 tip–190 tip); Pass A != B."
            if int(sid) >= 206
            else (
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. III Observata quaedam densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Cap. III Observata quaedam in primo & secundo "
                "Capite (digression/notanda; personale/Ezekiel; culpa-poena absurdities; "
                "propagation escape; fons/radix; Rom 5 transfusa; Laudo denique) before Cap. IV "
                "from locked 1661 Saumur Latin (pp. 186 tip–190 tip). Honest partial — Cap. IV+ "
                "remain. Reformed Saumur era disclosed. No new Greek lemma this slice; do not "
    ...
    block = f"""## 2026-09-22 Cap. III Observata quaedam densify (Cap. 1–2 notes → before Cap. IV)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. II Posterior quaestio de poena / before Cap. III)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. III Observata quaedam / personale / culpa-poena / fons-radix / Rom 5 / before Cap. IV)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 186 tip–190 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: no new Greek lemma this slice; do not revive ἔργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_206–212; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: catch-up ship tip 205 in flight; eng/src append after live>3809 or 12m (`sources/_ocr_cap3_observata/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_206-212",
        "tip_ready": "ok",
        "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IV Rursus quaedam observata & reprehensa in eodem Cap. II",
    }
