# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — missa / poena originale counter."""
...
OCR = BOOK / "sources/_ocr_cap9_missa_poena"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_missa_poena_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5646
PACKET_STEM = "cap9_missa_poena_densify"
LOCK_NAME = "_placeus_cap9_missa_poena_latin_lock.txt"
JX_PREFIX = "cap9_missa_poena"
...
Scope: Manuscriptum posterius Cap. IX — missa/poena originale counter printed pp. 354–355 tip. Stops before imputation-vs-inhaerentia block. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX imputation/inhaerentia+ / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX PLAC vere densify tip-640→646; need live>5646 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX missa/poena densify section {sid} "
                "from locked 1661 Latin (pp. 354–355 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (missa/poena tip) "
        "/ before imputation/inhaerentia"
    )
    remain = "Cap. IX imputation/inhaerentia+ / rest Cap. IX / ~494 pp Disputatio remain."
    meta.update({
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_pro_posteris_densify.packet.json").read_text(encoding="utf-8"))
    ...
    block = f"""## 2026-09-23 Cap. IX missa / poena originale counter densify

- Before: **{BEFORE}** sections (through Cap. IX vadem tip)
- After: **{AFTER}** sections (through Cap. IX missa/poena → deny original tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 354–355 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: missa; poena originale; loco nostro punitus; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_641–646; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5646 or 12m (`sources/_ocr_cap9_missa_poena/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_641-646", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — imputation vs inhaerentia / habitual (after missa tip)",
    }
