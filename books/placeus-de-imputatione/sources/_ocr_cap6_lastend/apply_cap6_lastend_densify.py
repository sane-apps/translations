# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — last-end / pleasure-vs-generation deepen tip; before original-stain syllogism."""
...
OCR = BOOK / "sources/_ocr_cap6_lastend"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_lastend_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4425
PACKET_STEM = "cap6_lastend_densify"
LOCK_NAME = "_placeus_cap6_lastend_latin_lock.txt"
JX_PREFIX = "cap6_lastend"
...
Scope: Manuscriptum posterius Cap. VI last-end / pleasure-vs-generation deepen (printed pp. 250 tip–252 tip). Stops before original-stain syllogism / absolute-right vs just-judgment deepen. Not Caput VII+.
...
Remaining: Cap. VI original-stain syllogism+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI lastend densify tip-335 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI lastend densify section {sid} "
                "from locked 1661 Latin (pp. 250 tip–252 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VI (Examen Cap. IV / causa condemnationis; last-end / pleasure-vs-generation tip) "
        "/ before original-stain syllogism"
    )
    remain = "Cap. VI original-stain syllogism+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI last-end / pleasure-vs-generation tip (before original-stain syllogism). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_genfac_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_lastend",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI last-end / pleasure-vs-generation tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "last-end / pleasure-vs-generation tip) before original-stain syllogism, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI original-stain syllogism+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI lastend densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "last-end / pleasure-vs-generation tip) before original-stain syllogism from locked 1661 Saumur Latin "
                "(pp. 250 tip–252 tip). Honest partial — Cap. VI original-stain syllogism+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. VI lastend densify (pleasure-vs-generation → before original-stain syllogism)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VI generative-faculty quarrel tip / before last-end deepen)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI last-end / pleasure-vs-generation tip / before original-stain syllogism)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 250 tip–252 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: last end / pleasure / generation / imputation; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_336–341; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap6_lastend/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_330-335", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VI original-stain syllogism / absolute-right vs just-judgment",
    }
