# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — imputation vs inhaerentia / habitual."""
...
OCR = BOOK / "sources/_ocr_cap9_inhaerentia"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_inhaerentia_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5646
PACKET_STEM = "cap9_inhaerentia_densify"
LOCK_NAME = "_placeus_cap9_inhaerentia_latin_lock.txt"
JX_PREFIX = "cap9_inhaerentia"
...
Scope: Manuscriptum posterius Cap. IX — imputation vs inhaerentia / habitual printed pp. 355–356 tip. Stops before equity/Martyr block. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX equity/Martyr+ / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX PLAC vere densify tip-646→652; need live>5646 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX inhaerentia densify section {sid} "
                "from locked 1661 Latin (pp. 355–356 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (imputation/inhaerentia tip) "
        "/ before equity/Martyr"
    )
    remain = "Cap. IX equity/Martyr+ / rest Cap. IX / ~494 pp Disputatio remain."
    meta.update({
    ...
    block = f"""## 2026-09-23 Cap. IX imputation vs inhaerentia / habitual densify

- Before: **{BEFORE}** sections (through Cap. IX missa/poena tip)
- After: **{AFTER}** sections (through Cap. IX habitual-imputation / not-in-us tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 355–356 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: inhaerentia; peccatum habituale; imputatur≠inest; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_647–652; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5646 or 12m (`sources/_ocr_cap9_inhaerentia/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_647-652", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — equity / Martyr communion (after inhaerentia tip)",
    }
