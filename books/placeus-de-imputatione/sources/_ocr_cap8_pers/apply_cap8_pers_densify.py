# -*- coding: utf-8 -*-
"""Man. Post. Cap. VIII densify — personal sentence words / quia paruisti; tip before Gospel promise."""
...
OCR = BOOK / "sources/_ocr_cap8_pers"
...
HOLD_START_PATH = Path("/tmp/placeus_cap8_pers_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4887
PACKET_STEM = "cap8_pers_densify"
LOCK_NAME = "_placeus_cap8_pers_latin_lock.txt"
JX_PREFIX = "cap8_pers"
...
Scope: Manuscriptum posterius Cap. VIII — personal sentence words / quia paruisti / Bellarmine (printed pp. 304 tip–306 tip). Stops before Gospel-promise / seed-of-woman argument. Not whole Cap. VIII / Disputatio.
...
Remaining: Cap. VIII Gospel promise+ / rest of Cap. VIII / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VIII pers densify tip-448 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VIII pers densify section {sid} "
                "from locked 1661 Latin (pp. 304 tip–306 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VIII (personal words / quia paruisti / Bellarmine) "
        "/ before Gospel promise"
    )
    remain = "Cap. VIII Gospel promise+ / rest of Cap. VIII / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VIII personal words / Bellarmine (before Gospel promise). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap8_sent_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap8_pers",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VIII personal words / quia paruisti (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VIII personal words / Bellarmine) before Gospel promise, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VIII Gospel promise deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. VIII pers densify from locked 1661 Latin "
            "(pp. 304 tip–306 tip); Pass A != B."
            if int(sid) >= 449
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VIII pers densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VIII personal words / Bellarmine) before Gospel promise from locked 1661 Saumur Latin "
                "(pp. 304 tip–306 tip). Honest partial — Cap. VIII Gospel promise+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VIII pers densify (personal words / quia paruisti → before Gospel promise)

- Before: **{BEFORE}** sections (through Cap. VIII sentence / mediate tip)
- After: **{AFTER}** sections (through Cap. VIII personal words / Bellarmine tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 304 tip–306 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: quia paruisti / personale / fermentum / Bellarminus; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_449–454; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap8_pers/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_449-454", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VIII — Gospel promise / seed of the woman",
    }
