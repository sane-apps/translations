# -*- coding: utf-8 -*-
"""Cap. XI open densify: Pelagian/Socinian/Remonstrant alienation through Cap. XII gate."""
...
OCR = BOOK / "sources/_ocr_cap11_open"
...
BIBLE = payload["bible"]
SIDS = ["131", "132", "133", "134", "135", "136", "137"]
BEFORE = 130
AFTER = 137
...
Scope: Caput XI A Pelagianorum, Socinianorum, & Remonstrantium… Placeum esse alienum (printed pp. 130–137 tip). Stops before Caput XII. Not further Cap. XII+.
Reconstruction: IA PDF page images via macOS Vision OCR + pdftotext; long-s/ligature OCR corrected. Locked Greek ἀτοπίαν (Hunnius; OCR soft). Do not revive OCR ἔργον.
No modern English used as copy-text.
Remaining: Cap. XII+ of the ~494-page Disputatio.
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. XI open densify section {sid} "
                "from locked 1661 Latin (pp. 130–137 tip)."
            ),
        }
        jp = BOOK / f"reviews/justifications/cap11_{sid}.json"
        jp.write_text(json.dumps(j, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        errs = check_pass_ab.check_file(jp)
        status = "ok" if not errs else "FAIL"
        print(f"check_pass_ab cap11_{sid}: {status} dist={j['a_neq_b_distance']}")
        if errs:
            for e in errs:
                print(" ", e)
...
def main() -> None:
    ...
    lock_path = BOOK / "sources/_placeus_cap11_open_latin_lock.txt"
    ...
    stop = (
        "Cap. XI Pelagian/Socinian/Remonstrant alienation / Origen–Jerome–Hunnius / "
        "Placeus vs Pelagius / Augustine–Martyr–Walaeus close / before Caput XII"
    )
    remain = "Cap. XII+ remain."
    meta.update({
        ...
        "edition": (
            ...
            "IA deimputationepri00lapl. Densify: Capita I–X + Caput XI (Pelagian/Socinian/"
            "Remonstrant alienation) before Caput XII. "
    ...
    if not any(str(w.get("path", "")).endswith("_placeus_cap11_open_latin_lock.txt") for w in wits):
        wits.append({
            "id": "latin_lock_cap11_open",
            "path": "sources/_placeus_cap11_open_latin_lock.txt",
            "role": "copy-text",
            "name": (
                "Locked Latin, Caput XI Pelagian/Socinian/Remonstrant alienation before Caput XII "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            "English follows the locked 1661 Saumur Latin of Capita I–XI partial through Cap. XI "
            "Pelagian/Socinian/Remonstrant alienation (Origen–Jerome–Hunnius; Placeus vs Pelagius; "
            "Augustine–Martyr–Walaeus) before Caput XII, reconstructed from IA PDF page "
    ...
    old_p = json.loads((BOOK / "reviews/audit/cap10_ultima_densify.packet.json").read_text(encoding="utf-8"))
    ...
    pkt_path = BOOK / "reviews/audit/cap11_open_densify.packet.json"
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. XI open densify from locked 1661 Latin "
            "(pp. 130–137 tip); Pass A != B."
            if int(sid) >= 131
            else (
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. XI open densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XI through Cap. XI Pelagian/Socinian/Remonstrant alienation "
                "(Origen–Jerome–Hunnius; Placeus vs Pelagius; Augustine–Martyr–Walaeus) before Caput XII "
                "from locked 1661 Saumur Latin (pp. 130–137 tip). Honest partial — Cap. XII+ remain. "
                "Reformed Saumur era disclosed. Locked Greek ἀτοπίαν (Hunnius); do not revive OCR ἔργον. "
                "Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    rev_path = BOOK / "reviews/audit/cap11_open_densify.review.json"
    ...
    block = """## 2026-09-22 Cap. XI open densify (Pelagian/Socinian/Remonstrant → Walaeus / before Cap. XII)
...
"""
    ...
    print(f"DONE before={BEFORE} after={AFTER} packet=cap11_open_densify PunchX=NO")
