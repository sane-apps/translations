# -*- coding: utf-8 -*-
"""Cap. VII Praeter aberrationem densify: vices of Garissoles Resp. 1–2 through Christ–Adam meritorious close."""
...
Scope: Caput VII Praeter aberrationem — vices in Garissoles's three responses through Resp. 1 (tautology + tertium ἐφ᾽ ᾧ construction), Secunda responsio (1 Cor. 15:22 / hereditary neglected), remote-cause blood-union reply, and Christ–Adam meritorious proximity close (printed pp. 77–82 tip). Stops before Sequitur ratio quarta / further Cap. VII. Not Cap. VIII+.
Reconstruction: IA PDF page images via macOS Vision OCR + tesseract + pdftotext; long-s/ligature OCR corrected; Greek ἐφ᾽ ᾧ πάντες ἥμαρτον / ἀνθρώπου / ἐγγύς confirmed against page image (ἐγγύς page-OCR soft).
No modern English used as copy-text.
Remaining: Cap. VII Quarta ratio onward and Cap. VIII+ of the ~494-page Disputatio.
...
def main() -> None:
    ...
    lock_path = BOOK / "sources/_placeus_cap7_praeter_latin_lock.txt"
    ...
    assert len(eng) == 48 and len(src) == 48, (len(eng), len(src))
    pre_eng = digest(eng)
    pre_src = digest(src)
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": f"Cap. VII Praeter densify section {sid} from locked 1661 Latin (pp. 77–82 tip).",
        }
    ...
    assert digest(eng[:48]) == pre_eng, "sections 1-48 english mutated"
    assert digest(src[:48]) == pre_src, "sections 1-48 source mutated"

    stop = "Cap. VII Praeter aberrationem / Resp. 1–2 vices / Christ–Adam meritorious close"
    meta.update({
        ...
        "edition": (
            ...
            "Densify: Capita I–VI + Caput VII through Praeter aberrationem vices of Garissoles Resp. 1 (tautology + tertium ἐφ᾽ ᾧ), "
            "Secunda responsio (1 Cor. 15:22 / hereditary), remote blood-union reply, and Christ–Adam meritorious proximity close. "
            "Not whole Disputatio; Cap. VII Quarta ratio onward and Cap. VIII+ remain."
        ),
        "blurb": (
            "Josue de la Place (Placeus) — Capita I–VII partial of De imputatione primi peccati Adami from the 1661 Saumur Latin, "
            f"through {stop}. Honest partial; Cap. VII Quarta ratio onward and later Capita remain."
        ),
        "first_english_note": (
    ...
    if not any(str(w.get("path", "")).endswith("_placeus_cap7_praeter_latin_lock.txt") for w in wits):
        wits.append({
            "id": "latin_lock_cap7_praeter",
            "path": "sources/_placeus_cap7_praeter_latin_lock.txt",
            "role": "copy-text",
            "name": "Locked Latin, Caput VII Praeter aberrationem through Christ–Adam close (Saumur 1661)",
            "language": "Latin",
            "url": "https://archive.org/details/deimputationepri00lapl",
        })
    meta["text_history"] = {
        "method": (
            "English follows the locked 1661 Saumur Latin of Capita I–VII partial through Cap. VII Praeter aberrationem "
            "(vices of Garissoles responses through Christ–Adam meritorious close), reconstructed from IA PDF page images with Vision OCR, tesseract, "
            "and pdftotext as check. No modern English was copied. Cap. VII Quarta ratio onward and Cap. VIII+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads((BOOK / "reviews/audit/cap7_alias_rationes_densify.packet.json").read_text(encoding="utf-8"))
    packet = copy.deepcopy(old_p)
    packet.pop("packet_id", None)
    packet["seed"] = 20260922
    packet["sample_size"] = 53
    packet["expected_sections"] = [str(i) for i in range(1, 54)]
    packet["explicit_selected_sections"] = packet["expected_sections"][:]
    packet["coverage"] = {"expected": 53, "english": 53, "source": 53}
    packet["identity"]["work"] = meta["title"]
    packet["identity"]["edition"] = meta["edition"]
    packet["identity"]["locus_aliases"] = {str(i): str(i) for i in range(1, 54)}
    packet["publication_scope"]["title"] = meta["title"]
    packet["publication_scope"]["section_count"] = 53
    ...
    for sid in packet["expected_sections"]:
        ...
        reviews.append({
            ...
            "notes": f"Section {sid}: compared Pass B to locked 1661 Latin Cap. VII Praeter aberrationem / vices through Christ–Adam close; Pass A != B.",
        })

    review = {
        "packet_id": packet["packet_id"],
        "reviewer": "scribe-placeus / Placeus Cap. VII Praeter aberrationem densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [
                "Section 53: Greek ἐγγύς in meritorious-causes quote is page-OCR soft (Vision crop evepze); sense locked by proximitas context."
            ],
            "notes": (
                "Cap. I–VI + Cap. VII through Praeter aberrationem vices of Garissoles Resp. 1–2 and Christ–Adam meritorious close "
                "from locked 1661 Saumur Latin (pp. 77–82 tip). Honest partial — Cap. VII Quarta ratio onward and Cap. VIII+ remain. "
                "Reformed Saumur era disclosed. Locked Greek ἐφ᾽ ᾧ / πάντες ἥμαρτον / ἀνθρώπου / ἐγγύς (do not revive OCR ἔργον); "
                "section 48 ἀποσχεδιάζει not reopened."
            ),
        },
        "reviews": reviews,
    }
    ...
    if "cap7_praeter_vices_densify" not in old[:1500]:
...
- Lemma note: locked Greek **ἐφ᾽ ᾧ πάντες ἥμαρτον** / **ἀνθρώπου** / **ἐγγύς** (ἐγγύς page-OCR soft); do not revive OCR ἔργον; section 48 ἀποσχεδιάζει not reopened
...
"""
        # keep rest after first heading
        if old.startswith("# Placeus"):
            rest = old.split("\n", 1)[1] if "\n" in old else ""
            # drop duplicate top heading from rest if present
        ...
        print("SESSION_HANDOFF already has praeter entry")
