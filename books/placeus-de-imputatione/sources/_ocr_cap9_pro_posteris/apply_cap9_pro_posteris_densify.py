# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — peccavit pro posteris / vadem."""
...
OCR = BOOK / "sources/_ocr_cap9_pro_posteris"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_pro_posteris_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5604
PACKET_STEM = "cap9_pro_posteris_densify"
LOCK_NAME = "_placeus_cap9_pro_posteris_latin_lock.txt"
JX_PREFIX = "cap9_pro_posteris"
...
Scope: Manuscriptum posterius Cap. IX — peccavit pro omnibus posteris / vadem printed pp. 353–354 tip. Stops before missa-persona / original-poena counter. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX missa/poena originale counter+ / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX PLAC vere densify tip-634→640; need live>5604 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX pro-posteris densify section {sid} "
                "from locked 1661 Latin (pp. 353–354 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (peccavit-pro-posteris/vadem tip) "
        "/ before missa/poena counter"
    )
    remain = "Cap. IX missa/poena counter+ / rest Cap. IX / ~494 pp Disputatio remain."
    meta.update({
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_aptitudo_densify.packet.json").read_text(encoding="utf-8"))
    ...
    block = f"""## 2026-09-23 Cap. IX peccavit pro omnibus posteris / vadem densify

- Before: **{BEFORE}** sections (through Cap. IX Zanchius actu tip)
- After: **{AFTER}** sections (through Cap. IX vadem / only-Adam-Eve tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 353–354 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: loco nostro; vadem; peccare loco alterius; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_635–640; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5604 or 12m (`sources/_ocr_cap9_pro_posteris/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_635-640", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — missa/poena originale counter (after vadem tip)",
    }
