# -*- coding: utf-8 -*-
"""Man. Post. Cap. VIII densify — death-reply / Ps 32; tip before Abraham justification."""
...
OCR = BOOK / "sources/_ocr_cap8_ps32"
...
HOLD_START_PATH = Path("/tmp/placeus_cap8_ps32_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4937
PACKET_STEM = "cap8_ps32_densify"
LOCK_NAME = "_placeus_cap8_ps32_latin_lock.txt"
JX_PREFIX = "cap8_ps32"
...
Scope: Manuscriptum posterius Cap. VIII — death-reply / Ps 32 imputation argument (printed pp. 308 tip–310 tip). Stops before Abraham justification proof. Not whole Cap. VIII / Disputatio.
...
Remaining: Cap. VIII Abraham justification+ / rest of Cap. VIII / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VIII ps32 densify tip-460 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VIII ps32 densify section {sid} "
                "from locked 1661 Latin (pp. 308 tip–310 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VIII (death-reply / Ps 32) "
        "/ before Abraham justification"
    )
    remain = "Cap. VIII Abraham justification+ / rest of Cap. VIII / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VIII death-reply / Ps 32 (before Abraham). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap8_gospel_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap8_ps32",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VIII death-reply / Ps 32 (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VIII death-reply / Ps 32) before Abraham, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VIII Abraham deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. VIII ps32 densify from locked 1661 Latin "
            "(pp. 308 tip–310 tip); Pass A != B."
            if int(sid) >= 461
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VIII ps32 densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VIII death-reply / Ps 32) before Abraham from locked 1661 Saumur Latin "
                "(pp. 308 tip–310 tip). Honest partial — Cap. VIII Abraham+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VIII ps32 densify (death-reply / Ps 32 → before Abraham)

- Before: **{BEFORE}** sections (through Cap. VIII Gospel promise / serpent tip)
- After: **{AFTER}** sections (through Cap. VIII Ps 32 reply tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 308 tip–310 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: opus Diaboli / Ps 32 / ἀνομία / ex gratia ex justitia; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_461–466; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap8_ps32/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_461-466", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VIII — Abraham justification / faith imputed",
    }
