# -*- coding: utf-8 -*-
"""Cap. IV Tertia GAR densify — three rationes (velle/nolle; separability; actus vs habitus); before Noli/Quarta."""
...
OCR = BOOK / "sources/_ocr_cap4_tertia_gar"
...
HOLD_START_PATH = Path("/tmp/placeus_cap4_tertia_gar_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 3884  # tip-231 catch-up ship; need live>3884 or 12m
PACKET_STEM = "cap4_tertia_gar_densify"
LOCK_NAME = "_placeus_cap4_tertia_gar_latin_lock.txt"
JX_PREFIX = "cap4_tertia_gar"
...
Scope: Manuscriptum posterius Cap. IV Tertia GAR three rationes against formal identity (velle/nolle; separability; actus vs habitus) through Placeus dilemma / passio≠privatio / expulsio (printed pp. 198 tip–202 tip). Stops before Noli mihi objicere / Quarta ratio. Not Cap. IV remainder+.
Reconstruction: IA PDF page images via macOS Vision OCR + pdftotext; long-s/ligature OCR corrected. Locked Greek ἀποστασία. Do not revive ἔργον.
No modern English used as copy-text.
Remaining: Cap. IV Noli/Quarta/Quinta+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "tip-231 catch-up ship in flight; re-probe before append; need live>3884 or 12m",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IV Tertia GAR densify section {sid} "
                "from locked 1661 Latin (pp. 195 tip–198 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Cap. IV Tertia GAR three rationes (velle/nolle; separability; actus vs habitus) "
        "/ PLAC dilemma–expulsio / before Noli mihi / Quarta"
    )
    remain = (
        "Cap. IV Noli/Quarta/Quinta+ / rest of MS examination / ~494 pp Disputatio remain."
    )
    meta.update({
        ...
        "edition": (
            ...
            "through Cap. IV Tertia GAR three rationes (before Noli/Quarta). "
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads(
                (BOOK / "reviews/audit/cap4_secunda_ratio_densify.packet.json").read_text(
                    encoding="utf-8"
                )
            )
            wits = list(
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap4_tertia_gar",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": (
                "Locked Latin, Man. Post. Cap. IV Tertia GAR three rationes "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Cap. IV Tertia GAR three rationes (velle/nolle; separability; "
            "actus vs habitus) and Placeus dilemma / passio≠privatio / expulsio before "
            "Noli/Quarta, reconstructed from IA PDF page images with Vision OCR and "
            "pdftotext as check. Locked Greek ἀποστασία. No modern English was copied. "
            "Cap. IV Quarta+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads(
        (BOOK / "reviews/audit/cap4_secunda_ratio_densify.packet.json").read_text(encoding="utf-8")
    )
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IV Tertia GAR three rationes densify from locked 1661 Latin "
            "(pp. 198 tip–202 tip); Pass A != B."
            if int(sid) >= 232
            else (
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IV Tertia GAR three rationes densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Cap. IV Tertia GAR three rationes "
                "(velle/nolle; separability; actus vs habitus) and Placeus dilemma / "
                "passio≠privatio / expulsio before Noli/Quarta from locked 1661 Saumur Latin "
                "(pp. 198 tip–202 tip). Honest partial — Cap. IV Quarta+ remain. "
                "Locked Greek ἀποστασία. Do not revive ἔργον. "
                "Punch X: NO."
            ),
        },
        "reviews": reviews,
    ...
    block = f"""## 2026-09-22 Cap. IV Tertia GAR densify (velle/nolle; separability; actus vs habitus → before Noli/Quarta)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. IV Secunda / Tertia GAR–PLAC actualis/habitualis / before three rationes)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. IV Tertia GAR three rationes / PLAC dilemma–expulsio / before Noli mihi / Quarta)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 198 tip–202 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: locked Greek **ἀποστασία**; do not revive ἔργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_232–238; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: tip-231 catch-up ship in flight; eng/src append after live>3884 or 12m (`sources/_ocr_cap4_tertia_gar/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_232-238",
        "tip_ready": "ok",
        "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IV Noli mihi objicere / Quarta ratio (poena after judicium)",
        "prior_packet_done": "cap4_secunda_ratio_densify (224→231)",
    }
