# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — PLAC evangelical-justice + Heb.7 / 1 Cor.15."""
...
OCR = BOOK / "sources/_ocr_cap9_plac_evang"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_plac_evang_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5498
PACKET_STEM = "cap9_plac_evang_densify"
LOCK_NAME = "_placeus_cap9_plac_evang_latin_lock.txt"
JX_PREFIX = "cap9_plac_evang"
...
Scope: Manuscriptum posterius Cap. IX — PLAC evangelical-justice reply + Heb. 7 / 1 Cor. 15 objections printed pp. 345–347 tip. Stops before GAR Resp. 1 (Confession Art. 10). Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX GAR Resp.1+ / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX PLAC vere densify tip-598→604; need live>5498 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX PLAC evang densify section {sid} "
                "from locked 1661 Latin (pp. 345–347 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (PLAC Rom5/communion tip) "
        "/ before GAR Resp. 1"
    )
    remain = "Cap. IX GAR Resp.1+ / rest Cap. IX / ~494 pp Disputatio remain."
    meta.update({
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_plac_vere_densify.packet.json").read_text(encoding="utf-8"))
    ...
    block = f"""## 2026-09-23 Cap. IX PLAC evangelical-justice + Heb.7 / 1 Cor.15 densify

- Before: **{BEFORE}** sections (through Cap. IX PLAC vere/proprie equity tip)
- After: **{AFTER}** sections (through Cap. IX PLAC Rom5/communion tip before GAR Resp.1)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 345–347 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_599–604; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5498 or 12m (`sources/_ocr_cap9_plac_evang/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_599-604", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — GAR Resp. 1 (Confession Art. 10) after PLAC communion tip",
    }
