# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — GAR Resp.3; tip before contradiction."""
...
OCR = BOOK / "sources/_ocr_cap9_gar3b"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_gar3b_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5314
PACKET_STEM = "cap9_gar3b_densify"
LOCK_NAME = "_placeus_cap9_gar3b_latin_lock.txt"
JX_PREFIX = "cap9_gar3b"
...
Scope: Manuscriptum posterius Cap. IX — GAR Resp.3 (Deus nemini peccatum imputat quin…) + PLAC start on three replies / Saul printed pp. 337–338 tip. Stops before PLAC contradiction on in alio peccare. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX PLAC contradiction on in alio+ / rest of Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX ephho densify tip-538→544; need live>5314 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX ephho densify section {sid} "
                "from locked 1661 Latin (pp. 337–338 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (eph-ho / eph-ho pantes) "
        "/ before GAR Resp.1 on peccare vs imputari"
    )
    remain = "Cap. IX GAR Resp.1+ / rest Cap. IX / rest of Cap. IX / ~494 pp Disputatio remain."
    meta.update({
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_gar2_densify.packet.json").read_text(encoding="utf-8"))
    ...
    old_p = json.loads((BOOK / "reviews/audit/cap9_proofs_densify.packet.json").read_text(encoding="utf-8"))
    ...
    review = {
        ...
        "scope_review": {
            ...
            "notes": (
                ...
                "(pp. 337–338 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX ephho densify (omit/death dilemma → before all-sinners)

- Before: **{BEFORE}** sections (through Cap. IX GAR 2 tip)
- After: **{AFTER}** sections (through Cap. IX GAR Resp.3 tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 337–338 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_557–562; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5314 or 12m (`sources/_ocr_cap9_gar3b/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_557-562", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — PLAC contradiction (in alio peccare vs imputation order)",
    }
