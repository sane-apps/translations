# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — Cap. VII sons/grandsons imputation / Deut 24 tip; before magistrate judgments / power-vs-justice."""
...
OCR = BOOK / "sources/_ocr_cap7_sons"
...
HOLD_START_PATH = Path("/tmp/placeus_cap7_sons_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4604
PACKET_STEM = "cap7_sons_densify"
LOCK_NAME = "_placeus_cap7_sons_latin_lock.txt"
JX_PREFIX = "cap7_sons"
...
Scope: Manuscriptum posterius Cap. VII first similitude — sons/grandsons imputation / Deut 24 (printed pp. 266 tip–268 tip). Stops before magistrate judgments / power-vs-justice deepen. Not whole Cap. VII / Disputatio.
...
Remaining: Cap. VII magistrate judgments+ / second–third similitudes / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VII sons densify tip-383 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VII sons densify section {sid} "
                "from locked 1661 Latin (pp. 266 tip–268 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VII (Examen trium similitudinum; sons/Deut 24 tip) "
        "/ before magistrate judgments / power-vs-justice"
    )
    remain = "Cap. VII magistrate judgments / power-vs-justice+ / second–third similitudes / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VII sons/Deut 24 tip (before magistrate judgments deepen). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap7_poena_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap7_sons",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VII sons/Deut 24 tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VII sons/Deut 24 tip) before magistrate judgments, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VII magistrate judgments deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VII sons densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VII sons/Deut 24 tip) before magistrate judgments from locked 1661 Saumur Latin "
                "(pp. 266 tip–268 tip). Honest partial — Cap. VII magistrate judgments deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VII sons densify (sons/grandsons / Deut 24 → before magistrate judgments)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VII penalties/meritorie tip / before sons-grandsons)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VII sons/Deut 24 tip / before magistrate judgments)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 266 tip–268 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: nepotes / Deut 24:16 / Ezek 18 / Amaziah; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_384–389; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap7_sons/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_384-389", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VII — magistrate judgments / God power vs justice",
    }
