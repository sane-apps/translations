# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — poena / permission / natural-propagation deepen tip; before judgment-locution quarrel."""
...
OCR = BOOK / "sources/_ocr_cap6_poena"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_poena_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4470
PACKET_STEM = "cap6_poena_densify"
LOCK_NAME = "_placeus_cap6_poena_latin_lock.txt"
JX_PREFIX = "cap6_poena"
...
Scope: Manuscriptum posterius Cap. VI poena / permission / natural-propagation deepen (printed pp. 254 tip–257 tip). Stops before judgment-locution quarrel. Not Caput VII+.
...
Remaining: Cap. VI judgment-locution quarrel+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI poena densify tip-347 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI poena densify section {sid} "
                "from locked 1661 Latin (pp. 254 tip–257 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VI (Examen Cap. IV / causa condemnationis; poena/permission tip) "
        "/ before judgment-locution quarrel"
    )
    remain = "Cap. VI judgment-locution quarrel+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI poena/permission/natural-propagation tip (before judgment-locution quarrel). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_stainsyl_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_poena",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI poena/permission/natural-propagation tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "poena/permission/natural-propagation tip) before judgment-locution quarrel, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI judgment-locution quarrel+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI poena densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "poena/permission tip) before judgment-locution quarrel from locked 1661 Saumur Latin "
                "(pp. 254 tip–257 tip). Honest partial — Cap. VI judgment-locution quarrel+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VI poena densify (permission / natural-propagation → before judgment-locution)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VI original-stain syllogism tip / before poena-permission deepen)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI poena/permission/natural-propagation tip / before judgment-locution quarrel)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 254 tip–257 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: poena / permissio / naturae vi & lege / justum judicium; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_348–353; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap6_poena/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_348-353", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VI judgment-locution / antecedent vs consequent judgment",
    }
