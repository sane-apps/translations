# -*- coding: utf-8 -*-
"""Man. Post. Cap. VIII densify — sentence / common-penalty syllogism; tip before personal words."""
...
OCR = BOOK / "sources/_ocr_cap8_sent"
...
HOLD_START_PATH = Path("/tmp/placeus_cap8_sent_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4860
PACKET_STEM = "cap8_sent_densify"
LOCK_NAME = "_placeus_cap8_sent_latin_lock.txt"
JX_PREFIX = "cap8_sent"
...
Scope: Manuscriptum posterius Cap. VIII — sentence / common-penalty syllogism / mediate imputation (printed pp. 302 tip–304 tip). Stops before personal sentence-words deepen. Not whole Cap. VIII / Disputatio.
...
Remaining: Cap. VIII personal sentence words+ / rest of Cap. VIII / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VIII sent densify tip-442 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VIII sent densify section {sid} "
                "from locked 1661 Latin (pp. 302 tip–304 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VIII (sentence / common-penalty / mediate) "
        "/ before personal sentence words"
    )
    remain = "Cap. VIII personal sentence words+ / rest of Cap. VIII / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VIII sentence / common-penalty (before personal words). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap8_hypo_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap8_sent",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VIII sentence / common-penalty (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VIII sentence / common-penalty) before personal words, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VIII personal sentence words deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. VIII sent densify from locked 1661 Latin "
            "(pp. 302 tip–304 tip); Pass A != B."
            if int(sid) >= 443
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VIII sent densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VIII sentence / common-penalty) before personal words from locked 1661 Saumur Latin "
                "(pp. 302 tip–304 tip). Honest partial — Cap. VIII personal sentence words+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VIII sent densify (sentence / common-penalty → before personal words)

- Before: **{BEFORE}** sections (through Cap. VIII Hypognosticon / event-reply tip)
- After: **{AFTER}** sections (through Cap. VIII sentence / mediate tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 302 tip–304 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_443–448; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap8_sent/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_443-448", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VIII — personal sentence words / quia paruisti",
    }
