# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — PLAC vs GAR 3 begin-to-be/Acts/Heb/Isa; tip before imputatum reductio."""
...
OCR = BOOK / "sources/_ocr_cap9_gar3"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_gar3_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5153
PACKET_STEM = "cap9_gar3_densify"
LOCK_NAME = "_placeus_cap9_gar3_latin_lock.txt"
JX_PREFIX = "cap9_gar3"
...
Scope: Manuscriptum posterius Cap. IX — PLAC vs GAR 3 (begin-to-be; Acts 1:21; Heb 1:6; Isa 53:6 / Pighius) printed pp. 325–326 tip. Stops before imputatum-est substitution reductio. Not whole Cap. IX / Disputatio.
...
Remaining: Cap. IX imputatum-est reductio+ / rest of Cap. IX / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX gar3 densify tip-508; need live>5153 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX gar3 densify section {sid} "
                "from locked 1661 Latin (pp. 325–326 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (PLAC vs GAR 3 / Acts–Heb–Isa) "
        "/ before imputatum-est reductio"
    )
    remain = "Cap. IX imputatum-est reductio+ / rest of Cap. IX / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. IX PLAC vs GAR 3 tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_innate_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap9_gar3",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. IX PLAC vs GAR 3 (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Caput IX (PLAC vs GAR 3 begin-to-be/Isa) before "
            "imputatum-est reductio, reconstructed from IA PDF page images with Vision OCR and "
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX gar3 densify from locked 1661 Latin "
            "(pp. 325–326 tip); Pass A != B."
            if int(sid) >= 509
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX gar3 densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput IX (PLAC vs GAR 3) "
                "before imputatum-est reductio from locked 1661 Saumur Latin "
                "(pp. 325–326 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX gar3 densify (PLAC vs GAR 3 → before imputatum reductio)

- Before: **{BEFORE}** sections (through Cap. IX GAR Cap. VIII Resp. 1 tip)
- After: **{AFTER}** sections (through Cap. IX PLAC vs GAR 3 tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 325–326 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: Acts 1:21; Heb 1:6; Isa 53:6; Pighius; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_509–514; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5153 or 12m (`sources/_ocr_cap9_gar3/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_509-514", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — imputatum-est substitution reductio (per unum hominem)",
    }
