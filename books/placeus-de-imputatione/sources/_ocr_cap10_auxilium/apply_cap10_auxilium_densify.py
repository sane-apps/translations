# -*- coding: utf-8 -*-
"""Cap. X Theologorum auxilium densify: Whitaker/Chamier/Sixtus/Bellarmine through retorsion."""
...
OCR = BOOK / "sources/_ocr_cap10_auxilium"
...
BIBLE = payload["bible"]
SIDS = ["118", "119", "120", "121", "122", "123", "124"]
...
Scope: Caput X Hac ratione… Theologorum auxilium — Whitaker / Chamier / Sixtus Senensis / Bellarmine rationes 1–4 + Placeus reply/retorsion (printed pp. 124–127 tip). Stops before Superest ultima… imputatione Iustitiae Christi. Not further Cap. X+.
...
Remaining: Cap. X ultima (Christi justitia)+ / Cap. XI+ of the ~494-page Disputatio.
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. X Theologorum auxilium densify section {sid} "
                "from locked 1661 Latin (pp. 124–127 tip)."
            ),
        }
...
def main() -> None:
    ...
    lock_path = BOOK / "sources/_placeus_cap10_auxilium_latin_lock.txt"
    ...
    assert len(eng) == 117 and len(src) == 117, (len(eng), len(src))
    assert str(eng[-1]["section"]) == "117", eng[-1]["section"]
    pre_eng = digest(eng)
    pre_src = digest(src)
    ...
    assert digest(eng[:117]) == pre_eng, "sections 1-117 english mutated"
    assert digest(src[:117]) == pre_src, "sections 1-117 source mutated"

    stop = (
        "Cap. X Theologorum auxilium / Whitaker–Chamier–Sixtus–Bellarmine / "
        "rationes 1–4 / Placeus reply–retorsion / before ultima Christi justitia"
    )
    remain = "Cap. X ultima (Christi justitia)+ / Cap. XI+ remain."
    meta.update({
        ...
        "edition": (
            ...
            "IA deimputationepri00lapl. Densify: Capita I–IX + Caput X through Theologorum "
            "auxilium (Whitaker/Chamier/Sixtus/Bellarmine) before ultima Christi justitia. "
    ...
    if not any(str(w.get("path", "")).endswith("_placeus_cap10_auxilium_latin_lock.txt") for w in wits):
        wits.append({
            "id": "latin_lock_cap10_auxilium",
            "path": "sources/_placeus_cap10_auxilium_latin_lock.txt",
            "role": "copy-text",
            "name": (
                "Locked Latin, Caput X Theologorum auxilium through Bellarmine retorsion "
                "before Christi justitia (Saumur 1661)"
            ),
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Theologorum auxilium Whitaker/Chamier/Sixtus/Bellarmine rationes 1–4 + Placeus "
            "reply/retorsion before ultima Christi justitia, reconstructed from IA PDF page "
            "images with Vision OCR and pdftotext as check. No modern English was copied. "
            "Cap. X ultima+ / Cap. XI+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads((BOOK / "reviews/audit/cap10_alteram_densify.packet.json").read_text(encoding="utf-8"))
    ...
    pkt_path = BOOK / "reviews/audit/cap10_auxilium_densify.packet.json"
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. X Theologorum auxilium densify from locked 1661 Latin "
            "(pp. 124–127 tip); Pass A != B."
            if int(sid) >= 118
            else (
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. X Theologorum auxilium densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–X through Cap. X Theologorum auxilium Whitaker/Chamier/Sixtus/"
                "Bellarmine rationes 1–4 + Placeus reply/retorsion before ultima Christi "
                "justitia from locked 1661 Saumur Latin (pp. 124–127 tip). Honest partial — "
                "Cap. X ultima+ / Cap. XI+ remain. Reformed Saumur era disclosed. No Greek "
                "lemma this slice; do not revive OCR ἔργον. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    rev_path = BOOK / "reviews/audit/cap10_auxilium_densify.review.json"
    ...
    block = """## 2026-09-22 Cap. X Theologorum auxilium densify (Whitaker / Chamier / Sixtus / Bellarmine → retorsion / before ultima)
...
"""
    ...
    print("DONE before=117 after=124 packet=cap10_auxilium_densify PunchX=NO")
