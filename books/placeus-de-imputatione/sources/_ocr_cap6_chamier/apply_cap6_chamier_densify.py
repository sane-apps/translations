# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — Chamier actiones sunt suppositorum; before Placeus erotemata replies."""
...
OCR = BOOK / "sources/_ocr_cap6_chamier"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_chamier_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4168
PACKET_STEM = "cap6_chamier_densify"
LOCK_NAME = "_placeus_cap6_chamier_latin_lock.txt"
JX_PREFIX = "cap6_chamier"
...
Scope: Manuscriptum posterius Cap. VI Chamier (actiones sunt suppositorum; judicial justice; other-sins / Rom 5; head fasciculus mediate-only; GAR erotemata; Pelagian-parallel / deny immediate-catholic tip) from after Isaiah 55 / vitium tip through before Placeus detailed replies (printed pp. 227 tip–230 tip). Stops before Placeus numbered answers to corruption parallel. Not Caput VII+.
...
Remaining: Cap. VI Placeus erotemata replies+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI Chamier tip-287 catch-up; need live>4168 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI Chamier densify section {sid} "
                "from locked 1661 Latin (pp. 227 tip–230 tip)."
            ),
        }
...
def main() -> None:
    ...
    remain = "Cap. VI Placeus erotemata replies+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI Chamier tip (before Placeus erotemata replies). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_isaiah55_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_chamier",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI Chamier tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI Placeus replies+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI Chamier densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "(pp. 227 tip–230 tip). Honest partial — Cap. VI Placeus replies+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. VI Chamier densify (suppositorum / Rom 5 / head fasciculus → before Placeus replies)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VI Isaiah 55 / vitium tip / before Chamier)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI Chamier / actiones suppositorum / head fasciculus tip / before Placeus replies)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 227 tip–230 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: Chamier / actiones suppositorum; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_288–293; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>4168 or 12m (`sources/_ocr_cap6_chamier/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_288-293", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VI Placeus replies to GAR erotemata / corruption parallel",
    }
