# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — GAR Cap. VIII Resp. 1 innate/introire; tip before GAR 3 PLAC reply."""
...
OCR = BOOK / "sources/_ocr_cap9_innate"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_innate_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5135
PACKET_STEM = "cap9_innate_densify"
LOCK_NAME = "_placeus_cap9_innate_latin_lock.txt"
JX_PREFIX = "cap9_innate"
...
Scope: Manuscriptum posterius Cap. IX — GAR Cap. VIII Resp. 1 (innate corruption / introire); Amyraut justice-entry; GAR 3 begin-to-be (printed pp. 324–325 tip). Stops before PLAC reply to GAR 3. Not whole Cap. IX / Disputatio.
...
Remaining: Cap. IX PLAC vs GAR 3+ / rest of Cap. IX / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX innate densify tip-502; need live>5135 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX innate densify section {sid} "
                "from locked 1661 Latin (pp. 324–325 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (GAR Cap. VIII Resp. 1 innate / Amyraut / GAR 3) "
        "/ before PLAC vs GAR 3"
    )
    remain = "Cap. IX PLAC vs GAR 3+ / rest of Cap. IX / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. IX GAR Cap. VIII Resp. 1 tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_reatus_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap9_innate",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. IX GAR Cap. VIII Resp. 1 (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Caput IX (GAR Cap. VIII Resp. 1 innate/introire) before "
            "PLAC vs GAR 3, reconstructed from IA PDF page images with Vision OCR and "
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX innate densify from locked 1661 Latin "
            "(pp. 324–325 tip); Pass A != B."
            if int(sid) >= 503
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX innate densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput IX (GAR Cap. VIII Resp. 1) "
                "before PLAC vs GAR 3 from locked 1661 Saumur Latin "
                "(pp. 324–325 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX innate densify (Cap. VIII Resp. 1 → before PLAC vs GAR 3)

- Before: **{BEFORE}** sections (through Cap. IX GAR Cap. VII § IX tip)
- After: **{AFTER}** sections (through Cap. IX GAR Cap. VIII Resp. 1 tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 324–325 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: innata; Amyraut; εἰς τὸ δικαστήριον; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_503–508; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5135 or 12m (`sources/_ocr_cap9_innate/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_503-508", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — PLAC vs GAR 3 (begin-to-be / Acts 1:21 / Heb 1:6)",
    }
