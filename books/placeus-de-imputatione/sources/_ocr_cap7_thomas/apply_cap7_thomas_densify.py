# -*- coding: utf-8 -*-
"""Man. Post. Cap. VII densify — Cap. VII second similitude (Thomas / human body) tip; before GAR Augustine reply."""
...
OCR = BOOK / "sources/_ocr_cap7_thomas"
...
HOLD_START_PATH = Path("/tmp/placeus_cap7_thomas_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4664
PACKET_STEM = "cap7_thomas_densify"
LOCK_NAME = "_placeus_cap7_thomas_latin_lock.txt"
JX_PREFIX = "cap7_thomas"
...
Scope: Manuscriptum posterius Cap. VII second similitude — Thomas Aquinas / human body (printed pp. 271 tip–278 tip). Stops before GAR Augustine reply on Adam's will as all men's. Not whole Cap. VII / Disputatio.
...
Remaining: Cap. VII GAR reply+ / third similitude / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VII thomas densify tip-395 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VII thomas densify section {sid} "
                "from locked 1661 Latin (pp. 271 tip–278 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VII (Examen trium similitudinum; 2nd similitude Thomas/body tip) "
        "/ before GAR Augustine reply"
    )
    remain = "Cap. VII GAR Augustine reply+ / third similitude / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VII second similitude (Thomas/body) tip (before GAR Augustine reply). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap7_magist_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap7_thomas",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VII second similitude Thomas/body tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VII second similitude Thomas/body tip) before GAR Augustine reply, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VII GAR Augustine reply deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VII thomas densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VII second similitude Thomas/body tip) before GAR Augustine reply from locked 1661 Saumur Latin "
                "(pp. 271 tip–278 tip). Honest partial — Cap. VII GAR Augustine reply deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VII thomas densify (2nd similitude Thomas/body → before GAR Augustine reply)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VII magist/Exod Deut Ezek tip)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VII second similitude Thomas/body tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 271 tip–278 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: Thomas / Porphyry / Chamier / Rom 5; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_396–401; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap7_thomas/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_396-401", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VII — GAR Augustine reply on Adam will = all",
    }
