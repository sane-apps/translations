# -*- coding: utf-8 -*-
"""Man. Post. Cap. VII densify — Cap. VII further Adam-will / 3rd similitude (civil body) tip; before God-as-judge deepen."""
...
OCR = BOOK / "sources/_ocr_cap7_will3"
...
HOLD_START_PATH = Path("/tmp/placeus_cap7_will3_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4708
PACKET_STEM = "cap7_will3_densify"
LOCK_NAME = "_placeus_cap7_will3_latin_lock.txt"
JX_PREFIX = "cap7_will3"
...
Scope: Manuscriptum posterius Cap. VII — further Adam-will + third similitude civil body / King-war (printed pp. 282 tip–287 tip). Stops before God-as-judge / 8th objection deepen. Not whole Cap. VII / Disputatio.
...
Remaining: Cap. VII God-as-judge+ / rest of Cap. VII / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VII will3 densify tip-407 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VII will3 densify section {sid} "
                "from locked 1661 Latin (pp. 282 tip–287 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VII (Examen trium similitudinum; 3rd similitude King/war tip) "
        "/ before God-as-judge / 8th objection"
    )
    remain = "Cap. VII God-as-judge / 8th objection+ / rest of Cap. VII / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VII 3rd similitude (King/war) tip (before God-as-judge). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap7_gar_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap7_will3",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VII 3rd similitude King/war tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VII 3rd similitude King/war tip) before God-as-judge, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VII God-as-judge deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VII will3 densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VII 3rd similitude King/war tip) before God-as-judge from locked 1661 Saumur Latin "
                "(pp. 282 tip–287 tip). Honest partial — Cap. VII God-as-judge deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VII will3 densify (further Adam-will / 3rd similitude → before God-as-judge)

- Before: **{BEFORE}** sections (through Cap. VII GAR liturgy tip)
- After: **{AFTER}** sections (through Cap. VII 3rd similitude King/war obj. 1–7 tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 282 tip–287 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: generatio / NATURAE VITIUM / Achan / causa principalis; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_408–413; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap7_will3/hold_receipt.json`; released {hold_note})
- Stopped before Cap. VII God-as-judge / 8th…
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_408-413", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VII — God-as-judge / 8th objection (Adam not actu father)",
    }
