# -*- coding: utf-8 -*-
"""Cap. VIII Ex vs.16 densify: after hereditary close through Chamier disertius / before IV vs.17."""
...
OCR = BOOK / "sources/_ocr_cap8_ex16"
...
Scope: Caput VIII III. Ex verf. 16 — Garissoles κρίμα/δώρημα syllogism through Chamier alien-guilt reply to "Nihil, opinor, desiderari potuit disertius" (printed pp. 97–98 tip). Stops before IV. Veniamus ad verf. 17 / further Cap. VIII. Not Cap. IX+.
Reconstruction: IA PDF page images via macOS Vision OCR + tesseract + pdftotext; long-s/ligature OCR corrected; Greek κρίμα / δώρημα locked against Rom 5:16 TR and page crops (Vision soft).
No modern English used as copy-text.
Remaining: Cap. VIII IV. Ex vs.17 onward + Cap. IX+ of the ~494-page Disputatio.
...
def main() -> None:
    ...
    lock_path = BOOK / "sources/_placeus_cap8_ex16_latin_lock.txt"
    ...
    assert len(eng) == 75 and len(src) == 75, (len(eng), len(src))
    pre_eng = digest(eng)
    pre_src = digest(src)
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": f"Cap. VIII Ex vs.16 densify section {sid} from locked 1661 Latin (pp. 97–98 tip).",
        }
    ...
    assert digest(eng[:75]) == pre_eng, "sections 1-75 english mutated"
    assert digest(src[:75]) == pre_src, "sections 1-75 source mutated"

    stop = "Cap. VIII Ex vs.16 κρίμα/δώρημα / Chamier alien-guilt close / before IV vs.17"
    meta.update({
        ...
        "edition": (
            ...
            "Densify: Capita I–VII + Caput VIII through III. Ex verf. 16 κρίμα/δώρημα / Chamier alien-guilt close before IV. Ex vs.17. "
            "Not whole Disputatio; Cap. VIII IV. Ex vs.17+ + Cap. IX+ remain."
        ),
        "blurb": (
            "Josue de la Place (Placeus) — Capita I–VIII partial of De imputatione primi peccati Adami from the 1661 Saumur Latin, "
            f"through {stop}. Honest partial; Cap. VIII IV. Ex vs.17+ + Cap. IX+ remain."
        ),
        "first_english_note": (
    ...
    if not any(str(w.get("path", "")).endswith("_placeus_cap8_ex16_latin_lock.txt") for w in wits):
        wits.append({
            "id": "latin_lock_cap8_ex16",
            "path": "sources/_placeus_cap8_ex16_latin_lock.txt",
            "role": "copy-text",
            "name": "Locked Latin, Caput VIII Ex vs.16 through Chamier disertius close (Saumur 1661)",
            "language": "Latin",
            "url": "https://archive.org/details/deimputationepri00lapl",
        })
    meta["text_history"] = {
        "method": (
            "English follows the locked 1661 Saumur Latin of Capita I–VIII partial through Cap. VIII Ex vs.16 "
            "κρίμα/δώρημα / Chamier alien-guilt close before IV vs.17, reconstructed from IA PDF page images with Vision OCR, tesseract, "
            "and pdftotext as check. No modern English was copied. Cap. VIII IV. Ex vs.17+ + Cap. IX+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads((BOOK / "reviews/audit/cap8_calvin_p20_densify.packet.json").read_text(encoding="utf-8"))
    packet = copy.deepcopy(old_p)
    packet.pop("packet_id", None)
    packet["seed"] = 20260922
    packet["sample_size"] = 80
    packet["expected_sections"] = [str(i) for i in range(1, 81)]
    packet["explicit_selected_sections"] = packet["expected_sections"][:]
    packet["coverage"] = {"expected": 80, "english": 80, "source": 80}
    packet["identity"]["work"] = meta["title"]
    packet["identity"]["edition"] = meta["edition"]
    packet["identity"]["locus_aliases"] = {str(i): str(i) for i in range(1, 81)}
    packet["publication_scope"]["title"] = meta["title"]
    packet["publication_scope"]["section_count"] = 80
    ...
    pkt_path = BOOK / "reviews/audit/cap8_ex16_densify.packet.json"
    ...
    for sid in packet["expected_sections"]:
        ...
        note = (
            f"Section {sid}: Cap. VIII Ex vs.16 densify; Pass B vs locked 1661 Latin; Pass A != B."
            if int(sid) >= 76
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VIII Ex vs.16 densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–VIII through Ex vs.16 κρίμα/δώρημα / Chamier alien-guilt close before IV vs.17 "
                "from locked 1661 Saumur Latin (pp. 97–98 tip). Honest partial — Cap. VIII IV. Ex vs.17+ + Cap. IX+ remain. "
                "Reformed Saumur era disclosed. Locked Greek κρίμα / δώρημα (Vision soft); do not revive OCR ἔργον. "
                "Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    rev_path = BOOK / "reviews/audit/cap8_ex16_densify.review.json"
    ...
    entry = (
        ...
        "- Packet: \n"
        "- Locked Latin:  (1661 PDF pp. 97–98 tip)\n"
        ...
        "- Claim  stays claimed (scribe-placeus); no second claim; not marked done\n"
