# -*- coding: utf-8 -*-
"""Man. Post. Cap. VII densify — Cap. VII God-as-judge / 8th / rebellious city / Abraham tip; before Ezek 21."""
...
OCR = BOOK / "sources/_ocr_cap7_judge"
...
HOLD_START_PATH = Path("/tmp/placeus_cap7_judge_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4741
PACKET_STEM = "cap7_judge_densify"
LOCK_NAME = "_placeus_cap7_judge_latin_lock.txt"
JX_PREFIX = "cap7_judge"
...
Scope: Manuscriptum posterius Cap. VII — God-as-judge / 8th objection / rebellious city / Abraham Gen 18 (printed pp. 287 tip–292 tip). Stops before GAR Ezek 21 deepen. Not whole Cap. VII / Disputatio.
...
Remaining: Cap. VII Ezek 21+ / rest of Cap. VII / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VII judge densify tip-413 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VII judge densify section {sid} "
                "from locked 1661 Latin (pp. 287 tip–292 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VII (Examen trium similitudinum; God-as-judge/Abraham tip) "
        "/ before GAR Ezek 21"
    )
    remain = "Cap. VII Ezek 21 reply+ / rest of Cap. VII / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VII God-as-judge/Abraham tip (before Ezek 21). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap7_will3_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap7_judge",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VII God-as-judge/Abraham tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VII God-as-judge/Abraham tip) before Ezek 21, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VII Ezek 21 deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VII judge densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VII God-as-judge/Abraham tip) before Ezek 21 from locked 1661 Saumur Latin "
                "(pp. 287 tip–292 tip). Honest partial — Cap. VII Ezek 21 deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VII judge densify (God-as-judge / Abraham → before Ezek 21)

- Before: **{BEFORE}** sections (through Cap. VII 3rd similitude obj. 7 tip)
- After: **{AFTER}** sections (through Cap. VII God-as-judge / Abraham immediate tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 287 tip–292 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_414–419; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap7_judge/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_414-419", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VII — GAR Ezek 21 / just with unjust reply",
    }
