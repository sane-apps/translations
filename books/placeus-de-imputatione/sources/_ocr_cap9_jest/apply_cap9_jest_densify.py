# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — GAR jest / PLAC serio John 8:58."""
...
OCR = BOOK / "sources/_ocr_cap9_jest"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_jest_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5366
PACKET_STEM = "cap9_jest_densify"
LOCK_NAME = "_placeus_cap9_jest_latin_lock.txt"
JX_PREFIX = "cap9_jest"
...
Scope: Manuscriptum posterius Cap. IX — GAR jest charge / PLAC serio on John 8:58 reductio + satisfaction parallel start printed pp. 340–341 tip. Stops before PLAC 'Nos non peccavimus sed alius' / further. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX alius-pro-nobis+ / Pelagian telum+ / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX ephho densify tip-538→544; need live>5366 or 12m; Punch X=NO",
    }
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                ...
                "from locked 1661 Latin (pp. 340–341 tip)."
            ),
        }
...
def main() -> None:
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_contr_densify.packet.json").read_text(encoding="utf-8"))
    ...
    review = {
        ...
        "scope_review": {
            ...
            "notes": (
                ...
                "(pp. 340–341 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
...
- Before: **{BEFORE}** sections (through Cap. IX PLAC contradiction tip)
- After: **{AFTER}** sections (through Cap. IX jest/serio tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 340–341 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_569–574; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5366 or 12m (`sources/_ocr_cap9_jest/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_569-574", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — PLAC alius-pro-nobis / double-question on satisfaction timing",
    }
