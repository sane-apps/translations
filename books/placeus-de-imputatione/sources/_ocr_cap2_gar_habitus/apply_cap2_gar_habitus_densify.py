# -*- coding: utf-8 -*-
"""Cap. II GAR Deinde qua ratione habitus densify — before Posterior quaestio de poena."""
...
OCR = BOOK / "sources/_ocr_cap2_gar_habitus"
...
HOLD_START_PATH = Path("/tmp/placeus_cap2_gar_habitus_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 3757
PACKET_STEM = "cap2_gar_habitus_densify"
LOCK_NAME = "_placeus_cap2_gar_habitus_latin_lock.txt"
JX_PREFIX = "cap2_gar_habitus"
...
Scope: Manuscriptum posterius Cap. II GAR Deinde qua ratione habitus ille through Placeus Manuscript mediate ratio / figmento (printed pp. 183 tip–185 tip). Stops before Posterior quaestio de hujus peccati imputati poena. Not Cap. II Posterior/Cap. III+.
...
Remaining: Cap. II Posterior quaestio de poena / Cap. III+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "catch-up ship in flight; do not append eng/src until live>3757 or 12m",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. II GAR Deinde habitus densify section {sid} "
                "from locked 1661 Latin (pp. 183 tip–185 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Cap. II GAR Deinde habitus (habitus fountain / inclination / consensus syllogism Rom 1:32 / "
        "Manuscript mediate ratio / before Posterior quaestio de poena)"
    )
    remain = (
        "Cap. II Posterior quaestio de poena / Cap. III+ / "
    ...
    meta.update({
        ...
        "edition": (
            ...
            "through Cap. II GAR Deinde habitus (before Posterior quaestio de poena). "
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads(
                (BOOK / "reviews/audit/cap2_sxix_nomine_densify.packet.json").read_text(
                    encoding="utf-8"
                )
            )
            wits = list(
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap2_gar_habitus",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": (
                "Locked Latin, Man. Post. Cap. II GAR Deinde habitus before Posterior quaestio "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Cap. II GAR Deinde qua ratione habitus (fountain/inclination/consensus/"
            "Manuscript mediate) before Posterior quaestio de poena, reconstructed from IA PDF page images "
            "with Vision OCR, tesseract, and pdftotext as check. No modern English was copied. "
            "Cap. II Posterior quaestio+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads(
        (BOOK / "reviews/audit/cap2_sxix_nomine_densify.packet.json").read_text(encoding="utf-8")
    )
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. II GAR Deinde habitus densify from locked 1661 Latin "
            "(pp. 183 tip–185 tip); Pass A != B."
            if int(sid) >= 193
            else (
    ...
    block = f"""## 2026-09-22 Cap. II GAR Deinde habitus densify (habitus fountain → before Posterior quaestio)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. II §XIX Tum in nomine / before Deinde habitus)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. II GAR Deinde habitus / inclination / Rom 1:32 consensus / Manuscript mediate / before Posterior quaestio de poena)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 183 tip–185 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: no new Greek lemma this slice; do not revive ἔργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_193–199; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: catch-up ship in flight; eng/src append after live>3757 or 12m (`sources/_ocr_cap2_gar_habitus/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_193-199",
        "tip_ready": "ok",
        "hold": hold_note,
