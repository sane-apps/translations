# -*- coding: utf-8 -*-
"""Man. Post. Cap. VIII densify — OT tree interdict open; tip before Hypognosticon."""
...
HOLD_START_PATH = Path("/tmp/placeus_cap8_ot_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4815
PACKET_STEM = "cap8_ot_densify"
LOCK_NAME = "_placeus_cap8_ot_latin_lock.txt"
JX_PREFIX = "cap8_ot"
...
Scope: Manuscriptum posterius Cap. VIII — OT arguments open / tree interdict / times distinction (printed pp. 297–300 tip). Stops before Hypognosticon / infant deepen. Not whole Cap. VIII / Disputatio.
...
Remaining: Cap. VIII Hypognosticon+ / rest of Cap. VIII / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VIII ot densify tip-430 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VIII ot densify section {sid} "
                "from locked 1661 Latin (pp. 297–300 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VIII (OT tree interdict / times distinction) "
        "/ before Hypognosticon"
    )
    remain = "Cap. VIII Hypognosticon+ / rest of Cap. VIII / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VIII OT tree interdict open (before Hypognosticon). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap7_quarto_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap8_ot",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VIII OT tree interdict open (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VIII OT tree interdict) before Hypognosticon, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VIII Hypognosticon deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. VIII OT densify from locked 1661 Latin "
            "(pp. 297–300 tip); Pass A != B."
            if int(sid) >= 431
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VIII ot densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VIII OT tree interdict) before Hypognosticon from locked 1661 Saumur Latin "
                "(pp. 297–300 tip). Honest partial — Cap. VIII Hypognosticon+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VIII ot densify (OT tree interdict → before Hypognosticon)

- Before: **{BEFORE}** sections (through Cap. VII Quarto/Quinto close)
- After: **{AFTER}** sections (through Cap. VIII OT tree interdict / times tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 297–300 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_431–436; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap8_ot/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_431-436", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VIII — Hypognosticon / infant time / interdict silence",
    }
