# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — PLAC vere/falso & proprie after GAR 5."""
...
OCR = BOOK / "sources/_ocr_cap9_plac_vere"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_plac_vere_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5471
PACKET_STEM = "cap9_plac_vere_densify"
LOCK_NAME = "_placeus_cap9_plac_vere_latin_lock.txt"
JX_PREFIX = "cap9_plac_vere"
...
Scope: Manuscriptum posterius Cap. IX — PLAC on vere/falso & proprie after GAR 5 (Augustine Ep. 106) printed pp. 344–345 tip. Stops before evangelical-justice reply / Heb. 7 / 1 Cor. 15 objections. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX PLAC evangelical-justice + Heb./1 Cor. replies / GAR Resp.1+ / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    import os
    if os.environ.get("PLACEUS_HOLD_ASYNC") == "1":
        if not HOLD_START_PATH.exists():
            HOLD_START_PATH.write_text(str(int(time.time())) + "\n")
        start = int(float(HOLD_START_PATH.read_text().strip()))
        ...
        receipt = {
            ...
            "probes": [{"unix": time.time(), "elapsed_s": 0, "live_sections": live, "probe_i": 1}],
        }
        if live is not None and live > LIVE_NEED:
            receipt["released_by"] = f"live_sections>{LIVE_NEED} ({live})"
        (OCR / "hold_receipt.json").write_text(json.dumps(receipt, indent=2) + "\n")
        print("hold async init live", live, "need>", LIVE_NEED, "active", "released_by" not in receipt, flush=True)
        return receipt
    # fall through to full wait
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX PLAC vere densify tip-592→598; need live>5471 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX PLAC vere densify section {sid} "
                "from locked 1661 Latin (pp. 344–345 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (PLAC vere/falso & proprie equity tip) "
        "/ before evangelical-justice reply"
    )
    remain = "Cap. IX PLAC evangelical-justice+Heb/1Cor / GAR Resp.1+ / rest Cap. IX / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. IX all-are-sinners tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_gar4_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap9_omit",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. IX omit/death dilemma (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Caput IX (eph-ho / eph-ho pantes) before "
            "eph-ho / eph-ho pantes, reconstructed from IA PDF page images with Vision OCR and "
            "pdftotext as check. No modern English was copied. Cap. IX deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    print("packet", packet["packet_id"], len(packet["sections"]))

    sec_by_id = {s["section"]: s for s in packet["sections"]}
    reviews = []
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX ephho densify from locked 1661 Latin "
            "(pp. 344–345 tip); Pass A != B."
            if int(sid) >= 593
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX ephho densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput IX (omit/death dilemma) "
                "before GAR Resp.1 on peccare vs imputari from locked 1661 Saumur Latin "
                "(pp. 343–344 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    print("post-tip live", live_now, "LIVE_NEED fixed", LIVE_NEED, flush=True)
    hold = wait_hold_post_tip()
    hold_note = hold.get("released_by") or "?"

    handoff = BOOK / "SESSION_HANDOFF.md"
    old = handoff.read_text(encoding="utf-8")
    block = f"""## 2026-09-23 Cap. IX PLAC vere/falso & proprie densify (after GAR 5)

- Before: **{BEFORE}** sections (through Cap. IX GAR 4 tip)
- After: **{AFTER}** sections (through Cap. IX PLAC vere/proprie equity tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 344–345 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_593–598; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5471 or 12m (`sources/_ocr_cap9_plac_vere/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "packet_id": packet["packet_id"],
        "before": BEFORE, "after": AFTER, "sids": SIDS,
        "phase": "appended_tip_ready", "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_593-598", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — PLAC evangelical-justice reply + Heb. 7 / 1 Cor. 15 objections (after vere/proprie tip)",
    }
