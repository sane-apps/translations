# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — equity / Martyr communion."""
...
OCR = BOOK / "sources/_ocr_cap9_equity"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_equity_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5693
PACKET_STEM = "cap9_equity_densify"
LOCK_NAME = "_placeus_cap9_equity_latin_lock.txt"
JX_PREFIX = "cap9_equity"
...
Scope: Manuscriptum posterius Cap. IX — equity / Martyr communion printed pp. 356–357 tip. Stops before Cap. X Rom.5.13. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. X open (Rom.5.13+) / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX equity/Martyr densify tip-652	o658; need live>5693 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX equity densify section {sid} "
                "from locked 1661 Latin (pp. 356–357 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (imputation/equity tip) "
        "/ before equity/Martyr"
    )
    remain = "Cap. X Rom.5.13+ / ~494 pp Disputatio remain."
    meta.update({
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX equity/Martyr densify from locked 1661 Latin "
            "(pp. 356–357 tip); Pass A != B."
            if int(sid) >= 653
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX equity/Martyr densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "(pp. 356–357 tip). Honest partial — Cap. X+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX equity / Martyr communion densify

- Before: **{BEFORE}** sections (through Cap. IX inhaerentia tip)
- After: **{AFTER}** sections (through Cap. IX equity/Martyr communion tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 356–357 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: philanthropia; vi communionis; tou einai; Petrum Martyrem
- Gates: `check_pass_ab` ok {JX_PREFIX}_653–658; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5693 or 12m (`sources/_ocr_cap9_equity/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_653-658", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. X — Rom. 5.13+ (after Cap. IX equity/Martyr tip)",
    }
