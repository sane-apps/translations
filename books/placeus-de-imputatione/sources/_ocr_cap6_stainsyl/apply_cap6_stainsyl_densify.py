# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — original-stain syllogism / absolute-right vs just-judgment tip; before poena-permission deepen."""
...
OCR = BOOK / "sources/_ocr_cap6_stainsyl"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_stainsyl_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4447
PACKET_STEM = "cap6_stainsyl_densify"
LOCK_NAME = "_placeus_cap6_stainsyl_latin_lock.txt"
JX_PREFIX = "cap6_stainsyl"
...
Scope: Manuscriptum posterius Cap. VI original-stain syllogism / absolute-right vs just-judgment (printed pp. 252 tip–254 tip). Stops before poena-permission deepen. Not Caput VII+.
...
Remaining: Cap. VI poena-permission deepen+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI stainsyl densify tip-341 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI stainsyl densify section {sid} "
                "from locked 1661 Latin (pp. 252 tip–254 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VI (Examen Cap. IV / causa condemnationis; original-stain syllogism tip) "
        "/ before poena-permission deepen"
    )
    remain = "Cap. VI poena-permission deepen+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI original-stain syllogism tip (before poena-permission deepen). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_lastend_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_stainsyl",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI original-stain syllogism tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "original-stain syllogism tip) before poena-permission deepen, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI poena-permission deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI stainsyl densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "original-stain syllogism tip) before poena-permission deepen from locked 1661 Saumur Latin "
                "(pp. 252 tip–254 tip). Honest partial — Cap. VI poena-permission deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VI stainsyl densify (original-stain syllogism → before poena-permission deepen)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VI last-end / pleasure-vs-generation tip / before original-stain syllogism)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI original-stain syllogism tip / before poena-permission deepen)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 252 tip–254 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_342–347; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap6_stainsyl/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_342-347", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VI poena / permission / natural-propagation deepen",
    }
