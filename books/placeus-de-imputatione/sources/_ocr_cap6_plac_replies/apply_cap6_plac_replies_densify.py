# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — Placeus replies Ad 1–6 / free-voluntary; before foedus deep critique."""
...
OCR = BOOK / "sources/_ocr_cap6_plac_replies"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_plac_replies_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4186
PACKET_STEM = "cap6_plac_replies_densify"
LOCK_NAME = "_placeus_cap6_plac_replies_latin_lock.txt"
JX_PREFIX = "cap6_plac_replies"
...
Scope: Manuscriptum posterius Cap. VI Placeus replies to GAR corruption-parallel (Ad 1–6); GAR free/voluntary imputation; Placeus Scripture/judicial/conscience; GAR erotema 1 foedus-head open / Placeus knot tip (printed pp. 230 tip–233 tip). Stops before deep foedus-condition critique. Not Caput VII+.
...
Remaining: Cap. VI foedus deep critique+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI Placeus-replies tip-293 catch-up; need live>4186 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI Placeus replies densify section {sid} "
                "from locked 1661 Latin (pp. 230 tip–233 tip)."
            ),
        }
...
def main() -> None:
    ...
    remain = "Cap. VI foedus deep critique+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI Placeus replies tip (before foedus deep critique). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_chamier_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_plac_replies",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI Placeus replies tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI foedus deep critique+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI Placeus-replies densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "(pp. 230 tip–233 tip). Honest partial — Cap. VI foedus deep critique+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. VI Placeus replies densify (Ad 1–6 / free-voluntary → before foedus deep critique)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VI Chamier / fasciculus tip / before Placeus replies)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI Placeus Ad 1–6 / free-voluntary / erotema-1 foedus tip / before foedus deep critique)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 230 tip–233 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: Placeus Ad replies; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_294–299; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>4186 or 12m (`sources/_ocr_cap6_plac_replies/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_294-299", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VI foedus-condition deep critique (GAR erotema 1+)",
    }
