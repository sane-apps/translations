# -*- coding: utf-8 -*-
"""Man. Post. Cap. X densify — GAR replies on Rom. 5.13 (after open tip)."""
...
OCR = BOOK / "sources/_ocr_cap10_mp_rom5_gar"
...
HOLD_START_PATH = Path("/tmp/placeus_cap10_mp_rom5_gar_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5751
PACKET_STEM = "cap10_mp_rom5_gar_densify"
LOCK_NAME = "_placeus_cap10_mp_rom5_gar_latin_lock.txt"
JX_PREFIX = "cap10_mp_rom5_gar"
...
Scope: Manuscriptum posterius Cap. X — GAR.1–3 replies on Rom. 5.13 through actus/reatus tip, printed pp. 359–360. Stops before PLAC on reatus-only. Not whole Cap. X / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. X PLAC on reatus / GAR+ / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.X GAR densify tip-664 to 670; need live>5751 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. X GAR densify section {sid} "
                "from locked 1661 Latin (pp. 359–360 tip)."
            ),
        }
...
def main() -> None:
    ...
    remain = "Cap. X PLAC on reatus+ / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. X GAR.1–3 tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap10_mp_rom5_densify.packet.json").read_text(encoding="utf-8"))
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. X GAR densify from locked 1661 Latin "
            "(pp. 359–360 tip); Pass A != B."
            if int(sid) >= 665
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. X GAR densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput X GAR.1–3 tip "
                "from locked 1661 Saumur Latin "
                "(pp. 359–360 tip). Honest partial — Cap. X PLAC on reatus+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. X GAR replies densify

- Before: **{BEFORE}** sections (through Cap. X Rom. 5.13 open tip)
- After: **{AFTER}** sections (through Cap. X GAR.1–3 tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 359–360 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: imputationi favorabilis; peccatum habituale; actus/reatus; Punch X=NO
- Gates: `check_pass_ab` ok {JX_PREFIX}_665–670; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5751 or 12m (`sources/_ocr_cap10_mp_rom5_gar/hold_receipt.json`; released {hold_note})
- Stopped before Cap. X PLAC on reatus-only…
- Honest partial: Cap. X PLAC on reatus+ / rest / ~494 pp Disputatio remain (**do not claim full Disputatio**)
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_665-670", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. X — PLAC on reatus-only (after GAR.3 tip)",
    }
