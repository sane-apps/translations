# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — Rom 5 omnes / Jeroboam + peccaverunt."""
...
OCR = BOOK / "sources/_ocr_cap9_rom5_omnes"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_rom5_omnes_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5570
PACKET_STEM = "cap9_rom5_omnes_densify"
LOCK_NAME = "_placeus_cap9_rom5_omnes_latin_lock.txt"
JX_PREFIX = "cap9_rom5_omnes"
...
Scope: Manuscriptum posterius Cap. IX — Rom 5 omnes / Jeroboam + peccaverunt syllogism printed pp. 349–351 tip. Stops mid PLAC on immediate vs mediate imputation. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX PLAC immediate-imputation expl / Calvin Clavis / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX PLAC vere densify tip-616→622; need live>5570 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX Rom5 omnes densify section {sid} "
                "from locked 1661 Latin (pp. 349–351 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (Rom5 omnes/Jeroboam tip) "
        "/ before immediate-imputation expl"
    )
    remain = "Cap. IX immediate-imputation+ / rest Cap. IX / ~494 pp Disputatio remain."
    meta.update({
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_gar2_mors_densify.packet.json").read_text(encoding="utf-8"))
    ...
    block = f"""## 2026-09-23 Cap. IX Rom 5 omnes / Jeroboam + peccaverunt densify

- Before: **{BEFORE}** sections (through Cap. IX GAR 2 mortem Adami tip)
- After: **{AFTER}** sections (through Cap. IX peccaverunt syllogism / GAR Resp tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 349–351 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_617–622; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5570 or 12m (`sources/_ocr_cap9_rom5_omnes/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_617-622", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — PLAC immediate-imputation expl + Calvin Clavis (after omnes syllogism)",
    }
