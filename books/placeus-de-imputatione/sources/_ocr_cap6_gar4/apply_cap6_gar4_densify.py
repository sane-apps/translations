# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — GAR 4 Rom. 5 / foedus; before Isaiah 55 tip."""
...
OCR = BOOK / "sources/_ocr_cap6_gar4"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_gar4_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4087
PACKET_STEM = "cap6_gar4_densify"
LOCK_NAME = "_placeus_cap6_gar4_latin_lock.txt"
JX_PREFIX = "cap6_gar4"
...
Scope: Manuscriptum posterius Cap. VI GAR 4 (Rom. 5 / foedus primarium; body/republic; Socrates; moral head; peccavimus via propagation; infection/reatus tip) from after Cap. VI forum tip through before Isaiah 55 / affected-ignorance (printed pp. 220 tip–223 tip). Stops before Subjungis Isaiah 55. Not Caput VII+.
...
Remaining: Cap. VI Isaiah 55+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI GAR4 tip-275 catch-up; need live>4087 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI GAR 4 densify section {sid} "
                "from locked 1661 Latin (pp. 220 tip–223 tip)."
            ),
        }
...
def main() -> None:
    ...
    remain = "Cap. VI Isaiah 55+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI GAR 4 tip (before Isaiah 55). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_examen_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_gar4",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI GAR 4 tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI Isaiah 55+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI GAR 4 densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "(pp. 220 tip–223 tip). Honest partial — Cap. VI Isaiah 55+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. VI GAR 4 densify (Rom. 5 / foedus / body / propagation → before Isaiah 55)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Caput VI Examen Cap. IV / forum tip / before GAR 4)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI GAR 4 / Rom. 5 / foedus / body / propagation / infection tip / before Isaiah 55)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 220 tip–223 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: Rom. 5 / foedus; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_276–281; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>4087 or 12m (`sources/_ocr_cap6_gar4/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_276-281", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VI Isaiah 55 / Subjungis / affected-ignorance",
    }
