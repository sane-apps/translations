# -*- coding: utf-8 -*-
"""Man. Post. Cap. X densify — foedus naturale / ceremonies."""
...
OCR = BOOK / "sources/_ocr_cap10_mp_rom5_foedus"
...
HOLD_START_PATH = Path("/tmp/placeus_cap10_mp_rom5_foedus_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5926
PACKET_STEM = "cap10_mp_rom5_foedus_densify"
LOCK_NAME = "_placeus_cap10_mp_rom5_foedus_latin_lock.txt"
JX_PREFIX = "cap10_mp_rom5_foedus"
...
Scope: Manuscriptum posterius Cap. X — PLAC on reatus/corruption after GAR.3, printed pp. 368–370 tip. Stops before further Cap. X. Not whole Cap. X / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. X Rom. 5:14+ / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.X foedus densify tip-700 to 706; need live>5926 or 12m; Punch X=NO",
    }
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                ...
                "from locked 1661 Latin (pp. 368–370 tip)."
            ),
        }
...
def main() -> None:
    ...
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. X foedus tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap10_mp_rom5_arbor_densify.packet.json").read_text(encoding="utf-8"))
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. X foedus densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput X foedus tip "
                "from locked 1661 Saumur Latin "
                "(pp. 368–370 tip). Honest partial — Cap. X Rom. 5:14+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. X foedus densify

- Before: **{BEFORE}** sections (through Cap. X arbor tip)
- After: **{AFTER}** sections (through Cap. X foedus tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 368–370 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: foedus naturale; lex naturae; Col 2:17; Punch X=NO
- Gates: `check_pass_ab` ok {JX_PREFIX}_701–706; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5926 or 12m (`sources/_ocr_cap10_mp_rom5_foedus/hold_receipt.json`; released {hold_note})
- Stopped before Cap. X Rom. 5:14…
- Honest partial: Cap. X Rom. 5:14+ / rest / ~494 pp Disputatio remain (**do not claim full Disputatio**)
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_701-706", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. X — Rom. 5:14 (after foedus tip)",
    }
