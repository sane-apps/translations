# -*- coding: utf-8 -*-
"""Man. Post. Cap. VII densify — Cap. VII magistrate tip continue / power-vs-justice / Exod Deut Ezek tip; before 2nd similitude (Thomas)."""
...
OCR = BOOK / "sources/_ocr_cap7_magist"
...
HOLD_START_PATH = Path("/tmp/placeus_cap7_magist_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4639
PACKET_STEM = "cap7_magist_densify"
LOCK_NAME = "_placeus_cap7_magist_latin_lock.txt"
JX_PREFIX = "cap7_magist"
...
Scope: Manuscriptum posterius Cap. VII first similitude continue — magistrate tip / power-vs-justice / Exod Deut Ezek (printed pp. 268 tip–271 tip). Stops before second similitude (Thomas / human body). Not whole Cap. VII / Disputatio.
...
Remaining: Cap. VII second–third similitudes / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VII magist densify tip-389 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VII magist densify section {sid} "
                "from locked 1661 Latin (pp. 268 tip–271 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VII (Examen trium similitudinum; magist/power-vs-justice tip) "
        "/ before second similitude (Thomas)"
    )
    remain = "Cap. VII second similitude (Thomas/body)+ / third similitude / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VII magist/power-vs-justice tip (before 2nd similitude). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap7_sons_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap7_magist",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VII magist/power-vs-justice tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VII magist/power-vs-justice tip) before second similitude, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VII second similitude deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VII magist densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VII magist/power-vs-justice tip) before second similitude from locked 1661 Saumur Latin "
                "(pp. 268 tip–271 tip). Honest partial — Cap. VII second similitude deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VII magist densify (magistrate tip continue / power-vs-justice → before 2nd similitude)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VII sons/Deut 24 / magistrate tip)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VII magist/power-vs-justice / Exod Deut Ezek tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 268 tip–271 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_390–395; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap7_magist/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_390-395", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VII — second similitude (Thomas / human body)",
    }
