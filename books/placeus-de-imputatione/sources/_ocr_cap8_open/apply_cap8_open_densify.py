# -*- coding: utf-8 -*-
"""Cap. VIII opening densify: Caput VIII Ex vs.14–15 tip through παράπτωμα/χάρισμα gift reductio."""
...
OCR = BOOK / "sources/_ocr_cap8_open"
...
Scope: Caput VIII opening — Caput VIII title through Garissoles Rom. 5:14 type/antitype syllogism, Placeus major/minor dissolve (Calvin / Gualther / Peter Martyr), Ex vs.15 proximate/remote reply, and παράπτωμα/χάρισμα opposition through "horresco pronuncians, ipsius Dei" (printed pp. 91–95 tip). Stops before Calvin p.20 citation block / further Cap. VIII. Not Cap. IX+.
Reconstruction: IA PDF page images via macOS Vision OCR + tesseract + pdftotext; long-s/ligature OCR corrected; Greek τὸ ἀντίτυπον / τὸ παράπτωμα / τὸ χάρισμα confirmed against page image (crop soft).
No modern English used as copy-text.
Remaining: Cap. VIII Calvin p.20 block onward + Cap. IX+ of the ~494-page Disputatio.
...
def main() -> None:
    ...
    lock_path = BOOK / "sources/_placeus_cap8_open_latin_lock.txt"
    ...
    assert len(eng) == 64 and len(src) == 64, (len(eng), len(src))
    pre_eng = digest(eng)
    pre_src = digest(src)
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": f"Cap. VIII opening densify section {sid} from locked 1661 Latin (pp. 91–95 tip).",
        }
    ...
    assert digest(eng[:64]) == pre_eng, "sections 1-64 english mutated"
    assert digest(src[:64]) == pre_src, "sections 1-64 source mutated"

    stop = "Cap. VIII opening through Rom. 5:15 παράπτωμα/χάρισμα gift reductio / before Calvin p.20"
    meta.update({
        ...
        "edition": (
            ...
            "Densify: Capita I–VII + Caput VIII opening (Rom. 5:14 type/antitype / Calvin–Gualther–Martyr / "
            "Rom. 5:15 proximate-remote / παράπτωμα·χάρισμα gift reductio). "
            "Not whole Disputatio; Cap. VIII remainder + Cap. IX+ remain."
        ),
        "blurb": (
            "Josue de la Place (Placeus) — Capita I–VIII partial of De imputatione primi peccati Adami from the 1661 Saumur Latin, "
            f"through {stop}. Honest partial; Cap. VIII remainder + Cap. IX+ remain."
        ),
        "first_english_note": (
    ...
    if not any(str(w.get("path", "")).endswith("_placeus_cap8_open_latin_lock.txt") for w in wits):
        wits.append({
            "id": "latin_lock_cap8_open",
            "path": "sources/_placeus_cap8_open_latin_lock.txt",
            "role": "copy-text",
            "name": "Locked Latin, Caput VIII opening through Rom. 5:15 gift reductio (Saumur 1661)",
            "language": "Latin",
            "url": "https://archive.org/details/deimputationepri00lapl",
        })
    meta["text_history"] = {
        "method": (
            "English follows the locked 1661 Saumur Latin of Capita I–VIII partial through Cap. VIII opening "
            "(Caput VIII title through Rom. 5:15 παράπτωμα/χάρισμα gift reductio before Calvin p.20), reconstructed from IA PDF page images with Vision OCR, tesseract, "
            "and pdftotext as check. No modern English was copied. Cap. VIII remainder + Cap. IX+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads((BOOK / "reviews/audit/cap7_quinta_ratio_densify.packet.json").read_text(encoding="utf-8"))
    packet = copy.deepcopy(old_p)
    packet.pop("packet_id", None)
    packet["seed"] = 20260922
    packet["sample_size"] = 70
    packet["expected_sections"] = [str(i) for i in range(1, 71)]
    packet["explicit_selected_sections"] = packet["expected_sections"][:]
    packet["coverage"] = {"expected": 70, "english": 70, "source": 70}
    packet["identity"]["work"] = meta["title"]
    packet["identity"]["edition"] = meta["edition"]
    packet["identity"]["locus_aliases"] = {str(i): str(i) for i in range(1, 71)}
    packet["publication_scope"]["title"] = meta["title"]
    packet["publication_scope"]["section_count"] = 70
    ...
    pkt_path = BOOK / "reviews/audit/cap8_open_densify.packet.json"
    ...
    for sid in packet["expected_sections"]:
        ...
        note = (
            f"Section {sid}: Cap. VIII opening densify; Pass B vs locked 1661 Latin; Pass A != B."
            if int(sid) >= 65
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VIII opening densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–VII + Cap. VIII opening (Rom. 5:14 type/antitype through Rom. 5:15 παράπτωμα/χάρισμα gift reductio before Calvin p.20) "
                "from locked 1661 Saumur Latin (pp. 91–95 tip). Honest partial — Cap. VIII remainder + Cap. IX+ remain. "
                "Reformed Saumur era disclosed. Locked Greek τὸ ἀντίτυπον / τὸ παράπτωμα / τὸ χάρισμα (Vision crop soft; do not revive OCR ἔργον). "
                "Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    rev_path = BOOK / "reviews/audit/cap8_open_densify.review.json"
