# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — judgment-locution / antecedent vs consequent judgment tip; before Calvin Tertio nature-vs-judgment."""
...
OCR = BOOK / "sources/_ocr_cap6_judloc"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_judloc_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4494
PACKET_STEM = "cap6_judloc_densify"
LOCK_NAME = "_placeus_cap6_judloc_latin_lock.txt"
JX_PREFIX = "cap6_judloc"
...
Scope: Manuscriptum posterius Cap. VI judgment-locution / antecedent vs consequent judgment (printed pp. 257 tip–259 tip). Stops before Calvin Tertio nature-vs-judgment. Not Caput VII+.
...
Remaining: Cap. VI Calvin Tertio nature-vs-judgment+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI judloc densify tip-353 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI judloc densify section {sid} "
                "from locked 1661 Latin (pp. 257 tip–259 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VI (Examen Cap. IV / causa condemnationis; judgment-locution tip) "
        "/ before Calvin Tertio nature-vs-judgment"
    )
    remain = "Cap. VI Calvin Tertio nature-vs-judgment+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI judgment-locution tip (before Calvin Tertio nature-vs-judgment). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_poena_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_judloc",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI judgment-locution tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "judgment-locution tip) before Calvin Tertio nature-vs-judgment, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI Calvin Tertio nature-vs-judgment+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI judloc densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "judgment-locution tip) before Calvin Tertio from locked 1661 Saumur Latin "
                "(pp. 257 tip–259 tip). Honest partial — Cap. VI Calvin Tertio nature-vs-judgment+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VI judloc densify (antecedent vs consequent → before Calvin Tertio)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VI poena/permission tip / before judgment-locution quarrel)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI judgment-locution tip / before Calvin Tertio nature-vs-judgment)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 257 tip–259 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: judicium antecedens / consequens / Rom 11 / locution; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_354–359; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap6_judloc/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_354-359", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VI Calvin Tertio — nature vs judgment",
    }
