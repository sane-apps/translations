# -*- coding: utf-8 -*-
"""Cap. II Posterior quaestio de hujus peccati imputati poena densify — before Cap. III."""
...
OCR = BOOK / "sources/_ocr_cap2_posterior_poena"
...
HOLD_START_PATH = Path("/tmp/placeus_cap2_posterior_poena_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 3787
PACKET_STEM = "cap2_posterior_poena_densify"
LOCK_NAME = "_placeus_cap2_posterior_poena_latin_lock.txt"
JX_PREFIX = "cap2_posterior_poena"
...
Scope: Manuscriptum posterius Cap. II Posterior quaestio de hujus peccati imputati poena through Placeus two-questions deferral (printed pp. 185 tip–186 tip). Stops before Cap. III Obseruata quaedam. Not Cap. III+.
...
Remaining: Cap. III+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "catch-up ship in flight; do not append eng/src until live>3787 or 12m",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. II Posterior quaestio de poena densify section {sid} "
                "from locked 1661 Latin (pp. 185 tip–186 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Cap. II Posterior quaestio de poena (distinguish / strictissime conditions / "
        "Ursinus / similitude denial / before Cap. III)"
    )
    remain = (
        "Cap. III+ / rest of MS examination / ~494 pp Disputatio remain."
    )
    meta.update({
        ...
        "edition": (
            ...
            "through Cap. II Posterior quaestio de poena (before Cap. III). "
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads(
                (BOOK / "reviews/audit/cap2_gar_habitus_densify.packet.json").read_text(
                    encoding="utf-8"
                )
            )
            wits = list(
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap2_posterior_poena",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": (
                "Locked Latin, Man. Post. Cap. II Posterior quaestio de poena before Cap. III "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    for wid, path, name in [
        ...
        (
            "latin_lock_cap2_sxix_nomine",
            "sources/_placeus_cap2_sxix_nomine_latin_lock.txt",
            "Locked Latin, Man. Post. Cap. II §XIX Tum in nomine before Deinde habitus (Saumur 1661)",
        ),
        (
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Cap. II Posterior quaestio de hujus peccati imputati poena "
            "(distinguish/strictissime/Ursinus/similitude) before Cap. III, reconstructed from IA PDF page images "
    ...
    old_p = json.loads(
        (BOOK / "reviews/audit/cap2_gar_habitus_densify.packet.json").read_text(encoding="utf-8")
    )
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. II Posterior quaestio de poena densify from locked 1661 Latin "
            "(pp. 185 tip–186 tip); Pass A != B."
            if int(sid) >= 200
            else (
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. II Posterior quaestio de poena densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Cap. II Posterior quaestio de hujus peccati "
                "imputati poena (distinguish; strictissime conditions; Ursinus; Fathers/Calvin "
                "similitude; two-questions deferral) before Cap. III from locked 1661 Saumur "
                "Latin (pp. 185 tip–186 tip). Honest partial — Cap. III+ remain. Reformed "
    ...
    block = f"""## 2026-09-22 Cap. II Posterior quaestio de poena densify (distinguish/similitude → before Cap. III)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. II GAR Deinde habitus / before Posterior quaestio de poena)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. II Posterior quaestio de poena / strictissime / Ursinus / similitude / before Cap. III)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 185 tip–186 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: no new Greek lemma this slice; do not revive ἔργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_200–205; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: catch-up ship in flight (tip 199); eng/src append after live>3787 or 12m (`sources/_ocr_cap2_posterior_poena/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_200-205",
        "tip_ready": "ok",
        "hold": hold_note,
