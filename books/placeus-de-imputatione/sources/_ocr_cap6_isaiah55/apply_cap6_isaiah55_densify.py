# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — Isaiah 55 / Subjungis / affected-ignorance; before Chamier."""
...
OCR = BOOK / "sources/_ocr_cap6_isaiah55"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_isaiah55_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4124
PACKET_STEM = "cap6_isaiah55_densify"
LOCK_NAME = "_placeus_cap6_isaiah55_latin_lock.txt"
JX_PREFIX = "cap6_isaiah55"
...
Scope: Manuscriptum posterius Cap. VI after GAR 4 infection tip: Subjungis / Isaiah 55 / affected ignorance (Job) / Deut 29:29 / Lavater / root-propagates-vice / vitium originis tip before Chamier (printed pp. 224 tip–227 tip). Stops before Chamier deep dive. Not Caput VII+.
...
Remaining: Cap. VI Chamier+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI Isaiah55 tip-281 catch-up; need live>4124 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI Isaiah 55 densify section {sid} "
                "from locked 1661 Latin (pp. 224 tip–227 tip)."
            ),
        }
...
def main() -> None:
    ...
    remain = "Cap. VI Chamier+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI Isaiah 55 / Subjungis tip (before Chamier). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_gar4_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_isaiah55",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI Isaiah 55 / Subjungis tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI Chamier+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI Isaiah 55 densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "(pp. 224 tip–227 tip). Honest partial — Cap. VI Chamier+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. VI Isaiah 55 densify (Subjungis / Job / Lavater / vitium → before Chamier)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VI GAR 4 / infection tip / before Isaiah 55)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI Isaiah 55 / Subjungis / Lavater / vitium originis tip / before Chamier)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 224 tip–227 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: Isaiah 55 / Lavater; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_282–287; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>4124 or 12m (`sources/_ocr_cap6_isaiah55/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_282-287", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VI Chamier (actiones sunt suppositorum)",
    }
