# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — imputatum-est reductio; tip before three senses."""
...
OCR = BOOK / "sources/_ocr_cap9_imputatum"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_imputatum_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5177
PACKET_STEM = "cap9_imputatum_densify"
LOCK_NAME = "_placeus_cap9_imputatum_latin_lock.txt"
JX_PREFIX = "cap9_imputatum"
...
Scope: Manuscriptum posterius Cap. IX — imputatum-est substitution reductio; GAR 5–6 reatus/curse; PLAC through-God absurdity (printed pp. 326–328 tip). Stops before three senses of imputation worked. Not whole Cap. IX / Disputatio.
...
Remaining: Cap. IX three senses of imputation+ / rest of Cap. IX / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX imputatum densify tip-514; need live>5177 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX imputatum densify section {sid} "
                "from locked 1661 Latin (pp. 326–328 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (imputatum-est reductio / GAR 5–6) "
        "/ before three senses of imputation"
    )
    remain = "Cap. IX three senses of imputation+ / rest of Cap. IX / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. IX imputatum-est reductio tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_gar3_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap9_imputatum",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. IX imputatum-est reductio (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Caput IX (imputatum-est reductio) before three senses "
            "of imputation, reconstructed from IA PDF page images with Vision OCR and "
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX imputatum densify from locked 1661 Latin "
            "(pp. 326–328 tip); Pass A != B."
            if int(sid) >= 515
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX imputatum densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput IX (imputatum-est reductio) "
                "before three senses from locked 1661 Saumur Latin "
                "(pp. 326–328 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX imputatum densify (reductio → before three senses)

- Before: **{BEFORE}** sections (through Cap. IX PLAC vs GAR 3 tip)
- After: **{AFTER}** sections (through Cap. IX imputatum-est reductio tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 326–328 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: imputatum est; reatus; Ps 109; per Deum absurd; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_515–520; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5177 or 12m (`sources/_ocr_cap9_imputatum/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_515-520", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — three senses of imputation (mind / declaration / penalty)",
    }
