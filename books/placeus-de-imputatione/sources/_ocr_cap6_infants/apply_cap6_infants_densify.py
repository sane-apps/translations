# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — infants death / Confessio XI / Pighius deepen tip; before generative-faculty quarrel."""
...
OCR = BOOK / "sources/_ocr_cap6_infants"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_infants_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4353
PACKET_STEM = "cap6_infants_densify"
LOCK_NAME = "_placeus_cap6_infants_latin_lock.txt"
JX_PREFIX = "cap6_infants"
...
Scope: Manuscriptum posterius Cap. VI infants death / Confessio XI / Pighius deepen through Placeus fountain-not-God and generative-corruption tip (printed pp. 243 tip–246 tip). Stops before generative-faculty moral quarrel. Not Caput VII+.
...
Remaining: Cap. VI generative-faculty quarrel+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI infants densify tip-323 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI infants densify section {sid} "
                "from locked 1661 Latin (pp. 243 tip–246 tip)."
            ),
        }
...
def main() -> None:
    ...
    remain = "Cap. VI generative-faculty quarrel+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI infants/Confessio/Pighius tip (before generative-faculty quarrel). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_gar6q_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_infants",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI infants/Confessio/Pighius tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI generative-faculty quarrel+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI infants densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "(pp. 243 tip–246 tip). Honest partial — Cap. VI generative-faculty quarrel+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. VI infants densify (Confessio XI / Pighius → before generative-faculty quarrel)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VI GAR erotema 6 tip / before infants/Pighius deepen)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI infants/Confessio/Pighius tip / before generative-faculty quarrel)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 243 tip–246 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: infants / Confessio XI / Pighius; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_324–329; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap6_infants/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_324-329", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VI generative faculty moral quarrel (GAR Unde / Decalogue)",
    }
