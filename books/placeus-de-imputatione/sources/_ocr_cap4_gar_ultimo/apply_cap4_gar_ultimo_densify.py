# -*- coding: utf-8 -*-
"""Cap. IV GAR Ultimo densify — mors spiritualis metaphorice / Luc 9:60; PLAC; GAR Resp./2-4; before Denique Bellarmine."""
...
OCR = BOOK / "sources/_ocr_cap4_gar_ultimo"
...
HOLD_START_PATH = Path("/tmp/placeus_cap4_gar_ultimo_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 3948  # tip-250 catch-up ship; need live>3948 or 12m
PACKET_STEM = "cap4_gar_ultimo_densify"
LOCK_NAME = "_placeus_cap4_gar_ultimo_latin_lock.txt"
JX_PREFIX = "cap4_gar_ultimo"
...
Scope: Manuscriptum posterius Cap. IV after Sexta morte morieris tip: GAR Ultimo (mors spiritualis metaphorice / Luc 9:60); PLAC truncation reply; GAR Resp. eternal torment; PLAC aeterna most proper; GAR 2-3 omnem mortem / Scripturis with PLAC spiritualis=peccatum and OT phrase lock (Ezek 3:18 / Hos 13:1 actualia); GAR 4 Decalogue metaphors with PLAC tip through before GAR Denique Bellarmine (printed pp. 209 tip-211 tip). Stops before GAR Denique (Pontificii / Chamier). Not Cap. IV remainder+ / Caput V.
...
Remaining: Cap. IV GAR Denique+ / Caput V+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "tip-250 catch-up ship in flight; re-probe before append; need live>3948 or 12m",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IV GAR Ultimo densify section {sid} "
                "from locked 1661 Latin (pp. 209 tip–211 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Cap. IV GAR Ultimo (mors spiritualis metaphorice / Luc 9:60; PLAC; "
        "GAR Resp./2-4 Decalogue tip) / before GAR Denique Bellarmine"
    )
    remain = (
        "Cap. IV GAR Denique+ / Caput V+ / rest of MS examination / ~494 pp Disputatio remain."
    )
    meta.update({
        ...
        "edition": (
            ...
            "through Cap. IV GAR Ultimo / PLAC / GAR Resp. 2-4 tip (before Denique). "
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads(
                (BOOK / "reviews/audit/cap4_quinta_gar_densify.packet.json").read_text(
                    encoding="utf-8"
                )
            )
            wits = list(
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap4_gar_ultimo",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": (
                "Locked Latin, Man. Post. Cap. IV GAR Ultimo / PLAC / GAR 2-4 tip "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Cap. IV GAR Ultimo (mors spiritualis metaphorice / Luc 9:60) through "
            "Placeus replies and GAR Resp./2-4 Decalogue tip before GAR Denique, "
            "reconstructed from IA PDF page images with Vision OCR and pdftotext as check. "
            "No modern English was copied. Cap. IV GAR Denique+ / Caput V+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads(
        (BOOK / "reviews/audit/cap4_quinta_gar_densify.packet.json").read_text(encoding="utf-8")
    )
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IV GAR Ultimo densify from locked 1661 Latin "
            "(pp. 209 tip–211 tip); Pass A != B."
            if int(sid) >= 251
            else (
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IV GAR Ultimo densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Cap. IV GAR Ultimo (mors spiritualis metaphorice / Luc 9:60; PLAC; GAR Resp./2-4) before Denique from locked 1661 Saumur Latin (pp. 209 tip–211 tip). Honest partial — Cap. IV GAR Denique+ / Caput V+ remain. Do not revive εργον. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. IV GAR Ultimo densify (mors spiritualis metaphorice / Luc 9:60 → PLAC → GAR Resp./2–4 → before Denique)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. IV Sexta tip / before GAR Ultimo)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. IV GAR Ultimo / PLAC / GAR Resp. 2–4 Decalogue tip / before Denique Bellarmine)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 209 tip–211 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_251–256; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: tip-250 catch-up ship; eng/src append after live>3948 or 12m (`sources/_ocr_cap4_gar_ultimo/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_251-256",
        "tip_ready": "ok",
        "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IV GAR Denique (Bellarmine/Chamier hoc est corpus / Placeus defendi) / Vir Doctus threefold before Caput V",
        "prior_packet_done": "cap4_quinta_gar_densify (244→250)",
    }
