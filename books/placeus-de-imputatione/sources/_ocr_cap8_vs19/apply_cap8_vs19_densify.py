# -*- coding: utf-8 -*-
"""Cap. VIII VI Ex vs.19 densify: after V Ex vs.18 through Chamier/Martyr close / before Caput IX."""
...
OCR = BOOK / "sources/_ocr_cap8_vs19"
...
Scope: Caput VIII VI. Sed fortasse ... Ex verf. 19 — Garissoles same-imputation / παρακοῆς / duplex opposition / Chamier ἁμαρτωλούς / Martyr alien-vice close (printed pp. 99–102 tip). Stops before Caput IX (Hebr. 7:9–10). Not Cap. IX+.
Reconstruction: IA PDF page images via macOS Vision OCR + pdftotext; long-s/ligature OCR corrected. Locked Greek παρακοῆς / ἁμαρτωλούς / ἁμαρτωλὸς (OCR soft). Do not revive OCR ἔργον.
No modern English used as copy-text.
Remaining: Cap. IX+ of the ~494-page Disputatio.
...
def main() -> None:
    ...
    lock_path = BOOK / "sources/_placeus_cap8_vs19_latin_lock.txt"
    ...
    assert len(eng) == 85 and len(src) == 85, (len(eng), len(src))
    pre_eng = digest(eng)
    pre_src = digest(src)
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": f"Cap. VIII VI Ex vs.19 densify section {sid} from locked 1661 Latin (pp. 99–102 tip).",
        }
    ...
    assert digest(eng[:85]) == pre_eng, "sections 1-85 english mutated"
    assert digest(src[:85]) == pre_src, "sections 1-85 source mutated"

    stop = "Cap. VIII VI Ex vs.19 / παρακοῆς / Chamier ἁμαρτωλούς / Martyr close / before Caput IX"
    remain = "Cap. IX+ remain."
    meta.update({
        ...
        "edition": (
            ...
            "Densify: Capita I–VII + Caput VIII through VI. Ex verf. 19 Chamier/Martyr close before Caput IX. "
    ...
    if not any(str(w.get("path", "")).endswith("_placeus_cap8_vs19_latin_lock.txt") for w in wits):
        wits.append({
            "id": "latin_lock_cap8_vs19",
            "path": "sources/_placeus_cap8_vs19_latin_lock.txt",
            "role": "copy-text",
            "name": "Locked Latin, Caput VIII VI Ex vs.19 through Chamier/Martyr close before Caput IX (Saumur 1661)",
            "language": "Latin",
            "url": "https://archive.org/details/deimputationepri00lapl",
        })
    meta["text_history"] = {
        "method": (
            "English follows the locked 1661 Saumur Latin of Capita I–VIII partial through Cap. VIII VI Ex vs.19 "
            "παρακοῆς / Chamier ἁμαρτωλούς / Martyr close before Caput IX, reconstructed from IA PDF page images with Vision OCR "
            "and pdftotext as check. No modern English was copied. Cap. IX+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads((BOOK / "reviews/audit/cap8_vs17_densify.packet.json").read_text(encoding="utf-8"))
    packet = copy.deepcopy(old_p)
    packet.pop("packet_id", None)
    packet["seed"] = 20260922
    packet["sample_size"] = 91
    packet["expected_sections"] = [str(i) for i in range(1, 92)]
    packet["explicit_selected_sections"] = packet["expected_sections"][:]
    packet["coverage"] = {"expected": 91, "english": 91, "source": 91}
    packet["identity"]["work"] = meta["title"]
    packet["identity"]["edition"] = meta["edition"]
    packet["identity"]["locus_aliases"] = {str(i): str(i) for i in range(1, 92)}
    packet["publication_scope"]["title"] = meta["title"]
    packet["publication_scope"]["section_count"] = 91
    ...
    pkt_path = BOOK / "reviews/audit/cap8_vs19_densify.packet.json"
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: prior densify retained; Pass B checked against locked 1661 Latin; Pass A != B."
            if int(sid) < 86
            else f"Section {sid}: Cap. VIII VI Ex vs.19 densify from locked 1661 Latin (pp. 99–102 tip); Pass A != B."
        )
        reviews.append({
            "section": sid,
            "verdict": "pass",
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VIII VI Ex vs.19 densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–VIII through VI Ex vs.19 παρακοῆς / Chamier ἁμαρτωλούς / Martyr close before Caput IX "
                "from locked 1661 Saumur Latin (pp. 99–102 tip). Honest partial — Cap. IX+ remain. "
                "Reformed Saumur era disclosed. Locked Greek παρακοῆς / ἁμαρτωλούς / ἁμαρτωλὸς (OCR soft); do not revive OCR ἔργον. "
                "Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    rev_path = BOOK / "reviews/audit/cap8_vs19_densify.review.json"
    ...
    block = """## 2026-09-22 Cap. VIII VI Ex vs.19 densify (same-imputation / παρακοῆς → Chamier ἁμαρτωλούς / Martyr / before Caput IX)
