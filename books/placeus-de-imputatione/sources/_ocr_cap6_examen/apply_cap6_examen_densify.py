# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — Examen Cap. IV / causa condemnationis; before GAR 4."""
...
OCR = BOOK / "sources/_ocr_cap6_examen"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_examen_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4050
PACKET_STEM = "cap6_examen_densify"
LOCK_NAME = "_placeus_cap6_examen_latin_lock.txt"
JX_PREFIX = "cap6_examen"
...
Scope: Manuscriptum posterius Caput VI (Examen Cap. IV / causa condemnationis) from after Caput V tip through PLAC forum / Noah-Abraham tip before GAR 4 (printed pp. 216 tip–220 tip). Stops before GAR 4. Not GAR 4+ / Caput VII+ / rest of MS examination.
Reconstruction: IA PDF page images via macOS Vision OCR + pdftotext; long-s/ligature OCR corrected. Porphyry / Amyraut named. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. VI GAR 4+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI tip-268 catch-up; need live>4050 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI Examen Cap. IV densify section {sid} "
                "from locked 1661 Latin (pp. 216 tip–220 tip)."
            ),
        }
...
def main() -> None:
    ...
    remain = "Cap. VI GAR 4+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Caput VI Examen Cap. IV tip (before GAR 4). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap5_imputari_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_examen",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Caput VI Examen Cap. IV tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI GAR 4+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI Examen Cap. IV densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "(pp. 216 tip–220 tip). Honest partial — Cap. VI GAR 4+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. VI Examen Cap. IV densify (one-man / Augustine / representation / forum → before GAR 4)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Caput V quid sit imputari tip / before Caput VI)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Caput VI Examen Cap. IV / one-man / Augustine / representation / forum tip / before GAR 4)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 216 tip–220 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: Porphyry / Amyraut; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_269–275; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>4050 or 12m (`sources/_ocr_cap6_examen/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_269-275", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VI GAR 4 (Rom. 5 / foedus tip)",
    }
