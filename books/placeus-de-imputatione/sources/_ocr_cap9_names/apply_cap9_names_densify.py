# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — name-distinctions; tip before GAR Beza reply."""
...
OCR = BOOK / "sources/_ocr_cap9_names"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_names_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5062
PACKET_STEM = "cap9_names_densify"
LOCK_NAME = "_placeus_cap9_names_latin_lock.txt"
JX_PREFIX = "cap9_names"
...
Scope: Manuscriptum posterius Cap. IX — name-distinctions (παράπτωμα vs other names; venom/propagation; Placeus syllogism; Beza ἡ ἁμαρτία) printed pp. 318–320 tip. Stops before GAR reply on Beza. Not whole Cap. IX / Disputatio.
...
Remaining: Cap. IX GAR/Beza interchange+ / rest of Cap. IX / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX names densify tip-484; need live>5062 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX names densify section {sid} "
                "from locked 1661 Latin (pp. 318–320 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (name-distinctions / Beza ἡ ἁμαρτία) "
        "/ before GAR reply"
    )
    remain = "Cap. IX GAR/Beza interchange+ / rest of Cap. IX / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. IX name-distinctions tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap9_names",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. IX name-distinctions (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Caput IX (name-distinctions / Beza ἡ ἁμαρτία) before GAR "
            "reply, reconstructed from IA PDF page images with Vision OCR and "
            "pdftotext as check. No modern English was copied. Cap. IX GAR/Beza+ remain."
        ),
        "witnesses": wits,
    }
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX names densify from locked 1661 Latin "
            "(pp. 318–320 tip); Pass A != B."
            if int(sid) >= 485
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX names densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput IX (name-distinctions / Beza) "
                "before GAR reply from locked 1661 Saumur Latin "
                "(pp. 318–320 tip). Honest partial — Cap. IX GAR/Beza+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX names densify (name-distinctions → before GAR)

- Before: **{BEFORE}** sections (through Cap. IX four proofs tip)
- After: **{AFTER}** sections (through Cap. IX name-distinctions tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 318–320 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: παράπτωμα; venenum; Seth–Kenan; Beza ἡ ἁμαρτία; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_485–490; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5062 or 12m (`sources/_ocr_cap9_names/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_485-490", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — GAR reply on Beza / ἡ ἁμαρτία",
    }
