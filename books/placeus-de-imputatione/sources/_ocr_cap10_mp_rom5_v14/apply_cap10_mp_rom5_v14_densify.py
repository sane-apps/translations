# -*- coding: utf-8 -*-
"""Man. Post. Cap. X densify — Rom. 5:14 Veniamus / typus futuri."""
...
OCR = BOOK / "sources/_ocr_cap10_mp_rom5_v14"
...
HOLD_START_PATH = Path("/tmp/placeus_cap10_mp_rom5_v14_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5955
PACKET_STEM = "cap10_mp_rom5_v14_densify"
LOCK_NAME = "_placeus_cap10_mp_rom5_v14_latin_lock.txt"
JX_PREFIX = "cap10_mp_rom5_v14"
...
Scope: Manuscriptum posterius Cap. X — PLAC on reatus/corruption after GAR.3, printed pp. 370–372 tip (Rom. 5:14). Stops before further Cap. X. Not whole Cap. X / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. X after v.14 tip (act/habit continue) / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    if os.environ.get("PLACEUS_HOLD_ASYNC") == "1":
        ...
        receipt = {
            ...
            "note": "tip-ready STOP; parent commits+ships; OWNER REVIEW — no next densify; Punch X=NO",
            "next_slice": "OWNER_REVIEW_HALT",
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.X v14 densify tip-706 to 712; need live>5955 or 12m; Punch X=NO; parent ships; no next densify",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. X Rom. 5:14 densify section {sid} "
                "from locked 1661 Latin (pp. 370–372 tip, Rom. 5:14)."
            ),
        }
...
def main() -> None:
    ...
    remain = "Cap. X Rom. 5:14+ / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. X Rom. 5:14 tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap10_mp_rom5_foedus_densify.packet.json").read_text(encoding="utf-8"))
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. X Rom. 5:14 densify from locked 1661 Latin "
            "(pp. 370–372 tip); Pass A != B."
            if int(sid) >= 707
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. X Rom. 5:14 densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput X Rom. 5:14 tip "
                "from locked 1661 Saumur Latin "
                "(pp. 370–372 tip). Honest partial — Cap. X after v.14 remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. X Rom. 5:14 densify

- Before: **{BEFORE}** sections (through Cap. X foedus tip)
- After: **{AFTER}** sections (through Cap. X Rom. 5:14 tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 370–372 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: typus futuri; reatus habitualis; Pighius; Punch X=NO
- Gates: `check_pass_ab` ok {JX_PREFIX}_707–712; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5955 or 12m (`sources/_ocr_cap10_mp_rom5_v14/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_707-712", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. X — after v.14 tip (act/habit continue); OWNER REVIEW — no auto densify",
    }
