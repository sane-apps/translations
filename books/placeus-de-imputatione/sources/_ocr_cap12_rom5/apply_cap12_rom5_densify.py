# -*- coding: utf-8 -*-
"""Cap. XII Tranfit vers. XIX densify: Daubuz Rom 5:19 / Chamier causal / Blancus — before Cap. XIV."""
...
OCR = BOOK / "sources/_ocr_cap12_rom5"
...
Scope: Caput XII Tranfit… vers. XIX (Daubuz Rom 5:19 / Chamier causal / Blancus 1648 Th. XLI–XLIII) through Nero–Caligula reductio (printed pp. 144–151 tip). Stops before Caput XIV (manuscript rationes). Not Cap. XIV+.
Reconstruction: IA PDF page images via macOS Vision OCR + pdftotext; long-s/ligature OCR corrected. Locked Greek κατάστασις / ἁμαρτία / παράπτωμα / ἁμαρτίαν / φρονημάτων (OCR soft). Do not revive OCR ἔργον.
No modern English used as copy-text.
Remaining: Cap. XIV+ / Cap. XIII+ of the ~494-page Disputatio.
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. XII Rom 5:19 densify section {sid} "
                "from locked 1661 Latin (pp. 144–151 tip)."
            ),
        }
...
def main() -> None:
    ...
    lock_path = BOOK / "sources/_placeus_cap12_rom5_latin_lock.txt"
    ...
    stop = (
        "Cap. XII Tranfit vers. XIX / Daubuz Rom 5:19 / Chamier causal / "
        "Confession XI / Blancus 1648 / Nero–Caligula close / before Cap. XIV"
    )
    remain = "Cap. XIV+ / manuscript examination / Cap. XIII+ remain."
    meta.update({
        "title": f"De imputatione primi peccati Adami (Cap. I–XII partial: through {stop})",
        "edition": (
            "De imputatione primi peccati Adami (Salmurii: Apud Ioannem Lesnerium, 1661). "
            "IA deimputationepri00lapl. Densify: Capita I–XI + Caput XII through Tranfit "
            "vers. XIX (Daubuz Rom 5:19 / Chamier causal / Blancus) before Cap. XIV. "
    ...
    if not any(str(w.get("path", "")).endswith("_placeus_cap12_rom5_latin_lock.txt") for w in wits):
        wits.append({
            "id": "latin_lock_cap12_rom5",
            "path": "sources/_placeus_cap12_rom5_latin_lock.txt",
            "role": "copy-text",
            "name": (
                "Locked Latin, Caput XII Tranfit vers. XIX Rom 5:19 / Chamier / Blancus "
                "before Cap. XIV (Saumur 1661)"
            ),
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Tranfit vers. XIX (Daubuz Rom 5:19; Chamier causaliter; Confession XI; Blancus 1648 "
            "Th. XLI–XLIII; Nero–Caligula reductio) before Cap. XIV, reconstructed from IA PDF "
            "page images with Vision OCR and pdftotext as check. No modern English was copied. "
            "Cap. XIV+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads((BOOK / "reviews/audit/cap12_open_densify.packet.json").read_text(encoding="utf-8"))
    ...
    pkt_path = BOOK / "reviews/audit/cap12_rom5_densify.packet.json"
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. XII Rom 5:19 densify from locked 1661 Latin "
            "(pp. 144–151 tip); Pass A != B."
            if int(sid) >= 145
            else (
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. XII Rom 5:19 densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XII through Cap. XII Tranfit vers. XIX (Daubuz Rom 5:19; Chamier causaliter; "
                "Confession XI; Blancus 1648 Th. XLI–XLIII; Nero–Caligula) before Cap. XIV from locked "
                "1661 Saumur Latin (pp. 144–151 tip). Honest partial — Cap. XIV+ remain. "
                "Reformed Saumur era disclosed. Locked Greek κατάστασις / ἁμαρτία / παράπτωμα / "
                "ἁμαρτίαν / φρονημάτων; do not revive OCR ἔργον. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    rev_path = BOOK / "reviews/audit/cap12_rom5_densify.review.json"
    ...
    block = """## 2026-09-22 Cap. XII Rom 5:19 densify (Tranfit vers. XIX / Chamier / Blancus → before Cap. XIV)
