# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — GAR/Beza ἡ ἁμαρτία; tip before GAR Cap. VII § IX."""
...
OCR = BOOK / "sources/_ocr_cap9_beza"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_beza_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5099
PACKET_STEM = "cap9_beza_densify"
LOCK_NAME = "_placeus_cap9_beza_latin_lock.txt"
JX_PREFIX = "cap9_beza"
...
Scope: Manuscriptum posterius Cap. IX — GAR reply on Beza / ἡ ἁμαρτία; Placeus defense; second reason εἰς τὸν κόσμον (printed pp. 320–322 tip). Stops before GAR Cap. VII § IX (actual by reatus). Not whole Cap. IX / Disputatio.
...
Remaining: Cap. IX GAR Cap. VII § IX+ / rest of Cap. IX / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX beza densify tip-490; need live>5099 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX beza densify section {sid} "
                "from locked 1661 Latin (pp. 320–322 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (GAR/Beza ἡ ἁμαρτία / εἰς τὸν κόσμον) "
        "/ before GAR Cap. VII § IX"
    )
    remain = "Cap. IX GAR Cap. VII § IX+ / rest of Cap. IX / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. IX GAR/Beza tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_names_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap9_beza",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. IX GAR/Beza (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Caput IX (GAR/Beza ἡ ἁμαρτία) before GAR Cap. VII § IX, "
            "reconstructed from IA PDF page images with Vision OCR and "
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX beza densify from locked 1661 Latin "
            "(pp. 320–322 tip); Pass A != B."
            if int(sid) >= 491
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX beza densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput IX (GAR/Beza ἡ ἁμαρτία) "
                "before GAR Cap. VII § IX from locked 1661 Saumur Latin "
                "(pp. 320–322 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX beza densify (GAR/Beza → before Cap. VII § IX)

- Before: **{BEFORE}** sections (through Cap. IX name-distinctions tip)
- After: **{AFTER}** sections (through Cap. IX GAR/Beza tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 320–322 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: Beza Annot.; ἀχώριστα; εἰς τὸν κόσμον; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_491–496; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5099 or 12m (`sources/_ocr_cap9_beza/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_491-496", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — GAR Cap. VII § IX (actual by reatus / introiit)",
    }
