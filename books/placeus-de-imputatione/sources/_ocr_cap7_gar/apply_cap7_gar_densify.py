# -*- coding: utf-8 -*-
"""Man. Post. Cap. VII densify — Cap. VII GAR Augustine reply / representation / liturgy tip; before further will-assertion."""
...
OCR = BOOK / "sources/_ocr_cap7_gar"
...
HOLD_START_PATH = Path("/tmp/placeus_cap7_gar_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4683
PACKET_STEM = "cap7_gar_densify"
LOCK_NAME = "_placeus_cap7_gar_latin_lock.txt"
JX_PREFIX = "cap7_gar"
...
Scope: Manuscriptum posterius Cap. VII — GAR Augustine reply on Adam's will / representation / French liturgy (printed pp. 278 tip–282 tip). Stops before further will-assertion deepen. Not whole Cap. VII / Disputatio.
...
Remaining: Cap. VII further will-assertion+ / third similitude / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VII gar densify tip-401 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VII gar densify section {sid} "
                "from locked 1661 Latin (pp. 278 tip–282 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VII (Examen trium similitudinum; GAR Augustine/representation tip) "
        "/ before further will-assertion"
    )
    remain = "Cap. VII further will-assertion+ / third similitude / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VII GAR Augustine/representation tip (before further will-assertion). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap7_thomas_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap7_gar",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VII GAR Augustine/representation tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VII GAR Augustine/representation tip) before further will-assertion, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VII further will-assertion deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VII gar densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VII GAR Augustine/representation tip) before further will-assertion from locked 1661 Saumur Latin "
                "(pp. 278 tip–282 tip). Honest partial — Cap. VII further will-assertion deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VII gar densify (GAR Augustine/representation → before further will-assertion)

- Before: **{BEFORE}** sections (through Cap. VII Thomas/body tip)
- After: **{AFTER}** sections (through Cap. VII GAR Augustine/representation/liturgy tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 278 tip–282 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: Augustine De Nupt. / Chamier / repraesentationis jus / liturgia; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_402–407; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap7_gar/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_402-407", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VII — further Adam-will assertion / third similitude",
    }
