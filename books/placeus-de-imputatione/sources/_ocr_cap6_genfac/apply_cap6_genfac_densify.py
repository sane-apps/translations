# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — generative faculty moral quarrel (GAR Unde / Decalogue) tip; before last-end deepen."""
...
OCR = BOOK / "sources/_ocr_cap6_genfac"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_genfac_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4399
PACKET_STEM = "cap6_genfac_densify"
LOCK_NAME = "_placeus_cap6_genfac_latin_lock.txt"
JX_PREFIX = "cap6_genfac"
...
Scope: Manuscriptum posterius Cap. VI generative faculty moral quarrel (GAR Decalogue / three respects / many last ends / mind-only tip) (printed pp. 247 tip–250 tip). Stops before last-end deepen. Not Caput VII+.
...
Remaining: Cap. VI last-end deepen+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI genfac densify tip-329 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI genfac densify section {sid} "
                "from locked 1661 Latin (pp. 247 tip–250 tip)."
            ),
        }
...
def main() -> None:
    ...
    remain = "Cap. VI last-end deepen+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI generative-faculty quarrel tip (before last-end deepen). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_infants_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_genfac",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI generative-faculty quarrel tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI last-end deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI genfac densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "(pp. 247 tip–250 tip). Honest partial — Cap. VI last-end deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. VI genfac densify (generative moral quarrel → before last-end deepen)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VI infants/Confessio/Pighius tip / before generative-faculty quarrel)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI generative-faculty quarrel tip / before last-end deepen)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 247 tip–250 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: generative faculty / Decalogue / last end; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_330–335; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap6_genfac/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "next_locus": "Man. Post. Cap. VI last-end / pleasure-vs-generation deepen",
    }
