# -*- coding: utf-8 -*-
"""Man. Post. Cap. VIII densify — Hypognosticon / infant / interdict silence; tip before sentence syllogism."""
...
OCR = BOOK / "sources/_ocr_cap8_hypo"
...
HOLD_START_PATH = Path("/tmp/placeus_cap8_hypo_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4840
PACKET_STEM = "cap8_hypo_densify"
LOCK_NAME = "_placeus_cap8_hypo_latin_lock.txt"
JX_PREFIX = "cap8_hypo"
...
Scope: Manuscriptum posterius Cap. VIII — Hypognosticon / infant time / interdict silence / event reply (printed pp. 300 tip–302 tip). Stops before sentence/common-penalty syllogism. Not whole Cap. VIII / Disputatio.
...
Remaining: Cap. VIII sentence syllogism+ / rest of Cap. VIII / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VIII hypo densify tip-436 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VIII hypo densify section {sid} "
                "from locked 1661 Latin (pp. 300 tip–302 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VIII (Hypognosticon / infant / interdict silence) "
        "/ before sentence syllogism"
    )
    remain = "Cap. VIII sentence syllogism+ / rest of Cap. VIII / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VIII Hypognosticon / interdict silence (before sentence syllogism). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap8_ot_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap8_hypo",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VIII Hypognosticon / interdict silence (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VIII Hypognosticon / interdict silence) before sentence syllogism, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VIII sentence syllogism deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. VIII hypo densify from locked 1661 Latin "
            "(pp. 300 tip–302 tip); Pass A != B."
            if int(sid) >= 437
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VIII hypo densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VIII Hypognosticon / interdict silence) before sentence syllogism from locked 1661 Saumur Latin "
                "(pp. 300 tip–302 tip). Honest partial — Cap. VIII sentence syllogism+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VIII hypo densify (Hypognosticon / infant / silence → before sentence syllogism)

- Before: **{BEFORE}** sections (through Cap. VIII OT tree interdict / times tip)
- After: **{AFTER}** sections (through Cap. VIII Hypognosticon / event-reply tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 300 tip–302 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_437–442; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap8_hypo/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_437-442", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VIII — sentence / common-penalty syllogism",
    }
