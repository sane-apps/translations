# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — GAR Cap. VII § IX actual/reatus/introiit; tip before GAR Resp. 1."""
...
OCR = BOOK / "sources/_ocr_cap9_reatus"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_reatus_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5117
PACKET_STEM = "cap9_reatus_densify"
LOCK_NAME = "_placeus_cap9_reatus_latin_lock.txt"
JX_PREFIX = "cap9_reatus"
...
Scope: Manuscriptum posterius Cap. IX — GAR Cap. VII § IX (actual by reatus / introiit; justification scope; singular act ≠ imputari) printed pp. 322–323 tip. Stops before GAR Cap. VIII Resp. 1 (innate corruption). Not whole Cap. IX / Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX reatus densify tip-496; need live>5117 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX reatus densify section {sid} "
                "from locked 1661 Latin (pp. 322–323 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (GAR Cap. VII § IX actual/reatus/introiit) "
        "/ before GAR Resp. 1"
    )
    remain = "Cap. IX GAR Resp. 1+ / rest of Cap. IX / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. IX GAR Cap. VII § IX tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_beza_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap9_reatus",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. IX GAR Cap. VII § IX (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Caput IX (GAR Cap. VII § IX actual/reatus) before GAR "
            "Resp. 1, reconstructed from IA PDF page images with Vision OCR and "
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX reatus densify from locked 1661 Latin "
            "(pp. 322–323 tip); Pass A != B."
            if int(sid) >= 497
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX reatus densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput IX (GAR Cap. VII § IX) "
                "before GAR Resp. 1 from locked 1661 Saumur Latin "
                "(pp. 322–323 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX reatus densify (Cap. VII § IX → before GAR Resp. 1)

- Before: **{BEFORE}** sections (through Cap. IX GAR/Beza tip)
- After: **{AFTER}** sections (through Cap. IX GAR Cap. VII § IX tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 322–323 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: inhaesione/reatu; introiit ≠ imputari; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_497–502; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5117 or 12m (`sources/_ocr_cap9_reatus/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_497-502", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — GAR Cap. VIII Resp. 1 (innate corruption / introire)",
    }
