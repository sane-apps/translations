# -*- coding: utf-8 -*-
"""Cap. IV Secunda ratio densify — hoc fac et vives / Garissolius silence / Tertia; before three rationes."""
...
OCR = BOOK / "sources/_ocr_cap4_secunda_ratio"
...
HOLD_START_PATH = Path("/tmp/placeus_cap4_secunda_ratio_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 3860  # tip-224 catch-up ship in flight; need live>3860 or 12m
PACKET_STEM = "cap4_secunda_ratio_densify"
LOCK_NAME = "_placeus_cap4_secunda_ratio_latin_lock.txt"
JX_PREFIX = "cap4_secunda_ratio"
...
Scope: Manuscriptum posterius Cap. IV Secunda ratio (hoc fac et vives), Garissolius silence, Tertia through GAR 2 formal/causal and Placeus actualis/habitualis (printed pp. 195 tip–198 tip). Stops before Garissoles three rationes against formal identity. Not Cap. IV remainder+.
Reconstruction: IA PDF page images via macOS Vision OCR + pdftotext + DJVU; long-s/ligature OCR corrected. No new Greek lemma. Do not revive ἔργον.
No modern English used as copy-text.
Remaining: Cap. IV Tertia GAR three rationes+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "tip-224 catch-up ship in flight (live 57/3860 context); re-probe before append; need live>3860 or 12m",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IV Secunda ratio densify section {sid} "
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Cap. IV Secunda ratio / Garissolius silence / Tertia "
        "through GAR 2 + PLAC actualis/habitualis / before three rationes"
    )
    remain = (
        "Cap. IV Tertia GAR three rationes+ / rest of MS examination / ~494 pp Disputatio remain."
    )
    meta.update({
        ...
        "edition": (
            ...
            "through Cap. IV Secunda/Tertia (before Garissoles three rationes). "
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads(
                (BOOK / "reviews/audit/cap4_gar_cameron_densify.packet.json").read_text(
                    encoding="utf-8"
                )
            )
            wits = list(
    ...
    old_p = json.loads(
        (BOOK / "reviews/audit/cap4_gar_cameron_densify.packet.json").read_text(encoding="utf-8")
    )
    ...
    review = {
        ...
        "scope_review": {
            ...
            "notes": (
                ...
                "(pp. 195 tip–198 tip). Honest partial — Cap. IV Secunda+ remain. "
    ...
    block = f"""## 2026-09-22 Cap. IV Secunda ratio densify (hoc fac et vives / silence / Tertia → before three rationes)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. IV GAR Cameron–Amyraut / before Secunda)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. IV Secunda / Garissolius silence / Tertia GAR–PLAC actualis/habitualis / before three rationes)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 195 tip–198 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: no new Greek lemma this slice; do not revive ἔργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_225–231; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: tip-224 catch-up ship in flight; eng/src append after live>3860 or 12m (`sources/_ocr_cap4_secunda_ratio/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_225-231",
        "tip_ready": "ok",
        "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IV Tertia GAR three rationes (velle/nolle; separability; actus vs habitus)",
        "prior_packet_done": "cap4_gar_cameron_densify (218→224)",
    }
