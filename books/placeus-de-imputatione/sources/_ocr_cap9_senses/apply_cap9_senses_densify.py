# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — three senses of imputation; tip before omit/death dilemma."""
...
OCR = BOOK / "sources/_ocr_cap9_senses"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_senses_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5195
PACKET_STEM = "cap9_senses_densify"
LOCK_NAME = "_placeus_cap9_senses_latin_lock.txt"
JX_PREFIX = "cap9_senses"
...
Scope: Manuscriptum posterius Cap. IX — three senses of imputation (mind / declaration / penalty); hereditary nearest subject of 'entered' (printed pp. 328–329 tip). Stops before omit-inhering / death-name dilemma worked. Not whole Cap. IX / Disputatio.
...
Remaining: Cap. IX omit/death dilemma+ / rest of Cap. IX / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX senses densify tip-520; need live>5195 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX senses densify section {sid} "
                "from locked 1661 Latin (pp. 328–329 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (three senses of imputation / hereditary nearest) "
        "/ before omit-inhering dilemma"
    )
    remain = "Cap. IX omit/death dilemma+ / rest of Cap. IX / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. IX three senses tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_imputatum_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap9_senses",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. IX three senses of imputation (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Caput IX (three senses of imputation) before omit-inhering "
            "dilemma, reconstructed from IA PDF page images with Vision OCR and "
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX senses densify from locked 1661 Latin "
            "(pp. 328–329 tip); Pass A != B."
            if int(sid) >= 521
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX senses densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput IX (three senses of imputation) "
                "before omit-inhering dilemma from locked 1661 Saumur Latin "
                "(pp. 328–329 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX senses densify (three senses → before omit dilemma)

- Before: **{BEFORE}** sections (through Cap. IX imputatum-est reductio tip)
- After: **{AFTER}** sections (through Cap. IX three senses tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 328–329 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: immanens; declaratio; mors vs peccatum; hereditary nearest; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_521–526; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5195 or 12m (`sources/_ocr_cap9_senses/hold_receipt.json`; released {hold_note})
- Stopped before Cap. IX omit-inhering / death-name dilemma…
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_521-526", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — omit-inhering / death-name dilemma (Pighius / Pelagians)",
    }
