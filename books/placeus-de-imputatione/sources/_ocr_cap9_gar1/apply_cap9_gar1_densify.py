# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — GAR Resp.1; tip before GAR 2."""
...
OCR = BOOK / "sources/_ocr_cap9_gar1"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_gar1_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5269
PACKET_STEM = "cap9_gar1_densify"
LOCK_NAME = "_placeus_cap9_gar1_latin_lock.txt"
JX_PREFIX = "cap9_gar1"
...
Scope: Manuscriptum posterius Cap. IX — GAR Resp.1 (aliud est peccare / in alio peccavisse) + PLAC reply printed pp. 335–336 tip. Stops before GAR 2 (peccandi verbum = reatus). Not whole Cap. IX / Disputatio.
...
Remaining: Cap. IX GAR 2+ / rest of Cap. IX / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX ephho densify tip-538→544; need live>5269 or 12m; Punch X=NO",
    }
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                ...
                "from locked 1661 Latin (pp. 335–336 tip)."
            ),
        }
...
def main() -> None:
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_ephho_densify.packet.json").read_text(encoding="utf-8"))
    ...
    review = {
        ...
        "scope_review": {
            ...
            "notes": (
                ...
                "(pp. 335–336 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
...
- Before: **{BEFORE}** sections (through Cap. IX eph-ho tip)
- After: **{AFTER}** sections (through Cap. IX GAR Resp.1 tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 335–336 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_545–550; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5269 or 12m (`sources/_ocr_cap9_gar1/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_545-550", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — GAR 2 (peccandi verbum aliquando = reatus / Judah Gen 43)",
    }
