# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — Cap. VII first similitude separability / judicial-action deepen tip; before penalties / meritorie."""
...
OCR = BOOK / "sources/_ocr_cap7_sep"
...
HOLD_START_PATH = Path("/tmp/placeus_cap7_sep_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4557
PACKET_STEM = "cap7_sep_densify"
LOCK_NAME = "_placeus_cap7_sep_latin_lock.txt"
JX_PREFIX = "cap7_sep"
...
Scope: Manuscriptum posterius Cap. VII first similitude — separability / judicial-action deepen (printed pp. 263 tip–265 tip). Stops before penalties / meritorie grace comparison. Not whole Cap. VII / Disputatio.
...
Remaining: Cap. VII penalties/meritorie/sons+ / second–third similitudes / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VII sep densify tip-371 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VII sep densify section {sid} "
                "from locked 1661 Latin (pp. 263 tip–265 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VII (Examen trium similitudinum; separability/judicial tip) "
        "/ before penalties / meritorie deepen"
    )
    remain = "Cap. VII penalties / meritorie / sons-grandsons+ / rest of three similitudes / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VII first-similitude separability/judicial tip (before penalties deepen). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap7_simil_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap7_sep",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VII separability/judicial tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VII separability/judicial tip) before penalties deepen, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VII penalties/meritorie deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VII sep densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VII separability/judicial tip) before penalties deepen from locked 1661 Saumur Latin "
                "(pp. 263 tip–265 tip). Honest partial — Cap. VII penalties/meritorie deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VII sep densify (separability / judicial-action → before penalties)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VII first courtier tip / before separability deepen)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VII separability/judicial tip / before penalties deepen)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 263 tip–265 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_372–377; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap7_sep/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_372-377", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VII — penalties / dying-you-die / meritorie grace comparison",
    }
