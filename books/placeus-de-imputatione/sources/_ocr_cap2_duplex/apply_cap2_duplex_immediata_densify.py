# -*- coding: utf-8 -*-
"""Cap. II duplex Immediata/Mediata densify — Est igitur quaestio… before §XIX."""
...
OCR = BOOK / "sources/_ocr_cap2_duplex"
...
HOLD_START_PATH = Path("/tmp/placeus_cap2_duplex_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 3696
PACKET_STEM = "cap2_duplex_immediata_densify"
LOCK_NAME = "_placeus_cap2_duplex_immediata_latin_lock.txt"
JX_PREFIX = "cap2_duplex"
...
Scope: Manuscriptum posterius Cap. II duplex Immediata/Mediata — Est igitur quaestio through Garissoles on mediate theologians / Placeus discrimen false (printed pp. 170 tip–175 tip). Stops before §XIX Tum in nomine imputationis. Not Cap. II §XIX+.
...
Remaining: Cap. II §XIX+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "tip-177 ship in flight; do not append eng/src until live>3696 or 12m",
    }
    while True:
        now = int(time.time())
        elapsed = now - start
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. II duplex densify section {sid} "
                "from locked 1661 Latin (pp. 170 tip–175 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Cap. II duplex Immediata/Mediata (Est igitur… Ego Nego; §XVI–XVII "
        "Garissoles Distinguitur / communem–singularem; Placeus discrimen false / before §XIX)"
    )
    remain = (
        "Cap. II §XIX+ / rest of MS examination / "
    ...
    meta.update({
        ...
        "edition": (
            ...
            "through duplex Immediata/Mediata (§XVI–XVII before §XIX). "
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads(
                (BOOK / "reviews/audit/cap2_s14_garissoles_densify.packet.json").read_text(
                    encoding="utf-8"
                )
            )
            wits = list(
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap2_duplex_immediata",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": (
                "Locked Latin, Man. Post. Cap. II duplex Immediata/Mediata before §XIX "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Cap. II duplex Immediata/Mediata (Est igitur quaestio; Ego Nego; "
            "§XVI–XVII Garissoles Distinguitur / communem–singularem; Placeus on mediate "
            "theologians) before §XIX Tum in nomine, reconstructed from IA PDF page images "
    ...
    old_p = json.loads(
        (BOOK / "reviews/audit/cap2_s14_garissoles_densify.packet.json").read_text(encoding="utf-8")
    )
    ...
    block = f"""## 2026-09-22 Cap. II duplex Immediata/Mediata densify (Est igitur… → before §XIX)
...
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 170 tip–175 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: no new Greek lemma this slice; do not revive ἔργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_178–184; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: tip-177 ship in flight; eng/src append after live>3696 or 12m (`sources/_ocr_cap2_duplex/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_178-184",
        "tip_ready": "ok",
        "hold": hold_note,
