# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — eph ho / eph ho pantes; tip before GAR Resp.1."""
...
OCR = BOOK / "sources/_ocr_cap9_ephho"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_ephho_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5243
PACKET_STEM = "cap9_ephho_densify"
LOCK_NAME = "_placeus_cap9_ephho_latin_lock.txt"
JX_PREFIX = "cap9_ephho"
...
Scope: Manuscriptum posterius Cap. IX — eph' ho / eph' ho pantes (in quo / because all sinned) printed pp. 333–335 tip. Stops before GAR Resp.1 (peccare vs in alio peccavisse). Not whole Cap. IX / Disputatio.
...
Remaining: Cap. IX GAR Resp.1+ / rest of Cap. IX / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX ephho densify tip-538→544; need live>5243 or 12m; Punch X=NO",
    }
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                ...
                "from locked 1661 Latin (pp. 333–335 tip)."
            ),
        }
...
def main() -> None:
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_allsinners_densify.packet.json").read_text(encoding="utf-8"))
    ...
    review = {
        ...
        "scope_review": {
            ...
            "notes": (
                ...
                "(pp. 333–335 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
...
- Before: **{BEFORE}** sections (through Cap. IX all-are-sinners tip)
- After: **{AFTER}** sections (through Cap. IX eph-ho tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 333–335 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_539–544; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5243 or 12m (`sources/_ocr_cap9_ephho/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_539-544", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — GAR Resp.1 (aliud est peccare / in alio peccavisse)",
    }
