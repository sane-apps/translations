# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — Calvin Tertio — nature vs judgment tip; before Cap. VII three similitudes."""
...
OCR = BOOK / "sources/_ocr_cap6_tertio"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_tertio_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4519
PACKET_STEM = "cap6_tertio_densify"
LOCK_NAME = "_placeus_cap6_tertio_latin_lock.txt"
JX_PREFIX = "cap6_tertio"
...
Scope: Manuscriptum posterius Cap. VI Calvin Tertio — nature vs judgment (printed pp. 259 tip–260 tip). Stops before Cap. VII three similitudes. Not whole Disputatio.
...
Remaining: Cap. VII three similitudes+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI tertio densify tip-359 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI tertio densify section {sid} "
                "from locked 1661 Latin (pp. 259 tip–260 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VI (Examen Cap. IV / causa condemnationis; Calvin Tertio tip) "
        "/ before Cap. VII three similitudes"
    )
    remain = "Cap. VII three similitudes+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI Calvin Tertio tip (before Cap. VII three similitudes). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_judloc_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_tertio",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI Calvin Tertio tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Calvin Tertio tip) before Cap. VII three similitudes, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VII three similitudes+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI tertio densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Calvin Tertio tip) before Cap. VII from locked 1661 Saumur Latin "
                "(pp. 259 tip–260 tip). Honest partial — Cap. VII three similitudes+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VI tertio densify (Calvin nature vs judgment → before Cap. VII)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VI judgment-locution tip / before Calvin Tertio)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI Calvin Tertio tip / before Cap. VII three similitudes)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 259 tip–260 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: Calvin / natura vs judicium / consilium / paradoxum; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_360–365; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap6_tertio/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_360-365", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VII — Examen trium similitudinum",
    }
