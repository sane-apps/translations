# -*- coding: utf-8 -*-
"""Cap. VII Quarta ratio densify: Sequitur ratio quarta through Sed pergamus (before Quinta)."""
...
OCR = BOOK / "sources/_ocr_cap7_quarta"
...
Scope: Caput VII Quarta ratio — Sequitur ratio quarta through Garissoles Resp. 1–2 and Placeus two-modes / Calvin Rom. 5:12 / Deo injuriosa close at Sed pergamus (printed pp. 82–86 tip). Stops before Quinta & ultima ratio / further Cap. VII. Not Cap. VIII+.
Reconstruction: IA PDF page images via macOS Vision OCR + tesseract + pdftotext; long-s/ligature OCR corrected; Greek ἐφ᾽ ᾧ πάντες ἥμαρτον confirmed against page image (crop soft).
No modern English used as copy-text.
Remaining: Cap. VII Quinta & ultima ratio onward and Cap. VIII+ of the ~494-page Disputatio.
...
def main() -> None:
    ...
    lock_path = BOOK / "sources/_placeus_cap7_quarta_latin_lock.txt"
    ...
    assert len(eng) == 53 and len(src) == 53, (len(eng), len(src))
    pre_eng = digest(eng)
    pre_src = digest(src)
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": f"Cap. VII Quarta densify section {sid} from locked 1661 Latin (pp. 82–86 tip).",
        }
    ...
    assert digest(eng[:53]) == pre_eng, "sections 1-53 english mutated"
    assert digest(src[:53]) == pre_src, "sections 1-53 source mutated"

    stop = "Cap. VII Quarta ratio through Sed pergamus (before Quinta)"
    meta.update({
        ...
        "edition": (
            ...
            "Densify: Capita I–VI + Caput VII through Quarta ratio (Sequitur ratio quarta; Garissoles Resp. 1–2; "
            "Placeus two-modes / Calvin Rom. 5:12 / Deo injuriosa close at Sed pergamus). "
            "Not whole Disputatio; Cap. VII Quinta & ultima ratio onward and Cap. VIII+ remain."
        ),
        "blurb": (
            "Josue de la Place (Placeus) — Capita I–VII partial of De imputatione primi peccati Adami from the 1661 Saumur Latin, "
            f"through {stop}. Honest partial; Cap. VII Quinta onward and later Capita remain."
        ),
        "first_english_note": (
    ...
    if not any(str(w.get("path", "")).endswith("_placeus_cap7_quarta_latin_lock.txt") for w in wits):
        wits.append({
            "id": "latin_lock_cap7_quarta",
            "path": "sources/_placeus_cap7_quarta_latin_lock.txt",
            "role": "copy-text",
            "name": "Locked Latin, Caput VII Quarta ratio through Sed pergamus (Saumur 1661)",
            "language": "Latin",
            "url": "https://archive.org/details/deimputationepri00lapl",
        })
    meta["text_history"] = {
        "method": (
            "English follows the locked 1661 Saumur Latin of Capita I–VII partial through Cap. VII Quarta ratio "
            "(Sequitur ratio quarta through Sed pergamus before Quinta), reconstructed from IA PDF page images with Vision OCR, tesseract, "
            "and pdftotext as check. No modern English was copied. Cap. VII Quinta & ultima ratio onward and Cap. VIII+ remain."
        ),
        "witnesses": wits,
    }
    ...
    packet["sample_size"] = 58
    packet["expected_sections"] = [str(i) for i in range(1, 59)]
    packet["explicit_selected_sections"] = packet["expected_sections"][:]
    packet["coverage"] = {"expected": 58, "english": 58, "source": 58}
    packet["identity"]["work"] = meta["title"]
    packet["identity"]["edition"] = meta["edition"]
    packet["identity"]["locus_aliases"] = {str(i): str(i) for i in range(1, 59)}
    packet["publication_scope"]["title"] = meta["title"]
    packet["publication_scope"]["section_count"] = 58
    ...
    pkt_path = BOOK / "reviews/audit/cap7_quarta_ratio_densify.packet.json"
    ...
    for sid in packet["expected_sections"]:
        ...
        reviews.append({
            ...
            "notes": f"Section {sid}: compared Pass B to locked 1661 Latin Cap. VII Quarta ratio; Pass A != B.",
        })

    review = {
        "packet_id": packet["packet_id"],
        "reviewer": "scribe-placeus / Placeus Cap. VII Quarta ratio densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [
                "Sections 57–58: Greek ἐφ᾽ ᾧ / πάντες ἥμαρτον page-OCR soft on Vision crop; locked to Cap. VII lemma (do not revive ἔργον).",
                "Section 58: vocibus ἐφ᾽ ᾧ ita is OCR-soft (Vision 'vocibus e ita'); sense locked by Romans 5:12 lemma + pertinentibus clause."
            ],
            "notes": (
                "Cap. I–VI + Cap. VII through Quarta ratio (Sequitur ratio quarta → Sed pergamus before Quinta) "
                "from locked 1661 Saumur Latin (pp. 82–86 tip). Honest partial — Cap. VII Quinta & ultima ratio onward and Cap. VIII+ remain. "
                "Reformed Saumur era disclosed. Locked Greek ἐφ᾽ ᾧ / πάντες ἥμαρτον (do not revive OCR ἔργον). Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    rev_path = BOOK / "reviews/audit/cap7_quarta_ratio_densify.review.json"
