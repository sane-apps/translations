# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — GAR Resp. 1 Confession Art. 10."""
...
OCR = BOOK / "sources/_ocr_cap9_gar_resp1"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_gar_resp1_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5525
PACKET_STEM = "cap9_gar_resp1_densify"
LOCK_NAME = "_placeus_cap9_gar_resp1_latin_lock.txt"
JX_PREFIX = "cap9_gar_resp1"
...
Scope: Manuscriptum posterius Cap. IX — GAR Resp. 1 (Confession Art. 10) + PLAC + GAR 2 open printed pp. 347–348 tip. Stops mid GAR 2 / before full PLAC on mortem Adami imputari. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX GAR 2+ / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX PLAC vere densify tip-604→610; need live>5525 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX GAR Resp.1 densify section {sid} "
                "from locked 1661 Latin (pp. 347–348 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (GAR Resp.1 / Art.10 tip) "
        "/ before GAR 2 mortem Adami imputari"
    )
    remain = "Cap. IX GAR 2+ / rest Cap. IX / ~494 pp Disputatio remain."
    meta.update({
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_plac_evang_densify.packet.json").read_text(encoding="utf-8"))
    ...
    block = f"""## 2026-09-23 Cap. IX GAR Resp. 1 (Confession Art. 10) densify

- Before: **{BEFORE}** sections (through Cap. IX PLAC Rom5/communion tip)
- After: **{AFTER}** sections (through Cap. IX GAR Resp.1 / GAR 2 open tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 347–348 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: morti addictos; Art. 10; in ipsa Adami persona; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_605–610; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5525 or 12m (`sources/_ocr_cap9_gar_resp1/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_605-610", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — GAR 2 / PLAC on mortem Adami imputari (after Resp.1 tip)",
    }
