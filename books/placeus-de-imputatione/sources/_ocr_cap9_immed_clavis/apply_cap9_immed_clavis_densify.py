# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — PLAC immediate-imputation + Calvin Clavis."""
...
OCR = BOOK / "sources/_ocr_cap9_immed_clavis"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_immed_clavis_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5570
PACKET_STEM = "cap9_immed_clavis_densify"
LOCK_NAME = "_placeus_cap9_immed_clavis_latin_lock.txt"
JX_PREFIX = "cap9_immed_clavis"
...
Scope: Manuscriptum posterius Cap. IX — PLAC immediate/antecedent imputation expl + Calvin Clavis on peccatum printed pp. 351–352 tip. Stops before aptitudo/Sohnius/Zanchius actu block. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX aptitudo/Sohnius/Zanchius+ / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX PLAC vere densify tip-622→628; need live>5570 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX immed/Clavis densify section {sid} "
                "from locked 1661 Latin (pp. 351–352 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (immediate-imputation/Clavis tip) "
        "/ before aptitudo/Sohnius/Zanchius"
    )
    remain = "Cap. IX aptitudo/Sohnius/Zanchius+ / rest Cap. IX / ~494 pp Disputatio remain."
    meta.update({
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_rom5_omnes_densify.packet.json").read_text(encoding="utf-8"))
    ...
    block = f"""## 2026-09-23 Cap. IX PLAC immediate-imputation + Calvin Clavis densify

- Before: **{BEFORE}** sections (through Cap. IX Rom5 omnes/Jeroboam tip)
- After: **{AFTER}** sections (through Cap. IX Calvin Clavis / Rom5 habitual tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 351–352 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_623–628; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5570 or 12m (`sources/_ocr_cap9_immed_clavis/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_623-628", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — aptitudo / Sohnius / Zanchius actu (after Clavis tip)",
    }
