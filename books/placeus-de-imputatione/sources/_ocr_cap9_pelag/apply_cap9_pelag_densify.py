# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — GAR Pelagian telum / when corrupted."""
...
OCR = BOOK / "sources/_ocr_cap9_pelag"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_pelag_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5418
PACKET_STEM = "cap9_pelag_densify"
LOCK_NAME = "_placeus_cap9_pelag_latin_lock.txt"
JX_PREFIX = "cap9_pelag"
...
Scope: Manuscriptum posterius Cap. IX — GAR Pelagian telum (corruptos in Adamo / when corrupted) + PLAC reply printed pp. 342–343 tip. Stops before GAR 4 (act+corruption together). Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX GAR 4+ / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX ephho densify tip-538→544; need live>5418 or 12m; Punch X=NO",
    }
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                ...
                "from locked 1661 Latin (pp. 342–343 tip)."
            ),
        }
...
def main() -> None:
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_alius_densify.packet.json").read_text(encoding="utf-8"))
    ...
    review = {
        ...
        "scope_review": {
            ...
            "notes": (
                ...
                "(pp. 342–343 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
...
- Before: **{BEFORE}** sections (through Cap. IX alius-pro-nobis tip)
- After: **{AFTER}** sections (through Cap. IX Pelagian telum tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 342–343 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_581–586; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5418 or 12m (`sources/_ocr_cap9_pelag/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_581-586", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — GAR 4 (peccatum Adami = actio + corruptio consequens)",
    }
