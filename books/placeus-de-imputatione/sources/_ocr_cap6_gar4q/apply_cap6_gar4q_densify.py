# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — GAR erotema 4 actuale/habituale tip; before GAR Q5."""
...
OCR = BOOK / "sources/_ocr_cap6_gar4q"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_gar4q_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4244
PACKET_STEM = "cap6_gar4q_densify"
LOCK_NAME = "_placeus_cap6_gar4q_latin_lock.txt"
JX_PREFIX = "cap6_gar4q"
...
Scope: Manuscriptum posterius Cap. VI GAR erotema 4 (actuale vs habituale / five replies) through Placeus imputabile tip (printed pp. 236 tip–238 tip). Stops before GAR Q5 Cain–Seth. Not Caput VII+.
...
Remaining: Cap. VI GAR Q5+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI GAR Q4 densify tip-305 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI GAR Q4 densify section {sid} "
                "from locked 1661 Latin (pp. 236 tip–238 tip)."
            ),
        }
...
def main() -> None:
    ...
    remain = "Cap. VI GAR Q5+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI GAR erotema 4 tip (before GAR Q5). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_foedus_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_gar4q",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI GAR erotema 4 tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI GAR Q5+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI GAR Q4 densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "(pp. 236 tip–238 tip). Honest partial — Cap. VI GAR Q5+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. VI GAR erotema 4 densify (actuale/habituale → before Q5)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VI foedus-condition / erotemata 2–3 tip / before GAR Q4)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI GAR erotema 4 tip / before GAR Q5)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 236 tip–238 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: GAR erotema 4 actuale/habituale; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_306–311; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap6_gar4q/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_306-311", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VI GAR erotema 5 first-generative / Cain–Seth tip",
    }
