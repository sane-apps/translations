# -*- coding: utf-8 -*-
"""Man. Post. Cap. VI densify — GAR erotema 6 Christ-righteousness parallel tip; before infants/Pighius deepen."""
...
OCR = BOOK / "sources/_ocr_cap6_gar6q"
...
HOLD_START_PATH = Path("/tmp/placeus_cap6_gar6q_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4327
PACKET_STEM = "cap6_gar6q_densify"
LOCK_NAME = "_placeus_cap6_gar6q_latin_lock.txt"
JX_PREFIX = "cap6_gar6q"
...
Scope: Manuscriptum posterius Cap. VI GAR erotema 6 (Christ-righteousness / Adam-sin parallel; replies 2–3; Placeus mode/gift/author-of-sin; corpus reatus; infants Confessio tip) (printed pp. 240 tip–243 tip). Stops before deeper Pighius/infant clash. Not Caput VII+.
...
Remaining: Cap. VI infants/Pighius+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VI GAR Q6 densify tip-317 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VI GAR Q6 densify section {sid} "
                "from locked 1661 Latin (pp. 240 tip–243 tip)."
            ),
        }
...
def main() -> None:
    ...
    remain = "Cap. VI infants/Pighius+ / Caput VII+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VI GAR erotema 6 tip (before infants/Pighius deepen). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap6_gar5q_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap6_gar6q",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VI GAR erotema 6 tip (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VI infants/Pighius+ remain."
        ),
        "witnesses": wits,
    }
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VI GAR Q6 densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "(pp. 240 tip–243 tip). Honest partial — Cap. VI infants/Pighius+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. VI GAR erotema 6 densify (Christ-righteousness parallel → before infants/Pighius deepen)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. VI GAR erotema 5 tip / before Q6 deep)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. VI GAR erotema 6 tip / before infants/Pighius deepen)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 240 tip–243 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: GAR erotema 6 Christ-righteousness parallel; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_318–323; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap6_gar6q/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_318-323", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VI infants death / Confessio XI / Pighius deepen",
    }
