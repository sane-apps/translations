# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — PLAC contradiction in alio vs imputation order."""
...
OCR = BOOK / "sources/_ocr_cap9_contr"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_contr_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5340
PACKET_STEM = "cap9_contr_densify"
LOCK_NAME = "_placeus_cap9_contr_latin_lock.txt"
JX_PREFIX = "cap9_contr"
...
Scope: Manuscriptum posterius Cap. IX — PLAC contradiction (in alio peccare vs imputation order) printed pp. 338–340 tip. Stops before GAR jest charge on John 8:58 reductio. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX GAR jest / PLAC serio+ / rest of Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX ephho densify tip-538→544; need live>5340 or 12m; Punch X=NO",
    }
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                ...
                "from locked 1661 Latin (pp. 338–340 tip)."
            ),
        }
...
def main() -> None:
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_gar3b_densify.packet.json").read_text(encoding="utf-8"))
    ...
    review = {
        ...
        "scope_review": {
            ...
            "notes": (
                ...
                "(pp. 338–340 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
...
- Before: **{BEFORE}** sections (through Cap. IX GAR Resp.3 tip)
- After: **{AFTER}** sections (through Cap. IX PLAC contradiction tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 338–340 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_563–568; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5340 or 12m (`sources/_ocr_cap9_contr/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_563-568", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — GAR jest charge / PLAC serio on John 8:58 reductio",
    }
