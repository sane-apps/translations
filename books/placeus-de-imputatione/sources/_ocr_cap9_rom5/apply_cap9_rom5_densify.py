# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — Rom 5:12 open; tip before four proofs."""
...
OCR = BOOK / "sources/_ocr_cap9_rom5"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_rom5_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4988
PACKET_STEM = "cap9_rom5_densify"
LOCK_NAME = "_placeus_cap9_rom5_latin_lock.txt"
JX_PREFIX = "cap9_rom5"
...
Scope: Manuscriptum posterius Cap. IX — NT open / Rom 5:12 entry / method (printed pp. 313–316 tip). Stops before four proofs that 'sin entered' = Adam's actual. Not whole Cap. IX / Disputatio.
...
Remaining: Cap. IX four proofs+ / rest of Cap. IX / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX rom5 densify tip-472 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX rom5 densify section {sid} "
                "from locked 1661 Latin (pp. 313–316 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (Rom 5:12 open / method) "
        "/ before four proofs"
    )
    remain = "Cap. IX four proofs+ / rest of Cap. IX / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. IX Rom 5:12 open (before four proofs). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap8_abr_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap9_rom5",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. IX Rom 5:12 open (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. IX Rom 5:12 open) before four proofs, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. IX four proofs deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX rom5 densify from locked 1661 Latin "
            "(pp. 313–316 tip); Pass A != B."
            if int(sid) >= 473
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX rom5 densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. IX Rom 5:12 open) before four proofs from locked 1661 Saumur Latin "
                "(pp. 313–316 tip). Honest partial — Cap. IX four proofs+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX rom5 densify (Rom 5:12 open → before four proofs)

- Before: **{BEFORE}** sections (through Cap. VIII Abraham / OT-close tip)
- After: **{AFTER}** sections (through Cap. IX Rom 5:12 entry tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 313–316 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_473–478; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap9_rom5/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_473-478", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — four proofs / sin-entered = Adam actual",
    }
