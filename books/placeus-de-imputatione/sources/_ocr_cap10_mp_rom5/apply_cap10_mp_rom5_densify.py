# -*- coding: utf-8 -*-
"""Man. Post. Cap. X densify — Rom. 5.13 open through minor tip."""
...
OCR = BOOK / "sources/_ocr_cap10_mp_rom5"
...
HOLD_START_PATH = Path("/tmp/placeus_cap10_mp_rom5_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5721
PACKET_STEM = "cap10_mp_rom5_densify"
LOCK_NAME = "_placeus_cap10_mp_rom5_latin_lock.txt"
JX_PREFIX = "cap10_mp_rom5"
...
Scope: Manuscriptum posterius Cap. X — Rom. 5.13 open through 2500-year minor tip, printed pp. 357–359. Stops before GAR replies. Not whole Cap. X / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. X GAR replies+ / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.X Rom.5.13 densify tip-658 to 664; need live>5721 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. X Rom.5.13 densify section {sid} "
                "from locked 1661 Latin (pp. 357–359 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput X (Rom. 5.13 open tip) "
        "/ before GAR replies"
    )
    remain = "Cap. X GAR replies+ / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. X Rom. 5.13 open tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_missa_poena_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            ...
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. X Rom.5.13 open tip (Saumur 1661)",
            "language": "Latin",
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. X Rom.5.13 densify from locked 1661 Latin "
            "(pp. 357–359 tip); Pass A != B."
            if int(sid) >= 659
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. X Rom.5.13 densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput X Rom.5.13 open tip "
                "from locked 1661 Saumur Latin "
                "(pp. 357–359 tip). Honest partial — Cap. X GAR+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. X Rom. 5.13 open densify

- Before: **{BEFORE}** sections (through Cap. IX equity/Martyr tip)
- After: **{AFTER}** sections (through Cap. X Rom. 5.13 open tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 357–359 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: γάρ; usque ad legem; vitium originale inhaerens; Punch X=NO
- Gates: `check_pass_ab` ok {JX_PREFIX}_659–664; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5721 or 12m (`sources/_ocr_cap10_mp_rom5/hold_receipt.json`; released {hold_note})
- Stopped before Cap. X GAR.1+…
- Honest partial: Cap. X GAR+ / rest of MS examination / ~494 pp Disputatio remain (**do not claim full Disputatio**)
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_659-664", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. X — GAR replies on Rom. 5.13 (after open tip)",
    }
