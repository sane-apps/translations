# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — GAR 2 / mortem Adami imputari."""
...
OCR = BOOK / "sources/_ocr_cap9_gar2_mors"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_gar2_mors_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5525
PACKET_STEM = "cap9_gar2_mors_densify"
LOCK_NAME = "_placeus_cap9_gar2_mors_latin_lock.txt"
JX_PREFIX = "cap9_gar2_mors"
...
Scope: Manuscriptum posterius Cap. IX — GAR 2 / PLAC on mortem Adami imputari printed pp. 348–349 tip. Stops before Jeroboam / Rom 5 omnes syllogism. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX Rom5 omnes / GAR Resp. peccati / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX PLAC vere densify tip-610→616; need live>5525 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX GAR2 mortem densify section {sid} "
                "from locked 1661 Latin (pp. 348–349 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (GAR 2 mortem Adami tip) "
        "/ before Rom5 omnes syllogism"
    )
    remain = "Cap. IX Rom5 omnes+ / rest Cap. IX / ~494 pp Disputatio remain."
    meta.update({
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_gar_resp1_densify.packet.json").read_text(encoding="utf-8"))
    ...
    block = f"""## 2026-09-23 Cap. IX GAR 2 / mortem Adami imputari densify

- Before: **{BEFORE}** sections (through Cap. IX GAR Resp.1 / GAR 2 open tip)
- After: **{AFTER}** sections (through Cap. IX PLAC why-only-Adam / dual reasons tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 348–349 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: mortem Adami imputari; in Adamo morimur; omnes; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_611–616; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5525 or 12m (`sources/_ocr_cap9_gar2_mors/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_611-616", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — Rom 5 omnes / Jeroboam + peccaverunt syllogism (after mortem Adami tip)",
    }
