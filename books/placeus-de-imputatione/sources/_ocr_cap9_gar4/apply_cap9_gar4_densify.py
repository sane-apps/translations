# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — GAR 4 act+corruption; tip into GAR 5."""
...
OCR = BOOK / "sources/_ocr_cap9_gar4"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_gar4_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5444
PACKET_STEM = "cap9_gar4_densify"
LOCK_NAME = "_placeus_cap9_gar4_latin_lock.txt"
JX_PREFIX = "cap9_gar4"
...
Scope: Manuscriptum posterius Cap. IX — GAR 4 (peccatum Adami = actio + corruptio consequens) + PLAC + GAR 5 open printed pp. 343–344 tip. Stops mid GAR 5 / before full PLAC on vere-proprie. Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX GAR 5 / PLAC vere-proprie+ / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX ephho densify tip-538→544; need live>5444 or 12m; Punch X=NO",
    }
    ...
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                ...
                "from locked 1661 Latin (pp. 343–344 tip)."
            ),
        }
...
def main() -> None:
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_pelag_densify.packet.json").read_text(encoding="utf-8"))
...
- Before: **{BEFORE}** sections (through Cap. IX Pelagian telum tip)
- After: **{AFTER}** sections (through Cap. IX GAR 4 tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 343–344 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_587–592; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5444 or 12m (`sources/_ocr_cap9_gar4/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_587-592", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — PLAC on vere/falso & proprie after GAR 5 (Augustine Ep. 106)",
    }
