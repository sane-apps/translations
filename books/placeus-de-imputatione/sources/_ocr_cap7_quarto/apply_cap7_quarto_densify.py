# -*- coding: utf-8 -*-
"""Man. Post. Cap. VII densify — Quarto/Quinto king & Gal/Rom; Cap. VII tip before Cap. VIII."""
...
OCR = BOOK / "sources/_ocr_cap7_quarto"
...
HOLD_START_PATH = Path("/tmp/placeus_cap7_quarto_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4793
PACKET_STEM = "cap7_quarto_densify"
LOCK_NAME = "_placeus_cap7_quarto_latin_lock.txt"
JX_PREFIX = "cap7_quarto"
...
Scope: Manuscriptum posterius Cap. VII — Quarto king / Quinto Gal 6:5 Rom 14:12 / Cap. VII close (printed p. 296). Stops before Cap. VIII. Not whole Cap. VII / Disputatio.
...
Remaining: Cap. VIII OT arguments+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VII quarto densify tip-425 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VII quarto densify section {sid} "
                "from locked 1661 Latin (p. 296)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VII (Examen trium similitudinum; Quarto/Quinto close) "
        "/ before Cap. VIII"
    )
    remain = "Cap. VIII OT arguments+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VII Quarto/Quinto close (before Cap. VIII). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap7_ezek_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap7_quarto",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VII Quarto/Quinto close (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VII Quarto/Quinto close) before Cap. VIII, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VIII OT deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. VII Quarto densify from locked 1661 Latin "
            "(p. 296); Pass A != B."
            if int(sid) >= 426
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VII quarto densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VII Quarto/Quinto close) before Cap. VIII from locked 1661 Saumur Latin "
                "(p. 296). Honest partial — Cap. VIII+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VII quarto densify (Quarto/Quinto → Cap. VII tip before Cap. VIII)

- Before: **{BEFORE}** sections (through Cap. VII GAR Ezek 21 / just-with-unjust tip)
- After: **{AFTER}** sections (through Cap. VII Quarto/Quinto Cap. VII close)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF p. 296)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_426–430; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap7_quarto/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_426-430", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VIII — OT arguments for imputing Adam actual sin",
    }
