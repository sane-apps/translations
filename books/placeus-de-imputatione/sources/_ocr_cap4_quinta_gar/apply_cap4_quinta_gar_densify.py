# -*- coding: utf-8 -*-
"""Cap. IV GAR 2 on Quinta densify — non mirum / temporal deferral; PLAC posterior; Sexta morte morieris tip; before GAR Ultimo."""
...
OCR = BOOK / "sources/_ocr_cap4_quinta_gar"
...
HOLD_START_PATH = Path("/tmp/placeus_cap4_quinta_gar_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 3930  # tip-244 catch-up ship; need live>3930 or 12m
PACKET_STEM = "cap4_quinta_gar_densify"
LOCK_NAME = "_placeus_cap4_quinta_gar_latin_lock.txt"
JX_PREFIX = "cap4_quinta_gar"
...
Scope: Manuscriptum posterius Cap. IV after Noli/Quarta/Quinta / PLAC on GAR Resp. 1: GAR 2 Nihil in eo mirum (spiritual immediate / temporal deferral / elect); Placeus posterior reply (syllogism, par ratio, contradiction, habitus); Sexta ratio morte morieris tip through before GAR Ultimo (printed pp. 206 tip–209 tip). Stops before GAR Ultimo on morte spiritualis metaphorice. Not Cap. IV remainder+.
Reconstruction: IA PDF page images via macOS Vision OCR + pdftotext; long-s/ligature OCR corrected. OCR fix: non credentem (vision credentem). Do not revive ἔργον.
No modern English used as copy-text.
Remaining: Cap. IV GAR Ultimo+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "tip-244 catch-up ship in flight; re-probe before append; need live>3930 or 12m",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IV Quinta GAR densify section {sid} "
                "from locked 1661 Latin (pp. 206 tip–209 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Cap. IV GAR 2 on Quinta (non mirum / temporal deferral; PLAC posterior; "
        "Sexta morte morieris tip) / before GAR Ultimo"
    )
    remain = (
        "Cap. IV GAR Ultimo+ / rest of MS examination / ~494 pp Disputatio remain."
    )
    meta.update({
        ...
        "edition": (
            ...
            "through Cap. IV GAR 2 on Quinta / Sexta tip (before GAR Ultimo). "
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads(
                (BOOK / "reviews/audit/cap4_noli_quarta_densify.packet.json").read_text(
                    encoding="utf-8"
                )
            )
            wits = list(
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap4_quinta_gar",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": (
                "Locked Latin, Man. Post. Cap. IV GAR 2 Quinta / Sexta tip "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Cap. IV GAR 2 on Quinta (non mirum / temporal deferral) through Placeus "
            "posterior reply and Sexta morte morieris tip before GAR Ultimo, "
            "reconstructed from IA PDF page images with Vision OCR and pdftotext as check. "
            "OCR fix non credentem. No modern English was copied. Cap. IV GAR Ultimo+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads(
        (BOOK / "reviews/audit/cap4_noli_quarta_densify.packet.json").read_text(encoding="utf-8")
    )
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IV Noli/Quarta/Quinta densify from locked 1661 Latin "
            "(pp. 202 tip–206 tip); Pass A != B."
            if int(sid) >= 245
            else (
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IV Quinta GAR densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Cap. IV GAR 2 on Quinta (non mirum / temporal "
                "deferral; PLAC posterior; Sexta tip) before GAR Ultimo "
                "from locked 1661 Saumur Latin (pp. 206 tip–209 tip). Honest partial — "
                "Cap. IV GAR Ultimo+ remain. OCR fix non credentem. Do not revive ἔργον. "
                "Punch X: NO."
            ),
        },
        "reviews": reviews,
    ...
    block = f"""## 2026-09-22 Cap. IV Quinta GAR densify (GAR 2 non mirum → PLAC posterior → Sexta tip → before GAR Ultimo)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. IV Noli/Quarta/Quinta / before GAR 2)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. IV GAR 2 / PLAC posterior / Sexta morte morieris tip / before GAR Ultimo)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 206 tip–209 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: OCR fix **non credentem** (vision credentem); do not revive ἔργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_245–250; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: tip-244 catch-up ship; eng/src append after live>3930 or 12m (`sources/_ocr_cap4_quinta_gar/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_245-250",
        "tip_ready": "ok",
        "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IV GAR Ultimo (mors spiritualis metaphorice / Luc 9:60 charge) / PLAC",
        "prior_packet_done": "cap4_noli_quarta_densify (238→244)",
    }
