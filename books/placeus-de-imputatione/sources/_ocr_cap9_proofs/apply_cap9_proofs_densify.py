# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — four proofs / sin-entered = Adam actual; tip before name-distinctions."""
...
OCR = BOOK / "sources/_ocr_cap9_proofs"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_proofs_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5026
PACKET_STEM = "cap9_proofs_densify"
LOCK_NAME = "_placeus_cap9_proofs_latin_lock.txt"
JX_PREFIX = "cap9_proofs"
...
Scope: Manuscriptum posterius Cap. IX — four proofs that 'sin entered' = Adam's actual (printed pp. 315–318 tip). Stops before Placeus name-distinctions on παράβασις / παράπτωμα / παρακοή. Not whole Cap. IX / Disputatio.
...
Remaining: Cap. IX name-distinctions+ / rest of Cap. IX / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX four-proofs densify tip-478; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX four-proofs densify section {sid} "
                "from locked 1661 Latin (pp. 315–318 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (four proofs / sin-entered = Adam actual) "
        "/ before name-distinctions"
    )
    remain = "Cap. IX name-distinctions+ / rest of Cap. IX / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. IX four proofs tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_rom5_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap9_proofs",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. IX four proofs (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Caput IX (four proofs / sin-entered = Adam actual) before name-"
            "distinctions, reconstructed from IA PDF page images with Vision OCR and "
            "pdftotext as check. No modern English was copied. Cap. IX name-distinctions+ remain."
        ),
        "witnesses": wits,
    }
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX four-proofs densify from locked 1661 Latin "
            "(pp. 315–318 tip); Pass A != B."
            if int(sid) >= 479
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX four-proofs densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput IX (four proofs / sin-entered = Adam "
                "actual) before name-distinctions from locked 1661 Saumur Latin "
                "(pp. 315–318 tip). Honest partial — Cap. IX name-distinctions+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX four-proofs densify (sin-entered = Adam actual → before names)

- Before: **{BEFORE}** sections (through Cap. IX Rom 5:12 open tip)
- After: **{AFTER}** sections (through Cap. IX four proofs tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 315–318 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: originale/actual mix; Pighius; formaliter/causaliter; ἡ ἁμαρτία; παράβασις/παράπτωμα/παρακοή; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_479–484; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap9_proofs/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_479-484", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — name-distinctions (παράβασις / παράπτωμα / παρακοή)",
    }
