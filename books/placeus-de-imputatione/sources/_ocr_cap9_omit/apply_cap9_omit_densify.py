# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — omit-inhering / death-name dilemma; tip before all-sinners proof."""
...
OCR = BOOK / "sources/_ocr_cap9_omit"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_omit_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5207
PACKET_STEM = "cap9_omit_densify"
LOCK_NAME = "_placeus_cap9_omit_latin_lock.txt"
JX_PREFIX = "cap9_omit"
...
Scope: Manuscriptum posterius Cap. IX — omit-inhering / death-name dilemma (Pighius / Pelagians / Augustine) printed pp. 329–330 tip. Stops before all-are-sinners proof. Not whole Cap. IX / Disputatio.
...
Remaining: Cap. IX all-are-sinners proof+ / rest of Cap. IX / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX omit densify tip-526; need live>5207 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX omit densify section {sid} "
                "from locked 1661 Latin (pp. 329–330 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (omit-inhering / death-name dilemma) "
        "/ before all-are-sinners proof"
    )
    remain = "Cap. IX all-are-sinners proof+ / rest of Cap. IX / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. IX omit/death dilemma tip. Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_senses_densify.packet.json").read_text(encoding="utf-8"))
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Caput IX (omit-inhering / death-name dilemma) before "
            "all-are-sinners proof, reconstructed from IA PDF page images with Vision OCR and "
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IX omit densify from locked 1661 Latin "
            "(pp. 329–330 tip); Pass A != B."
            if int(sid) >= 527
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IX omit densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "before all-are-sinners proof from locked 1661 Saumur Latin "
                "(pp. 329–330 tip). Honest partial — Cap. IX deepen+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. IX omit densify (omit/death dilemma → before all-sinners)

- Before: **{BEFORE}** sections (through Cap. IX three senses tip)
- After: **{AFTER}** sections (through Cap. IX omit/death dilemma tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 329–330 tip)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_527–532; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5207 or 12m (`sources/_ocr_cap9_omit/hold_receipt.json`; released {hold_note})
- Stopped before Cap. IX all-are-sinners proof…
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_527-532", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — all-are-sinners proof (peccatum in mundo / in hominibus)",
    }
