# -*- coding: utf-8 -*-
"""Man. Post. Cap. VIII densify — Gospel promise / seed of woman; tip before Ps 32."""
...
OCR = BOOK / "sources/_ocr_cap8_gospel"
...
HOLD_START_PATH = Path("/tmp/placeus_cap8_gospel_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4911
PACKET_STEM = "cap8_gospel_densify"
LOCK_NAME = "_placeus_cap8_gospel_latin_lock.txt"
JX_PREFIX = "cap8_gospel"
...
Scope: Manuscriptum posterius Cap. VIII — Gospel promise / seed of woman / serpent-cause (printed pp. 306 tip–308 tip). Stops before death-reply / Ps 32. Not whole Cap. VIII / Disputatio.
...
Remaining: Cap. VIII Ps 32+ / rest of Cap. VIII / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VIII gospel densify tip-454 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VIII gospel densify section {sid} "
                "from locked 1661 Latin (pp. 306 tip–308 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VIII (Gospel promise / seed of woman / serpent) "
        "/ before Ps 32"
    )
    remain = "Cap. VIII Ps 32+ / rest of Cap. VIII / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VIII Gospel promise / serpent (before Ps 32). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap8_pers_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap8_gospel",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VIII Gospel promise / seed of woman (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VIII Gospel promise / serpent) before Ps 32, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. VIII Ps 32 deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. VIII gospel densify from locked 1661 Latin "
            "(pp. 306 tip–308 tip); Pass A != B."
            if int(sid) >= 455
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VIII gospel densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VIII Gospel promise / serpent) before Ps 32 from locked 1661 Saumur Latin "
                "(pp. 306 tip–308 tip). Honest partial — Cap. VIII Ps 32+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VIII gospel densify (Gospel promise / seed → before Ps 32)

- Before: **{BEFORE}** sections (through Cap. VIII personal words / Bellarmine tip)
- After: **{AFTER}** sections (through Cap. VIII Gospel promise / serpent tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 306 tip–308 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: semine mulieris / Remedio communi / serpens / venenum; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_455–460; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap8_gospel/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_455-460", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. VIII — death-reply / Ps 32 imputation argument",
    }
