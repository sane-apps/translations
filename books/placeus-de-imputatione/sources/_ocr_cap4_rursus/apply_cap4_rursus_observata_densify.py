# -*- coding: utf-8 -*-
"""Cap. IV Rursus quaedam observata densify — through Prima ratio morte morieris; before GAR."""
...
OCR = BOOK / "sources/_ocr_cap4_rursus"
...
HOLD_START_PATH = Path("/tmp/placeus_cap4_rursus_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 3821  # hold cleared at 3822; first probe should release
PACKET_STEM = "cap4_rursus_observata_densify"
LOCK_NAME = "_placeus_cap4_rursus_observata_latin_lock.txt"
JX_PREFIX = "cap4_rursus"
...
Scope: Manuscriptum posterius Cap. IV Rursus quaedam observata & reprehensa in eodem Cap. II through Prima ratio on morte morieris (printed pp. 190 tip–193 tip). Stops before GAR Resp. / Cameron–Amyraut on promised life. Not Cap. IV remainder+.
Reconstruction: IA PDF page images via macOS Vision OCR + tesseract lat+eng + pdftotext; long-s/ligature OCR corrected. Locked Hebrew חַיִּים / נִשְׁמַת חַיִּים. Do not revive ἔργον.
No modern English used as copy-text.
Remaining: Cap. IV GAR/PLAC on morte morieris+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "hold cleared context (live 3822); re-probe before append; need live>3821 or 12m",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IV Rursus observata densify section {sid} "
                "from locked 1661 Latin (pp. 190 tip–193 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Cap. IV Rursus observata (spiraculum/lignum/morte morieris Prima ratio "
        "/ before GAR Cameron–Amyraut)"
    )
    remain = (
        "Cap. IV GAR/PLAC remainder+ / rest of MS examination / ~494 pp Disputatio remain."
    )
    meta.update({
        ...
        "edition": (
            ...
            "through Cap. IV Rursus observata Prima ratio (before GAR). "
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads(
                (BOOK / "reviews/audit/cap3_observata_densify.packet.json").read_text(
                    encoding="utf-8"
                )
            )
            wits = list(
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap4_rursus_observata",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": (
                "Locked Latin, Man. Post. Cap. IV Rursus observata through Prima ratio "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Cap. IV Rursus quaedam observata (spiraculum; plural חַיִּים; "
            "sanctitas; arbor vitae; morte morieris Prima ratio) before GAR, reconstructed "
            "from IA PDF page images with Vision OCR, tesseract, and pdftotext as check. "
            "No modern English was copied. Cap. IV remainder+ remain."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads(
        (BOOK / "reviews/audit/cap3_observata_densify.packet.json").read_text(encoding="utf-8")
    )
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. IV Rursus observata densify from locked 1661 Latin "
            "(pp. 190 tip–193 tip); Pass A != B."
            if int(sid) >= 213
            else (
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. IV Rursus observata densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Cap. IV Rursus quaedam observata (spiraculum "
                "vitae; plural חַיִּים; Zanchius/Pareus; Adam vs Christ sanctity; arbor vitae; "
                "morte morieris Prima ratio) before GAR Resp./Cameron–Amyraut from locked 1661 "
                "Saumur Latin (pp. 190 tip–193 tip). Honest partial — Cap. IV remainder+ remain. "
                "Reformed Saumur era disclosed. Locked Hebrew חַיִּים / נִשְׁמַת חַיִּים; do not "
                "revive ἔργον. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-22 Cap. IV Rursus observata densify (spiraculum/חיים/morte Prima → before GAR)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. III Observata / before Cap. IV)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Cap. IV Rursus observata / plural חיים / sanctitas / arbor / morte Prima / before GAR)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 190 tip–193 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: locked Hebrew **חַיִּים** / **נִשְׁמַת חַיִּים**; do not revive ἔργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_213–218; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: cleared context (live 3822); eng/src append after live>3821 or 12m (`sources/_ocr_cap4_rursus/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_213-218",
        "tip_ready": "ok",
        "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IV GAR Resp. / Cameron–Amyraut on vita promissa (after Prima ratio)",
        "prior_packet_done": "cap3_observata_densify (205→212)",
    }
