# -*- coding: utf-8 -*-
"""Cap. X Superest ultima densify: imputatione Iustitiae Christi through Cap. XI gate."""
...
OCR = BOOK / "sources/_ocr_cap10_ultima"
...
Scope: Caput X Superest ultima… imputatione Iustitiae Christi — Garissoles last ratio + Placeus reply/retorsion/contradiction (printed pp. 127–130 tip). Stops before Caput XI. Not further Cap. XI+.
...
Remaining: Cap. XI+ of the ~494-page Disputatio.
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. X Superest ultima densify section {sid} "
                "from locked 1661 Latin (pp. 127–130 tip)."
            ),
        }
...
def main() -> None:
    ...
    lock_path = BOOK / "sources/_placeus_cap10_ultima_latin_lock.txt"
    ...
    stop = (
        "Cap. X Superest ultima / Garissoles last ratio from imputed Christ’s justice / "
        "Placeus reply–retorsion–contradiction / before Caput XI"
    )
    remain = "Cap. XI+ remain."
    meta.update({
        ...
        "edition": (
            ...
            "IA deimputationepri00lapl. Densify: Capita I–IX + Caput X through Superest ultima "
            "(imputatione Iustitiae Christi) before Caput XI. "
    ...
    if not any(str(w.get("path", "")).endswith("_placeus_cap10_ultima_latin_lock.txt") for w in wits):
        wits.append({
            "id": "latin_lock_cap10_ultima",
            "path": "sources/_placeus_cap10_ultima_latin_lock.txt",
            "role": "copy-text",
            "name": (
                "Locked Latin, Caput X Superest ultima through Placeus reply before Caput XI "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Superest ultima imputatione Iustitiae Christi (Garissoles last ratio + Placeus "
            "reply/retorsion/contradiction) before Caput XI, reconstructed from IA PDF page "
    ...
    old_p = json.loads((BOOK / "reviews/audit/cap10_auxilium_densify.packet.json").read_text(encoding="utf-8"))
    ...
    pkt_path = BOOK / "reviews/audit/cap10_ultima_densify.packet.json"
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. X Superest ultima densify from locked 1661 Latin "
            "(pp. 127–130 tip); Pass A != B."
            if int(sid) >= 125
            else (
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. X Superest ultima densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–X through Cap. X Superest ultima imputatione Iustitiae Christi "
                "(Garissoles last ratio + Placeus reply/retorsion/contradiction) before Caput XI "
                "from locked 1661 Saumur Latin (pp. 127–130 tip). Honest partial — Cap. XI+ remain. "
    ...
    rev_path = BOOK / "reviews/audit/cap10_ultima_densify.review.json"
    ...
    block = """## 2026-09-22 Cap. X Superest ultima densify (Christi justitia → retorsion / before Cap. XI)
...
"""
    ...
    print(f"DONE before={BEFORE} after={AFTER} packet=cap10_ultima_densify PunchX=NO")
