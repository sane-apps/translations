# -*- coding: utf-8 -*-
"""Man. Post. Cap. IX densify — aptitudo / Sohnius / Zanchius actu."""
...
OCR = BOOK / "sources/_ocr_cap9_aptitudo"
...
HOLD_START_PATH = Path("/tmp/placeus_cap9_aptitudo_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 5604
PACKET_STEM = "cap9_aptitudo_densify"
LOCK_NAME = "_placeus_cap9_aptitudo_latin_lock.txt"
JX_PREFIX = "cap9_aptitudo"
...
Scope: Manuscriptum posterius Cap. IX — aptitudo / Sohnius / Zanchius actu printed pp. 352–353 tip. Stops before "Minus etiam… peccavit pro omnibus." Not whole Cap. IX / Disputatio.
Reconstruction: IA PDF via pdftotext; long-s/ligature OCR corrected. Do not revive εργον.
No modern English used as copy-text.
Remaining: Cap. IX peccavit-pro-posteris / rest Cap. IX / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.IX PLAC vere densify tip-628→634; need live>5604 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. IX aptitudo densify section {sid} "
                "from locked 1661 Latin (pp. 352–353 tip)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput IX (aptitudo/Sohnius/Zanchius tip) "
        "/ before peccavit-pro-posteris"
    )
    remain = "Cap. IX peccavit-pro-posteris+ / rest Cap. IX / ~494 pp Disputatio remain."
    meta.update({
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap9_immed_clavis_densify.packet.json").read_text(encoding="utf-8"))
    ...
    block = f"""## 2026-09-23 Cap. IX aptitudo / Sohnius / Zanchius actu densify

- Before: **{BEFORE}** sections (through Cap. IX Calvin Clavis tip)
- After: **{AFTER}** sections (through Cap. IX Zanchius actu / original vitium tip)
- Packet: `{PACKET_STEM}` id `{packet["packet_id"]}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 352–353 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: aptitudo; Sohnius; Zanchius; peccare actu; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_629–634; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>5604 or 12m (`sources/_ocr_cap9_aptitudo/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_629-634", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — peccavit pro omnibus posteris / vadem (after Zanchius tip)",
    }
