# -*- coding: utf-8 -*-
"""Man. Post. Cap. V densify — quid sit imputari / non imputari; before Caput VI."""
...
OCR = BOOK / "sources/_ocr_cap5_imputari"
...
HOLD_START_PATH = Path("/tmp/placeus_cap5_imputari_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4025  # post tip-ready catch-up; need live>4025 or 12m
PACKET_STEM = "cap5_imputari_densify"
LOCK_NAME = "_placeus_cap5_imputari_latin_lock.txt"
JX_PREFIX = "cap5_imputari"
...
Scope: Manuscriptum posterius Caput V (Examen Cap. III quid sit imputari vel non imputari peccatum) from after Cap. IV Caeterum tip through mediate-imputation close before Caput VI (printed pp. 213 tip–215 tip). Stops before Caput VI. Not Caput VI+ / rest of MS examination.
Reconstruction: IA PDF page images via macOS Vision OCR + pdftotext; long-s/ligature OCR corrected. Locked Greek παραλογισμός. Do not revive εργον.
No modern English used as copy-text.
Remaining: Caput VI+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    """After tip-ready: hold until live>4025 or 12m (do not ship)."""
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; tip-262/Cap.V catch-up ship; need live>4025 or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. V quid sit imputari densify section {sid} "
                "from locked 1661 Latin (pp. 213 tip–215 tip)."
            ),
        }
...
def main() -> None:
    ...
    # Hold already cleared (live 4025>4012); append now.
    ...
    stop = (
        "Man. Post. Caput V (Examen Cap. III quid sit imputari / non imputari; "
        "de jure vs mercy; judicial / mediate tip) / before Caput VI"
    )
    remain = (
        "Caput VI+ / rest of MS examination / ~494 pp Disputatio remain."
    )
    meta.update({
        ...
        "edition": (
            ...
            "IA deimputationepri00lapl. Densify: Capita I–XII + Caput XIV + Man. Post. Cap. II–V "
            "through Caput V quid sit imputari tip (before Caput VI). "
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads(
                (BOOK / "reviews/audit/cap4_gar_denique_densify.packet.json").read_text(
                    encoding="utf-8"
                )
            )
            wits = list(
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap5_imputari",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": (
                "Locked Latin, Man. Post. Caput V quid sit imputari tip "
                "(Saumur 1661)"
            ),
            "language": "Latin",
    ...
    for wid, path, name in [
        (
            "latin_lock_cap4_gar_denique",
            "sources/_placeus_cap4_gar_denique_latin_lock.txt",
            "Locked Latin, Man. Post. Cap. IV GAR Denique / Vir Doctus threefold tip (Saumur 1661)",
        ),
    ]:
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Man. Post. Caput V (Examen Cap. III quid sit imputari vel non imputari; de jure vs "
            "mercy; judicial imputation; mediate tip) before Caput VI, "
            "reconstructed from IA PDF page images with Vision OCR and pdftotext as check. "
            "No modern English was copied. Caput VI+ remain. Locked Greek παραλογισμός."
        ),
        "witnesses": wits,
    }
    ...
    old_p = json.loads(
        (BOOK / "reviews/audit/cap4_gar_denique_densify.packet.json").read_text(encoding="utf-8")
    )
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. V quid sit imputari densify from locked 1661 Latin "
            "(pp. 213 tip–215 tip); Pass A != B."
            if int(sid) >= 263
            else (
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. V quid sit imputari densify review, 2026-09-22",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {
            ...
            "uncertainties": [],
            "notes": (
                "Cap. I–XIV through Man. Post. Caput V (quid sit imputari / de jure / judicial / mediate tip) before Caput VI from locked 1661 Saumur Latin (pp. 213 tip–215 tip). Honest partial — Caput VI+ remain. Locked Greek παραλογισμός. Do not revive εργον. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    # Post tip-ready hold (no ship)
    hold = wait_hold_post_tip()
    hold_note = hold.get("released_by") or "?"

    handoff = BOOK / "SESSION_HANDOFF.md"
    old = handoff.read_text(encoding="utf-8")
    block = f"""## 2026-09-22 Cap. V quid sit imputari densify (Examen Cap. III → de jure / judicial / mediate → before Caput VI)

- Before: **{BEFORE}** sections (Cap. I–XIV through Man. Post. Cap. IV GAR Denique / Vir Doctus threefold / Caeterum tip / before Caput V)
- After: **{AFTER}** sections (Cap. I–XIV through Man. Post. Caput V quid sit imputari / de jure / judicial / mediate tip / before Caput VI)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 213 tip–215 tip)
- Pass A≠B; OUR; English-first; no PBB/ops TNs
- Lemma note: locked Greek παραλογισμός; do not revive εργον
- Gates: `check_pass_ab` ok {JX_PREFIX}_263–268; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>4025 or 12m (`sources/_ocr_cap5_imputari/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "punch_x": "NO",
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_263-268",
        "tip_ready": "ok",
        "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Caput VI (Examen Cap. IV)",
    }
