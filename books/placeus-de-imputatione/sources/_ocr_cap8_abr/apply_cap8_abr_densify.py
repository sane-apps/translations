# -*- coding: utf-8 -*-
"""Man. Post. Cap. VIII densify — Abraham / OT close; tip before Cap. IX Rom 5."""
...
OCR = BOOK / "sources/_ocr_cap8_abr"
...
HOLD_START_PATH = Path("/tmp/placeus_cap8_abr_post_hold_start.txt")
HOLD_SECONDS = 12 * 60
LIVE_NEED = 4963
PACKET_STEM = "cap8_abr_densify"
LOCK_NAME = "_placeus_cap8_abr_latin_lock.txt"
JX_PREFIX = "cap8_abr"
...
Scope: Manuscriptum posterius Cap. VIII — Abraham justification / OT close / law-detects-sin (printed pp. 310 tip–312). Stops before Cap. IX NT Rom 5. Not whole Cap. VIII / Disputatio.
...
Remaining: Cap. IX NT Rom 5+ / rest of MS examination / ~494 pp Disputatio.
...
def wait_hold_post_tip() -> dict:
    ...
    receipt = {
        ...
        "probes": [],
        "released_by": None,
        "note": "post tip-ready; Cap.VIII abr densify tip-466 catch-up; need live>LIVE_NEED or 12m; Punch X=NO",
    }
...
def write_justifications() -> None:
    for sid in SIDS:
        ...
        j = {
            ...
            "notes": (
                f"Cap. VIII abr densify section {sid} "
                "from locked 1661 Latin (pp. 310 tip–312)."
            ),
        }
...
def main() -> None:
    ...
    stop = (
        "Man. Post. Caput VIII (Abraham / OT close) "
        "/ before Cap. IX Rom 5"
    )
    remain = "Cap. IX NT Rom 5+ / rest of MS examination / ~494 pp Disputatio remain."
    meta.update({
        ...
        "edition": (
            ...
            f"through Cap. VIII Abraham / OT close (before Cap. IX). Not whole Disputatio; {remain}"
        ),
        "blurb": (
    ...
    if len(wits) < 30:
        try:
            old_p = json.loads((BOOK / "reviews/audit/cap8_ps32_densify.packet.json").read_text(encoding="utf-8"))
    ...
    if not any(str(w.get("path", "")).endswith(LOCK_NAME) for w in wits):
        wits.append({
            "id": "latin_lock_cap8_abr",
            "path": f"sources/{LOCK_NAME}",
            "role": "copy-text",
            "name": "Locked Latin, Man. Post. Cap. VIII Abraham / OT close (Saumur 1661)",
            "language": "Latin",
    ...
    meta["text_history"] = {
        "method": (
            ...
            "Cap. VIII Abraham / OT close) before Cap. IX, reconstructed from IA PDF page images with "
            "Vision OCR and pdftotext as check. No modern English was copied. Cap. IX Rom 5 deepen+ remain."
        ),
        "witnesses": wits,
    }
    ...
    for sid in packet["expected_sections"]:
        note = (
            f"Section {sid}: Cap. VIII abr densify from locked 1661 Latin "
            "(pp. 310 tip–312); Pass A != B."
            if int(sid) >= 467
    ...
    review = {
        ...
        "reviewer": "scribe-placeus / Placeus Cap. VIII abr densify review, 2026-09-23",
        "verdict": "pass",
        "scope_review": {
            "verdict": "pass",
            "checks": {k: True for k in (
            ...
            "notes": (
                ...
                "Cap. VIII Abraham / OT close) before Cap. IX from locked 1661 Saumur Latin "
                "(pp. 310 tip–312). Honest partial — Cap. IX Rom 5+ remain. Punch X: NO."
            ),
        },
        "reviews": reviews,
    }
    ...
    block = f"""## 2026-09-23 Cap. VIII abr densify (Abraham / OT close → before Cap. IX)

- Before: **{BEFORE}** sections (through Cap. VIII Ps 32 reply tip)
- After: **{AFTER}** sections (through Cap. VIII Abraham / OT-close tip)
- Packet: `{PACKET_STEM}`
- Locked Latin: `sources/{LOCK_NAME}` (1661 PDF pp. 310 tip–312)
...
- Gates: `check_pass_ab` ok {JX_PREFIX}_467–472; `--tip-ready` 0; packet `reviews/audit/{PACKET_STEM}.packet.json`
- Hold: after tip-ready; live>LIVE_NEED or 12m (`sources/_ocr_cap8_abr/hold_receipt.json`; released {hold_note})
...
"""
    ...
    print("SESSION_HANDOFF prepended")
        ...
        "claim": "placeus-de-imputatione-densify stays claimed",
        "check_pass_ab": f"ok {JX_PREFIX}_467-472", "tip_ready": "ok", "hold": hold_note,
        "hostname": subprocess.check_output(["hostname"], text=True).strip(),
        "next_locus": "Man. Post. Cap. IX — NT arguments / Rom 5:12ff",
    }
