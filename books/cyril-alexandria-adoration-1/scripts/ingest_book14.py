#!/usr/bin/env python3
"""Ingest PG 68 Book 14 Greek into adoration14_source.json from the locked PDF text."""
...
CLEAN = BOOK / "sources" / "adoration_book14_greek_clean.txt"
OUT = BOOK / "translations" / "adoration14_source.json"
...
def slice_book14(raw: str) -> str:
    raw = nfc(raw)
    # OCR form ΤΕΣΣΑΡΑΚΛΙ∆ΕΚΑΤΟΣ seen in pdftotext
    m = re.search(
        r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΤΕΣΣΑΡΑΚΛΙ[∆Δ]ΕΚΑΤΟΣ|ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΤΕΣΣΑΡΑΚΑΙ[∆Δ]ΕΚΑΤΟΣ|ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΤΕΣΣΑΡΕΣΚΑΙ[∆Δ]ΕΚΑΤΟΣ",
        raw,
    )
    if not m:
    ...
    if head >= 0 and start - head < 250:
        start = head
    end_m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΠΕΝΤΕΚΑΙ[∆Δ]ΕΚΑΤΟΣ", raw[start + 50 :])
    if not end_m:
        raise SystemExit("Book 15 start not found")
...
def parse(clean: str) -> list[dict]:
    ...
    # Book 14 opens ~PG 68.885 (first inline marker often soon after)
    spans = [(0, cols[0][0] if cols else len(clean), "68.885")]
