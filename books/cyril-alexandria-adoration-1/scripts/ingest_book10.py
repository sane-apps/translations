#!/usr/bin/env python3
"""Ingest PG 68 Book 10 Greek into adoration10_source.json from the locked PDF text."""
...
CLEAN = BOOK / "sources" / "adoration_book10_greek_clean.txt"
OUT = BOOK / "translations" / "adoration10_source.json"
...
def slice_book10(raw: str) -> str:
    raw = nfc(raw)
    start = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ∆ΕΚΑΤΟΣ")
    if start < 0:
        start = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΔΕΚΑΤΟΣ")
    if start < 0:
        m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ [∆Δ]ΕΚΑΤΟΣ", raw)
        if not m:
            raise SystemExit("Book 10 start not found")
        start = m.start()
    head = raw.rfind("ΠΕΡΙ ΤΗΣ ΕΝ ΠΝΕΥΜΑΤΙ", 0, start)
    if head >= 0 and start - head < 250:
        start = head
    end = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΝ∆ΕΚΑΤΟΣ", start + 20)
    if end < 0:
        end = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΝΔΕΚΑΤΟΣ", start + 20)
    if end < 0:
        m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΝ[∆Δ]ΕΚΑΤΟΣ", raw[start + 20 :])
        if not m:
...
def parse(clean: str) -> list[dict]:
    ...
    # Book 10 opens ~PG 68.649 (first inline marker often 68.652)
    spans = [(0, cols[0][0] if cols else len(clean), "68.649")]
