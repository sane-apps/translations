#!/usr/bin/env python3
"""Ingest PG 68 Book 8 Greek into adoration8_source.json from the locked PDF text."""
...
CLEAN = BOOK / "sources" / "adoration_book8_greek_clean.txt"
OUT = BOOK / "translations" / "adoration8_source.json"
...
def slice_book8(raw: str) -> str:
    raw = nfc(raw)
    start = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΟΓ∆ΟΟΣ.")
    if start < 0:
        start = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΟΓΔΟΟΣ.")
    if start < 0:
        m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΟΓ.ΟΟΣ\.", raw)
        if not m:
            raise SystemExit("Book 8 start not found")
        start = m.start()
    head = raw.rfind("ΠΕΡΙ ΤΗΣ ΕΝ ΠΝΕΥΜΑΤΙ", 0, start)
    if head >= 0 and start - head < 250:
        start = head
    end = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΝΝΑΤΟΣ.", start + 20)
    if end < 0:
        end = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΝΑΤΟΣ.", start + 20)
    if end < 0:
        m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΝ+ΑΤΟΣ\.", raw[start + 20 :])
        if not m:
...
def parse(clean: str) -> list[dict]:
    ...
    # Book 8 opens ~PG 68.533 (first inline marker often 68.481)
    spans = [(0, cols[0][0] if cols else len(clean), "68.533")]
