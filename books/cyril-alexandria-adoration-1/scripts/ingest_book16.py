#!/usr/bin/env python3
"""Ingest PG 68 Book 16 Greek into adoration16_source.json from the locked PDF text."""
...
CLEAN = BOOK / "sources" / "adoration_book16_greek_clean.txt"
OUT = BOOK / "translations" / "adoration16_source.json"
...
def slice_book16(raw: str) -> str:
    raw = nfc(raw)
    m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΚΚΑΙ[∆Δ]ΕΚΑΤΟΣ", raw)
    if not m:
    ...
    if head >= 0 and start - head < 250:
        start = head
    end_m = re.search(r"ΛΟΓΟΣ ΕΠΤΑΚΑΙ[∆Δ]ΕΚΑΤΟΣ", raw[start + 50 :])
    if not end_m:
...
def parse(clean: str) -> list[dict]:
    ...
    # Book 16 opens ~PG 68.1009 (first inline marker often soon after)
    spans = [(0, cols[0][0] if cols else len(clean), "68.1009")]
