#!/usr/bin/env python3
"""Ingest PG 68 Book 11 Greek into adoration11_source.json from the locked PDF text."""
...
CLEAN = BOOK / "sources" / "adoration_book11_greek_clean.txt"
OUT = BOOK / "translations" / "adoration11_source.json"
...
def slice_book11(raw: str) -> str:
    raw = nfc(raw)
    start = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΝ∆ΕΚΑΤΟΣ")
    if start < 0:
        start = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΝΔΕΚΑΤΟΣ")
    if start < 0:
        m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΝ[∆Δ]ΕΚΑΤΟΣ", raw)
        if not m:
            raise SystemExit("Book 11 start not found")
        start = m.start()
    head = raw.rfind("ΠΕΡΙ ΤΗΣ ΕΝ ΠΝΕΥΜΑΤΙ", 0, start)
    if head >= 0 and start - head < 250:
        start = head
    end = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ∆Ω∆ΕΚΑΤΟΣ", start + 20)
    if end < 0:
        end = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΔΩΔΕΚΑΤΟΣ", start + 20)
    if end < 0:
        end = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ∆Ω∆ΕΚΑΤΟΣ", start + 20)
    if end < 0:
        m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ [∆Δ][ΩΩ][∆Δ]ΕΚΑΤΟΣ", raw[start + 20 :])
        if not m:
            raise SystemExit("Book 12 start not found")
...
def parse(clean: str) -> list[dict]:
    ...
    # Book 11 opens ~PG 68.725 (first inline marker often soon after)
    spans = [(0, cols[0][0] if cols else len(clean), "68.725")]
