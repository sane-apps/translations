#!/usr/bin/env python3
"""Ingest PG 68 Book 12 Greek into adoration13_source.json from the locked PDF text."""
...
CLEAN = BOOK / "sources" / "adoration_book13_greek_clean.txt"
OUT = BOOK / "translations" / "adoration13_source.json"
...
def slice_book13(raw: str) -> str:
    raw = nfc(raw)
    m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΤΡΙΣΚΑΙ[∆Δ]ΕΚΑΤΟΣ", raw)
    if not m:
        raise SystemExit("Book 13 start not found")
    start = m.start()
    head = raw.rfind("ΠΕΡΙ ΤΗΣ ΕΝ ΠΝΕΥΜΑΤΙ", 0, start)
    if head >= 0 and start - head < 250:
        start = head
    end_m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΤΕΣΣΑΡΑ[Κκ]?ΛΙ[∆Δ]ΕΚΑΤΟΣ|ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΤΕΣΣΑΡΕΣΚΑΙ[∆Δ]ΕΚΑΤΟΣ|ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΤΕΣΣΑΡΑΚΑΙ[∆Δ]ΕΚΑΤΟΣ", raw[start + 50 :])
    if not end_m:
        # OCR form ΤΕΣΣΑΡΑΚΛΙ∆ΕΚΑΤΟΣ seen in pdftotext
        end_m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΤΕΣΣΑΡΑΚΛΙ[∆Δ]ΕΚΑΤΟΣ", raw[start + 50 :])
    if not end_m:
        raise SystemExit("Book 14 start not found")
    end = start + 50 + end_m.start()
    peri = raw.rfind("ΠΕΡΙ ΤΗΣ ΕΝ ΠΝΕΥΜΑΤΙ", start + 50, end)
    if peri > start:
        end = peri
...
def parse(clean: str) -> list[dict]:
    ...
    # Book 13 opens ~PG 68.781 (first inline marker often soon after)
    spans = [(0, cols[0][0] if cols else len(clean), "68.844")]
