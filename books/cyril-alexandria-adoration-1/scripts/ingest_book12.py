#!/usr/bin/env python3
"""Ingest PG 68 Book 12 Greek into adoration12_source.json from the locked PDF text."""
...
CLEAN = BOOK / "sources" / "adoration_book12_greek_clean.txt"
OUT = BOOK / "translations" / "adoration12_source.json"
...
def slice_book12(raw: str) -> str:
    raw = nfc(raw)
    m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ [∆Δ][ΩΩ][∆Δ]ΕΚΑΤΟΣ", raw)
    if not m:
    ...
    if head >= 0 and start - head < 250:
        start = head
    # Prefer the Book 13 title after Book 12 body (not stray page headers).
    end_m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΤΡΙΣΚΑΙ[∆Δ]ΕΚΑΤΟΣ", raw[start + 50 :])
    if not end_m:
...
def parse(clean: str) -> list[dict]:
    ...
    # Book 12 opens ~PG 68.781 (first inline marker often soon after)
    spans = [(0, cols[0][0] if cols else len(clean), "68.781")]
