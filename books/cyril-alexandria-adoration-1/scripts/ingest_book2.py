#!/usr/bin/env python3
"""Ingest PG 68 Book 2 Greek into adoration2_source.json from the locked PDF text."""
...
CLEAN = BOOK / "sources" / "adoration_book2_greek_clean.txt"
OUT = BOOK / "translations" / "adoration2_source.json"
...
def slice_book2(raw: str) -> str:
    raw = nfc(raw)
    # OCR variants: ∆ for Δ, Ω for Ω
    start_idx = raw.find("ΛΟΓΟΣ ∆ΕΥΤΕΡΟΣ")
    if start_idx < 0:
        start_idx = raw.find("ΛΟΓΟΣ ΔΕΥΤΕΡΟΣ")
    end_idx = raw.find("ΛΟΓΟΣ ΤΡΙΤΟΣ", start_idx + 10 if start_idx >= 0 else 0)
    if start_idx < 0 or end_idx < 0:
        raise SystemExit(f"book2 bounds missing start={start_idx} end={end_idx}")
    # include the ΠΕΡΙ heading a few lines above ΔΕΥΤΕΡΟΣ
    head = raw.rfind("ΠΕΡΙ ΤΗΣ ΕΝ ΠΝΕΥΜΑΤΙ", 0, start_idx)
    if head >= 0 and start_idx - head < 400:
        start_idx = head
...
def apply_hyphen_joins(text: str, notes: list[str]) -> str:
    t = text.replace("σα-φὴς", "σαφὴς").replace("σα-φής", "σαφής")
    t = re.sub(r"([Α-Ωα-ωἈ-Ὧἀ-ὧά-ώΆ-Ώ]+)[\-‐‑]\s*", r"\1", t)
...
def parse(clean: str) -> list[dict]:
    ...
    spans = [(0, cols[0][0] if cols else len(clean), "68.212")]
    ...
    for i, (a, b, col) in enumerate(spans):
        ...
        if not paras:
            continue
        # skip a span that is only a stray page numeral
