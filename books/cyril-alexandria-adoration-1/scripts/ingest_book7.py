#!/usr/bin/env python3
"""Ingest PG 68 Book 7 Greek into adoration7_source.json from the locked PDF text."""
...
CLEAN = BOOK / "sources" / "adoration_book7_greek_clean.txt"
OUT = BOOK / "translations" / "adoration7_source.json"
...
def slice_book7(raw: str) -> str:
    raw = nfc(raw)
    start = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΒ∆ΟΜΟΣ.")
    if start < 0:
        start = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΒΔΟΜΟΣ.")
    if start < 0:
        m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΒ.ΟΜΟΣ\.", raw)
        if not m:
    ...
    if head >= 0 and start - head < 250:
        start = head
    end = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΟΓ∆ΟΟΣ.", start + 20)
    if end < 0:
        end = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΟΓΔΟΟΣ.", start + 20)
    if end < 0:
        m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΟΓ.ΟΟΣ\.", raw[start + 20 :])
        if not m:
...
def parse(clean: str) -> list[dict]:
    ...
    # Book 7 opens ~PG 68.480 (first inline marker often 68.481)
    spans = [(0, cols[0][0] if cols else len(clean), "68.480")]
