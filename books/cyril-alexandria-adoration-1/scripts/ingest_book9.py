#!/usr/bin/env python3
"""Ingest PG 68 Book 9 Greek into adoration9_source.json from the locked PDF text."""
...
CLEAN = BOOK / "sources" / "adoration_book9_greek_clean.txt"
OUT = BOOK / "translations" / "adoration9_source.json"
...
def slice_book9(raw: str) -> str:
    raw = nfc(raw)
    start = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΝΝΑΤΟΣ.")
    if start < 0:
        start = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΝΑΤΟΣ.")
    if start < 0:
        m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΝ+ΑΤΟΣ\.", raw)
        if not m:
            raise SystemExit("Book 9 start not found")
        start = m.start()
    head = raw.rfind("ΠΕΡΙ ΤΗΣ ΕΝ ΠΝΕΥΜΑΤΙ", 0, start)
    if head >= 0 and start - head < 250:
        start = head
    end = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ∆ΕΚΑΤΟΣ", start + 20)
    if end < 0:
        end = raw.find("ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΔΕΚΑΤΟΣ", start + 20)
    if end < 0:
        m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ [∆Δ]ΕΚΑΤΟΣ", raw[start + 20 :])
        if not m:
...
def parse(clean: str) -> list[dict]:
    ...
    # Book 9 opens ~PG 68.589 (first inline marker often 68.481)
    spans = [(0, cols[0][0] if cols else len(clean), "68.589")]
