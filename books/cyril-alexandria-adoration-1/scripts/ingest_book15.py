#!/usr/bin/env python3
"""Ingest PG 68 Book 15 Greek into adoration15_source.json from the locked PDF text."""
...
CLEAN = BOOK / "sources" / "adoration_book15_greek_clean.txt"
OUT = BOOK / "translations" / "adoration15_source.json"
...
def slice_book15(raw: str) -> str:
    raw = nfc(raw)
    m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΠΕΝΤΕΚΑΙ[∆Δ]ΕΚΑΤΟΣ", raw)
    if not m:
    ...
    if head >= 0 and start - head < 250:
        start = head
    end_m = re.search(r"ΛΑΤΡΕΙΑΣ ΛΟΓΟΣ ΕΚΚΑΙ[∆Δ]ΕΚΑΤΟΣ", raw[start + 50 :])
    if not end_m:
        raise SystemExit("Book 16 start not found")
...
def parse(clean: str) -> list[dict]:
    ...
    # Book 15 opens ~PG 68.885 (first inline marker often soon after)
    spans = [(0, cols[0][0] if cols else len(clean), "68.949")]
